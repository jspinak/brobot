...
public class MockActionLogger implements ActionLogger {
    ...
    public LogData logStateTransition(
            ...
            boolean success,
            long transitionTime) {
        String fromStatesString =
                fromStates.stream()
                        .map(State::getName)
                        .reduce((a, b) -> a + ", " + b)
                        .orElse("None");
        System.out.println(
                "Mock logStateTransition: SessionId="
                        + sessionId
                        + ", From State="
                        + fromStatesString
                        + ", To States=["
                        + String.join(", ", toStates.stream().map(State::getName).toList())
                        + "]"
                        + ", Success="
                        + success
                        + ", Time="
    ...
    public LogData logPerformanceMetrics(
        ...
        System.out.println(
                "Mock logPerformanceMetrics: SessionId="
                        + sessionId
                        + ", ActionDuration="
                        + actionDuration
                        + ", PageLoadTime="
