...
/**
 * Comprehensive test suite for PathFinder class. Tests path finding algorithms for state
 * navigation.
 *
 * <p>NOTE: These tests have complex mock interactions with the PathFinder implementation. Disabled
 * as they require refactoring to match the actual implementation behavior.
 */
...
class PathFinderTest {
    ...
    class MultiplePathTests {
        ...
        void testFindMultiplePaths() {
            ...
            // Execute
            Paths paths =
                    pathFinder.getPathsToState(List.of(startState1, startState2), targetState);
            ...
            boolean hasPath1 =
                    paths.getPaths().stream().anyMatch(p -> p.getStates().equals(List.of(1L, 3L)));
            boolean hasPath2 =
                    paths.getPaths().stream().anyMatch(p -> p.getStates().equals(List.of(2L, 3L)));
            assertTrue(hasPath1);
            assertTrue(hasPath2);
        }
