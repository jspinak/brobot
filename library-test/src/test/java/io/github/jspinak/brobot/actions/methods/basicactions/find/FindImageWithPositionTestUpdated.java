...
/**
 * Updated tests for find operations with position settings using new ActionConfig API.
 ...
 * - Position settings are now only on Pattern/StateImage, not in ActionConfig
...
public class FindImageWithPositionTestUpdated extends BrobotIntegrationTestBase {
    ...
    void findWithDifferentStrategiesAndPositions() {
        ...
        StateImage imageWithPosition = new StateImage.Builder()
                .addPattern(new Pattern.Builder()
                        .setFilename(TestPaths.getImagePath("topLeft"))
        ...
        // All matches should have the same calculated position since they use the same pattern
        for (Location loc : allResult.getMatchLocations()) {
            assertEquals(bestLoc.getCalculatedX(), loc.getCalculatedX(), 
                        "All matches should have same calculated X position");
            assertEquals(bestLoc.getCalculatedY(), loc.getCalculatedY(), 
    ...
    void setPositionForAllPatternsTest() {
        ...
        StateImage multiPattern = new StateImage.Builder()
                .addPattern(new Pattern.Builder()
                        .setFilename(TestPaths.getImagePath("topLeft"))
                        .setTargetPosition(new Position(10, 10))  // This will be overridden
                        .build())
                .addPattern(new Pattern.Builder()
                        .setFilename(TestPaths.getImagePath("topLeft"))
                        .setTargetPosition(new Position(20, 20))  // This will be overridden
                        .build())
        ...
        if (result.size() > 1) {
            ...
            for (Location loc : result.getMatchLocations()) {
                assertEquals(firstLoc.getCalculatedX(), loc.getCalculatedX(), 
                            "All matches should have same calculated X");
                assertEquals(firstLoc.getCalculatedY(), loc.getCalculatedY(), 
    ...
    void compareOldAndNewPositionAPI() {
        ...
        */
        ...
        Pattern patternWithPosition = new Pattern.Builder()
                .setFilename("test.png")
                .setTargetPosition(new Position(100, 100))
                .build();
        
        StateImage imageWithPosition = new StateImage.Builder()
                .addPattern(patternWithPosition)
                .build();
