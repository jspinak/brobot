...
/**
 * Comprehensive test configuration that provides all required beans for Brobot testing. This
 * configuration ensures that Spring context can be properly initialized for tests without requiring
 * actual GUI resources or screen capture capabilities.
 */
@TestConfiguration
public class BrobotTestConfiguration {
    ...
    public Action mockAction() {
        ...
        io.github.jspinak.brobot.action.ActionResult successResult =
                new io.github.jspinak.brobot.action.ActionResult();
        successResult.setSuccess(true);

        // Add a mock match to the result for tests that expect matches
        io.github.jspinak.brobot.model.match.Match mockMatch =
                new io.github.jspinak.brobot.model.match.Match.Builder()
    ...
    public Click mockClick() {
        ...
        doAnswer(
                        invocation -> {
                            io.github.jspinak.brobot.action.ActionResult result =
    ...
    public ActionChainExecutor mockActionChainExecutor() {
        ...
        when(executor.executeChain(any(), any(), any()))
                .thenReturn(new io.github.jspinak.brobot.action.ActionResult());
    ...
    /**
     * Mock MockModeManager bean for testing. Note: MockModeManager uses static methods, so we'll
     * configure the static mock mode.
     */
    @Bean
    @Primary
