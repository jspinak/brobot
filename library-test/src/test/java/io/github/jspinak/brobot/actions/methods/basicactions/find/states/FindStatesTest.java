...
/**
 * Tests for finding states functionality using saved FloraNext screenshots. Works in headless/CI
...
class FindStatesTest extends BrobotIntegrationTestBase {
    ...
    void testFindStatesWithScreenshots() {
        ...
        System.out.println(
                "Created " + testStates.size() + " test states from FloraNext screenshots");
                                        ...
                                        stateImage
                                                ...
                                                .forEach(
                                                        pattern -> {
                                                            ...
                                                            // Typical minimum size for reliable
                                                            // matching
                                                            assertTrue(
                                                                    region.w() >= 10
                                                                            || region.h() >= 10,
                                                                    "Pattern should have reasonable"
                                                                            + " minimum size");
        ...
        if (screenshotDir != null && screenshotDir.exists()) {
            File[] screenshots =
                    screenshotDir.listFiles(
                            (dir, name) -> name.startsWith("floranext") && name.endsWith(".png"));

            if (screenshots != null && screenshots.length > 0) {
                System.out.println("Found " + screenshots.length + " FloraNext screenshots:");
                for (File screenshot : screenshots) {
                    System.out.println(
                            "  - " + screenshot.getName() + " (" + screenshot.length() + " bytes)");
                }
            }
        }
