...
/**
 * Updated tests for finding fixed pixel matches using new ActionConfig API. Demonstrates migration
 * from ActionOptions to FixedPixelsFindOptions.
 *
 * <p>Key changes: - Uses FixedPixelsFindOptions instead of generic ActionOptions - ActionResult
...
class FindFixedPixelMatchesTestUpdated {
    ...
    @Autowired FindFixedPixelMatches findFixedPixelMatches;
    ...
    void find_samePatternNewAPI() {
        ObjectCollection objectCollection =
                new ObjectCollection.Builder().withPatterns(pattern1(), pattern1()).build();
    ...
    void find_differentPatternsNewAPI() {
        ObjectCollection objectCollection =
                new ObjectCollection.Builder().withPatterns(pattern1(), pattern2()).build();
    ...
    void testFixedPixelsWithCustomSettings() {
        ...
        FixedPixelsFindOptions customOptions =
                new FixedPixelsFindOptions.Builder()
        ...
        if (matches.getActionConfig() instanceof FixedPixelsFindOptions) {
            FixedPixelsFindOptions resultOptions =
                    (FixedPixelsFindOptions) matches.getActionConfig();
    ...
    void testUsingActionService() {
        ...
        FixedPixelsFindOptions fixedPixelsOptions =
                new FixedPixelsFindOptions.Builder().setMaxMovement(3).build();

        ActionResult matches = new ActionResult();
        matches.setActionConfig(fixedPixelsOptions);

        Optional<ActionInterface> fixedPixelsFindActionOpt =
                actionService.getAction(fixedPixelsOptions);
        assertTrue(
                fixedPixelsFindActionOpt.isPresent(),
                "Fixed pixels find action should be available");
    ...
    void compareOldAndNewFixedPixelsAPI() {
        ...
        ActionOptions oldOptions = new ActionOptions.Builder()
                ...
                .setFind(PatternFindOptions.FindStrategy.FIXED_PIXELS)
                .build();
        ...
        */
        ...
        // NEW API:
        FixedPixelsFindOptions newOptions =
                new FixedPixelsFindOptions.Builder()
                        .setMaxMovement(2)
