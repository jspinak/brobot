...
/**
 * Tests for creating states from visual matches using saved FloraNext screenshots. Works in
 * headless/CI environments without requiring live OCR.
 */
class CreateStatesFromMatchesTest extends BrobotIntegrationTestBase {
    ...
    void testCreateStatesFromMatches() {
        ...
        assertFalse(
                matches.getMatchList().isEmpty(), "Should have created matches from screenshots");

        System.out.println(
                "Created " + matches.getMatchList().size() + " matches from FloraNext screenshots");
                    ...
                    state.getStateImages()
                            .forEach(
                                    stateImage -> {
                                        assertNotNull(
                                                stateImage.getName(),
                                                "StateImage should have a name");
                                        assertFalse(
                                                stateImage.getPatterns().isEmpty(),
                                        ...
                                        stateImage
                                                ...
                                                .forEach(
                                                        pattern -> {
                                                            assertNotNull(
                                                                    pattern.getMat(),
                                                                    "Pattern should have a Mat");
                                                            assertNotNull(
                                                                    pattern.getRegion(),
                                                                    "Pattern should have a region");
        ...
        for (int i = 0; i <= 4; i++) {
            ...
            if (screenshot.exists()) {
                try {
                    ...
                    assertTrue(
                            mat.cols() > 0 && mat.rows() > 0, "Mat should have valid dimensions");
