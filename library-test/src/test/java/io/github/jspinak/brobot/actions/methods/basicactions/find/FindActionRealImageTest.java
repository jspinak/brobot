...
/**
 * Integration test for Find Action using real images and screenshots.
 * Tests the complete find pipeline with actual image matching.
 * 
 * This test verifies:
 * - Find operations with real images 
 * - Pattern matching in screenshots
 * - Illustration generation for both successful and failed finds
...
public class FindActionRealImageTest {
    ...
    void testSuccessfulFindInScreenshot() {
        ...
        assertTrue(result.isSuccess(), "Find operation should succeed when topLeft is in floranext0");
        ...
        System.out.println("✓ Successful find test completed - found " + result.getMatchList().size() + " matches");
    }
    ...
    void testFailedFindInScreenshot() {
        ...
        assertFalse(result.isSuccess(), "Find operation should fail when bottomRight is not in floranext0");
    ...
    void testFindWithMultiplePatterns() {
        ...
        if (result.isSuccess()) {
            ...
            System.out.println("✓ Multi-pattern test completed - found " + result.getMatchList().size() + " matches");
        } else {
            System.out.println("✓ Multi-pattern test completed - no matches found (this may be expected)");
        }
    ...
    void testFindWithDifferentSimilarityThresholds() {
        ...
        PatternFindOptions flexibleOptions = new PatternFindOptions.Builder()
                .setStrategy(PatternFindOptions.Strategy.FIRST)
                .setSimilarity(0.5)
                .build();
        ...
        assertTrue(flexibleResult.getMatchList().size() >= strictResult.getMatchList().size(),
            "Flexible matching should find more or equal matches than strict matching");

        System.out.println("✓ Similarity threshold test completed - Strict: " + 
            strictResult.getMatchList().size() + " matches, Flexible: " + 
            flexibleResult.getMatchList().size() + " matches");
    }
