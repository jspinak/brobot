...
/**
 * Tests for image comparison functionality. Works in headless mode as image comparison uses OpenCV
 * which works without display. Migrated to use TestDataUpdated.
 */
@SpringBootTest(
...
class CompareImagesTest extends BrobotIntegrationTestBase {
    ...
    @Autowired ImageComparer compareImages;
    ...
    void compareImages1() {
        try {
            ...
            StateImage flora1 =
                    new StateImage.Builder().addPattern(testData.getFloranext0()).build();
            StateImage flora2 =
                    new StateImage.Builder().addPattern(testData.getFloranext1()).build();
    ...
    private void compareWithFallbackImages() {
        ...
        assertTrue(
                selfMatch.getScore() > 0.99,
                "Image compared with itself should have very high similarity");
    }
