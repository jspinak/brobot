 ...
 * 
 * This configuration is activated when running tests to prevent Spring context initialization
...
public class TestActionConfiguration {
    ...
    /**
     * Provides a mocked Find action that immediately returns success.
     * This breaks the circular dependency chain: Find → FindPipeline → Click → Find
     */
    @Bean("testFind")
    @Primary
    ...
    /**
     * Provides a simplified Click action that doesn't depend on Find.
     * Uses the mocked Find for any find operations.
     */
    @Bean("testClick")
    @Primary
    ...
    public WaitVanish testWaitVanish() {
        ...
        doAnswer(invocation -> {
            ...
            matches.setSuccess(true);
            return null;
        }).when(mockVanish).perform(any(ActionResult.class), any(ObjectCollection[].class));
    ...
    public MoveMouse testMoveMouse() {
        ...
        doAnswer(invocation -> {
            ...
            matches.setSuccess(true);
            return null;
        }).when(mockMove).perform(any(ActionResult.class), any(ObjectCollection[].class));
