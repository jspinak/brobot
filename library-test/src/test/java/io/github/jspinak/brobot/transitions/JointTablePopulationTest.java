...
/**
 * Test that verifies the fix for joint table population with state IDs.
 * This test directly tests the joint table behavior without requiring
 * the full AnnotationProcessor dependencies.
 */
public class JointTablePopulationTest extends BrobotTestBase {
    ...
    public void testJointTableRequiresStateIdsNotNames() {
        ...
        JavaStateTransition transitionWithOnlyNames = new JavaStateTransition.Builder()
        ...
        assertTrue(parentsOfWorking.isEmpty(), 
        ...
        assertTrue(paths.isEmpty(), 
            "Path finding should fail when joint table has no IDs");
    }
    ...
    public void testJointTableWorksWithStateIds() {
        ...
        JavaStateTransition transitionWithIds = new JavaStateTransition.Builder()
        ...
        assertFalse(parentsOfWorking.isEmpty(), 
            "Joint table SHOULD find parents when transition has IDs");
        assertTrue(parentsOfWorking.contains(promptId), 
            "Prompt should be a parent of Working");
        ...
        assertFalse(paths.isEmpty(), 
            "Path finding should succeed when joint table has IDs");
    ...
    public void testAnnotationProcessorFixPattern() {
        ...
        assertEquals(Set.of("Working"), transition.getActivateNames(),
            "Should have state name");
        assertEquals(Set.of(workingId), transition.getActivate(),
            "Should have state ID");
        ...
        assertTrue(parents.contains(promptId),
            "Joint table should have the transition");
