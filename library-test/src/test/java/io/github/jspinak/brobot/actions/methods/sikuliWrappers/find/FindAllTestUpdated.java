...
/**
 * Updated tests for FindAll functionality using new ActionConfig API. Works in headless mode using
 * real image processing. Demonstrates migration from ActionOptions to specific config classes.
 */
...
class FindAllTestUpdated extends BrobotIntegrationTestBase {
    ...
    void findWithPatternFindOptions() {
        try {
            ...
            DefineRegionOptions legacyOptions =
                    new DefineRegionOptions.Builder()
                            .setDefineAs(DefineRegionOptions.DefineAs.INSIDE_ANCHORS)
                            .build();
    ...
    void findWithAllStrategy() {
        try {
            ...
            for (Match match : matches) {
                assertTrue(match.getScore() >= 0.7, "Match score should be >= minimum similarity");
            }
    ...
    void findWithBestStrategy() {
        try {
            ...
            if (!matches.isEmpty()) {
                ...
                assertTrue(matches.size() <= 1, "BEST strategy should return at most one match");
