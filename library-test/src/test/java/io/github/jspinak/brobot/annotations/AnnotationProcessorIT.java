 ...
 *
 * <p>Since AnnotationProcessor runs automatically via ApplicationReadyEvent in Spring Boot, these
 * tests focus on verifying the results of annotation processing rather than calling processor
 * methods directly.
 */
@SpringBootTest
@ActiveProfiles("test")
...
public class AnnotationProcessorIT extends BrobotTestBase {
    ...
    @Autowired private AnnotationProcessor processor;

    @Autowired private StateService stateService;

    @Autowired private StateTransitionService transitionService;

    @Autowired private InitialStates initialStates;
    ...
    class StateProcessingVerification {
        ...
        public void testStateServiceQueryByName() {
            ...
            assertTrue(unknownState.isPresent(), "StateService should contain the UNKNOWN state");

            assertEquals(
                    "UNKNOWN",
                    unknownState.get().getName(),
        ...
        public void testStateServiceProvidesIdsAndNames() {
            ...
            assertTrue(allNames.contains("UNKNOWN"), "State names should include UNKNOWN");
        }
    }
    ...
    class RealAnnotationProcessingTests {
        ...
        public void testProcessorProcessesViaSpring() {
            // Since AnnotationProcessor runs automatically via
        ...
        public void testAnnotationProcessingOccurred() {
            ...
            assertFalse(
                    stateService.onlyTheUnknownStateExists()
                            && stateService.getAllStates().size() == 1,
                    "If only UNKNOWN state exists, either no states were processed or only default"
                            + " states exist");
    ...
    class StateAnnotationAttributesVerification {
        ...
        public void testStateAnnotationAttributes() {
            ...
            io.github.jspinak.brobot.annotations.State stateAnnotation =
                    TestState2.class.getAnnotation(
                            io.github.jspinak.brobot.annotations.State.class);
        ...
        public void testStateWithCustomProperties() {
            io.github.jspinak.brobot.annotations.State stateAnnotation =
                    TestState3.class.getAnnotation(
                            io.github.jspinak.brobot.annotations.State.class);
    ...
    class ErrorHandlingTests {
        ...
        public void testStateQueriesById() {
            ...
            if (!allStates.isEmpty()) {
                ...
                assertTrue(retrievedState.isPresent(), "Should be able to retrieve state by ID");
                assertEquals(
                        stateId,
                        retrievedState.get().getId(),
        ...
        public void testStateNameIdConversions() {
            ...
            if (!allStates.isEmpty()) {
                ...
                assertEquals(stateId, retrievedId, "State ID should match for name: " + stateName);

                // Test ID to name conversion
                String retrievedName = stateService.getStateName(stateId);
                assertEquals(
                        stateName, retrievedName, "State name should match for ID: " + stateId);
            }
        }
    }
