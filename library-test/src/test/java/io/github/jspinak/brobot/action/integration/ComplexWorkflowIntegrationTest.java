...
class ComplexWorkflowIntegrationTest extends BrobotTestBase {
    ...
    class LoginWorkflowTests {
        ...
        void shouldExecuteCompleteLoginWorkflow() {
            ...
            if (findLoginResult.isSuccess()) {
                ActionResult clickLoginResult =
                        action.perform(new ClickOptions.Builder().build(), loginButtonColl);
                ...
                ActionResult findPasswordResult =
                        action.perform(new PatternFindOptions.Builder().build(), passwordColl);
                ...
                ActionResult submitResult =
                        action.perform(new ClickOptions.Builder().build(), submitColl);
        ...
        void shouldHandleLoginWithWaitingForLoading() {
            ...
            ActionResult dashboardResult =
                    action.perform(new PatternFindOptions.Builder().build(), dashboardColl);
    ...
    class ConditionalActionChainTests {
        ...
        void shouldHandleRetryPatternInConditionalChain() {
            ...
            ConditionalActionChain chain =
                    ConditionalActionChain.find(dashboard)
                            .ifNotFoundDo(
                                    result -> {
    ...
    class ActionChainExecutorTests {
        ...
        void shouldExecuteNestedSearchChain() {
            ...
            PatternFindOptions findContainer =
                    new PatternFindOptions.Builder()
                            // .setSimilarity(0.8) // Not available on builder
                            .build();

            PatternFindOptions findButtonInContainer =
                    new PatternFindOptions.Builder()
                            // .setSimilarity(0.9) // Not available on builder
                            .build();

            ActionChainOptions chainOptions =
                    new ActionChainOptions.Builder(findContainer)
                            .setStrategy(ActionChainOptions.ChainingStrategy.NESTED)
                            .then(findButtonInContainer)
                            .build();

            ObjectCollection containerColl =
                    new ObjectCollection.Builder().withImages(loginButton).build();

            // When
            ActionResult initialResult = new ActionResult();
            ActionResult chainResult =
                    chainExecutor.executeChain(chainOptions, initialResult, containerColl);
        ...
        void shouldExecuteConfirmStrategyChain() {
            ...
            PatternFindOptions initialFind =
                    new PatternFindOptions.Builder().setSimilarity(0.7).build();

            PatternFindOptions confirmFind =
                    new PatternFindOptions.Builder()
                            // .setSimilarity(0.9) // Not available on builder
                            .build();

            ActionChainOptions chainOptions =
                    new ActionChainOptions.Builder(initialFind)
            ...
            ActionResult initialResult = new ActionResult();
            ActionResult chainResult =
                    chainExecutor.executeChain(chainOptions, initialResult, targetColl);
    ...
    class DragAndDropWorkflowTests {
        ...
        void shouldHandleComplexDragWorkflow() {
            ...
            ObjectCollection item1Coll = new ObjectCollection.Builder().withImages(item1).build();

            ObjectCollection item2Coll = new ObjectCollection.Builder().withImages(item2).build();

            ObjectCollection dropColl = new ObjectCollection.Builder().withImages(dropZone).build();
    ...
    class ParallelActionExecutionTests {
        ...
        void shouldExecuteActionsInParallel() throws InterruptedException {
            ...
            ObjectCollection coll1 = new ObjectCollection.Builder().withImages(loginButton).build();

            ObjectCollection coll2 =
                    new ObjectCollection.Builder().withImages(usernameField).build();

            ObjectCollection coll3 =
                    new ObjectCollection.Builder().withImages(passwordField).build();
            ...
            Thread t1 =
                    new Thread(
                            () -> {
                                ActionResult result =
                                        action.perform(
                                                new PatternFindOptions.Builder().build(), coll1);
            ...
            Thread t2 =
                    new Thread(
                            () -> {
                                ActionResult result =
                                        action.perform(
                                                new PatternFindOptions.Builder().build(), coll2);
                                if (result != null) completedActions.incrementAndGet();
                                latch.countDown();
                            });
            ...
            Thread t3 =
                    new Thread(
                            () -> {
                                ActionResult result =
                                        action.perform(
                                                new PatternFindOptions.Builder().build(), coll3);
                                if (result != null) completedActions.incrementAndGet();
                                latch.countDown();
                            });
    ...
    class StateTransitionWorkflowTests {
        ...
        void shouldHandleStateBasedNavigation() {
            ...
            ActionResult transitionResult = action.perform(ActionType.CLICK, submitButton);
    ...
    class ErrorRecoveryWorkflowTests {
        ...
        void shouldRecoverFromActionFailures() {
            ...
            while (retryCount.get() < 3 && !recovered.get()) {
                result = action.perform(new PatternFindOptions.Builder().build(), targetColl);
