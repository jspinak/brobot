 ...
 * Provides a mock Screen bean for tests running in mock mode.
 */
@TestConfiguration
public class MockScreenConfig {
    ...
    @Bean
    @Primary
    @ConditionalOnProperty(name = "brobot.mock.enabled", havingValue = "true", matchIfMissing = true)
    public Screen mockScreen() {
        ...
        when(mockScreen.capture(anyInt(), anyInt(), anyInt(), anyInt())).thenReturn(mockScreenImage);
