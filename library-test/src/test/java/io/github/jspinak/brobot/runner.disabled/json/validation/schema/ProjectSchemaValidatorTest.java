...
class ProjectSchemaValidatorTest {
    ...
    void validate_whenStateReferencesAreInvalid_shouldReturnErrors() {
        ...
        if (result.getErrors().stream()
                .noneMatch(e -> "Invalid state reference".equals(e.errorCode()))) {
            System.out.println(
                    "Debug: Errors found in"
                            + " validate_whenStateReferencesAreInvalid_shouldReturnErrors:");
            result.getErrors()
                    .forEach(
                            error ->
                                    System.out.println(
            ...
            boolean schemaErrorPresent =
                    result.getErrors().stream()
                            .anyMatch(
                                    e ->
                                            "Schema validation failed".equals(e.errorCode())
                                                    || "Schema validation error"
                                                            .equals(e.errorCode()));
            if (schemaErrorPresent) {
                System.out.println(
                        "Debug: Schema validation errors are present, preventing custom"
                                + " validation.");
            }
        }
