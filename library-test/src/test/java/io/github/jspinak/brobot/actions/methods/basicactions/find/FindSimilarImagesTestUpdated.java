 ...
 *
 * <p>The SIMILAR_IMAGES strategy compares images between two ObjectCollections: - First collection
 * contains base images for comparison - Second collection contains images to compare against the
 * base - Returns one Match per image in the second collection with similarity scores
 *
 * <p>This is useful for: 1. Screen state recognition - determining which known screen is displayed
 * 2. Image classification - categorizing images based on similarity 3. Change detection - finding
 * which images have changed 4. Duplicate detection - identifying similar or duplicate images
 */
...
public class FindSimilarImagesTestUpdated extends BrobotIntegrationTestBase {
    ...
    void testFindSimilarImagesBetweenCollections() {
        ...
        Pattern basePattern1 =
                new Pattern.Builder().setBufferedImage(baseImage1).setName("BasePattern1").build();

        Pattern basePattern2 =
                new Pattern.Builder().setBufferedImage(baseImage2).setName("BasePattern2").build();

        StateImage baseStateImage1 =
                new StateImage.Builder().addPattern(basePattern1).setName("BaseImage1").build();

        StateImage baseStateImage2 =
                new StateImage.Builder().addPattern(basePattern2).setName("BaseImage2").build();
        ...
        ObjectCollection baseCollection =
                new ObjectCollection.Builder().withImages(baseStateImage1, baseStateImage2).build();

        // Second collection - images to compare
        ObjectCollection compareCollection =
                new ObjectCollection.Builder()
                        .withImages(compareStateImage1, compareStateImage2, compareStateImage3)
                        .build();

        // Use SimilarImagesFindOptions
        SimilarImagesFindOptions findOptions =
                new SimilarImagesFindOptions.Builder()
                        .setSimilarity(0.7)
        ...
        for (Match match : matches) {
            assertNotNull(match);
            assertTrue(
                    match.getScore() >= 0 && match.getScore() <= 1.0,
    ...
    void testScreenRecognitionPreset() {
        ...
        ObjectCollection knownScreens =
                new ObjectCollection.Builder().withImages(knownStateImage).build();

        ObjectCollection currentScreenCapture =
                new ObjectCollection.Builder().withImages(currentStateImage).build();
        ...
        assertEquals(
                SimilarImagesFindOptions.ComparisonMethod.BEST_MATCH,
    ...
    void testMultipleBaseImages() {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            Pattern pattern =
                    new Pattern.Builder()
                            ...
                            .build();
            baseStateImages[i] =
                    new StateImage.Builder().addPattern(pattern).setName("BaseImage" + i).build();
        }
        ...
        ObjectCollection baseCollection =
                new ObjectCollection.Builder().withImages(baseStateImages).build();

        ObjectCollection compareCollection =
                new ObjectCollection.Builder().withImages(compareStateImages).build();
    ...
    void testDuplicateDetectionPreset() {
        ...
        ObjectCollection originals =
                new ObjectCollection.Builder().withImages(originalImage).build();

        ObjectCollection candidates =
                new ObjectCollection.Builder().withImages(duplicateImage).build();
        ...
        assertEquals(
                SimilarImagesFindOptions.ComparisonMethod.ALL_PATTERNS_MATCH,
                findOptions.getComparisonMethod());
    ...
    void testChangeDetectionPreset() {
        ...
        ObjectCollection beforeCollection =
                new ObjectCollection.Builder().withImages(beforeState).build();

        ObjectCollection afterCollection =
                new ObjectCollection.Builder().withImages(afterState).build();
        ...
        assertEquals(
                SimilarImagesFindOptions.ComparisonMethod.ANY_PATTERN_MATCHES,
                findOptions.getComparisonMethod());
        assertTrue(findOptions.isIncludeNoMatches());
    ...
    void testEmptyCollections() {
        ...
        ObjectCollection nonEmptyCollection =
                new ObjectCollection.Builder()
                        .withImages(
                                new StateImage.Builder()
                                        .addPattern(
        ...
        SimilarImagesFindOptions findOptions =
                new SimilarImagesFindOptions.Builder().setSimilarity(0.8).build();
    ...
    void testPatternsWithSearchRegions() {
        ...
        StateImage baseStateImage =
                new StateImage.Builder()
                        .addPattern(basePattern)
                        .setName("BaseImageWithRegion")
                        .build();

        StateImage compareStateImage =
                new StateImage.Builder()
        ...
        ObjectCollection baseCollection =
                new ObjectCollection.Builder().withImages(baseStateImage).build();

        ObjectCollection compareCollection =
                new ObjectCollection.Builder().withImages(compareStateImage).build();
