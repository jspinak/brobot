...
/**
 * Comprehensive end-to-end integration tests simulating real-world workflows. Tests complete user
 * scenarios across multiple components and states.
 */
@TestPropertySource(
        properties = {
...
class EndToEndWorkflowIntegrationTest extends BrobotIntegrationTestBase {
    ...
    @Autowired private StateAwareScheduler scheduler;
    ...
    private void createUIElements() {
        ...
        uiElements.put(
                "usernameField", createStateImage("username-field", "images/login/username.png"));
        uiElements.put(
                "passwordField", createStateImage("password-field", "images/login/password.png"));
        uiElements.put("loginButton", createStateImage("login-button", "images/login/submit.png"));

        // Dashboard elements
        uiElements.put(
                "dashboardHeader",
                createStateImage("dashboard-header", "images/dashboard/header.png"));
        uiElements.put(
                "inventoryTile",
                createStateImage("inventory-tile", "images/dashboard/inventory.png"));
        uiElements.put(
                "reportsTile", createStateImage("reports-tile", "images/dashboard/reports.png"));
        uiElements.put(
                "settingsTile", createStateImage("settings-tile", "images/dashboard/settings.png"));

        // Inventory elements
        uiElements.put(
                "inventoryGrid", createStateImage("inventory-grid", "images/inventory/grid.png"));
        ...
        uiElements.put(
                "checkoutButton",
                createStateImage("checkout-button", "images/checkout/proceed.png"));
        uiElements.put(
                "paymentForm", createStateImage("payment-form", "images/checkout/payment.png"));
    }
    ...
    private void createApplicationStates() {
        ...
        dashboardState =
                createState(
                        "Dashboard",
                        List.of(
                                uiElements.get("dashboardHeader"),
        ...
        checkoutState =
                createState(
                        "Checkout",
                        List.of(
                                uiElements.get("cartIcon"),
                                uiElements.get("checkoutButton"),
                                uiElements.get("paymentForm")));

        reportState = createState("Reports", List.of()); // Would have report-specific images

        settingsState = createState("Settings", List.of()); // Would have settings-specific images
    }
    ...
    class CompleteUserJourneyTests {
        ...
        private ActionResult performLogin() {
            ...
            ActionResult findResult =
                    action.perform(new PatternFindOptions.Builder().build(), loginColl);
        ...
        private ActionResult browseInventory() {
            ObjectCollection invColl =
                    new ObjectCollection.Builder()
                            .withImages(uiElements.get("inventoryTile"))
                            .build();

            ActionResult clickResult = action.perform(new ClickOptions.Builder().build(), invColl);

            if (clickResult.isSuccess()) {
                // Wait for inventory to load
                ObjectCollection gridColl =
                        new ObjectCollection.Builder()
        ...
        private ActionResult addItemsToCart() {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                ObjectCollection addColl =
                        new ObjectCollection.Builder()
                                .withImages(uiElements.get("addItemButton"))
                                .build();

                ActionResult addResult =
                        action.perform(new ClickOptions.Builder().build(), addColl);
        ...
        private ActionResult performCheckout() {
            ObjectCollection cartColl =
                    new ObjectCollection.Builder().withImages(uiElements.get("cartIcon")).build();

            ActionResult cartResult = action.perform(new ClickOptions.Builder().build(), cartColl);

            if (cartResult.isSuccess()) {
                ObjectCollection checkoutColl =
                        new ObjectCollection.Builder()
    ...
    class StateRecoveryTests {
        ...
        void shouldHandleStateDetectionAfterCrash() {
            ...
            assertTrue(
                    detectedState == null || stateService.getAllStates().contains(detectedState));
        }
    ...
    class WorkflowResult {
        ...
        WorkflowStep getLastStep() {
            return steps.isEmpty()
                    ? new WorkflowStep("", new ActionResult())
    ...
    class RecoveryResult {
        ...
        String getRecoveredState() {
            return recoveredState;
        }

        void setRecoveredState(String recoveredState) {
            this.recoveredState = recoveredState;
        }

        Duration getRecoveryTime() {
            return recoveryTime;
        }

        void setRecoveryTime(Duration recoveryTime) {
