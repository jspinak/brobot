...
/**
 * Monitors system resources during parallel test execution. Helps identify memory leaks, thread
...
public class ParallelExecutionMonitor {
    ...
    public void startMonitoring() {
        if (monitoring.compareAndSet(false, true)) {
            ...
            scheduler.scheduleAtFixedRate(this::logSummary, 5, 5, TimeUnit.SECONDS);
    ...
    private void generateFinalReport() {
        ...
        System.out.println(
                String.format(
                        "  Peak Heap Usage:     %.0f MB", peakMemoryUsage.get() / (1024.0 * 1024)));
        System.out.println(
                String.format(
                        "  Max Heap Available:  %.0f MB",
                        memoryBean.getHeapMemoryUsage().getMax() / (1024.0 * 1024)));
        System.out.println(
                String.format(
                        "  Peak Usage Percent:  %.1f%%",
                        (peakMemoryUsage.get() * 100.0)
        ...
        System.out.println(
                String.format("  Daemon Threads:      %d", threadBean.getDaemonThreadCount()));
        ...
        if (lastSnapshot != null) {
            ...
            System.out.println(
                    String.format("  Process CPU:         %.1f%%", lastSnapshot.processCpuLoad));
            System.out.println(
                    String.format("  System CPU:          %.1f%%", lastSnapshot.systemCpuLoad));
        }
