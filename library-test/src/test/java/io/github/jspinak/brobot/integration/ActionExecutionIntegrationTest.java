...
/**
 * Integration test for Action execution with various options and configurations. Tests the complete
 * action flow including pattern finding, clicking, typing, and dragging.
 */
...
public class ActionExecutionIntegrationTest extends IntegrationTestBase {
    ...
    private void setupTestState() {
        // Create test images
        buttonImage = new StateImage.Builder().setName("TestButton").build();

        iconImage =
                new StateImage.Builder()
                        .setName("TestIcon")
                        .setSearchRegionForAllPatterns(
                                Region.builder().withRegion(100, 100, 50, 50).build())
                        .build();

        textField =
                new StateString.Builder().setName("TextField").setString("Enter text here").build();

        // Create and register test state
        testState =
                new State.Builder("TestActionState")
    ...
    public void testClickWithOptions() {
        ...
        MousePressOptions mouseOptions =
        ...
        ObjectCollection objects = new ObjectCollection.Builder().withImages(iconImage).build();
    ...
    public void testTypeAction() {
        ...
        ObjectCollection objects = new ObjectCollection.Builder().withStrings(textToType).build();
    ...
    public void testDragAction() {
        ...
        DragOptions dragOptions =
                new DragOptions.Builder().setDelayBetweenMouseDownAndMove(0.5).build();

        ObjectCollection objects =
                new ObjectCollection.Builder().withLocations(fromLocation, toLocation).build();
    ...
    public void testMoveAction() {
        ...
        MouseMoveOptions moveOptions =
                new MouseMoveOptions.Builder().setMoveMouseDelay(0.1f).build();
    ...
    public void testFindAllMatches() {
        ...
        StateImage repeatingPattern = new StateImage.Builder().setName("RepeatingPattern").build();

        PatternFindOptions findAllOptions =
                new PatternFindOptions.Builder()
        ...
        ObjectCollection objects =
                new ObjectCollection.Builder().withImages(repeatingPattern).build();
        ...
        if (result.getMatchList().size() > 1) {
            ...
            assertNotEquals(
                    first.getRegion().getX(),
                    second.getRegion().getX(),
    ...
    public void testActionWithSearchRegion() {
        ...
        Region searchRegion = Region.builder().withRegion(50, 50, 200, 200).build();

        StateImage imageWithSearchRegion =
                new StateImage.Builder()
                        .setName("ButtonInRegion")
                        .setSearchRegionForAllPatterns(searchRegion)
                        .build();

        PatternFindOptions findInRegion = new PatternFindOptions.Builder().build();

        ObjectCollection objects =
                new ObjectCollection.Builder().withImages(imageWithSearchRegion).build();
        ...
        if (!result.getMatchList().isEmpty()) {
            ...
            assertTrue(
                    matchRegion.getX() >= searchRegion.getX(),
                    "Match X should be within search region");
            assertTrue(
                    matchRegion.getY() >= searchRegion.getY(),
    ...
    public void testCompositeAction() {
        ...
        ObjectCollection findObjects =
                new ObjectCollection.Builder().withStrings(textField).build();
        ...
        ClickOptions clickOptions =
                new ClickOptions.Builder().setPressOptions(pressOptions).build();
        ...
        TypeOptions typeOptions = new TypeOptions.Builder().setTypeDelay(0.01).build();

        ObjectCollection typeObjects =
                new ObjectCollection.Builder().withStrings("Composite action test").build();
    ...
    public void testActionWithTimeout() {
        ...
        PatternFindOptions timeoutOptions =
                new PatternFindOptions.Builder()
