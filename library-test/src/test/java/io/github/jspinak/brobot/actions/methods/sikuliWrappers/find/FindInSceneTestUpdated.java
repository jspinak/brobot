...
/**
 * Updated tests for FindInScene functionality using the new ActionConfig API. Demonstrates
 * migration from ActionOptions to DefineRegionOptions.
 *
 * <p>Key changes: - Uses DefineRegionOptions instead of generic ActionOptions for define actions -
 * ActionResult requires setActionConfig() before perform() - Uses ActionService to get the
 * appropriate action - DefineAs enum is now DefineRegionOptions.DefineAs
 */
@SpringBootTest(
...
class FindInSceneTestUpdated extends BrobotIntegrationTestBase {
    ...
    @Autowired ScenePatternMatcher findInScene;

    @Autowired SceneProvider getScenes;
    ...
    void defineWithDifferentStrategies_newAPI() {
        ...
        try {
            ...
            // Test each strategy
            for (DefineRegionOptions options :
                    List.of(insideAnchorsOptions, boundsOptions, matchesOptions)) {
                ActionResult result = new ActionResult();
    ...
    void compareOldAndNewAPI() {
        ...
        */
        ...
        // NEW API:
        DefineRegionOptions newOptions =
                new DefineRegionOptions.Builder()
                        .setDefineAs(DefineRegionOptions.DefineAs.INSIDE_ANCHORS)
                        .build();
