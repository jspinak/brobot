...
/**
 * Integration test for Action execution. Tests the action execution with the new ActionConfig API.
 */
@SpringBootTest(
...
class ActionServiceIntegrationTest {
    ...
    @Autowired private io.github.jspinak.brobot.action.Action action;

    @Autowired private Find find;

    @Autowired private Click click;

    @Autowired private TypeText typeText;
    ...
    void testBasicTypeAction() {
        ...
        ObjectCollection collection = new ObjectCollection.Builder().withStrings("test").build();
