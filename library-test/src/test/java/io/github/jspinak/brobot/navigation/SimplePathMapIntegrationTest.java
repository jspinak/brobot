...
/**
 * Simplified integration test for path map setup and path finding.
 * Uses actual API methods available in the codebase.
 */
...
class SimplePathMapIntegrationTest {
    ...
    void testSimplePathMap() {
        ...
        State start = new State.Builder("Start")
            .setPathScore(0)
            .build();
        State middle = new State.Builder("Middle")
            .setPathScore(10)
            .build();
        State end = new State.Builder("End")
            .setPathScore(0)
            .build();
        ...
        StateTransitions startTrans = new StateTransitions.Builder("Start")
            .addTransition(() -> true, "Middle")
            .build();
        startTrans.setStateId(start.getId());
        
        StateTransitions middleTrans = new StateTransitions.Builder("Middle")
            .addTransition(() -> true, "End")
            .build();
        middleTrans.setStateId(middle.getId());
        
        StateTransitions endTrans = new StateTransitions.Builder("End")
            .build();
    ...
    void testMultiplePaths() {
        ...
        // PathA goes to End
        StateTransitions pathATrans = new StateTransitions.Builder("PathA")
            .addTransition(() -> true, "End")
            .build();
        pathATrans.setStateId(pathA.getId());
        
        // PathB goes to End  
        StateTransitions pathBTrans = new StateTransitions.Builder("PathB")
            .addTransition(() -> true, "End")
            .build();
        ...
        Path pathThroughB = allPaths.stream()
            .filter(p -> p.contains(pathB.getId()))
            .findFirst()
            .orElseThrow();
        
        // Find path through A (higher score)
        Path pathThroughA = allPaths.stream()
            .filter(p -> p.contains(pathA.getId()))
            .findFirst()
            .orElseThrow();
    ...
    void testComplexGraphWithCycles() {
        ...
        // Hub -> NodeA, NodeB
        StateTransitions hubTrans = new StateTransitions.Builder("Hub")
            .addTransition(() -> true, "NodeA", "NodeB")
            .build();
        hubTrans.setStateId(hub.getId());
        
        // NodeA -> Target, Hub (cycle)
        StateTransitions nodeATrans = new StateTransitions.Builder("NodeA")
            .addTransition(() -> true, "Target", "Hub")
            .build();
        nodeATrans.setStateId(nodeA.getId());
        
        // NodeB -> Target
        StateTransitions nodeBTrans = new StateTransitions.Builder("NodeB")
            .addTransition(() -> true, "Target")
            .build();
    ...
    void testJointTableQueries() {
        ...
        // S1 -> S2 -> S3
        StateTransitions t1 = new StateTransitions.Builder("S1")
            .addTransition(() -> true, "S2")
            .build();
        t1.setStateId(s1.getId());
        
        StateTransitions t2 = new StateTransitions.Builder("S2")
            .addTransition(() -> true, "S3")
            .build();
