...
/**
 * Updated tests for finding all words in images with OCR using new ActionConfig API. Demonstrates
 * migration from ActionOptions to TextFindOptions. These tests require Tesseract OCR to be
 * installed.
 *
 * <p>Key changes: - Uses TextFindOptions instead of generic ActionOptions for OCR - TextFindOptions
 * automatically uses ActionOptions.Find.ActionOptions.Find.ALL strategy - ActionResult requires
 * setActionConfig() before perform() - Uses ActionService to get the appropriate action - Fusion
 * and area filtering handled differently in new API
 */
...
public class FindAllWordsTestUpdated extends OcrTestBase {
    ...
    void findWordsWithMinSizeNewAPI() {
        ...
        ActionResult wordMatches =
                performOcrOperation(
                        () -> {
                            ObjectCollection screens =
                            ...
                            TextFindOptions textFindOptions =
                                    new TextFindOptions.Builder()
                                            ...
                                            // Note: minArea filtering might be handled differently
                                            // in new API
                                            // Check if MatchAdjustmentOptions or post-processing is
                                            // needed
                                            .build();
                            ...
                            Optional<ActionInterface> textFindActionOpt =
                                    actionService.getAction(textFindOptions);
                            assertTrue(
                                    textFindActionOpt.isPresent(),
                                    "Text find action should be available");
        ...
        if (!wordMatches.isEmpty()) {
            ...
            if (wordMatches.getMatchList() != null && !wordMatches.getMatchList().isEmpty()) {
                System.out.println(
                        "Match list contains " + wordMatches.getMatchList().size() + " matches");
            }
        } else {
    ...
    void findWordsWithCustomSettings() {
                        ...
                        () -> {
                            ...
                            TextFindOptions textFindOptions =
                                    new TextFindOptions.Builder()
                                            ...
                                            .setSimilarity(
                                                    0.7) // Lower threshold for OCR confidence
        ...
        if (!wordMatches.isEmpty()) {
            ...
            boolean hasImages =
                    wordMatches.getMatchList().stream().anyMatch(match -> match.getMat() != null);
    ...
    void compareOldAndNewOcrAPI() {
        ...
        */
        ...
        // NEW API:
        TextFindOptions newOptions =
                new TextFindOptions.Builder().setMaxMatchRetries(3).setPauseAfterEnd(0.5).build();
        ...
        assertEquals(
                "ActionOptions.Find.ActionOptions.Find.ALL",
                newOptions.getFindStrategy().toString());
    }
    ...
    void testTextFindOptionsSpecificFeatures() {
        ...
        assertEquals(
                "ActionOptions.Find.ActionOptions.Find.ALL", options.getFindStrategy().toString());
    }
}
