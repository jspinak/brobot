 ...
 *
 * <p>Original test intended to verify: 1. DragOptions creation with custom locations and pauses 2.
 * PatternFindOptions with search regions and strategies 3. ClickOptions with mouse movement after
 * action 4. Default pause values from framework settings 5. Complex drag configurations 6. Preset
 * find strategies 7. JSON serialization of options
 *
 * <p>Rewritten to use actual available APIs: - DragOptions only has mousePressOptions,
 * delayBetweenMouseDownAndMove, and delayAfterDrag - PatternFindOptions has Strategy enum and
 * similarity settings - ClickOptions has numberOfClicks and mousePressOptions - No direct location
 * setters on DragOptions (locations come from ObjectCollection)
 */
...
public class ActionOptionsIntegrationTestUpdated extends BrobotIntegrationTestBase {
    ...
    void testActionConfigInheritance() {
        ...
        DragOptions dragOptions =
                new DragOptions.Builder().setPauseAfterEnd(1.5).setPauseBeforeBegin(0.5).build();

        // Test inherited properties
        assertEquals(1.5, dragOptions.getPauseAfterEnd(), 0.001);
        assertEquals(0.5, dragOptions.getPauseBeforeBegin(), 0.001);

        ClickOptions clickOptions = new ClickOptions.Builder().setPauseAfterEnd(2.0).build();
