...
/**
 * Full-screen transparent overlay for capturing screen regions. Similar to SikuliX IDE's capture
 * functionality.
 */
@Component
public class CaptureOverlay extends JFrame {
    ...
    @Autowired private BrobotCaptureService captureService;
    ...
    private void drawDimensions(Graphics2D g2d) {
        ...
        String dimensions = String.format("%d x %d", selectionBounds.width, selectionBounds.height);
    ...
    public void startCapture() {
        ...
        try {
            ...
            if (screenshot.getWidth() > screenSize.width
                    || screenshot.getHeight() > screenSize.height) {
    ...
    private BufferedImage createDarkenedImage(BufferedImage original) {
        ...
        g2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, DARKEN_FACTOR));
