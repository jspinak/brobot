...
/**
 * Demonstrates form automation with comprehensive logging. Shows best practices for logging
...
public class FormAutomation {
    ...
    private boolean executeFormFilling(Map<String, String> formData) {
        ...
        ObjectCollection firstNameCollection =
                new ObjectCollection.Builder().withImages(firstNameField).build();

        ActionResult firstNameResult =
                action.perform(
        ...
        action.perform(
                new ClickOptions.Builder().setPauseAfterEnd(0.2).build(), firstNameCollection);
        action.perform(
        ...
        if (submitResult.isSuccess()) {
            action.perform(
                    new ClickOptions.Builder().setPauseAfterEnd(1.0).build(), submitCollection);
    ...
    private void attemptFormRecovery(Map<String, String> formData) {
        ...
        StateImage formIndicator =
                new StateImage.Builder().setName("FormTitle").addPatterns("form_title.png").build();

        ObjectCollection formCheckObjects =
                new ObjectCollection.Builder().withImages(formIndicator).build();

        ActionResult formCheck =
                action.perform(
    ...
    private boolean fillField(String fieldPattern, String value, String fieldName) {
        ...
        ObjectCollection fieldObjects = new ObjectCollection.Builder().withImages(field).build();

        // Find field
        ActionResult findResult =
                action.perform(
                        new PatternFindOptions.Builder().setSimilarity(0.8).build(), fieldObjects);
        ...
        action.perform(
                ...
                new ObjectCollection.Builder().withStrings("^a", "{DELETE}").build());
    ...
    private void submitForm() {
        ...
        ActionResult result =
                action.perform(
                        new ClickOptions.Builder().setPauseAfterEnd(2.0).build(), submitObjects);
    ...
    /**
     * Form automation example directly from the documentation. Shows action chaining with proper
     * logging. From: /docs/03-core-library/guides/automatic-action-logging.md
     */
    public void fillAndSubmitFormViaActionChain(
            StateImage firstNameField, StateImage submitButton) {
        log.info("Starting form automation via action chain");

        // Create form data
        ObjectCollection formData =
        ...
        PatternFindOptions findFirstName = new PatternFindOptions.Builder().build();

        ActionChainOptions fillForm =
                new ActionChainOptions.Builder(findFirstName)
