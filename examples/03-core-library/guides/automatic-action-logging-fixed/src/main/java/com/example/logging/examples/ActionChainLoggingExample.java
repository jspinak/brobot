 ...
 * 
 * Key features shown:
 * - Step-by-step logging in action chains
 * - Conditional logging based on action results
 * - MDC (Mapped Diagnostic Context) for structured logging
 * - Performance timing for chains
 */
@Component
...
public class ActionChainLoggingExample {
    ...
    private final StateImage errorMessage = new StateImage.Builder()
    ...
    public void loginChainWithLogging() {
        ...
        ActionResult loginFindResult = action.perform(
        ...
        if (loginFindResult.isSuccess()) {
            ...
            ActionResult loginClickResult = action.perform(
        ...
        if (success) {
            ...
            ActionResult usernameFindResult = action.perform(
        ...
        if (success) {
            ...
            ActionResult passwordFindResult = action.perform(
                new PatternFindOptions.Builder().build(),
                passwordCollection
            );
