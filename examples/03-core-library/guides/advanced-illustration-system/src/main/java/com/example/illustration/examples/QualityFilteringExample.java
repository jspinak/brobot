...
/**
 * Demonstrates quality-based filtering for illustrations. Shows how to focus on meaningful,
 * high-quality visualizations in Brobot v1.1.0.
 */
@Component
@Slf4j
public class QualityFilteringExample {
    ...
    public void demonstrateQualityThresholds() {
        ...
        for (int i = 0; i < testElements.length; i++) {
            ...
            ObjectCollection collection =
                    new ObjectCollection.Builder().withImages(testElements[i]).build();
                    ...
                    result.getBestMatch()
                            .ifPresent(
                                    match -> {
                                        ...
                                        log.info(
                                                "Found with threshold {}: score = {}",
                                                threshold,
                                                matchScore);

                                        // Only illustrate high-quality matches
                                        if (matchScore >= 0.85
                                                && illustrationController.okToIllustrate(
                                                        findOptions, collection)) {
    ...
    public void demonstrateCriticalElements() {
        ...
        for (StateImage critical : criticalElements) {
            ...
            ObjectCollection collection =
                    new ObjectCollection.Builder().withImages(critical).build();
            ...
            if (result.isSuccess()) {
                ...
                if (illustrationController.okToIllustrate(findOptions, collection)) {
                    ...
                    illustrationController.illustrateWhenAllowed(
                            result, new ArrayList<>(), findOptions, collection);
                }
                ...
                if (illustrationController.okToIllustrate(clickOptions, collection)) {
                    illustrationController.illustrateWhenAllowed(
                            clickResult, new ArrayList<>(), clickOptions, collection);
                }
            } else {
    ...
    public void demonstrateContextAwareFiltering() {
        ...
        for (String context : contexts) {
            ...
            ObjectCollection collection =
                    new ObjectCollection.Builder().withImages(contextualElement).build();
            ...
            if (result.isSuccess()) {
                ...
                if (shouldIllustrate
                        && illustrationController.okToIllustrate(findOptions, collection)) {
    ...
    public void demonstrateMultiMatchAnalysis() {
        ...
        ObjectCollection collection =
                new ObjectCollection.Builder().withImages(repeatingElement).build();
        ...
        if (result.isSuccess()) {
            ...
            if (variance > 0.2
                    && illustrationController.okToIllustrate(findAllOptions, collection)) {
                log.info("Significant quality variance detected - illustrating");
                illustrationController.illustrateWhenAllowed(
                        result, new ArrayList<>(), findAllOptions, collection);
            }
        }
    }
