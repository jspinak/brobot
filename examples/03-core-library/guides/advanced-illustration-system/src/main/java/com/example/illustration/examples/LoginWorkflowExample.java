...
/**
 * Demonstrates illustration configuration for a login workflow. Shows how the
 * IllustrationController works with authentication steps in Brobot v1.1.0.
 */
@Component
@Slf4j
public class LoginWorkflowExample {
    ...
    public void executeLogin(String username, String password) {
        ...
        ActionResult loginClick =
                action.perform(new ClickOptions.Builder().build(), loginCollection);

        // Illustrate the login click if allowed
        if (illustrationController.okToIllustrate(
                new ClickOptions.Builder().build(), loginCollection)) {
        ...
        if (loginClick.isSuccess()) {
            ...
            ActionResult usernameClick =
                    action.perform(new ClickOptions.Builder().build(), usernameCollection);
            if (illustrationController.okToIllustrate(
                    new ClickOptions.Builder().build(), usernameCollection)) {
                illustrationController.illustrateWhenAllowed(
                        usernameClick,
                        new ArrayList<>(),
                        new ClickOptions.Builder().build(),
            ...
            ActionResult passwordClick =
                    action.perform(new ClickOptions.Builder().build(), passwordCollection);
            if (illustrationController.okToIllustrate(
                    new ClickOptions.Builder().build(), passwordCollection)) {
                illustrationController.illustrateWhenAllowed(
                        passwordClick,
                        new ArrayList<>(),
                        new ClickOptions.Builder().build(),
            ...
            if (illustrationController.okToIllustrate(
                    new ClickOptions.Builder().build(), submitCollection)) {
                illustrationController.illustrateWhenAllowed(
                        submitResult,
                        new ArrayList<>(),
                        new ClickOptions.Builder().build(),
            ...
            if (errorCheck.isSuccess()) {
                ...
                if (illustrationController.okToIllustrate(
                        new PatternFindOptions.Builder().build(), errorCollection)) {
                    illustrationController.illustrateWhenAllowed(
                            errorCheck,
                            new ArrayList<>(),
                ...
                ActionResult dashboardCheck =
                        action.perform(
                                new PatternFindOptions.Builder().build(), dashboardCollection);
    ...
    private void performDashboardOperations() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            ObjectCollection menuCollection =
                    new ObjectCollection.Builder().withImages(menuItem).build();

            ActionResult menuClick =
                    action.perform(new ClickOptions.Builder().build(), menuCollection);

            // Only illustrate if the action fails (for demonstration)
            if (!menuClick.isSuccess()) {
                if (illustrationController.okToIllustrate(
                        new ClickOptions.Builder().build(), menuCollection)) {
                    illustrationController.illustrateWhenAllowed(
                            menuClick,
                            new ArrayList<>(),
                            new ClickOptions.Builder().build(),
