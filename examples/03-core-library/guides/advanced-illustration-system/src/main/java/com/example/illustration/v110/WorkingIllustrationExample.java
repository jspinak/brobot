 ...
 *
 * <p>The IllustrationController in v1.1.0 provides: - okToIllustrate() - Check if illustration
 * should happen - illustrateWhenAllowed() - Create illustration if allowed - No setConfig() method
 * - configuration is via application properties
 */
@Component
...
public class WorkingIllustrationExample {
    ...
    public void demonstrateIllustrationChecking() {
        ...
        // Create test objects
        StateImage button =
                new StateImage.Builder().setName("TestButton").addPatterns("button").build();

        ObjectCollection buttonCollection =
        ...
        boolean shouldIllustrateFInd =
                illustrationController.okToIllustrate(findOptions, buttonCollection);
        ...
        boolean shouldIllustrateClick =
                illustrationController.okToIllustrate(clickOptions, buttonCollection);
    ...
    public void demonstrateManualIllustration() {
        ...
        StateImage loginButton =
                new StateImage.Builder().setName("LoginButton").addPatterns("login_button").build();
        ...
        if (result.isSuccess()) {
            ...
            boolean illustrated =
                    illustrationController.illustrateWhenAllowed(
                            result, searchRegions, findConfig, loginCollection);
    ...
    public void demonstrateIllustrationFiltering() {
        ...
        StateImage element =
                new StateImage.Builder().setName("RepeatedElement").addPatterns("element").build();

        ObjectCollection elementCollection =
                new ObjectCollection.Builder().withImages(element).build();
        ...
        for (int i = 0; i < 5; i++) {
            ...
            ActionResult result =
                    action.perform(new PatternFindOptions.Builder().build(), elementCollection);

            // Check if this would be illustrated
            PatternFindOptions findConfig = new PatternFindOptions.Builder().build();
            boolean wouldIllustrate =
                    illustrationController.okToIllustrate(findConfig, elementCollection);
    ...
    public void demonstrateBestPractices() {
        ...
        StateImage autoButton =
                new StateImage.Builder().setName("AutoButton").addPatterns("auto_button").build();

        // This will automatically illustrate based on settings
        action.perform(
                new ClickOptions.Builder().build(),
                new ObjectCollection.Builder().withImages(autoButton).build());

        // 2. Use ActionConfig.illustrate for explicit control
        log.info("\n2. Explicit control with ActionConfig:");
        ClickOptions forceIllustrate =
                new ClickOptions.Builder()
                        .setIllustrate(ActionConfig.Illustrate.YES) // Force illustration
                        .build();

        action.perform(
                forceIllustrate, new ObjectCollection.Builder().withImages(autoButton).build());

        // 3. Disable illustration for specific actions
        log.info("\n3. Disable illustration:");
        ClickOptions noIllustrate =
                new ClickOptions.Builder()
                        .setIllustrate(ActionConfig.Illustrate.NO) // Prevent illustration
                        .build();

        action.perform(noIllustrate, new ObjectCollection.Builder().withImages(autoButton).build());
    }
    ...
    public void demonstrateIllustrationState() {
        ...
        log.info("Last collections count: {}", illustrationController.getLastCollections().size());

        // Action permissions from configuration
        log.info("\nAction permissions:");
        illustrationController
                .getActionPermissions()
                .forEach((action, allowed) -> log.info("  {} : {}", action, allowed));
    }
