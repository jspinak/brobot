...
/**
 * Demonstrates performance optimization strategies for the illustration system. Shows how to use
 * IllustrationController efficiently in high-volume scenarios.
 */
@Component
@Slf4j
public class PerformanceOptimizationExample {
    ...
    public PerformanceOptimizationExample(
            Action action, IllustrationController illustrationController) {
        this.action = action;
    ...
    public void demonstrateAdaptiveSampling() {
        ...
        for (int i = 0; i < 100; i++) {
            Region targetRegion =
                    new Region(random.nextInt(1600) + 100, random.nextInt(800) + 100, 50, 50);

            ObjectCollection moveTarget =
                    new ObjectCollection.Builder().withRegions(targetRegion).build();

            ActionResult moveResult =
                    action.perform(new MouseMoveOptions.Builder().build(), moveTarget);
            ...
            if (i % 10 == 0 && illustrationController.okToIllustrate(moveOptions, moveTarget)) {
                illustrationController.illustrateWhenAllowed(
                        moveResult, new ArrayList<>(), moveOptions, moveTarget);
            }
        }
    ...
    public void demonstrateBatchedOperations() {
        ...
        for (int batch = 0; batch < dataItems.size() / batchSize; batch++) {
            ...
            for (int i = batch * batchSize;
                    i < (batch + 1) * batchSize && i < dataItems.size();
                    i++) {
                ObjectCollection itemCollection =
                        new ObjectCollection.Builder().withImages(dataItems.get(i)).build();

                ActionResult result =
                        action.perform(new PatternFindOptions.Builder().build(), itemCollection);
        ...
        for (int scenario = 0; scenario < 3; scenario++) {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                ActionResult searchResult =
                        action.perform(new ClickOptions.Builder().build(), searchCollection);
                ...
                if (shouldIllustrate
                        && illustrationController.okToIllustrate(clickOptions, searchCollection)) {
                    illustrationController.illustrateWhenAllowed(
                            searchResult, new ArrayList<>(), clickOptions, searchCollection);
                }
    ...
    public void demonstrateSmartCaching() {
        ...
        StateImage cachedElement =
                new StateImage.Builder()
                        .setName("CachedElement")
                        .addPatterns("ui-elements/cached-element")
                        .build();

        ObjectCollection cachedCollection =
                new ObjectCollection.Builder().withImages(cachedElement).build();

        log.info("Performing repeated checks on the same element...");

        for (int i = 0; i < 20; i++) {
            ActionResult result =
                    action.perform(new PatternFindOptions.Builder().build(), cachedCollection);
            ...
            if ((isFirstOccurrence || isFailed || isPeriodicCheck)
                ...
                log.debug(
                        "Illustrating check {} (first: {}, failed: {}, periodic: {})",
                        i,
                        isFirstOccurrence,
                        isFailed,
                        isPeriodicCheck);
                illustrationController.illustrateWhenAllowed(
                        result, new ArrayList<>(), findOptions, cachedCollection);
            }
        }
