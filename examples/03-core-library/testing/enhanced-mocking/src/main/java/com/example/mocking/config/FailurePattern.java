...
public class FailurePattern {
    ...
    @Builder.Default private double failureRate = 0.1;
    ...
    @Builder.Default
    private java.util.Map<io.github.jspinak.brobot.action.ActionType, Double> cascadesToActions =
    ...
    @Builder.Default private boolean cascading = false;

    // Multiplier for cascading failures
    @Builder.Default private double cascadeMultiplier = 1.5;

    // Maximum consecutive failures before forced success
    @Builder.Default private int maxConsecutiveFailures = 3;
    ...
    @Builder.Default private Class<? extends Exception> exceptionType = RuntimeException.class;

    // Probability decay per failure occurrence
    @Builder.Default private double probabilityDecay = 0.1;

    // Current state (mutable during execution)
    @Builder.Default private transient int currentConsecutiveFailures = 0;
    @Builder.Default private transient double currentProbability = 0;
    @Builder.Default private transient long lastFailureTime = 0;
    ...
    public double getCurrentFailureProbability() {
        ...
        if (cascading && currentConsecutiveFailures > 0) {
            return Math.min(
                    1.0,
                    currentProbability * Math.pow(cascadeMultiplier, currentConsecutiveFailures));
        }
