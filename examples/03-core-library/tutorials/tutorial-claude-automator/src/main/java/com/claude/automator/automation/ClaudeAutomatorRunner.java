 ...
 *
 * <p>Key features shown: - State-based automation - Automatic state transitions - Declarative
 * search regions that update dynamically - Documentation examples from
 * /docs/03-core-library/tutorials/tutorial-claude-automator/
 */
@Component
...
public class ClaudeAutomatorRunner implements CommandLineRunner {
    ...
    private void monitorClaude() throws InterruptedException {
        ...
        // Check initial state
        ObjectCollection promptCollection =
                new ObjectCollection.Builder().withImages(promptState.getClaudePrompt()).build();
        if (action.perform(new PatternFindOptions.Builder().build(), promptCollection)
                .isSuccess()) {
            ...
            ObjectCollection workingCollection =
                    new ObjectCollection.Builder().withImages(workingState.getClaudeIcon()).build();
            if (action.perform(new PatternFindOptions.Builder().build(), workingCollection)
                    .isSuccess()) {
                log.info("Claude has transitioned to Working state!");
                log.info(
                        "Note: The icon was found using a search region calculated from the prompt"
                                + " location");
            }
        } else {
    ...
    /**
     * Run documentation examples if they are available. From:
     * /docs/03-core-library/tutorials/tutorial-claude-automator/automation.md
     */
