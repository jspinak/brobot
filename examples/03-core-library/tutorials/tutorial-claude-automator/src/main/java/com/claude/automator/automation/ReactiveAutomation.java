...
/**
 * Reactive Approach example from documentation. From:
 * /docs/03-core-library/tutorials/tutorial-claude-automator/automation.md
 *
 * <p>This demonstrates reactive monitoring using Project Reactor. Note: This is conditional on
 * Reactor being available in the classpath.
 */
@Component
...
public class ReactiveAutomation {
    ...
    /**
     * Monitor Claude icon using reactive streams. Returns a Flux that emits true when the icon is
     * missing.
     */
    ...
    /** Check if the Claude icon is missing. Returns true if icon is NOT found (i.e., missing). */
    private boolean checkIconStatus() {
        try {
            PatternFindOptions findOptions =
                    new PatternFindOptions.Builder().setPauseBeforeBegin(0.1).build();

            boolean iconFound =
                    action.perform(findOptions, workingState.getClaudeIcon()).isSuccess();
    ...
    public Flux<Boolean> monitorWithErrorRecovery() {
        return Flux.interval(Duration.ofSeconds(2))
                ...
                .onErrorContinue(
                        (throwable, obj) -> {
                            log.warn(
                                    "Error during monitoring tick, continuing: {}",
                ...
                .retryWhen(
                        reactor.util.retry.Retry.backoff(3, Duration.ofSeconds(5))
                                .doBeforeRetry(
                                        retrySignal ->
                                                log.info(
                                                        "Retrying reactive monitoring, attempt: {}",
