...
public class PureActionsDemo {
    ...
    /**
     * Old Way vs New Way comparison Code from:
     * docs/docs/01-getting-started/pure-actions-quickstart.md lines 14-38
     */
    public void demonstrateOldVsNew() {
        log.info("=== Old Way vs New Way ===");

        StateImage buttonImage = new StateImage.Builder().addPatterns("submit-button").build();
        ...
        ConditionalActionChain.find(new PatternFindOptions.Builder().build())
                ...
                .perform(action, new ObjectCollection.Builder().withImages(buttonImage).build());
    }

    /**
     * Common Use Case 1: Click a Button Code from:
     * docs/docs/01-getting-started/pure-actions-quickstart.md lines 68-77
     */
    public void clickButtonExample() {
        ...
        ConditionalActionChain.find(new PatternFindOptions.Builder().build())
                .ifFoundClick()
                .perform(action, new ObjectCollection.Builder().withImages(submitButton).build());
    }

    /**
     * Common Use Case 2: Type in a Field Code from:
     * docs/docs/01-getting-started/pure-actions-quickstart.md lines 80-89
     */
    public void typeInFieldExample() {
        ...
        StateImage textField = new StateImage.Builder().addPatterns("text-field").build();

        ConditionalActionChain.find(new PatternFindOptions.Builder().build())
                .ifFoundClick()
                .then(new TypeOptions.Builder().setTypeDelay(0.1).build())
                .perform(
                        action,
    ...
    public void highlightFoundElements() {
        ...
        StateImage targetPattern = new StateImage.Builder().addPatterns("target-element").build();
    ...
    public void rightClickMenuExample() {
        ...
        StateImage targetElement = new StateImage.Builder().addPatterns("target-element").build();

        StateImage deleteOption = new StateImage.Builder().addPatterns("delete-option").build();
    ...
    public void workWithResults() {
        ...
        StateImage targetImage = new StateImage.Builder().addPatterns("multi-element").build();
    ...
    public void demonstrateErrorHandling() {
        ...
        StateImage criticalButton = new StateImage.Builder().addPatterns("critical-button").build();

        ActionResult result =
                action.perform(
                        new PatternFindOptions.Builder().build(),
                        new ObjectCollection.Builder().withImages(criticalButton).build());

        if (result.isSuccess()) {
            action.perform(
                    new ClickOptions.Builder().build(),
                    new ObjectCollection.Builder().withMatches(result).build());
        } else {
    ...
    public void handleBothSuccessAndFailure() {
        ...
        StateImage saveButton = new StateImage.Builder().addPatterns("save-button").build();
    ...
    /**
     * Best Practice: Reuse find results Code from:
     * docs/docs/01-getting-started/pure-actions-quickstart.md lines 199-212
     */
    public void reuseFindResults() {
        log.info("=== Reuse Find Results ===");

        StateImage allButtons = new StateImage.Builder().addPatterns("button-pattern").build();
        ...
        for (Match button : buttons.getMatchList()) {
            ...
            action.perform(
                    highlight,
                    new ObjectCollection.Builder().withRegions(button.getRegion()).build());
    ...
    /**
     * Login Flow Example using Conditional Chains Code from:
     * docs/docs/03-core-library/action-config/15-conditional-chains-examples.md lines 38-56
     */
    public ActionResult performLogin(String username, String password) {
        log.info("=== Login Flow Example ===");

        StateImage loginButton = new StateImage.Builder().addPatterns("login-button").build();

        StateImage usernameField = new StateImage.Builder().addPatterns("username-field").build();

        StateImage passwordField = new StateImage.Builder().addPatterns("password-field").build();
        ...
        return ConditionalActionChain.find(new PatternFindOptions.Builder().build())
                ...
                .perform(action, new ObjectCollection.Builder().withImages(loginButton).build());
    }

    /**
     * Save with Confirmation Dialog Example Code from:
     * docs/docs/03-core-library/action-config/15-conditional-chains-examples.md lines 62-76
     */
    public ActionResult saveWithConfirmation() {
        ...
        StateImage confirmDialog = new StateImage.Builder().addPatterns("confirm-dialog").build();

        return ConditionalActionChain.find(new PatternFindOptions.Builder().build())
                .ifFoundClick()
                .ifNotFoundLog("Save button not found")
                .perform(action, new ObjectCollection.Builder().withImages(saveButton).build());
    }
