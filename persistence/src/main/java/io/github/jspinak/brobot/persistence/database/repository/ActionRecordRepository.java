...
public interface ActionRecordRepository extends JpaRepository<ActionRecordEntity, Long> {
    ...
    @Query(
            "SELECT a FROM ActionRecordEntity a WHERE a.session.id = :sessionId ORDER BY"
                    + " a.timestamp")
    ...
    @Query(
            "SELECT a FROM ActionRecordEntity a WHERE a.session.id = :sessionId AND a.actionSuccess"
                    + " = :success")
    List<ActionRecordEntity> findBySessionIdAndSuccess(
            @Param("sessionId") Long sessionId, @Param("success") boolean success);

    @Query(
            "SELECT a FROM ActionRecordEntity a WHERE a.timestamp >= :startTime AND a.timestamp <="
                    + " :endTime")
    List<ActionRecordEntity> findByTimeRange(
    ...
    @Query(
            "SELECT COUNT(a) FROM ActionRecordEntity a WHERE a.session.id = :sessionId AND"
                    + " a.actionSuccess = true")
