...
/**
 * Spring Boot auto-configuration for Brobot persistence.
 * Automatically configures persistence based on application properties.
 */
@Configuration
    ...
    public PersistenceProvider persistenceProvider(
        ...
        if (configuration.getType() == PersistenceConfiguration.PersistenceType.DATABASE) {
            if (sessionRepository == null || recordRepository == null) {
                log.error("Database persistence requires JPA repositories but they are not available");
                throw new IllegalStateException("Database persistence requires Spring Data JPA");
            }
            return new DatabasePersistenceProvider(configuration, sessionRepository, recordRepository);
        }
