...
public interface RecordingSessionRepository extends JpaRepository<RecordingSessionEntity, Long> {
    ...
    @Query("SELECT s FROM RecordingSessionEntity s WHERE s.startTime >= :startTime AND s.endTime <= :endTime ORDER BY s.startTime DESC")
    List<RecordingSessionEntity> findByDateRange(LocalDateTime startTime, LocalDateTime endTime);
    
    @Query("SELECT s FROM RecordingSessionEntity s WHERE s.status = 'ACTIVE' OR s.status = 'PAUSED'")
