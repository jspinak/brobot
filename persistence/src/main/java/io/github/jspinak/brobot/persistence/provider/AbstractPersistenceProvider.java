...
public abstract class AbstractPersistenceProvider implements PersistenceProvider {
    ...
    public String stopSession() {
        ...
        log.info("Stopped recording session: {} ({} actions)", 
                 currentSessionId, currentSession != null ? currentSession.getTotalActions() : 0);
    ...
    private void recordAsync(ActionRecord record, StateObject stateObject) {
        try {
            if (!recordQueue.offer(new RecordTask(currentSessionId, record, stateObject), 
    ...
    protected abstract void doRecordAction(String sessionId, ActionRecord record, StateObject stateObject);
