 ...
 * Stores all data in memory - useful for testing and temporary sessions.
 * Optionally can export to file on shutdown.
 */
@Slf4j
public class InMemoryPersistenceProvider extends AbstractPersistenceProvider {
    ...
    protected void doStopSession(String sessionId, SessionMetadata metadata) {
        ...
        if (session != null) {
            ...
            session.metadata = metadata;
            log.debug("Stopped in-memory session: {} with {} records", 
                     sessionId, session.records.size());
        }
    }
    ...
    protected void doRecordAction(String sessionId, ActionRecord record, StateObject stateObject) {
        ...
        if (session != null) {
            ...
            log.trace("Recorded action to session {} (total: {})", 
                     sessionId, session.records.size());
        }
    }
    ...
    public ActionHistory exportSession(String sessionId) {
        ...
        log.info("Exported in-memory session {} with {} records", 
                sessionId, session.records.size());
        return history;
    }
    ...
    public String importSession(ActionHistory history, String sessionName) {
        ...
        metadata.setSuccessfulActions((int) history.getSnapshots().stream()
    ...
    public MemoryStatistics getMemoryStatistics() {
        ...
        stats.totalRecords = sessions.values().stream()
    ...
    public Map<String, List<ActionRecord>> getAllSessionData() {
        ...
        for (Map.Entry<String, InMemorySession> entry : sessions.entrySet()) {
            List<ActionRecord> records = entry.getValue().records.stream()
                .map(r -> r.record)
    ...
    private void enforceSessionLimit() {
        ...
        while (sessionOrder.size() > maxSessions) {
            ...
            if (removed != null) {
                log.debug("Evicted old session {} to maintain limit of {} sessions", 
                         oldestSession, maxSessions);
            }
        }
    }
    ...
    private void persistOnShutdown() {
        ...
        try {
            ...
            FileBasedPersistenceProvider fileProvider = new FileBasedPersistenceProvider(fileConfig);
            
            // Export each session
            for (Map.Entry<String, InMemorySession> entry : sessions.entrySet()) {
                ActionHistory history = exportSession(entry.getKey());
                if (history != null) {
                    String sessionName = entry.getValue().metadata.getName() + "_shutdown_" + 
    ...
    public static class MemoryStatistics {
        ...
        public String toString() {
            return String.format("Sessions: %d, Records: %d, Est. Memory: %.2f MB",
