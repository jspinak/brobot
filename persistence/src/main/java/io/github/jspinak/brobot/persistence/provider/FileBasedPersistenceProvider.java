 ...
 * Stores action records in JSON or CSV files without requiring a database.
 */
@Slf4j
    ...
    private void writeJsonRecords(Path path, List<ActionRecord> records) throws IOException {
        try (OutputStream os = createOutputStream(path);
             Writer writer = new OutputStreamWriter(os, 
                 Charset.forName(configuration.getFile().getEncoding()))) {
    ...
    private void writeCsvRecords(Path path, List<ActionRecord> records) throws IOException {
        try (OutputStream os = createOutputStream(path);
             ...
             CSVPrinter csvPrinter = new CSVPrinter(writer, CSVFormat.DEFAULT
                 .withHeader("Timestamp", "ActionType", "Success", "Duration", 
                            "State", "Object", "Text", "MatchCount"))) {
            
            for (ActionRecord record : records) {
                csvPrinter.printRecord(
                    LocalDateTime.now(),
                    record.getActionConfig() != null ? record.getActionConfig().getClass().getSimpleName() : "",
    ...
    private List<ActionRecord> readJsonRecords(Path path) throws IOException {
        try (InputStream is = createInputStream(path)) {
            return objectMapper.readValue(is, 
                objectMapper.getTypeFactory().constructCollectionType(List.class, ActionRecord.class));
        }
    }
