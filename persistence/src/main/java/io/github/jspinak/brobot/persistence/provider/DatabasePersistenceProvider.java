...
/**
 * Database persistence provider using JPA.
 * Extracted from the runner module to make it reusable.
 */
@Slf4j
public class DatabasePersistenceProvider extends AbstractPersistenceProvider {
    ...
    protected void doStopSession(String sessionId, SessionMetadata metadata) {
        ...
        if (entity != null) {
            ...
            log.debug("Finalized database session: {} (total actions: {})", 
                     sessionId, entity.getTotalActions());
        }
    }
    ...
    protected void doRecordAction(String sessionId, ActionRecord record, StateObject stateObject) {
        ...
        session.setSuccessRate((double) session.getSuccessfulActions() / session.getTotalActions() * 100);
    ...
    public ActionHistory exportSession(String sessionId) {
        ...
        List<ActionRecordEntity> records = recordRepository.findBySessionIdOrderByTimestamp(session.getId());
    ...
    public String importSession(ActionHistory history, String sessionName) {
        ...
        session.setSuccessfulActions((int) history.getSnapshots().stream()
            .filter(ActionRecord::isActionSuccess)
            .count());
        session.setFailedActions(session.getTotalActions() - session.getSuccessfulActions());
        session.setSuccessRate(session.getTotalActions() > 0 ? 
            (double) session.getSuccessfulActions() / session.getTotalActions() * 100 : 0);
        ...
        log.info("Imported session {} with {} records to database", sessionName, history.getSnapshots().size());
        return sessionId;
    }
        ...
        if (entity.getMatches() != null) {
            for (MatchEntity matchEntity : entity.getMatches()) {
                Match match = new Match.Builder()
                    .setRegion(matchEntity.getX(), matchEntity.getY(), 
                              matchEntity.getWidth(), matchEntity.getHeight())
