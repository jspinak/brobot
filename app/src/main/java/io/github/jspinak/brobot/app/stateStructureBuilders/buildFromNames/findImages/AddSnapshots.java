...
public class AddSnapshots {
     ...
     */
    public void ifNeededAddSnapshot(ExtendedStateImageDTO image, List<Match> matches,
                                    List<AttributeTypes.Attribute> attributes) {
        //Report.println("size of matches = "+matches.size());
        if (!imageShouldExist(attributes)) return;
        if (matches.isEmpty()) image.getStateImage().getAllMatchSnapshots().add(new MatchSnapshot());
        else image.getStateImage().getAllMatchSnapshots().add(getSnapshot(matches));
    }
