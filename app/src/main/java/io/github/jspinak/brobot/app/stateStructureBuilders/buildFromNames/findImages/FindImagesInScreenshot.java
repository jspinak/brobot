...
import io.github.jspinak.brobot.app.stateStructureBuilders.buildFromNames.attributes.PrintAttribute;
import io.github.jspinak.brobot.app.stateStructureBuilders.buildFromNames.attributes.UseAttribute;
...
public class FindImagesInScreenshot {
    ...
    public List<Match> findIn(ExtendedStateImageDTO image, String filename, int page, ImageGroup imageGroup) {
    ...
    private List<Match> getMatches(ExtendedStateImageDTO image, String filename) {
        File file = new File(BrobotSettings.screenshotPath + filename);
        String path = file.getAbsolutePath();
        List<Match> matches = new ArrayList<>();
        image.getStateImage().getPatterns().forEach(p -> {
            Finder f = new Finder(path);
            f.findAll(p.sikuli());
            while (f.hasNext()) matches.add(new Match.Builder().setSikuliMatch(f.next()).build());
            f.destroy();
        });
        return matches;
    }
