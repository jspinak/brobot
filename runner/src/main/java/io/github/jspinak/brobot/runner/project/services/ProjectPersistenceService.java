...
/**
 * Service responsible for persisting and loading automation projects.
 * Handles all file I/O operations for project data.
 */
@Slf4j
@Service
public class ProjectPersistenceService implements DiagnosticCapable {
    ...
    public void saveProjectAs(ProjectDefinition project, Path projectPath) {
        ...
        try {
            ...
            eventBus.publish(LogEvent.info(this, "Project saved: " + project.getName(), "Persistence"));
            
        } catch (IOException e) {
            log.error("Failed to save project: {}", project.getName(), e);
            eventBus.publish(LogEvent.error(this, "Failed to save project: " + e.getMessage(), "Persistence", e));
    ...
    public Optional<ProjectDefinition> loadProject(Path projectPath) {
        ...
        try {
            ProjectDefinition project = objectMapper.readValue(projectFile.toFile(), ProjectDefinition.class);
            ...
            eventBus.publish(LogEvent.info(this, "Project loaded: " + project.getName(), "Persistence"));
            
            return Optional.of(project);
            
        } catch (IOException e) {
            log.error("Failed to load project from: {}", projectPath, e);
            eventBus.publish(LogEvent.error(this, "Failed to load project: " + e.getMessage(), "Persistence", e));
            return Optional.empty();
        }
    }
    ...
    public boolean deleteProject(Path projectPath) {
        ...
        try {
            ...
            eventBus.publish(LogEvent.error(this, "Failed to delete project: " + e.getMessage(), "Persistence", e));
            return false;
        }
    }
     ...
     */
    private void copyProjectResources(ProjectDefinition project, Path targetPath) throws IOException {
        ...
        if (project.getConfigPath() != null && !project.getConfigPath().equals(targetPath.resolve("config"))) {
            copyDirectory(project.getConfigPath(), targetPath.resolve("config"));
        }
        
        // Copy image files if source differs from target
        if (project.getImagePath() != null && !project.getImagePath().equals(targetPath.resolve("images"))) {
            copyDirectory(project.getImagePath(), targetPath.resolve("images"));
        }
        
        // Copy data files if source differs from target
        if (project.getDataPath() != null && !project.getDataPath().equals(targetPath.resolve("data"))) {
