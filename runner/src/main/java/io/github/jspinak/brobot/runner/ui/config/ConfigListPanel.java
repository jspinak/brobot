...
/**
 * Configuration list panel component.
 * Single responsibility: Displays and manages the list of configurations.
 */
@Slf4j
public class ConfigListPanel extends VBox {
    ...
    public ConfigListPanel(EventBus eventBus) {
        ...
        // Bind selection
        configTable.getSelectionModel().selectedItemProperty().addListener((obs, oldVal, newVal) -> {
    ...
    private HBox createSearchBar() {
        ...
        searchField.textProperty().addListener((obs, oldVal, newVal) -> filterConfigurations(newVal));
    ...
    private void filterConfigurations(String searchText) {
        if (searchText == null || searchText.isEmpty()) {
            ...
            ObservableList<ConfigEntry> filtered = configData.filtered(config ->
                config.getName().toLowerCase().contains(lowerSearch) ||
                config.getProject().toLowerCase().contains(lowerSearch) ||
                config.getPath().toLowerCase().contains(lowerSearch)
            );
