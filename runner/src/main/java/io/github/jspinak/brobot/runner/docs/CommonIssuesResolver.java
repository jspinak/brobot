...
public class CommonIssuesResolver {
    ...
    public ResolutionSuggestions analyzeAndSuggest(Throwable error, ErrorContext context) {
        log.debug("Analyzing error for resolution suggestions: {}", error.getClass().getSimpleName());
        ...
        if (guidance.isPresent()) {
            ...
            // Add solutions
            entry.getSolutions().forEach(solution -> {
    ...
    private void addContextSpecificSuggestions(Throwable error, ErrorContext context, 
                                               List<String> suggestions, List<String> automatedFixes) {
    ...
    private void addErrorSpecificSuggestions(Throwable error, List<String> suggestions, 
