 ...
 *
 * <p>This service adds runtime information such as memory usage, CPU load, thread count, and other
 * system metrics to error contexts. It also handles error categorization and ID generation.
 *
...
public class ErrorEnrichmentService implements DiagnosticCapable {
    ...
    private final Map<ErrorContext.ErrorCategory, AtomicLong> categoryCounts =
    ...
    public ErrorContext enrichContext(ErrorContext context) {
        ...
        ErrorContext enriched =
                ErrorContext.builder()
                        .errorId(
                                context.getErrorId() != null
                                        ? context.getErrorId()
                                        : generateErrorId())
        ...
        // Track category
        categoryCounts
                .computeIfAbsent(enriched.getCategory(), k -> new AtomicLong())
                .incrementAndGet();

        if (diagnosticMode.get()) {
            log.info(
                    "[DIAGNOSTIC] Enriched context {} - Memory: {}MB, Threads: {}, CPU: {}%",
                    enriched.getErrorId(),
    ...
    public DiagnosticInfo getDiagnosticInfo() {
        ...
        return DiagnosticInfo.builder().component("ErrorEnrichmentService").states(states).build();
    }
