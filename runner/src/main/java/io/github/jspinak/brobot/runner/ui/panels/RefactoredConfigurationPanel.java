...
/**
 * Refactored configuration panel using the new architecture.
 * Extends BasePanel for standardized lifecycle management and prevents label duplication.
 */
@Slf4j
@Component
public class RefactoredConfigurationPanel extends BasePanel {
    ...
    protected void doInitialize() {
        ...
        statusLabel = labelManager.getOrCreateLabel(
            "config_status",
            "Ready",
            "status-label"
        );
        ...
        autoLoadCheckbox.selectedProperty().addListener((obs, oldVal, newVal) -> 
            runnerProperties.setAutoStartAutomation(newVal)
        );
        ...
        eventBus.subscribe(BrobotEvent.EventType.CONFIG_LOADED, 
            evt -> Platform.runLater(this::refresh));
        eventBus.subscribe(BrobotEvent.EventType.CONFIG_LOADING_FAILED, 
            evt -> Platform.runLater(this::refresh));
    ...
    private GridPane createConfigurationGrid() {
        ...
        Label projectConfigLabel = labelManager.getOrCreateLabel(
        ...
        // DSL configuration
        Label dslConfigLabel = labelManager.getOrCreateLabel(
            "dsl_config_label",
            "DSL Configuration:",
            "config-field-label"
        );
        ...
        // Images directory
        Label imagePathLabel = labelManager.getOrCreateLabel(
            "image_path_label",
            "Images Directory:",
            "config-field-label"
        );
                ...
                try {
                    ...
                    if (success) {
                        Platform.runLater(() -> {
                            ...
                            ConfigurationEvent event = ConfigurationEvent.loaded(
                                this,
                                "Project Configuration",
                                "Successfully loaded configuration from " + projectConfigField.getText()
                            );
                    ...
                    Platform.runLater(() -> {
                        updateStatus("Failed to load configuration: " + e.getMessage(), true);
                        loadButton.setDisable(false);
                        ...
                        ConfigurationEvent event = ConfigurationEvent.loadingFailed(
                            this,
                            "Project Configuration",
                            e.getMessage(),
                            e
    ...
    private void saveConfiguration() {
        try {
            ...
            log.info("Configuration saved: project={}, dsl={}, images={}",
