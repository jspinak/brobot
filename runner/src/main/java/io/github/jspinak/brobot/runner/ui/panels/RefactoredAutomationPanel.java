...
/**
 * Refactored automation panel using the new architecture. Demonstrates proper use of BasePanel,
...
public class RefactoredAutomationPanel extends BasePanel {
    ...
    private void setupUI() {
        ...
        Label titleLabel =
                labelManager.getOrCreateLabel(
                        "automation_title", "Automation Control", "title-label");
        ...
        Label actionsLabel =
                labelManager.getOrCreateLabel(
                        "actions_section", "Available Actions", "section-label");

        Label logLabel =
                labelManager.getOrCreateLabel("log_section", "Execution Log", "section-label");
    ...
    private HBox createControlSection() {
        ...
        autoMinimizeCheck
                .selectedProperty()
                .addListener(
                        (obs, oldVal, newVal) -> windowController.setAutoMinimizeEnabled(newVal));
    ...
    private VBox createProgressSection() {
        ...
        Label statusLabel =
                labelManager.getOrCreateLabel("status_label", "Status: Ready", "status-label");
        ...
        Label progressLabel = labelManager.getOrCreateLabel("progress_label", "0%");
        ...
        Platform.runLater(
                () -> {
                    try {
                        ...
                        if (project.getAutomation() == null
                                || project.getAutomation().getButtons() == null) {
                            clearButtons();
    ...
    private Button createTaskButton(TaskButton buttonDef) {
        ...
        Button button =
                renderedButtons.computeIfAbsent(
                        buttonId,
                        k -> {
        ...
        Platform.runLater(
                () -> {
                    ...
                    labelManager.updateLabel(
                            "status_label", "Status: " + status.getState().getDescription());

                    // Update progress
                    progressBar.setProgress(status.getProgress());
                    labelManager.updateLabel(
                            "progress_label", String.format("%.0f%%", status.getProgress() * 100));
