...
/**
 * Refactored unified automation panel using service-oriented architecture.
...
public class RefactoredUnifiedAutomationPanel extends VBox {
    ...
    private void createUI() {
        ...
        controlBar.getConfigureHotkeysButton().setOnAction(e -> hotkeyService.showConfigurationDialog());
        controlBar.getAutoMinimizeCheckBox().setSelected(windowController.isAutoMinimizeEnabled());
        controlBar.getAutoMinimizeCheckBox().selectedProperty().addListener((obs, old, val) -> 
            windowController.setAutoMinimizeEnabled(val)
        );
        ...
        // Assemble the panel
        assembledPanel = uiFactory.assembleMainPanel(
            titleLabel, statusPanel, progressSection, 
            controlBar, buttonPane, logArea
        );
    ...
    public String getStatistics() {
        return String.format(
            "Status: %s\nLog entries: %d",
            statusService.getStatusSummary(),
            logService.getStatistics().getTotalLogs()
        );
    }
}