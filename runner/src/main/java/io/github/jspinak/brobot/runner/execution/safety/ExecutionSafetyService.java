 ...
 *
 * <p>This service implements safety checks and controls to prevent: - Runaway mouse movements -
 * Excessive key presses - Unintended system interactions - Resource exhaustion
 *
...
public class ExecutionSafetyService implements DiagnosticCapable {
    ...
    public boolean checkActionSafety(ExecutionContext context, String actionType) {
        ...
        if (emergencyStop.get()) {
            log.warn(
                    "Action blocked due to emergency stop - Type: {}, Task: {}",
                    actionType,
                    context.getTaskName());
            return false;
        }
        ...
        ActionTracker tracker =
                actionTrackers.computeIfAbsent(context.getId(), k -> new ActionTracker());

        if (!tracker.checkRate(actionType)) {
            log.warn(
                    "Action rate limit exceeded - Type: {}, Task: {}, Rate: {}/s",
                    actionType,
                    context.getTaskName(),
                    tracker.getCurrentRate(actionType));
            return false;
        }

        if (diagnosticMode.get()) {
            log.info(
                    "[DIAGNOSTIC] Safety check passed - Type: {}, Task: {}, Rate: {}/s",
                    actionType,
                    context.getTaskName(),
                    tracker.getCurrentRate(actionType));
        }
    ...
    public boolean validateMousePosition(int x, int y) {
        ...
        if (!inBounds) {
            log.warn(
                    "Mouse position outside safe bounds - X: {}, Y: {}, Bounds: {}",
                    x,
                    y,
                    SAFE_SCREEN_BOUNDS);
        }
    ...
    public void recordFailure(ExecutionContext context, Throwable error) {
        ...
        log.error(
                "Execution failure recorded - Task: {}, Consecutive failures: {}",
                context.getTaskName(),
                failures,
                error);
    ...
    public DiagnosticInfo getDiagnosticInfo() {
        ...
        return DiagnosticInfo.builder().component("ExecutionSafetyService").states(states).build();
    }
    ...
    private static class ActionTracker {
        ...
        public boolean checkRate(String actionType) {
            RateWindow window =
                    actionWindows.computeIfAbsent(
                            actionType, k -> new RateWindow(ACTION_RATE_WINDOW));
