...
/**
 * Refactored panel for selecting recent configurations and managing configuration history.
 * Uses service-oriented architecture to separate concerns and improve maintainability.
 */
@Slf4j
@Component
@Getter
public class RefactoredConfigSelectionPanel extends VBox {
    ...
    private void configureServices() {
        ...
        loadingService.setLoadingCompleteHandler(result -> {
        ...
        importService.setImportSuccessHandler(entry -> {
            // Add to history
            ...
            tableService.selectConfiguration(assembledUI.getSplitPane().getConfigTable(), entry);
    ...
    private void removeConfiguration(ConfigEntry entry) {
        if (entry != null) {
            loadingService.confirmRemoval(entry).ifPresent(confirmed -> {
                if (confirmed) {
                    // Remove from history
                    ...
                    if (assembledUI.getSplitPane().getDetailsPanel().getConfiguration() == entry) {
                        assembledUI.getSplitPane().getDetailsPanel().clearConfiguration();
                    }
