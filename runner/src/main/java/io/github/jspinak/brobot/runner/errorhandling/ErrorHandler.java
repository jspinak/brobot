...
/**
 * Central error handling orchestrator that coordinates error processing across specialized
 * services.
 *
 * <p>This component acts as a thin orchestrator that delegates specific responsibilities to
 * specialized services following the Single Responsibility Principle:
 *
 * <p>- ErrorProcessingService: Manages error processor pipeline - ErrorStrategyService: Manages
 * error handling strategies - ErrorHistoryService: Tracks error history - ErrorEnrichmentService:
 * Enriches error context - ErrorStatisticsService: Collects error statistics
 *
 * <p>The error handler coordinates these services to provide comprehensive error handling,
...
public class ErrorHandler implements DiagnosticCapable {
    ...
    public void initialize() {
        ...
        log.info(
                "Error handler initialized with {} processors and {} strategies",
                processingService.getProcessorCount(),
                strategyService.getStrategyCount());
    }
    ...
    public ErrorResult handleError(Throwable error, ErrorContext context) {
        ...
        try {
            return circuitBreaker.execute(
                    () -> {
                        ...
                        if (diagnosticMode.get()) {
                            log.info(
                                    "[DIAGNOSTIC] Handled error {} - Type: {}, Result: {},"
                                            + " Recoverable: {}",
                        ...
                        if (!result.isSuccess()
                                && context.getSeverity() == ErrorContext.ErrorSeverity.CRITICAL) {
                            throw new RuntimeException(
                                    "Critical error handling failed: " + result.getUserMessage());
                        }
            ...
            log.error(
                    "Circuit breaker is OPEN for operation '{}', error handling blocked",
                    operation);
            ...
            return ErrorResult.unrecoverable(
                    "Error handler failed: " + e.getMessage(), context.getErrorId());
        }
    }
     ...
     */
    public Map<String, io.github.jspinak.brobot.runner.errorhandling.circuit.CircuitBreakerMetrics>
            getCircuitBreakerMetrics() {
        Map<String, io.github.jspinak.brobot.runner.errorhandling.circuit.CircuitBreakerMetrics>
                metrics = new ConcurrentHashMap<>();
        circuitBreakers.forEach(
                (operation, breaker) -> metrics.put(operation, breaker.getMetrics()));
        return metrics;
    }
    ...
    public DiagnosticInfo getDiagnosticInfo() {
        ...
        circuitBreakers.forEach(
                (op, breaker) -> circuitBreakerStates.put(op, breaker.getState().get().name()));
        ...
        return DiagnosticInfo.builder().component("ErrorHandler").states(states).build();
    }
    ...
    public void enableDiagnosticMode(boolean enabled) {
        ...
        log.info(
                "Diagnostic mode {} for ErrorHandler and all services",
    ...
    private ErrorResult attemptRecovery(ErrorResult result, ErrorContext context) {
        try {
            ...
            return ErrorResult.unrecoverable(
                    "Recovery failed: " + e.getMessage(), context.getErrorId());
        }
    }
        ...
        public void process(Throwable error, ErrorContext context) {
            ...
            io.github.jspinak.brobot.runner.events.ErrorEvent.ErrorSeverity eventSeverity =
                    switch (context.getSeverity()) {
                        case LOW ->
                                io.github.jspinak.brobot.runner.events.ErrorEvent.ErrorSeverity.LOW;
                        case MEDIUM ->
                                io.github.jspinak.brobot.runner.events.ErrorEvent.ErrorSeverity
                                        .MEDIUM;
                        case HIGH ->
                                io.github.jspinak.brobot.runner.events.ErrorEvent.ErrorSeverity
                                        .HIGH;
