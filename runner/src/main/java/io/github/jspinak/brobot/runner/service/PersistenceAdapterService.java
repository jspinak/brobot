...
/**
 * Adapter service that bridges the runner UI with the persistence module. This replaces the
 * previous ActionRecordingService implementation with the new modular persistence architecture.
 */
@Service
...
public class PersistenceAdapterService {
    ...
    public List<SessionMetadata> getAllSessionMetadata() {
        return getAllSessions().stream().map(this::getSessionMetadata).toList();
    }
}
