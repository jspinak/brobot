...
/**
 * Service for filtering log entries based on various criteria. Provides predicate builders for
...
public class LogFilterService {
    ...
    public Predicate<LogEntry> createFilter(FilterCriteria criteria) {
        ...
        if (criteria.searchText != null && !criteria.searchText.trim().isEmpty()) {
            predicates.add(
                    createSearchFilter(
                            criteria.searchText, criteria.useRegex, criteria.caseSensitive));
        }
        ...
        return predicates.stream().reduce(entry -> true, Predicate::and);
    }

    /** Creates a search text filter. */
    private Predicate<LogEntry> createSearchFilter(
            String searchText, boolean useRegex, boolean caseSensitive) {
        if (useRegex) {
            try {
                Pattern pattern =
                        caseSensitive
    ...
    private Predicate<LogEntry> createSourceFilter(String source) {
        ...
        return entry ->
                entry.getSource() != null && entry.getSource().toLowerCase().contains(lowerSource);
    }
    ...
    public Predicate<LogEntry> createQuickFilter(QuickFilterType type) {
        switch (type) {
            case ERRORS_ONLY:
                return entry ->
                        entry.getLevel() == LogEntry.LogLevel.ERROR
    ...
    /**
     * Creates a highlighted search filter that marks matching text. Returns a predicate that also
     * populates highlight ranges.
     */
    public SearchResult createHighlightedSearchFilter(
            String searchText, boolean useRegex, boolean caseSensitive) {
        Pattern pattern;
        ...
        Predicate<LogEntry> predicate =
                entry -> {
                    ...
                    while (matcher.find()) {
                        found = true;
                        highlights.add(
                                new HighlightRange(entry.getId(), matcher.start(), matcher.end()));
                    }
    ...
    public List<String> getUniqueTypes(List<LogEntry> entries) {
        return entries.stream().map(LogEntry::getType).distinct().sorted().toList();
    }
