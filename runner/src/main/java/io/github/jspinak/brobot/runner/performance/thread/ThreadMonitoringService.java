 ...
 *
 * <p>This service provides comprehensive thread monitoring capabilities including thread count
 * tracking, contention detection, deadlock detection, and performance metrics collection.
 *
...
public class ThreadMonitoringService implements DiagnosticCapable {
            ...
            if (thread != null && thread.getBlockedCount() > 0) {
                ...
                if (existing != null) {
                    ...
                    if (currentBlocked > existing.getBlockedCount()
                        ...
                        ThreadContentionInfo updated =
                                new ThreadContentionInfo(
    ...
    public List<ThreadInfo> detectDeadlocks() {
        ...
        for (ThreadInfo thread : result) {
            log.error(
                    "Deadlocked thread: {} (ID: {})", thread.getThreadName(), thread.getThreadId());
        }
    ...
    public void monitorThreadHealth() {
        ...
        if (stats.getTotalStartedThreads() - stats.getCurrentThreadCount() > 1000) {
            log.warn(
                    "Possible thread leak detected. Total started: {}, Current: {}",
                    stats.getTotalStartedThreads(),
                    stats.getCurrentThreadCount());
        }

        if (diagnosticMode.get()) {
            log.info(
                    "[DIAGNOSTIC] Thread health check - Current: {}, Peak: {}, Daemon: {}",
                    stats.getCurrentThreadCount(),
    ...
    public DiagnosticInfo getDiagnosticInfo() {
        ...
        stats.getThreadGroups()
                .forEach(
                        (group, count) -> {
                            states.put("threadGroup." + group, count);
                        });

        return DiagnosticInfo.builder().component("ThreadMonitoringService").states(states).build();
    }
    ...
    public void enableDiagnosticMode(boolean enabled) {
        ...
        log.info(
                "Diagnostic mode {} for ThreadMonitoringService", enabled ? "enabled" : "disabled");
    }
    ...
    public static class ThreadStatistics {
        ...
        public int getTotalPoolThreads() {
            return poolHealth.values().stream().mapToInt(ThreadPoolHealth::poolSize).sum();
        }

        public int getTotalActivePoolThreads() {
            return poolHealth.values().stream().mapToInt(ThreadPoolHealth::activeThreads).sum();
        }
    }
