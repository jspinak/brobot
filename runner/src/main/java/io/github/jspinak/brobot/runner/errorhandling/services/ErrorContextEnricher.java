 ...
 * Adds runtime metrics, system properties, and environmental data to error contexts.
 */
@Slf4j
@Service
public class ErrorContextEnricher {
    ...
    public ErrorContext enrich(ErrorContext context) {
        ...
        try {
            return ErrorContext.builder()
                ...
                .additionalData(mergeAdditionalData(context.getAdditionalData(), collectSystemData()))
     ...
     */
    public ErrorContext createEnrichedContext(String operation, ErrorContext.ErrorCategory category) {
     ...
     */
    private Map<String, Object> mergeAdditionalData(Map<String, Object> existing, Map<String, Object> systemData) {
    ...
    public ErrorContext.ErrorCategory categorizeError(Throwable error) {
        ...
        } else if (className.contains("database") || className.contains("sql") ||
                   className.contains("jdbc") || message.contains("database")) {
            return ErrorContext.ErrorCategory.DATABASE;
        } else if (className.contains("validation") || className.contains("invalid") ||
                   className.contains("illegal") || message.contains("invalid")) {
        ...
        } else if (className.contains("config") || className.contains("property") ||
