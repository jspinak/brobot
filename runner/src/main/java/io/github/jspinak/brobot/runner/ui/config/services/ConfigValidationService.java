...
/**
 * Service for validating configuration data and user inputs. Provides validation rules for
...
public class ConfigValidationService {
    ...
    /**
     * Validates a version string. Expected format: major.minor[.patch][-qualifier] Examples: 1.0,
    ...
    public ValidationResult validateVersion(String version) {
        ...
        if (!VERSION_PATTERN.matcher(version).matches()) {
            return ValidationResult.error(
                    "Invalid version format. Expected: major.minor[.patch][-qualifier] (e.g., 1.0,"
                            + " 1.0.0, 1.0.0-SNAPSHOT)");
        }
    ...
    /**
     * Validates a project name. Must start with a letter and contain only letters, numbers,
    ...
    public ValidationResult validateProjectName(String projectName) {
        ...
        if (!PROJECT_NAME_PATTERN.matcher(projectName).matches()) {
            return ValidationResult.error(
                    "Invalid project name. Must start with a letter and contain only letters,"
                            + " numbers, hyphens, and underscores");
        }

        if (projectName.length() > 50) {
            return ValidationResult.warning(
                    "Project name is very long. Consider using a shorter name");
        }

        return ValidationResult.success();
    }

    /** Validates a configuration key. Must be a valid identifier that can be used in code. */
    public ValidationResult validateKey(String key) {
        ...
        if (!KEY_PATTERN.matcher(key).matches()) {
            return ValidationResult.error(
                    "Invalid key format. Must start with a letter and contain only letters,"
                            + " numbers, underscores, and dots");
        }

        if (RESERVED_WORDS.contains(key.toLowerCase())) {
            return ValidationResult.error(
                    "'" + key + "' is a reserved word and cannot be used as a key");
        }
    ...
    public ValidationResult validatePackageName(String packageName) {
        ...
        if (!PACKAGE_NAME_PATTERN.matcher(packageName).matches()) {
            return ValidationResult.error(
                    "Invalid package name. Must be lowercase letters separated by dots (e.g.,"
                            + " com.example.app)");
        }
    ...
    /** Validates a list of configuration entries. Returns all validation errors found. */
