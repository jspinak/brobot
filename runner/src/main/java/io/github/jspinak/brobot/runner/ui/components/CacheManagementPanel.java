...
/**
 * Panel for managing cache operations. Extracted from ResourceMonitorPanel to improve modularity.
 */
public class CacheManagementPanel extends VBox {
    ...
    private HBox createCacheControls() {
        ...
        controls.getChildren().addAll(cacheSelector, clearCacheButton, new Spacer());
        ...
        if ("All Caches".equals(selectedCache)) {
            ...
            for (Map.Entry<String, Map<String, Long>> entry : allStats.entrySet()) {
                ...
                if (stats != null) {
                    ...
                    statsText.append(stats.getOrDefault("hitRatio", 0L)).append("% hit ratio\n");
                }
            }
        } else {
            ...
            if (stats != null) {
                statsText
                        ...
                        .append(stats.getOrDefault("maxSize", 0L))
                        .append("\n");
                statsText.append("Hits: ").append(stats.getOrDefault("hits", 0L)).append("\n");
                statsText.append("Misses: ").append(stats.getOrDefault("misses", 0L)).append("\n");
                statsText
                        .append("Hit Ratio: ")
                        .append(stats.getOrDefault("hitRatio", 0L))
                        .append("%\n");
                statsText.append("Puts: ").append(stats.getOrDefault("puts", 0L));
            } else {
