 ...
 *
 * <p>Single Responsibility: Create and manage ProjectContext instances
 */
@Slf4j
@Service
...
public class ProjectContextService {
    ...
    private final AtomicReference<ProjectContext> currentContext =
            new AtomicReference<>(ProjectContext.empty());
    ...
    public ProjectContext createContext(ProjectDefinition projectDefinition) {
        ...
        if (projectDefinition.getAutomationProjectId() != null) {
            try {
                automationProject =
                        automationLoader.loadAutomation(
                                projectDefinition.getConfigPath(),
                                projectDefinition.getAutomationProjectId());
            } catch (Exception e) {
                log.warn(
                        "Failed to load automation for project: {}",
                        projectDefinition.getName(),
                        e);
                eventBus.publish(
                        LogEvent.warning(
                                this, "Automation loading failed: " + e.getMessage(), "Project"));
            }
        }
    ...
    public void updateAutomation(AutomationProject automationProject) {
        ...
        if (current.isValid()) {
            ...
            if (automationProject != null) {
                current.getProjectDefinition()
                        .setAutomationProjectId(String.valueOf(automationProject.getId()));
                lifecycleService.updateProjectMetadata(
                        "automationProjectId", automationProject.getId());
            }
        }
    }
