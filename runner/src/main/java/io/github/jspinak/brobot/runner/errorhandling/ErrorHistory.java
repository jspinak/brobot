...
public class ErrorHistory {
    ...
    public void record(Throwable error, ErrorContext context) {
        ...
        errorsByCategory.computeIfAbsent(context.getCategory(), k -> new ArrayList<>()).add(record);
    ...
    public List<ErrorRecord> getRecentErrors(int count) {
        return errorRecords.stream().limit(count).collect(Collectors.toList());
    }
    ...
    public ErrorHistoryStatistics getStatistics() {
        ...
        return new ErrorHistoryStatistics(
                errorRecords.size(), categoryCount, severityCount, getMostFrequentErrors(5));
    }
    ...
    private void cleanupOldRecords() {
        ...
        errorsByCategory
                .values()
                .forEach(list -> list.removeIf(record -> record.getTimestamp().isBefore(cutoff)));
    }
