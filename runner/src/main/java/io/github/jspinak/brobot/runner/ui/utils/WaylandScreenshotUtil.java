...
/**
 * Screenshot utility for Wayland environments.
 * Uses system screenshot tools (gnome-screenshot, spectacle, etc.) as fallback.
 */
public class WaylandScreenshotUtil {
    ...
    public static boolean isWayland() {
        ...
        logger.info("Wayland detection - WAYLAND_DISPLAY: {}, XDG_SESSION_TYPE: {}, isWayland: {}", 
            waylandDisplay, sessionType, isWayland);
    ...
    public static CompletableFuture<String> captureWithSystemTool(String filename) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            String[] commands = {
                ...
                String.format("dbus-send --session --type=method_call --dest=org.gnome.Shell.Screenshot " +
                    "/org/gnome/Shell/Screenshot org.gnome.Shell.Screenshot.Screenshot " +
                    "boolean:false boolean:false string:'%s'", outputPath)
            };
        ...
        try {
            ...
            if (completed && exitCode == 0) {
                ...
                if (outputFile.exists() && outputFile.length() > 0) {
                    logger.info("Screenshot captured successfully with command: {}", command.split(" ")[0]);
                    return true;
                }
            }
            ...
            if (!completed) {
                ...
                logger.warn("Screenshot command failed with exit code {}: {}", exitCode, command.split(" ")[0]);
            }
