 ...
 *
 * <p>This service analyzes thread pool performance, system resources, and workload characteristics
 * to make optimization decisions. It can adjust pool sizes, recommend configuration changes, and
...
public class ThreadOptimizationService implements DiagnosticCapable {
    ...
    @Autowired
    public ThreadOptimizationService(
            ThreadPoolManagementService poolManagement, ThreadMonitoringService monitoring) {
    ...
    public boolean applyOptimizations(List<OptimizationAction> actions) {
        ...
        for (OptimizationAction action : actions) {
            try {
                ...
                optimizationsByType
                        .computeIfAbsent(action.getType().name(), k -> new AtomicLong())
                        .incrementAndGet();

                if (diagnosticMode.get()) {
                    log.info(
                            "[DIAGNOSTIC] Applied optimization: {} - Success: {}", action, success);
                }
            } catch (Exception e) {
    ...
    public void adjustPoolSize(String poolName, PoolAdjustment adjustment) {
        poolManagement.adjustPoolSize(poolName, adjustment.getCoreSize(), adjustment.getMaxSize());
    }
                ...
                .forEach(
                        (name, health) -> {
                            if (health.utilization() < 0.5 && health.poolSize() > 1) {
                                ...
                                actions.add(
                                        OptimizationAction.builder()
                                                .type(ActionType.REDUCE_POOL_SIZE)
                                                .targetPool(name)
                                                .description(
                                                        "Reduce pool size due to low utilization")
                ...
                .forEach(
                        (name, health) -> {
                            if (name.contains("cpu")
                                    && health.activeThreads() > metrics.getAvailableCores()) {
                                actions.add(
                                        OptimizationAction.builder()
    ...
    private boolean applyAction(OptimizationAction action) {
        switch (action.getType()) {
            ...
            case THROTTLE_POOL:
                ...
                return poolManagement.adjustPoolSize(
                        action.getTargetPool(), maxThreads, maxThreads);

            case REDUCE_QUEUE_SIZE:
                // Queue size adjustment would require pool recreation
                log.warn(
                        "Queue size adjustment not implemented for pool: {}",
                        action.getTargetPool());
                return false;
    ...
    public DiagnosticInfo getDiagnosticInfo() {
        ...
        states.put(
                ...
                totalOptimizations.get() > 0
                        ? (successfulOptimizations.get() * 100.0 / totalOptimizations.get())
                        : 100.0);
    ...
    public void enableDiagnosticMode(boolean enabled) {
        ...
        log.info(
                "Diagnostic mode {} for ThreadOptimizationService",
