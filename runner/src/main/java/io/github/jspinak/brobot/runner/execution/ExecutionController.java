 ...
 *
 * <p>This refactored controller delegates responsibilities to specialized services: -
 * ExecutionService: Core execution logic - ExecutionThreadManager: Thread management -
 * ExecutionTimeoutManager: Timeout handling - ExecutionSafetyService: Safety checks
 *
 * <p>The controller now focuses solely on: - API coordination - Control flow (pause/resume/stop) -
 * Status access
 *
...
public class ExecutionController implements AutoCloseable, DiagnosticCapable {
    ...
    // Status tracking
    @Getter private final ExecutionStatus status = new ExecutionStatus();

    // Logging callback
    @Setter private Consumer<String> logCallback;

    // Diagnostic mode
    private final AtomicBoolean diagnosticMode = new AtomicBoolean(false);

    @Autowired
    public ExecutionController(ExecutionService executionService, ResourceManager resourceManager) {
    ...
    public void executeAutomation(
        ...
        executionService.addStatusListener(
                executionId,
                status -> {
        ...
        CompletableFuture<Void> future =
                executionService.execute(wrappedTask, button.getLabel(), options);
    ...
    public boolean isPaused() {
        ExecutionControl control = currentControl.get();
        return control != null && control.isPaused() && status.getState() == ExecutionState.PAUSED;
    }
    ...
    public DiagnosticInfo getDiagnosticInfo() {
        ...
        return DiagnosticInfo.builder().component("ExecutionController").states(states).build();
    }
    ...
    /**
     * Updates our status from the execution service status. Thread-safe: synchronized to prevent
