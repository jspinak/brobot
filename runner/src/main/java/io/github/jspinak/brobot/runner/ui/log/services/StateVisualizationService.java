...
/**
 * Service for creating and managing state transition visualizations. Provides various visualization
 * styles and layouts for state diagrams.
 */
@Slf4j
@Service
public class StateVisualizationService {
    ...
    public static class VisualizationTheme {
        ...
        public double getArrowStrokeWidth() {
    ...
    public void visualizeTransition(
            StateVisualizationPanel panel, List<String> fromStates, List<String> toStates) {
        Platform.runLater(
                () -> {
                    ...
                    switch (panel.visualizationType) {
                        case CIRCLE_NODES:
                            drawCircleTransition(
                                    panel, fromStates, toStates, canvasWidth, canvasHeight);
                            break;
                        case RECTANGLE_NODES:
                            drawRectangleTransition(
                                    panel, fromStates, toStates, canvasWidth, canvasHeight);
                            break;
                        case HIERARCHICAL:
                            drawHierarchicalTransition(
                                    panel, fromStates, toStates, canvasWidth, canvasHeight);
                            break;
                        case RADIAL:
                            drawRadialTransition(
                                    panel, fromStates, toStates, canvasWidth, canvasHeight);
                            break;
                        case FLOW:
                            drawFlowTransition(
        ...
        Platform.runLater(
                () -> {
                    ...
                    switch (panel.visualizationType) {
                        case CIRCLE_NODES:
                            drawCircleNode(
                                    panel, stateName, canvasWidth / 2, canvasHeight / 2, true);
                            break;
                        case RECTANGLE_NODES:
                            drawRectangleNode(
                                    panel, stateName, canvasWidth / 2, canvasHeight / 2, true);
    ...
    private void drawCircleTransition(
            StateVisualizationPanel panel,
        ...
        // Draw from states
        List<Node> fromNodes =
                drawStateGroup(panel, fromStates, leftX, canvasHeight / 2, this::drawCircleNode);

        // Draw to states
        List<Node> toNodes =
                drawStateGroup(panel, toStates, rightX, canvasHeight / 2, this::drawCircleNode);
    ...
    private void drawRectangleTransition(
            StateVisualizationPanel panel,
            List<String> fromStates,
            List<String> toStates,
            double canvasWidth,
        ...
        // Draw from states
        List<Node> fromNodes =
                drawStateGroup(panel, fromStates, leftX, canvasHeight / 2, this::drawRectangleNode);

        // Draw to states
        List<Node> toNodes =
                drawStateGroup(panel, toStates, rightX, canvasHeight / 2, this::drawRectangleNode);
    ...
    // Hierarchical layout
    private void drawHierarchicalTransition(
            StateVisualizationPanel panel,
            List<String> fromStates,
            List<String> toStates,
            double canvasWidth,
        ...
        for (int i = 0; i < fromStates.size(); i++) {
            Node node =
                    drawCircleNode(panel, fromStates.get(i), fromSpacing * (i + 1), topY, false);
            fromNodes.add(node);
        }
        ...
        for (int i = 0; i < toStates.size(); i++) {
            Node node = drawCircleNode(panel, toStates.get(i), toSpacing * (i + 1), bottomY, false);
            toNodes.add(node);
        }
    ...
    // Radial layout
    private void drawRadialTransition(
            StateVisualizationPanel panel,
            List<String> fromStates,
            List<String> toStates,
            double canvasWidth,
    ...
    // Flow chart style
    private void drawFlowTransition(
            StateVisualizationPanel panel,
            List<String> fromStates,
            List<String> toStates,
            double canvasWidth,
    ...
    private List<Node> drawStateGroup(
            StateVisualizationPanel panel,
            List<String> states,
            double x,
            double y,
    ...
    private interface StateNodeDrawer {
        Node draw(
                StateVisualizationPanel panel, String name, double x, double y, boolean highlight);
    }

    // Draw circle node
    private Node drawCircleNode(
            StateVisualizationPanel panel,
            String stateName,
            double x,
            double y,
    ...
    // Draw rectangle node
    private Node drawRectangleNode(
            StateVisualizationPanel panel,
            String stateName,
            double x,
            double y,
    ...
    private void drawArrows(
            StateVisualizationPanel panel, List<Node> fromNodes, List<Node> toNodes) {
    ...
    private void drawArrow(
            StateVisualizationPanel panel, double startX, double startY, double endX, double endY) {
        VisualizationTheme theme = panel.theme;
