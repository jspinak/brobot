...
/**
 * Service responsible for tracking error metrics and statistics. Provides insights into error
...
public class ErrorMetricsService {
    ...
    private final Map<ErrorContext.ErrorCategory, AtomicLong> errorsByCategory =
            new ConcurrentHashMap<>();
    private final Map<ErrorContext.ErrorSeverity, AtomicLong> errorsBySeverity =
            new ConcurrentHashMap<>();
    ...
    @Getter private final ErrorHistory errorHistory = new ErrorHistory();
    ...
    public void recordError(Throwable error, ErrorContext context) {
        ...
        // Count by category
        errorsByCategory
                .computeIfAbsent(context.getCategory(), k -> new AtomicLong())
                .incrementAndGet();

        // Count by severity
        errorsBySeverity
                .computeIfAbsent(context.getSeverity(), k -> new AtomicLong())
                .incrementAndGet();

        // Count by component
        if (context.getComponent() != null) {
            errorsByComponent
                    .computeIfAbsent(context.getComponent(), k -> new AtomicLong())
                    .incrementAndGet();
        }
    ...
    public ErrorStatistics getStatistics() {
        ...
        errorsByCategory.forEach((category, count) -> categoryStats.put(category, count.get()));
    ...
    public PeriodMetrics getMetricsForPeriod(LocalDateTime start, LocalDateTime end) {
        ...
        return new PeriodMetrics(
                start, end, periodErrors, calculateRateForPeriod(periodErrors, start, end));
    }

    private void recordTimestamp(String errorType) {
        errorTimestamps.compute(
                errorType,
                (key, timestamps) -> {
                    if (timestamps == null) {
                    ...
                    if (timestamps.size() > MAX_TIMESTAMPS_PER_ERROR) {
                        timestamps =
                                new ArrayList<>(
                                        timestamps.subList(
                                                timestamps.size() - MAX_TIMESTAMPS_PER_ERROR,
                                                timestamps.size()));
                    }
