...
/**
 * Refactored icon generator that orchestrates between services.
 * Simplified from 610 lines to a thin orchestrator.
 */
@Slf4j
@Component
@Data
                ...
                for (String iconName : iconNames) {
                    for (int size : sizes) {
                        try {
                            ...
                            log.warn("Failed to preload icon: {} at size {}", iconName, size, e);
                        }
                    }
                }
                log.info("Preloaded {} icon types in {} sizes", iconNames.length, sizes.length);
            });
        });
    }
