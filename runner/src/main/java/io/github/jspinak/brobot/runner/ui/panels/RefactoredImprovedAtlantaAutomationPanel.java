...
/**
 * Refactored improved automation panel using service-oriented architecture. Acts as a thin
...
public class RefactoredImprovedAtlantaAutomationPanel extends AtlantaBasePanel {
    ...
    private Region createMainContent() {
        ...
        // Create split layout
        return uiFactory.createResponsiveSplitLayout(taskPanel.getCard(), logPanel.getCard());
    }
    ...
    private void refreshProjectTasks() {
        Platform.runLater(
                () -> {
                    // Load tasks
                    TaskManagementService.ProjectTasksResult result =
                            taskService.loadProjectTasks();

                    // Update project label
                    projectInfo
                            .getProjectLabel()
                            .setText(
                                    result.getProjectName() != null
        ...
        Platform.runLater(
                () -> {
                    ...
                    statusSection
                            .getStatusLabel()
                            .getStyleClass()
                            .removeAll("status-running", "status-paused", "status-stopped");
