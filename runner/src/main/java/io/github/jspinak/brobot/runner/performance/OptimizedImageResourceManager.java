...
public class OptimizedImageResourceManager implements MemoryReleasable {
    ...
    public void initialize() {
        ...
        log.info(
                "Initialized optimized image resource manager with {}MB cache",
    ...
    private void evictLeastRecentlyUsed(long requiredSpace) {
        ...
        Iterator<Map.Entry<String, SoftReference<Image>>> softIter =
                softCache.entrySet().iterator();
    ...
    public void preloadImages(List<String> paths) {
        CompletableFuture<?>[] futures =
                paths.stream().map(this::getImageAsync).toArray(CompletableFuture[]::new);

        CompletableFuture.allOf(futures)
                .thenRun(() -> log.info("Preloaded {} images", paths.size()));
    }
    ...
    public long releaseMemory() {
        ...
        List<String> unpinnedPaths =
                strongCache.keySet().stream()
                        .filter(
                                path -> {
                                    ImageMetadata metadata = metadataCache.get(path);
    ...
    public record CacheStatistics(
        ...
        public String toString() {
            return String.format(
                    "Cache Stats: Strong=%d, Soft=%d, Size=%dMB, Hit Rate=%.1f%% (Hits=%d,"
                            + " Misses=%d)",
                    strongCacheSize,
                    softCacheSize,
