...
/**
 * Service responsible for managing error handling strategies.
 * Provides registration, lookup, and default strategy initialization.
 */
@Slf4j
@Service
public class ErrorStrategyManager {
    ...
    public <T extends Throwable> void registerStrategy(Class<T> errorType, IErrorStrategy strategy) {
        ...
        log.debug("Registered strategy {} for error type {}", 
                 strategy.getClass().getSimpleName(), errorType.getSimpleName());
    }
    
    /**
     * Finds the appropriate strategy for the given error.
     * Searches for exact match first, then superclasses, finally returns default.
     */
    public IErrorStrategy findStrategy(Throwable error) {
        ...
        while (currentClass != null && Throwable.class.isAssignableFrom(currentClass)) {
            ...
            if (strategy != null) {
                log.trace("Found superclass strategy match for {} using {}", 
        ...
        for (Class<?> interfaceClass : error.getClass().getInterfaces()) {
            if (Throwable.class.isAssignableFrom(interfaceClass)) {
                ...
                if (strategy != null) {
                    log.trace("Found interface strategy match for {} using {}", 
