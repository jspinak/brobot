...
/**
 * Fixes tab clicking issues in JavaFX TabPane.
 * This addresses the problem where tab clicks don't register or are delayed.
 */
@Slf4j
public class TabClickFix {
    ...
    private static void handleTabClick(MouseEvent event, TabPane tabPane) {
        ...
        if (clickedTab != null && clickedTab != tabPane.getSelectionModel().getSelectedItem()) {
            log.debug("Tab {} event detected on: {}", event.getEventType().getName(), clickedTab.getText());
    ...
    private static Tab findTabByPosition(TabPane tabPane, Bounds bounds) {
        ...
        double clickX = bounds.getMinX() - tabHeaderArea.localToScene(tabHeaderArea.getBoundsInLocal()).getMinX();
    ...
    public static void simulateTabClick(TabPane tabPane, int tabIndex) {
        ...
        Platform.runLater(() -> {
            ...
            if (tabHeaderArea != null) {
                double tabWidth = tabHeaderArea.getBoundsInLocal().getWidth() / tabPane.getTabs().size();
