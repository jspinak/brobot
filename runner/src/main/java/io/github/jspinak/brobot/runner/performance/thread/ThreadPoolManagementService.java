 ...
 * 
 * This service handles creation, registration, monitoring, and shutdown
 * of thread pools used throughout the application. It maintains a registry
 * of all managed pools and provides health monitoring capabilities.
 * 
...
public class ThreadPoolManagementService implements DiagnosticCapable {
    ...
    public ExecutorService createPool(String name, ThreadPoolConfig config) {
        ...
        if (diagnosticMode.get()) {
            log.info("[DIAGNOSTIC] Pool '{}' created - Core: {}, Max: {}, Queue: {}", 
        ...
        if (awaitTermination) {
            try {
                if (!pool.awaitTermination(timeoutSeconds, TimeUnit.SECONDS)) {
                    log.warn("Pool '{}' did not terminate within {} seconds, forcing shutdown", 
                            name, timeoutSeconds);
                    pool.shutdownNow();
                }
    ...
    public int getTotalQueuedTasks() {
        return pools.values().stream()
                .mapToInt(pool -> pool.getQueue().size())
                .sum();
    }
    ...
    public void enableDiagnosticMode(boolean enabled) {
        ...
        log.info("Diagnostic mode {} for ThreadPoolManagementService", 
