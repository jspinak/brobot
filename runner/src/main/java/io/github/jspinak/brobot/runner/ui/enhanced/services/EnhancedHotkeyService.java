...
/**
 * Service for managing hotkeys in the enhanced automation panel.
 * Handles hotkey registration, configuration, and action binding.
 */
@Slf4j
@Service
public class EnhancedHotkeyService {
    ...
    @Getter
    @Setter
    private HotkeyConfiguration configuration;
     ...
     */
    public void registerHotkeyActions(Runnable pauseAction, Runnable resumeAction,
                                      Runnable stopAction, Runnable togglePauseAction) {
        log.debug("Registering hotkey actions");
        
        hotkeyManager.registerAction(HotkeyAction.PAUSE, () -> {
            logHotkeyAction("PAUSE");
            pauseAction.run();
        });
        
        hotkeyManager.registerAction(HotkeyAction.RESUME, () -> {
            logHotkeyAction("RESUME");
            resumeAction.run();
        });
        
        hotkeyManager.registerAction(HotkeyAction.STOP, () -> {
            logHotkeyAction("STOP");
            stopAction.run();
        });
        
        hotkeyManager.registerAction(HotkeyAction.TOGGLE_PAUSE, () -> {
