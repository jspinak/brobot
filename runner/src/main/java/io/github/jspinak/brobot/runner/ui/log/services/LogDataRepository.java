...
/**
 * Repository service for managing log data. Provides thread-safe storage and querying of log
 * entries.
 */
@Slf4j
@Service
public class LogDataRepository {
    ...
    public static class RepositoryStats {
        ...
        public RepositoryStats(
                int totalEntries,
                Map<LogEntry.LogLevel, Integer> entriesByLevel,
                Map<String, Integer> entriesByType,
        ...
        public Map<LogEntry.LogLevel, Integer> getEntriesByLevel() {
    ...
    public void addLogEntry(LogEntry entry) {
        ...
        try {
            ...
            typeIndex
                    .computeIfAbsent(entry.getType(), k -> ConcurrentHashMap.newKeySet())
                    .add(entry.getId());
            sourceIndex
                    .computeIfAbsent(entry.getSource(), k -> ConcurrentHashMap.newKeySet())
                    .add(entry.getId());
            ...
            boolean matches =
                    entry.getMessage().toLowerCase().contains(searchLower)
                            ...
                            || (entry.getDetails() != null
                                    && entry.getDetails().toLowerCase().contains(searchLower));
            if (!matches) {
                return false;
            }
        }
    ...
    public RepositoryStats getStatistics() {
        ...
        try {
            ...
            return new RepositoryStats(
                    logEntries.size(), levelCounts, typeCounts, oldest, newest, totalSize);
