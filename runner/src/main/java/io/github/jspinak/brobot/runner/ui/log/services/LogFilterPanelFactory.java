...
/**
 * Factory service for creating log filter UI components.
 * Provides consistent filter panels with various configurations.
 */
@Slf4j
@Service
public class LogFilterPanelFactory {
    ...
    public static class FilterControls {
        ...
        public ComboBox<String> getLevelFilter() { return levelFilter; }
        public DatePicker getStartDatePicker() { return startDatePicker; }
        public DatePicker getEndDatePicker() { return endDatePicker; }
        public CheckBox getAutoScrollCheckBox() { return autoScrollCheckBox; }
        public Button getClearButton() { return clearButton; }
        public ComboBox<String> getPresetCombo() { return presetCombo; }
    }
     ...
     */
    public VBox createFilterPanel(FilterPanelConfiguration config, 
                                  Consumer<FilterState> onFilterChange) {
     ...
     */
    public HBox createFilterBar(FilterPanelConfiguration config,
                               Consumer<FilterState> onFilterChange) {
        ...
        if (config.showDateFilters) {
            controls.startDatePicker = createDatePicker("Start Date", date -> {
                state.setStartDate(date);
                onFilterChange.accept(state);
            });
            controls.endDatePicker = createDatePicker("End Date", date -> {
        ...
        if (config.showAutoScroll) {
            ...
            controls.autoScrollCheckBox.setOnAction(e -> {
        ...
        if (config.showClearButton) {
            ...
            controls.clearButton.setOnAction(e -> {
     ...
     */
    private ComboBox<String> createTypeFilter(FilterPanelConfiguration config,
                                            FilterState state,
                                            Consumer<FilterState> onChange) {
     ...
     */
    private ComboBox<String> createLevelFilter(FilterPanelConfiguration config,
                                             FilterState state,
                                             Consumer<FilterState> onChange) {
     ...
     */
    private ComboBox<String> createPresetSelector(FilterControls controls,
                                                 FilterState state,
                                                 Consumer<FilterState> onChange) {
    ...
    private void applyPreset(String presetName, FilterControls controls, FilterState state) {
        FilterPreset preset = defaultPresets.stream()
     ...
     */
    public HBox createActionBar(boolean showExport, 
                               boolean showClear,
                               Runnable onExport,
                               Runnable onClear) {
