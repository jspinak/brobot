...
/**
 * Unified AutomationPanel that merges functionality from both AutomationPanel and
 * EnhancedAutomationPanel without using singleton pattern.
 *
 * <p>Key improvements: - No singleton pattern (proper Spring DI) - Prevents label duplication -
 * Includes hotkey support - Window control features - Thread-safe UI updates
 */
@Slf4j
@Component
@Getter
...
public class UnifiedAutomationPanel extends VBox {
    ...
    /**
     * Refreshes the automation buttons based on the currently loaded project. Prevents duplicate
        ...
        Platform.runLater(
                () -> {
                    ...
                    try {
                        ...
                        for (Map.Entry<String, List<TaskButton>> entry :
                                buttonsByCategory.entrySet()) {
    ...
    private void showHotkeyConfiguration() {
        ...
        alert.setContentText(
                "Hotkey configuration dialog not yet implemented.\n\nDefault hotkeys:\n"
                        + "Ctrl+P - Pause\n"
                        + "Ctrl+R - Resume\n"
                        + "Ctrl+S - Stop\n"
