...
/**
 * Service that provides screenshot capture functionality for the Desktop Runner. Integrates both
 * JavaFX scene capture and Brobot's desktop screenshot capabilities.
 */
@Service
public class ScreenshotService {
            ...
            CompletableFuture.runAsync(
                    () -> {
                        try {
                            ...
                            if (!initialized) {
                                logger.warn(
                                        "Desktop screenshot capability not fully initialized -"
                                                + " screenshots may fail");
                            } else {
                                logger.info(
                                        "Desktop screenshot capability initialized successfully");
                            }
                        } catch (Exception e) {
    ...
    /**
     * Takes a screenshot of the desktop (full screen or active monitor). Uses pure Java AWT Robot
     * for reliability and runs asynchronously to prevent freezing.
     *
    ...
    /**
     * Takes a screenshot of the desktop asynchronously. This method returns immediately and
     * performs the capture in the background.
     *
     * @param baseFileName Base name for the screenshot file
     * @param callback Called when screenshot is complete (or fails)
     */
    public void captureDesktopAsync(
            String baseFileName, java.util.function.Consumer<String> callback) {
        String timestamp = LocalDateTime.now().format(TIMESTAMP_FORMAT);
        String filename = baseFileName + "-" + timestamp;
    ...
    /**
     * Takes a screenshot of the desktop using AWT Robot. This captures the primary screen
    ...
    public void captureTimedSeries(int duration, double frequency) {
        ...
        logger.info(
                "Starting timed screenshot series: {} seconds at {} second intervals",
                duration,
                frequency);
    ...
    /**
     * Takes both application and desktop screenshots. Useful for comparing JavaFX rendering with
     * desktop appearance.
     *
