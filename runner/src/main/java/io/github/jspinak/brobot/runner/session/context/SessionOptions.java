 ...
 *
 * <p>This class encapsulates all configurable parameters for a session, following the principle of
...
public class SessionOptions {
    ...
    @Builder.Default private final boolean autosaveEnabled = true;

    /** Interval between automatic saves */
    @Builder.Default private final Duration autosaveInterval = Duration.ofMinutes(5);

    /** Maximum number of session history entries to keep */
    @Builder.Default private final int maxSessionHistory = 10;

    /** Whether to capture screenshots during session */
    @Builder.Default private final boolean captureScreenshots = true;

    /** Whether to save execution results in session */
    @Builder.Default private final boolean saveExecutionResults = true;

    /** Maximum size for session data (in MB) */
    @Builder.Default private final int maxSessionSizeMB = 100;

    /** Whether to compress session files */
    @Builder.Default private final boolean compressSessionFiles = false;

    /** Session retention period (how long to keep old sessions) */
    @Builder.Default private final Duration retentionPeriod = Duration.ofDays(30);
