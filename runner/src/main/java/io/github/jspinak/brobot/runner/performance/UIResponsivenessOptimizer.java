...
public class UIResponsivenessOptimizer {
    ...
    private final ScheduledExecutorService scheduler =
            Executors.newSingleThreadScheduledExecutor(r -> new Thread(r, "ui-scheduler"));
    ...
    public void batchUpdate(String groupId, Runnable update, long maxDelayMs) {
        ...
        BatchedUIUpdate batch =
                batchedUpdates.computeIfAbsent(
                        groupId, k -> new BatchedUIUpdate(groupId, maxDelayMs));
    ...
    private void optimizeUIPerformance() {
        ...
        if (currentFps < lowFrameRateThreshold) {
            ...
            batchedUpdates
                    .values()
                    .forEach(batch -> batch.setMaxDelay(Math.min(batch.getMaxDelay() * 2, 200)));
        } else if (currentFps > targetFrameRate * 0.9) {
            // Performance is good, reduce batching delays
            batchedUpdates
                    .values()
                    .forEach(batch -> batch.setMaxDelay(Math.max(batch.getMaxDelay() / 2, 10)));
        }
    }

    @FunctionalInterface
    public interface IProgressTask<T> {
        T execute(IProgressUpdater progressUpdater, IMessageUpdater messageUpdater)
                throws Exception;
    }
    ...
    private static class FrameRateMonitor extends AnimationTimer {
        ...
        public double getCurrentFPS() {
            synchronized (frameRates) {
                ...
                return frameRates.stream().mapToDouble(Double::doubleValue).average().orElse(60.0);
            }
        }
    }
    ...
    public static class VirtualDataProvider<T> {
        ...
        private final Map<Integer, CompletableFuture<List<T>>> pageCache =
