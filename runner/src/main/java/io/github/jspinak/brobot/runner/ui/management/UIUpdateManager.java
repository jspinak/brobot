...
/**
 * Centralized manager for UI updates that ensures all updates happen on the JavaFX thread and
 * provides performance tracking and throttling capabilities.
 */
@Slf4j
@Component
public class UIUpdateManager {
    ...
    /**
     * Queues a UI update for batch processing. This is more efficient for high-frequency updates.
     *
     * @param taskId Identifier for tracking
     * @param update The update to execute
     */
     ...
     */
    public boolean schedulePeriodicUpdate(
            String taskId, Runnable update, long initialDelay, long period, TimeUnit unit) {
        ...
        ScheduledFuture<?> future =
                scheduler.scheduleAtFixedRate(
                        () -> executeUpdate(taskId, update), initialDelay, period, unit);
     ...
     * @return true if scheduled successfully, false if a task with this ID already exists
     */
    public boolean scheduleDelayedUpdate(
            String taskId, Runnable update, long delay, TimeUnit unit) {
        if (scheduledTasks.containsKey(taskId)) {
            log.warn("Task already scheduled: {}", taskId);
            return false;
        }
        ...
        Platform.runLater(
                () -> {
                    ...
                    if (executed > 0) {
                        ...
                        log.trace(
                                "Processed batch of {} updates in {}ms",
                                executed,
    ...
    private void recordMetrics(String taskId, long startTime) {
        ...
        metricsMap.computeIfAbsent(taskId, k -> new UpdateMetrics(k)).recordUpdate(duration);
    }
    ...
    public static class UpdateMetrics {
        ...
        void recordUpdate(long durationNanos) {
            ...
            do {
                currentMax = maxDuration.get();
                    ...
                    && !maxDuration.compareAndSet(currentMax, durationNanos));
            ...
            do {
                currentMin = minDuration.get();
                    ...
                    && !minDuration.compareAndSet(currentMin, durationNanos));
        }
        ...
        public String toString() {
            return String.format(
                    "UpdateMetrics[task=%s, updates=%d, avg=%.2fms, max=%.2fms, min=%.2fms]",
                    taskId,
                    getTotalUpdates(),
