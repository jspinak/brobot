...
/**
 * Error processor that logs errors with appropriate severity levels.
...
public class LoggingErrorProcessor implements IErrorProcessor {
    ...
    private String formatLogMessage(Throwable error, ErrorContext context) {
        ...
        if (context.getSeverity().getLevel() >= ErrorContext.ErrorSeverity.HIGH.getLevel()) {
            message.append(" [Memory: ").append(formatBytes(context.getMemoryUsed()))
                   .append(", Threads: ").append(context.getActiveThreads())
                   .append(", CPU: ").append(String.format("%.1f%%", context.getCpuUsage() * 100))
                   .append("]");
        }
