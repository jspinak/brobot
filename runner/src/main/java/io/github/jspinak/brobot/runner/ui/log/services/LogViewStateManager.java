...
/**
 * Service for managing the view state of the log panel. Tracks selection, auto-scroll, and status
 * information.
 */
@Slf4j
@Service
public class LogViewStateManager {
    ...
    private final ObjectProperty<DetailLevel> detailLevel =
            new SimpleObjectProperty<>(DetailLevel.NORMAL);
    ...
    public LogViewStateManager() {
        ...
        autoScroll.addListener(
                (obs, oldVal, newVal) -> log.debug("Auto-scroll changed to: {}", newVal));

        selectedEntry.addListener(
                (obs, oldVal, newVal) -> {
                    if (newVal != null) {
        ...
        detailLevel.addListener(
                (obs, oldVal, newVal) -> log.debug("Detail level changed to: {}", newVal));
    }
    ...
    public ViewState saveState() {
        String selectedId = selectedEntry.get() != null ? selectedEntry.get().getId() : null;

        return new ViewState(
                autoScroll.get(), showDetails.get(), wrapText.get(), detailLevel.get(), selectedId);
    }
    ...
    public String formatDetails(LogEntry entry) {
        ...
        if (level == DetailLevel.DETAILED || level == DetailLevel.DEBUG) {
            ...
            if (entry.hasException()) {
                details.append("\nException:\n")
                        .append(entry.getExceptionStackTrace())
                        .append("\n");
            }
    ...
    public DisplayOptions getDisplayOptions() {
        return new DisplayOptions(
                showDetails.get(), wrapText.get(), detailLevel.get(), autoScroll.get());
    }
