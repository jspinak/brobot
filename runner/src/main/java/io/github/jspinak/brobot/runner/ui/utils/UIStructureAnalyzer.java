...
public class UIStructureAnalyzer {
    ...
    private static void analyzeNode(Node node, int depth) {
        ...
        if (node instanceof TabPane) {
            ...
            for (Tab tab : tabPane.getTabs()) {
                logger.info("{}  Tab: '{}' (selected: {})", indent, tab.getText(), tab.isSelected());
    ...
    private static String getNodeInfo(Node node) {
        ...
        if (text != null && !text.isEmpty()) {
            info.append(" \"").append(text.length() > 30 ? text.substring(0, 30) + "..." : text).append("\"");
        }
        
        // Add bounds info
        info.append(" [").append(String.format("%.0fx%.0f", 
            node.getBoundsInLocal().getWidth(),
            node.getBoundsInLocal().getHeight())).append("]");
    ...
    private static void collectTextNodes(Node node, java.util.Map<String, java.util.List<Node>> textToNodes) {
        if (node == null || !node.isVisible()) {
            return;
        }
