 ...
 *
 * <p>- ThreadPoolManagementService: Manages thread pool lifecycle - ThreadMonitoringService:
 * Monitors thread health and metrics - ThreadPoolFactoryService: Creates thread pools with
 * configurations - ThreadOptimizationService: Optimizes thread usage
 *
 * <p>The orchestrator maintains backward compatibility with the original API while providing
...
public class ThreadManagementOptimizer implements DiagnosticCapable {
    ...
    public void initialize() {
        ...
        // Create shared pool
        sharedPool =
                factory.createWorkStealingPool(
                        "shared", Runtime.getRuntime().availableProcessors());
    ...
    private void monitorThreads() {
        try {
            ...
            // Detect contention
            List<ThreadMonitoringService.ThreadContentionInfo> contentions =
                    monitoring.detectContention();
    ...
    private void optimizeThreadUsage() {
        ThreadMonitoringService.ThreadStatistics stats = monitoring.getCurrentStatistics();
        ThreadOptimizationService.OptimizationResult result =
                optimization.optimizeThreadUsage(stats);
    ...
    public void reportThreadStatistics() {
        ...
        report.append(
                String.format("  Active pool threads: %d\n", stats.getTotalActivePoolThreads()));
    ...
    public void enableDiagnosticMode(boolean enabled) {
        ...
        log.info(
                "Diagnostic mode {} for ThreadManagementOptimizer and all services",
