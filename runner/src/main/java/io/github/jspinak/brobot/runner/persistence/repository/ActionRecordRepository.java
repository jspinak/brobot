...
/** Repository for ActionRecord entities with custom queries for filtering and analysis. */
@Repository
public interface ActionRecordRepository extends JpaRepository<ActionRecordEntity, Long> {
    ...
    @Query(
            "SELECT ar FROM ActionRecordEntity ar WHERE ar.session.id = :sessionId AND ar.stateName"
                    + " = :stateName")
    List<ActionRecordEntity> findBySessionIdAndStateName(
            @Param("sessionId") Long sessionId, @Param("stateName") String stateName);

    /** Find successful records in a session */
    @Query(
            "SELECT ar FROM ActionRecordEntity ar WHERE ar.session.id = :sessionId AND"
                    + " ar.actionSuccess = true")
    List<ActionRecordEntity> findSuccessfulBySessionId(@Param("sessionId") Long sessionId);

    /** Find failed records in a session */
    @Query(
            "SELECT ar FROM ActionRecordEntity ar WHERE ar.session.id = :sessionId AND"
                    + " ar.actionSuccess = false")
    List<ActionRecordEntity> findFailedBySessionId(@Param("sessionId") Long sessionId);

    /** Count records by action type in a session */
    @Query(
            "SELECT ar.actionConfigType, COUNT(ar) FROM ActionRecordEntity ar "
    ...
    @Query(
            "SELECT ar FROM ActionRecordEntity ar WHERE ar.session.id = :sessionId "
                    + "ORDER BY ar.duration DESC")
    Page<ActionRecordEntity> findSlowestInSession(
            @Param("sessionId") Long sessionId, Pageable pageable);

    /** Calculate average duration by action type */
    @Query(
            "SELECT ar.actionConfigType, AVG(ar.duration) FROM ActionRecordEntity ar "
                    + "WHERE ar.session.id = :sessionId GROUP BY ar.actionConfigType")
    List<Object[]> averageDurationByType(@Param("sessionId") Long sessionId);

    /** Find records with matches */
    @Query(
            "SELECT DISTINCT ar FROM ActionRecordEntity ar LEFT JOIN FETCH ar.matches "
    ...
    @Query(
            "SELECT ar FROM ActionRecordEntity ar WHERE ar.objectName = :objectName "
                    + "ORDER BY ar.timestamp DESC")
    Optional<ActionRecordEntity> findMostRecentByObjectName(
            @Param("objectName") String objectName, Pageable pageable);
