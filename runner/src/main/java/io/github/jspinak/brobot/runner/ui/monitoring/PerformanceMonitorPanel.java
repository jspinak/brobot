...
/**
 * Performance monitoring panel that displays real-time metrics from LabelManager and
 * UIUpdateManager.
 *
 * <p>This panel demonstrates how to monitor the refactored UI architecture and can be used during
...
public class PerformanceMonitorPanel extends VBox {
    ...
    private final ObservableList<ComponentMetric> componentMetrics =
    ...
    private TableView<ComponentMetric> createComponentMetricsTable() {
        ...
        table.setPlaceholder(
                labelManager.getOrCreateLabel(this, "noComponents", "No registered components"));
    ...
    private void startMonitoring() {
        ...
        uiUpdateManager.schedulePeriodicUpdate(
                UPDATE_TASK_ID, this::updateMetrics, 0, 2, TimeUnit.SECONDS);
    }
    ...
    private void updateSummary() {
        ...
        long slowTasks = taskMetrics.stream().filter(t -> "Slow".equals(t.getStatus())).count();
    ...
    public String exportMetricsReport() {
        ...
        taskMetrics.forEach(
                task -> {
                    report.append(
                            String.format(
                                    "  %s: %d updates, avg %.2s ms, max %s ms (%s)\n",
                                    task.getTaskId(),
                                    task.getUpdateCount(),
                                    task.getAverageTime(),
                                    task.getMaxTime(),
