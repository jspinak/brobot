...
/**
 * Refactored logs panel using service-oriented architecture.
 * Acts as a thin orchestrator coordinating between specialized services.
 */
@Slf4j
@Component
public class RefactoredAtlantaLogsPanel extends VBox implements AutoCloseable {
    ...
    private LogFilterService.FilterCriteria currentCriteria = LogFilterService.FilterCriteria.builder().build();
    ...
    private HBox createFilterBar() {
        ...
        searchField.textProperty().addListener((obs, old, text) -> {
            currentCriteria = LogFilterService.FilterCriteria.builder()
                .searchText(text)
    ...
    private HBox createMainContent() {
        ...
        LogTableFactory.TableConfiguration tableConfig = LogTableFactory.TableConfiguration.builder()
            .enableRowStyling(true)
            .enableTooltips(true)
            .enableLevelIndicators(true)
            .selectionHandler(entry -> viewStateManager.selectedEntryProperty().set(entry))
            .build();
        ...
        viewStateManager.selectedEntryProperty().addListener((obs, oldVal, newVal) -> {
            if (newVal != null) {
    ...
    private void setupEventHandling() {
        ...
        filteredLogs.addListener((javafx.collections.ListChangeListener<LogEntry>) c -> 
            viewStateManager.updateStatus(dataRepository.size(), filteredLogs.size())
        );
        
        // Auto-scroll handling
        viewStateManager.autoScrollProperty().addListener((obs, oldVal, newVal) -> {
    ...
    private void loadInitialLogs() {
        ...
        Task<List<LogData>> loadTask = new Task<>() {
            @Override
        ...
        loadTask.setOnSucceeded(e -> {
            ...
            List<LogEntry> entries = logData.stream()
                .map(this::convertLogData)
                .toList();
    ...
    private LogEntry convertLogData(LogData logData) {
        ...
        return LogEntry.builder()
            ...
            .source(logData.getCurrentStateName() != null ? logData.getCurrentStateName() : "System")
            .build();
    }
    ...
    private void exportLogs() {
        ...
        String defaultFilename = exportService.getDefaultFilename(LogExportService.ExportFormat.TEXT);
