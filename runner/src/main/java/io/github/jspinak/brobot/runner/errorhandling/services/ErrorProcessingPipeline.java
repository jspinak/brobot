...
/**
 * Service responsible for managing the error processing pipeline. Coordinates multiple error
 * processors that handle errors in sequence.
 */
@Slf4j
@Service
public class ErrorProcessingPipeline {
    ...
    public void initialize() {
        ...
        log.info(
                "Error processing pipeline initialized with {} processors", errorProcessors.size());
    }
    ...
    public void process(Throwable error, ErrorContext context) {
        ...
        for (IErrorProcessor processor : errorProcessors) {
            try {
                ...
                log.error(
                        "Error processor {} failed while processing {}",
    ...
    public int removeProcessorsByType(Class<? extends IErrorProcessor> processorType) {
        List<IErrorProcessor> toRemove =
                errorProcessors.stream().filter(processorType::isInstance).toList();
    ...
    public boolean executeProcessor(
            Class<? extends IErrorProcessor> processorType, Throwable error, ErrorContext context) {
        for (IErrorProcessor processor : errorProcessors) {
