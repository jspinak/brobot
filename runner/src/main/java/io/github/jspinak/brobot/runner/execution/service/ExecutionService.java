 ...
 *
 * <p>This service orchestrates the execution of automation tasks, coordinating with thread
 * management, timeout monitoring, and safety services.
 *
...
public class ExecutionService implements DiagnosticCapable {
    ...
    private final Map<String, Consumer<ExecutionStatus>> statusListeners =
    ...
    public ExecutionService(
            ExecutionThreadManager threadManager,
    ...
    public CompletableFuture<Void> execute(
            Runnable task, String taskName, ExecutionOptions options) {
        ...
        ExecutionOptions effectiveOptions =
                options != null ? options : ExecutionOptions.defaultOptions();
        ...
        log.info(
                "Starting execution - Task: {}, ID: {}, Correlation: {}",
                taskName,
                context.getId(),
                            ...
                            () -> {
                                try {
                                    ...
                                    throw new RuntimeException(
                                            "Execution interrupted during start delay", e);
                                }
                            })
    ...
    public boolean cancel(String executionId) {
        ...
        log.info(
                "Cancelling execution - ID: {}, Task: {}",
                executionId,
                handle.getContext().getTaskName());
        ...
        boolean cancelled =
                threadManager.cancel(
                        executionId, handle.getContext().getOptions().isInterruptible());
    ...
    public DiagnosticInfo getDiagnosticInfo() {
        ...
        activeExecutions.forEach(
                (id, handle) -> {
                    ...
                    states.put(
                            "execution." + id + ".duration",
                            status.getDuration() != null
                                    ? status.getDuration().toSeconds() + "s"
                                    : "not started");
                });

        return DiagnosticInfo.builder().component("ExecutionService").states(states).build();
    }
    ...
    private CompletableFuture<Void> executeInternal(
            Runnable task, ExecutionContext context, ExecutionHandle handle) {
        ...
        TimeoutHandler timeoutHandler =
                (id, taskName, elapsed) -> {
                    log.error(
                            "Execution timeout - ID: {}, Task: {}, Elapsed: {}",
                            id,
                            taskName,
                            elapsed);
                    ...
                    future.completeExceptionally(
                            new java.util.concurrent.TimeoutException(
                                    "Execution timed out after " + elapsed));
                };
        ...
        if (diagnosticMode.get()) {
            log.info(
                    "[DIAGNOSTIC] Execution started - ID: {}, Task: {}, Timeout: {}",
                    context.getId(),
                    context.getTaskName(),
    ...
    private void handleExecutionError(
            ExecutionContext context, ExecutionHandle handle, Exception error) {
        log.error(
                "Execution failed - ID: {}, Task: {}",
                context.getId(),
                context.getTaskName(),
                error);
