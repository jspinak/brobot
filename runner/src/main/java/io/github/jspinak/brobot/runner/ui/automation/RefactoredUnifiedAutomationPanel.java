...
/**
 * Refactored unified panel for automation management. Uses separated services and components
...
public class RefactoredUnifiedAutomationPanel extends BrobotPanel {
    ...
    private HBox createTopBar() {
        ...
        topBar.getChildren()
                .addAll(projectLabel, projectSelector, refreshButton, spacer, controlPanel);
    ...
    private void updateProjectList() {
        ...
        List<String> projectNames =
                projects.stream().map(ProjectInfo::getName).collect(Collectors.toList());
    ...
    private void loadProject(String projectName) {
        ...
        try {
            ...
            Optional<ProjectInfo> projectInfo =
                    projects.stream().filter(p -> p.getName().equals(projectName)).findFirst();
    ...
    private void loadCurrentProject() {
        ...
        Map<String, List<String>> categorizedAutomations =
                categorizeAutomations(project.getAutomationNames());
        ...
        for (Map.Entry<String, List<String>> entry : categorizedAutomations.entrySet()) {
            ...
            if (!automations.isEmpty()) {
                // Create category box
                VBox categoryBox =
                        buttonFactory.createCategoryBox(
                                category, "Contains " + automations.size() + " automation(s)");
    ...
    private Map<String, List<String>> categorizeAutomations(List<String> automations) {
        return automations.stream().collect(Collectors.groupingBy(this::extractCategory));
    }
    ...
    private String formatAutomationName(String name) {
        ...
        return name.replaceAll("([a-z])([A-Z])", "$1 $2").replaceAll("_", " ").trim();
    }
    ...
    private void setupEventListeners() {
        ...
        controlService.addExecutionListener(
                event -> {
                    Platform.runLater(
                            () -> {
                                switch (event.getType()) {
                                    ...
                                    case FAILED:
                                        log.debug(
                                                "Automation failed: {}", event.getAutomationName());
