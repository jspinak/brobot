...
/**
 * Refactored LogViewerPanel using extracted services. Coordinates between services and manages the
 * UI.
 */
@Slf4j
@Component
    ...
    public void postConstruct() {
        ...
        if (logTableView != null) {
            LogEntry welcomeEntry =
                    parsingService.createSimpleEntry(
                            "Log viewer initialized", LogEntry.LogLevel.INFO);
    ...
    private void exportLogs() {
        ...
        for (LogExportService.ExportFormat format : LogExportService.ExportFormat.values()) {
            FileChooser.ExtensionFilter filter =
                    new FileChooser.ExtensionFilter(format.getDescription(), format.getExtension());
                ...
                new Task<>() {
                    ...
                    protected Void call() throws Exception {
                        exportService.exportLogs(
                                logTableView.getAllEntries(), file.toPath(), format, options);
                        return null;
                    }
                };
                ...
                .ifPresent(
                        response -> {
                            if (response == ButtonType.OK) {
                                ...
                                // Add cleared message
                                LogEntry clearedEntry =
                                        parsingService.createSimpleEntry(
                                                "Log entries cleared", LogEntry.LogLevel.INFO);
