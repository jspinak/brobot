...
/**
 * Smart overlap detector that intelligently filters out false positives
 * and only reports genuine UI layout issues.
 */
public class SmartOverlapDetector {
                    ...
                    if (shouldCheckOverlap(node1, node2)) {
                        ...
                        if (boundsOverlap(bounds1, bounds2)) {
                            ...
                            if (intersection.getWidth() > 1 && intersection.getHeight() > 1) {
                                overlaps.add(new OverlapDetector.OverlapInfo(node1, node2, intersection));
                            }
                        }
                    }
                }
    ...
    /**
     * Determines if two nodes should be checked for overlap.
     * This method contains all the intelligence about what constitutes a "real" overlap.
     */
    ...
    private static boolean isButtonLabelOverlap(Node node1, Node node2) {
        boolean isButton1 = node1 instanceof ButtonBase || 
                           node1.getClass().getName().contains("Button");
        boolean isButton2 = node2 instanceof ButtonBase || 
                           node2.getClass().getName().contains("Button");
        boolean isLabel1 = node1 instanceof Label || node1 instanceof Text ||
                          node1.getClass().getName().contains("Label") ||
                          node1.getClass().getName().contains("Text");
        boolean isLabel2 = node2 instanceof Label || node2 instanceof Text ||
