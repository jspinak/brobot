...
/**
 * Refactored ResourceMonitorPanel using LabelManager and UIUpdateManager. This panel coordinates
...
public class RefactoredResourceMonitorPanel extends BrobotPanel {
    ...
    @Autowired
    public RefactoredResourceMonitorPanel(
        ...
        this.monitoringService =
                new ResourceMonitoringService(resourceManager, imageResourceManager, cacheManager);
    ...
    public void preDestroy() {
        ...
        if (metrics != null) {
            log.info(
                    "Resource monitor update performance - Total updates: {}, Average duration:"
                            + " {:.2f}ms",
    ...
    /**
     * Perform scheduled update - retrieves latest data and updates UI. This method is called by
     * UIUpdateManager on the scheduled interval.
     */
    ...
    /**
     * Handle monitoring data from the monitoring service. This is called whenever new monitoring
     * data is available.
     */
    ...
    /**
     * Update UI with latest monitoring data. This method is guaranteed to run on the JavaFX thread.
     */
    private void updateUI(ResourceMonitoringService.ResourceData data) {
        ...
        if (sessionManager.isSessionActive()) {
            ...
            String sessionInfo =
                    formatSessionInfo(sessionManager.getCurrentSession().getId(), lastAutosave);
        ...
        log.trace(
                "UI updated with resource data - Total: {}, Cached: {}, Memory: {:.2f}MB",
                data.getTotalResources(),
                data.getCachedImages(),
                data.getMemoryMB());
    }

    private String formatSessionInfo(String sessionId, LocalDateTime lastAutosave) {
        String lastAutosaveStr =
                lastAutosave != null
                        ? lastAutosave.format(DateTimeFormatter.ofPattern("HH:mm:ss"))
                        : "never";

        return String.format(
                "Active session: %s (last autosaved: %s)",
                sessionId.substring(0, 8) + "...", lastAutosaveStr);
    }

    /**
     * Inner class for the resource status card. Uses LabelManager for centralized label management.
     */
