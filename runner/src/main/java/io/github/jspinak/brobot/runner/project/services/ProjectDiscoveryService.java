...
/**
 * Service responsible for discovering and indexing automation projects.
 * Scans configured directories for valid projects and maintains an index.
 */
@Slf4j
@Service
public class ProjectDiscoveryService implements DiagnosticCapable {
    ...
    public ProjectDiscoveryService(EventBus eventBus, ProjectPersistenceService persistenceService) {
    ...
    public void scanForProjects() {
        ...
        eventBus.publish(LogEvent.info(this, 
            String.format("Discovered %d projects", foundCount), "Discovery"));
    }
    ...
    private int scanDirectory(Path directory) {
        ...
        try {
            Files.walkFileTree(directory, EnumSet.noneOf(FileVisitOption.class), 3, 
    ...
    private void trimRecentProjects() {
        ...
        List<Map.Entry<String, RecentProject>> sorted = recentProjects.entrySet().stream()
    ...
    public DiagnosticInfo getDiagnosticInfo() {
        ...
        diagnosticStates.put("lastScanTime", lastScanTime != null ? lastScanTime.toString() : "Never");
