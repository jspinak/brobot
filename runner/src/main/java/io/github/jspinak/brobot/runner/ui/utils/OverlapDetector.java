...
/**
 * Detects overlapping UI elements, particularly text and labels. This helps identify spacing and
 * layout issues in the UI.
 */
public class OverlapDetector {
    ...
    public static class OverlapInfo {
        ...
        public OverlapInfo(Node node1, Node node2, Bounds intersection) {
            ...
            this.description =
                    String.format(
                            "%s overlaps with %s",
                            getNodeDescription(node1), getNodeDescription(node2));
        }
    ...
    /**
     * Creates visual markers for overlapping areas. Adds red rectangles to the scene showing
     * overlap areas.
     *
    ...
    private static boolean boundsOverlap(Bounds b1, Bounds b2) {
        return b1.getMinX() < b2.getMaxX()
                && b1.getMaxX() > b2.getMinX()
                && b1.getMinY() < b2.getMaxY()
                && b1.getMaxY() > b2.getMinY();
    }
    ...
    /**
     * Checks if this is a button-internal overlap (button with its own label/text). This prevents
     * false positives from JavaFX's internal button structure.
     */
    private static boolean isButtonInternalOverlap(Node node1, Node node2) {
        ...
        if (class1.contains("Tab") || class2.contains("Tab")) {
            ...
            if (class1.contains("TabPaneSkin")
                    || class2.contains("TabPaneSkin")
                    || class1.contains("TabHeaderSkin")
                    || class2.contains("TabHeaderSkin")
                    || class1.contains("TabContentRegion")
                    || class2.contains("TabContentRegion")) {
                return true;
            }
        }
    ...
    /** Analyzes spacing issues between elements. Reports elements that are too close together. */
        ...
        for (int i = 0; i < allNodes.size(); i++) {
            for (int j = i + 1; j < allNodes.size(); j++) {
                ...
                if (distance < minSpacing && distance >= 0) {
                    spacingIssues++;
                    logger.warn(
                            "Spacing issue: {} and {} are only {} pixels apart",
    ...
    /**
     * Calculates minimum distance between two bounds. Returns 0 if they overlap, otherwise the
     * minimum gap.
     */
