...
/**
 * Special fix for making tabs clickable in TestFX tests.
 * This works around JavaFX's complex tab header structure.
 */
@Slf4j
    ...
    public static void makeTabsClickable(TabPane tabPane) {
        ...
        Platform.runLater(() -> {
            ...
            if (tabHeaderArea != null) {
                tabHeaderArea.setOnMouseClicked(event -> {
    ...
    private static void handleTabHeaderClick(MouseEvent event, TabPane tabPane) {
        ...
        if (tabCount > 0) {
            ...
            if (clickedIndex >= 0 && clickedIndex < tabCount) {
                ...
                log.debug("Tab header clicked at index: {} ({})", clickedIndex, clickedTab.getText());
