...
/**
 * Wrapper for tab content to ensure proper layout and prevent overlapping with tab headers. This
 * class provides a container that properly isolates tab content from the tab header area.
 */
public class TabContentWrapper extends VBox {
    ...
    public Node getContent() {
        return contentContainer.getChildren().isEmpty()
                ? null
                : contentContainer.getChildren().get(0);
    }

    /**
     * Creates a wrapped version of the given content. This is a convenience method for wrapping
