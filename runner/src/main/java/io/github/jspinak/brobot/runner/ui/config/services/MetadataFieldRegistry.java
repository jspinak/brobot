...
/**
 * Registry service for metadata field types. Manages field definitions and provides field creation
 * functionality.
 */
@Slf4j
@Service
public class MetadataFieldRegistry {
    ...
    private void registerDefaultFieldTypes() {
        // Text field
        registerFieldType(
                MetadataFieldDefinition.builder()
                        .type("text")
                        .displayName("Text")
                        ...
                        .controlFactory(
                                config -> {
                                    ...
                                    if (config.getInitialValue() != null) {
                                        try {
                                            LocalDate date =
                                                    LocalDate.parse(config.getInitialValue());
        ...
        registerFieldType(
                MetadataFieldDefinition.builder()
                        ...
                        .valueSetter(
                                (control, value) -> ((ComboBox<String>) control).setValue(value))
                        .defaultValue("")
                        .multiValue(false)
                        .maxLength(255)
                        .build());
                                ...
                                config -> {
                                    ...
                                    if (config.getInitialValue() != null) {
                                        checkBox.setSelected(
                                                Boolean.parseBoolean(config.getInitialValue()));
                                    }
                                    return checkBox;
                                })
                        .validator(value -> true)
                        .valueExtractor(
                                control -> String.valueOf(((CheckBox) control).isSelected()))
                        .valueSetter(
                                (control, value) ->
                                        ((CheckBox) control)
    ...
    public String normalizeKey(String key) {
        ...
        return key.toLowerCase().replaceAll("\\s+", "_").replaceAll("[^a-z0-9_]", "");
    }
}
