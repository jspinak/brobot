...
/**
 * Orchestrates automation execution by coordinating between the UI, event system, and execution
 * layer. This class acts as a high-level facade that manages automation tasks initiated from
 * TaskButtons, publishes events throughout the execution lifecycle, and provides control over
 * automation flow.
 */
@Getter
@Setter
...
public class AutomationOrchestrator {
    ...
    public AutomationOrchestrator(
        ...
        this.executionController.setLogCallback(
                message -> {
                    if (logCallback != null) {
    ...
    public void executeAutomation(TaskButton button) {
                ...
                () -> {
                    try {
                        ...
                        eventBus.publish(
                                LogEvent.info(
                                        this,
                                        "Starting automation: " + button.getLabel(),
                                        "Automation"));

                        // This is where you would integrate with the Brobot library to execute the
                        // function
                        // For example:
                        ...
                        eventBus.publish(
                                LogEvent.info(
                                        this,
                                        "Completed automation: " + button.getLabel(),
                                        "Automation"));
                    } catch (Exception e) {
                        // Log error
        ...
        try {
            executionController.executeAutomation(
                    button,
                    automationTask,
                    timeoutMillis,
                    executionEventPublisher.getStatusConsumer());
        } catch (Exception e) {
