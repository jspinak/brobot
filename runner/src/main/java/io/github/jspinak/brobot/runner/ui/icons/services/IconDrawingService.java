...
/**
 * Service that provides icon drawing implementations. Each icon type has its own drawing method.
 */
@Slf4j
@Service
public class IconDrawingService {
    ...
    private void drawThemeIcon(GraphicsContext gc, int size) {
        ...
        // Draw half moon
        gc.fillArc(
                centerX - radius, centerY - radius, radius * 2, radius * 2, 90, 180, ArcType.ROUND);
    }
    ...
    private void drawRefreshIcon(GraphicsContext gc, int size) {
        ...
        // Draw circular arrow
        gc.strokeArc(
                centerX - radius, centerY - radius, radius * 2, radius * 2, 45, 270, ArcType.OPEN);
    ...
    private void drawKeyboardIcon(GraphicsContext gc, int size) {
        ...
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 8; col++) {
                ...
                double y =
                        size - margin - keyboardHeight + keySpacing + row * (keySize + keySpacing);
