...
/** Validator for configuration metadata. Provides validation rules and error messages. */
public class ConfigMetadataValidator {
    ...
    private void validateProjectName(String projectName, ValidationResult result) {
        if (projectName == null || projectName.trim().isEmpty()) {
            ...
            result.addError("projectName", "Project name contains invalid characters");
        }
    }
        ...
        if (version != null && !version.trim().isEmpty()) {
            if (version.length() > MAX_VERSION_LENGTH) {
                result.addError(
                        "version",
                        String.format(
                                "Version must be less than %d characters", MAX_VERSION_LENGTH));
            } else if (!VERSION_PATTERN.matcher(version).matches()) {
                result.addError(
                        "version", "Invalid version format. Use semantic versioning (e.g., 1.0.0)");
            }
        }
    }
    ...
    private void validateAdditionalFields(
            Map<String, String> additionalFields, ValidationResult result) {
        ...
        for (Map.Entry<String, String> entry : additionalFields.entrySet()) {
            ...
            if (key == null || key.trim().isEmpty()) {
                ...
                result.addError(
                        "additionalField_" + key,
                        "Field key '"
                                + key
                                + "' contains invalid characters. Use lowercase letters, numbers,"
            ...
            if (value != null && value.length() > MAX_FIELD_LENGTH) {
                result.addError(
                        "additionalField_" + key,
                        String.format(
                                "Field '%s' value must be less than %d characters",
