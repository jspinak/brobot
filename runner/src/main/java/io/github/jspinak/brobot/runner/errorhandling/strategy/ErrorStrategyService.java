 ...
 *
 * <p>This service maintains a registry of error strategies mapped to exception types, finds the
 * appropriate strategy for a given error, and executes it.
 *
...
public class ErrorStrategyService implements DiagnosticCapable {
    ...
    private final Map<Class<? extends Throwable>, IErrorStrategy> strategies =
    ...
    public IErrorStrategy findStrategy(Throwable error) {
        ...
        if (strategy != null) {
            if (diagnosticMode.get()) {
                log.debug(
                        "[DIAGNOSTIC] Found exact strategy match for {}",
        ...
        while (currentClass != null && Throwable.class.isAssignableFrom(currentClass)) {
            ...
            if (strategy != null) {
                if (diagnosticMode.get()) {
                    log.debug(
                            "[DIAGNOSTIC] Found superclass strategy match for {} using {}",
                            error.getClass().getSimpleName(),
        ...
        for (Class<?> interfaceClass : error.getClass().getInterfaces()) {
            if (Throwable.class.isAssignableFrom(interfaceClass)) {
                ...
                if (strategy != null) {
                    if (diagnosticMode.get()) {
                        log.debug(
                                "[DIAGNOSTIC] Found interface strategy match for {} using {}",
                                error.getClass().getSimpleName(),
        ...
        if (diagnosticMode.get()) {
            log.debug(
                    "[DIAGNOSTIC] Using default strategy for {}", error.getClass().getSimpleName());
        }
    ...
    public ErrorResult executeStrategy(Throwable error, ErrorContext context) {
        ...
        if (strategy == defaultStrategy) {
            ...
            strategyExecutions
                    .computeIfAbsent(strategyName, k -> new AtomicLong())
                    .incrementAndGet();
        }
        ...
        try {
            ...
            if (diagnosticMode.get()) {
                log.info(
                        "[DIAGNOSTIC] Strategy {} returned result - Success: {}, Recoverable: {}",
                        strategyName,
                        result.isSuccess(),
    ...
    public DiagnosticInfo getDiagnosticInfo() {
        ...
        strategies.forEach(
                (errorType, strategy) -> {
                    states.put(
                            "mapping." + errorType.getSimpleName(),
                            strategy.getClass().getSimpleName());
                });

        return DiagnosticInfo.builder().component("ErrorStrategyService").states(states).build();
    }
