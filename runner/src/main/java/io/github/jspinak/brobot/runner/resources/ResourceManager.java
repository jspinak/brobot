...
public class ResourceManager {
    ...
    public void releaseResource(AutoCloseable resource) {
        try {
            String description = resourceTracker.getOrDefault(resource, resource.getClass().getSimpleName());
            resource.close();
            managedResources.remove(resource);
            int count = resourceCount.decrementAndGet();
            eventBus.publish(LogEvent.debug(this,
                    String.format("Resource released: %s (remaining: %d)", description, count),
                    "Resources"));
        } catch (Exception e) {
            eventBus.publish(LogEvent.error(this, "Failed to release resource: " + e.getMessage(), "Resources", e));
        }
    }
        ...
        for (Mat mat : mats) {
            if (mat != null && !mat.isNull()) {
                try {
                    mat.release();
                    eventBus.publish(LogEvent.debug(this,
                            "OpenCV Mat resource released", "Resources"));
                } catch (Exception e) {
                    eventBus.publish(LogEvent.error(this,
                            "Failed to release Mat resource: " + e.getMessage(), "Resources", e));
                }
            }
        }
    }
    ...
    public void shutdown() {
        ...
        for (int i = resourcesToClose.size() - 1; i >= 0; i--) {
            ...
            try {
                ...
                managedResources.remove(resource);
                eventBus.publish(LogEvent.debug(this,
                        "Resource cleaned up: " + description, "Resources"));
            } catch (Exception e) {
                eventBus.publish(LogEvent.error(this,
                        "Error during resource cleanup: " + e.getMessage(), "Resources", e));
            }
        }
        ...
        eventBus.publish(LogEvent.info(this,
                "Resource cleanup completed, remaining resources: " + managedResources.size(),
                "Resources"));
    }
