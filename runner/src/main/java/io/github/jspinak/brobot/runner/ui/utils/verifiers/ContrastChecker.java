...
public class ContrastChecker {
    ...
    public List<TextVisibilityIssue> checkTextVisibility(List<Node> nodes) {
        ...
        for (Node node : nodes) {
            ...
            if (backgroundColor == null) {
                // If we can't determine background, assume white for light theme, dark for dark
                // theme
            ...
            if (contrastRatio < minRatio) {
                issues.add(
                        new TextVisibilityIssue(
                                node, text, textColor, backgroundColor, contrastRatio));
            }
        }
    ...
    private String extractStyleValue(String style, String property) {
        ...
        while (start < style.length()
                && (style.charAt(start) == ':' || style.charAt(start) == ' ')) {
            start++;
        }
