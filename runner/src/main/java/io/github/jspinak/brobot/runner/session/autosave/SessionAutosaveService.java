 ...
 *
 * <p>This service handles scheduling and execution of periodic session saves, ensuring data is not
 * lost during long-running automation tasks.
 *
...
public class SessionAutosaveService implements DiagnosticCapable {
                ...
                Executors.newScheduledThreadPool(
                        ...
                        r -> {
                            Thread thread =
                                    new Thread(
                                            r,
                                            "session-autosave-" + Thread.currentThread().getId());
    ...
    public void enableAutosave(SessionContext context, Consumer<Session> saveHandler) {
        ...
        if (diagnosticMode.get()) {
            log.info(
                    "[DIAGNOSTIC] Autosave enabled - Session: {}, Interval: {}",
                    sessionId,
                    interval);
        }
    }
    ...
    public DiagnosticInfo getDiagnosticInfo() {
        ...
        return DiagnosticInfo.builder().component("SessionAutosaveService").states(states).build();
    }
    ...
    private void performAutosave(String sessionId, Consumer<Session> saveHandler) {
        ...
        try {
            ...
            if (diagnosticMode.get()) {
                log.info(
                        "[DIAGNOSTIC] Autosave completed - Session: {}, Total saves: {}",
                        sessionId,
                        status.getSaveCount());
            }
