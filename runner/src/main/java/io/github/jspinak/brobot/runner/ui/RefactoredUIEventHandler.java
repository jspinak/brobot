...
/**
 * Refactored UI event handler that uses dependency injection instead of singletons. This handler
 * subscribes to events and updates UI components through the UIComponentRegistry.
 */
@Component
@Slf4j
@Data
public class RefactoredUIEventHandler {
    ...
    public void cleanup() {
        ...
        for (Map.Entry<BrobotEvent.EventType, Consumer<BrobotEvent>> entry :
    ...
    private void setupEventHandlers() {
        ...
        eventHandlers.put(
                BrobotEvent.EventType.EXECUTION_COMPLETED, this::handleExecutionCompleted);
        ...
        eventHandlers.put(
                BrobotEvent.EventType.CONFIG_LOADING_FAILED, this::handleConfigLoadingFailed);
    }
    ...
    /**
     * Publishes an event to notify UI components to update themselves. This approach decouples the
     * event handler from specific UI components.
     */
    ...
    private void handleExecutionProgress(BrobotEvent event) {
        ...
        notifyUIComponents("execution.progress", Map.of("progress", statusEvent.getProgress()));
    }
    ...
    private void handleExecutionCompleted(BrobotEvent event) {
        ...
        notifyUIComponents("execution.completed", Map.of("message", "Completed successfully"));
    }
    ...
    private void handleExecutionFailed(BrobotEvent event) {
        ...
        notifyUIComponents("execution.failed", Map.of("message", statusEvent.getMessage()));
    }
    ...
    private void handleExecutionPaused(BrobotEvent event) {
        ...
        notifyUIComponents("execution.paused", Map.of("message", "Paused"));
    }
    ...
    private void handleExecutionResumed(BrobotEvent event) {
        ...
        notifyUIComponents("execution.resumed", Map.of("message", "Running"));
    }
    ...
    private void handleExecutionStopped(BrobotEvent event) {
        ...
        notifyUIComponents("execution.stopped", Map.of("message", "Stopped"));
    }
    ...
    private void handleLogWarning(BrobotEvent event) {
        ...
        notifyUIComponents("log.warning", Map.of("message", "WARNING: " + logEvent.getMessage()));
    }
    ...
    private void handleLogError(BrobotEvent event) {
        ...
        notifyUIComponents("log.error", Map.of("message", "ERROR: " + logEvent.getMessage()));
    }
    ...
    private void handleConfigLoaded(BrobotEvent event) {
        ...
        notifyUIComponents("config.loaded", Map.of("configName", configEvent.getConfigName()));
    }
        ...
        Platform.runLater(
                () -> {
                    ...
                    showErrorDialog(
                            ...
                            configEvent.getDetails(),
                            configEvent.getError());
                });
    }
    ...
    private void showErrorDialog(String title, String header, String content, Exception exception) {
        ...
        if (exception != null) {
            ...
            if (stackTrace.length > 0) {
                ...
                Arrays.stream(stackTrace)
                        ...
                        .forEach(
                                element ->
                                        fullContent.append("  at ").append(element).append("\n"));
        ...
        io.github.jspinak.brobot.runner.ui.dialogs.ErrorDialog.show(
                title, header, fullContent.toString());
    }
}
