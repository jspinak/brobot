...
/**
 * Service for controlling automation execution. Provides methods to run, pause, resume, and stop
 * automations.
 */
@Slf4j
@Service
public class AutomationControlService {
    ...
    /**
     * Executes an automation by name. Returns a CompletableFuture that completes when the
     * automation finishes.
     */
    public CompletableFuture<ExecutionResult> executeAutomation(String automationName) {
        ...
        io.github.jspinak.brobot.runner.execution.ExecutionStatus status =
                orchestrator.getExecutionStatus();
        if (status != null
                && status.getState()
                        == io.github.jspinak.brobot.runner.execution.ExecutionState.RUNNING) {
        ...
        if (project == null) {
            ...
            return CompletableFuture.completedFuture(ExecutionResult.error("No project loaded"));
        }

        // Find the task button for this automation
        io.github.jspinak.brobot.runner.project.TaskButton taskButton =
                project.getAutomations().stream()
                ...
                CompletableFuture.supplyAsync(
                        () -> {
                            try {
                                ...
                                // Wait for completion
                                io.github.jspinak.brobot.runner.execution.ExecutionStatus
                                        execStatus;
                                do {
                                    Thread.sleep(100);
                                    execStatus = orchestrator.getExecutionStatus();
                                                ...
                                                && execStatus.getState()
                                                        == io.github.jspinak.brobot.runner.execution
                            ...
                            () -> {
                                if (result.isSuccess()) {
                                    notifyExecutionListeners(
                                            ExecutionEvent.completed(automationName));
                                } else {
                                    notifyExecutionListeners(
                                            ExecutionEvent.failed(
                                                    automationName, result.getError()));
                                }
                            });
                });
    ...
    public boolean pauseAutomation() {
        ...
        String automationName =
                status.getCurrentTaskName() != null ? status.getCurrentTaskName() : "Unknown";
    ...
    public boolean resumeAutomation() {
        ...
        if (status == null
                || (status.getState()
                                != io.github.jspinak.brobot.runner.execution.ExecutionState.RUNNING
                        && status.getState()
                                != io.github.jspinak.brobot.runner.execution.ExecutionState
                                        .PAUSED)) {
    ...
    public boolean isRunning() {
        ...
        return status != null
                && (status.getState()
                                == io.github.jspinak.brobot.runner.execution.ExecutionState.RUNNING
                        || status.getState()
                                == io.github.jspinak.brobot.runner.execution.ExecutionState.PAUSED);
    }
    ...
    private CompletableFuture<ExecutionResult> runAutomationsSequentially(
            List<String> automations) {
        if (automations.isEmpty()) {
