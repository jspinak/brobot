 ...
 *
 * <p>This class is responsible for thread lifecycle management, including creation, scheduling,
 * monitoring, and cleanup of execution threads.
 *
...
public class ExecutionThreadManager implements DiagnosticCapable {
    ...
    public ExecutionThreadManager() {
        ...
        log.info(
                "ExecutionThreadManager initialized with core pool size: {}, max pool size: {}",
                CORE_POOL_SIZE,
                MAX_POOL_SIZE);
    }
    ...
    public Future<?> submit(Runnable task, ExecutionContext context) {
        ...
        log.debug(
                "Submitting task {} with correlation ID {}",
                context.getTaskName(),
        ...
        if (diagnosticMode.get()) {
            log.info(
                    "[DIAGNOSTIC] Task submitted - ID: {}, Name: {}, Active tasks: {}",
                    context.getId(),
                    context.getTaskName(),
    ...
    private Runnable wrapTask(Runnable task, ExecutionContext context) {
        return () -> {
            ...
            try {
                ...
                Thread.currentThread().setName(originalThreadName + "-" + context.getTaskName());
                ...
                log.debug(
                        "Starting execution of task {} on thread {}",
                        context.getTaskName(),
                ...
                log.error(
                        "Error executing task {} with correlation ID {}",
                        context.getTaskName(),
                        context.getCorrelationId(),
                        e);
                throw e;
