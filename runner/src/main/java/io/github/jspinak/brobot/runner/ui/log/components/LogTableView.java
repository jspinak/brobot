...
/**
 * Custom TableView for displaying log entries with advanced features. Includes filtering, sorting,
...
public class LogTableView extends TableView<LogEntry> {
    ...
    private void setupColumns() {
        ...
        timeColumn.setCellValueFactory(
                data ->
                        new SimpleStringProperty(
                                data.getValue().getTimestamp().format(TIME_FORMAT)));
        ...
        levelColumn.setCellValueFactory(
                data -> new SimpleObjectProperty<>(data.getValue().getLevel()));
    ...
    private Callback<
                    TableColumn<LogEntry, LogEntry.LogLevel>,
                    TableCell<LogEntry, LogEntry.LogLevel>>
            createLevelCellFactory() {
        return column ->
                new TableCell<>() {
                    @Override
                ...
                tv -> {
                                ...
                                protected void updateItem(LogEntry item, boolean empty) {
                                    ...
                                    if (empty || item == null) {
                                        ...
                                        switch (item.getLevel()) {
                                            case ERROR:
                                            case FATAL:
                                                setStyle("-fx-background-color: #ffebee;"); // Light
                                                // red
                                                break;
                                            case WARNING:
                                                setStyle("-fx-background-color: #fff3e0;"); // Light
                                                // orange
                                                break;
                                            default:
                                                setStyle("");
                    ...
                    row.setOnContextMenuRequested(
                            event -> {
                                if (!row.isEmpty()) {
                                    showContextMenu(
                                            row.getItem(), event.getScreenX(), event.getScreenY());
                                }
                            });
