 ...
 *
 * <p>This class acts as a thin orchestrator that delegates responsibilities to specialized
 * services:
 *
 * <ul>
 *   <li>{@link SessionLifecycleService} - Manages session lifecycle (start, end, active sessions)
 *   <li>{@link SessionPersistenceService} - Handles all file I/O operations
 *   <li>{@link SessionStateService} - Captures and restores application state
 *   <li>{@link SessionAutosaveService} - Manages automatic periodic saving
 *   <li>{@link SessionDiscoveryService} - Provides session search and discovery
 * </ul>
 *
 * <p>Thread Safety: This class is thread-safe through delegation to thread-safe services.
 *
 * @see Session
 * @see SessionContext
 * @see SessionOptions
...
public class SessionManager implements AutoCloseable, DiagnosticCapable {
    ...
    public Session startNewSession(String projectName, String configPath, String imagePath) {
        ...
        eventBus.publish(LogEvent.info(this, "Session started: " + session.getId(), "Session"));
    ...
    public void endCurrentSession() {
        ...
        eventBus.publish(LogEvent.info(this, "Session ended: " + sessionId, "Session"));
    }
    ...
    public void saveSession(Session session) {
        ...
        try {
            ...
            eventBus.publish(
                    LogEvent.error(
                            this, "Failed to save session: " + e.getMessage(), "Session", e));
        }
    }
    ...
    public Optional<Session> loadSession(String sessionId) {
        ...
        try {
            ...
            if (session.isPresent()) {
                eventBus.publish(LogEvent.info(this, "Session loaded: " + sessionId, "Session"));
            } else {
                eventBus.publish(
                        LogEvent.warning(this, "Session not found: " + sessionId, "Session"));
            }
            ...
            eventBus.publish(
                    LogEvent.error(
                            this, "Failed to load session: " + e.getMessage(), "Session", e));
    ...
    public boolean restoreSession(String sessionId) {
        ...
        try {
            ...
            eventBus.publish(LogEvent.info(this, "Session restored: " + sessionId, "Session"));
            ...
            eventBus.publish(
                    LogEvent.error(
                            this, "Failed to restore session: " + e.getMessage(), "Session", e));
            return false;
        }
    }
    ...
    public boolean deleteSession(String sessionId) {
        ...
        if (deleted) {
            eventBus.publish(LogEvent.info(this, "Session deleted: " + sessionId, "Session"));
        }
    ...
    public DiagnosticInfo getDiagnosticInfo() {
        ...
        states.put(
                "currentSessionId",
                getCurrentSession() != null ? getCurrentSession().getId() : null);

        return DiagnosticInfo.builder().component("SessionManager").states(states).build();
    }
    ...
    public void enableDiagnosticMode(boolean enabled) {
        ...
        log.info(
                "Diagnostic mode {} for SessionManager and all services",
