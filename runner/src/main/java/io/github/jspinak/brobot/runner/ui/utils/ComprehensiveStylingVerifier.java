...
/**
 * Comprehensive styling verification system that detects: 1. Duplicate rendering of components 2.
 * Tab content isolation issues 3. Z-order and layering problems 4. Container boundary violations 5.
 * Text visibility and contrast issues
 *
 * <p>This class now delegates to specialized verifiers for better maintainability.
 */
public class ComprehensiveStylingVerifier {
    private static final Logger logger =
            LoggerFactory.getLogger(ComprehensiveStylingVerifier.class);

    private static final StylingVerificationOrchestrator orchestrator =
            new StylingVerificationOrchestrator();
    ...
    public static class VerificationResult {
        ...
        private VerificationResult(
                StylingVerificationOrchestrator.VerificationResult orchestratorResult) {
            ...
            for (var issue : orchestratorResult.tabIsolationIssues) {
                this.tabIsolationIssues.add(
                        new TabIsolationIssue(
                                issue.activeTab, issue.leakingTab, issue.leakingNode));
            }
            ...
            for (var violation : orchestratorResult.containerViolations) {
                this.containerViolations.add(
                        new ContainerViolation(violation.node, violation.container));
            }

            this.textVisibilityIssues = new ArrayList<>();
            for (var issue : orchestratorResult.textVisibilityIssues) {
                this.textVisibilityIssues.add(
                        new TextVisibilityIssue(
                                issue.node,
                                issue.text,
                                issue.textColor,
            ...
            for (var issue : orchestratorResult.styleDivergenceResult.issues) {
                this.styleDivergenceIssues.add(
                        new StyleDivergenceIssue(
                                issue.node, issue.issueType, issue.description, issue.severity));
            }

            this.overallDivergenceScore =
                    orchestratorResult.styleDivergenceResult.overallDivergenceScore;
            this.totalDivergenceIssues =
                    orchestratorResult.styleDivergenceResult.totalDivergenceIssues;
        }
    ...
    public static class TabIsolationIssue {
        ...
        public TabIsolationIssue(
                javafx.scene.control.Tab activeTab,
                javafx.scene.control.Tab leakingTab,
                javafx.scene.Node leakingNode) {
            this.activeTab = activeTab;
            this.leakingTab = leakingTab;
            this.leakingNode = leakingNode;
            this.description =
                    String.format(
                            "Content from tab '%s' visible while tab '%s' is active: %s",
                            leakingTab.getText(),
                            activeTab.getText(),
                            NodeDescriptionUtil.getNodeDescription(leakingNode));
        }
    }
    ...
    public static class StyleDivergenceIssue {
        ...
        public StyleDivergenceIssue(
                javafx.scene.Node node, String issueType, String description, double severity) {
            this.node = node;
    ...
    public static void logVerificationResults(VerificationResult result) {
        orchestrator.logVerificationResults(
                new StylingVerificationOrchestrator.VerificationResult(
                        // Convert back to orchestrator types for logging
                        result.duplicates.stream()
                                .map(d -> new DuplicateDetector.DuplicateIssue(d.text, d.nodes))
                                .toList(),
                        result.tabIsolationIssues.stream()
                                .map(
                                        i ->
                                                new TabIsolationVerifier.TabIsolationIssue(
                                                        i.activeTab, i.leakingTab, i.leakingNode))
                                .toList(),
                        result.zOrderIssues.stream()
                                .map(z -> new ZOrderAnalyzer.ZOrderIssue(z.frontNode, z.backNode))
                                .toList(),
                        result.containerViolations.stream()
                                .map(
                                        c ->
                                                new ContainerBoundaryChecker.ContainerViolation(
                                                        c.node, c.container))
                                .toList(),
                        result.textVisibilityIssues.stream()
                                .map(
                                        t ->
                                                new ContrastChecker.TextVisibilityIssue(
                                                        t.node,
                                                        t.text,
                                                        t.textColor,
                                                        t.backgroundColor,
                        ...
                        new StyleDivergenceAnalyzer.StyleDivergenceResult() {
                            {
                                issues.addAll(
                                        result.styleDivergenceIssues.stream()
                                                .map(
                                                        s ->
                                                                new StyleDivergenceAnalyzer
                                                                        .StyleDivergenceIssue(
                                                                        s.node,
                                                                        s.issueType,
                                                                        s.description,
                                                                        s.severity))
