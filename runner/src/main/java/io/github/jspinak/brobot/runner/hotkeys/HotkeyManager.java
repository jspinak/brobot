 ...
 * Provides configurable key combinations for pause, resume, and stop operations.
 */
@Component
public class HotkeyManager {
    ...
    // Default hotkeys
    private static final KeyCombination DEFAULT_PAUSE_KEY = new KeyCodeCombination(KeyCode.P, KeyCombination.CONTROL_DOWN);
    private static final KeyCombination DEFAULT_RESUME_KEY = new KeyCodeCombination(KeyCode.R, KeyCombination.CONTROL_DOWN);
    private static final KeyCombination DEFAULT_STOP_KEY = new KeyCodeCombination(KeyCode.S, KeyCombination.CONTROL_DOWN);
    private static final KeyCombination DEFAULT_TOGGLE_PAUSE_KEY = new KeyCodeCombination(KeyCode.SPACE, KeyCombination.CONTROL_DOWN);
    ...
    public void registerWithScene(Scene scene) {
        ...
        for (Map.Entry<HotkeyAction, KeyCombination> entry : hotkeys.entrySet()) {
            ...
            scene.getAccelerators().put(keyCombination, () -> {
                ...
                if (handler != null) {
                    logger.debug("Hotkey triggered: {} ({})", action, keyCombination);
                    Platform.runLater(handler);
                } else {
                    logger.warn("No handler registered for hotkey action: {}", action);
                }
            });
        }
    ...
    public void updateHotkey(HotkeyAction action, KeyCombination newCombination) {
        ...
        if (registeredScene != null) {
            registeredScene.getAccelerators().put(newCombination, () -> {
                Runnable handler = actions.get(action);
                if (handler != null) {
                    Platform.runLater(handler);
                }
