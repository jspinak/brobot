...
/**
 * Registry for UI components that tracks all created components. Uses WeakReferences to prevent
 * memory leaks.
 *
 * <p>This helps prevent duplicate component creation and provides a central place to track all UI
...
public class UIComponentRegistry {
    ...
    /**
     * Gets the count of registered components. Note: This includes components that may have been
     * garbage collected.
     *
    ...
    /**
     * Gets a summary of all registered components. Useful for debugging.
     *
    ...
    /** Logs the current state of the registry. Useful for debugging duplicate component issues. */
    public void logRegistryState() {
        ...
        if (log.isDebugEnabled()) {
            summary.forEach((id, className) -> log.debug("  {} -> {}", id, className));
        }
    }
}
