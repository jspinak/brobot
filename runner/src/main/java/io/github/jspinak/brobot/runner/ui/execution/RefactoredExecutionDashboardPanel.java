...
/**
 * Refactored comprehensive dashboard for execution control and monitoring. Uses LabelManager and
 * UIUpdateManager for better resource management.
 *
 * <p>This dashboard combines multiple specialized panels to provide a complete interface for
 * controlling and monitoring automation execution. It includes:
 *
 * <ul>
 *   <li>Execution control buttons (play, pause, stop)
 *   <li>Status display with progress tracking
 *   <li>Performance metrics and charts
 *   <li>State transition history
 *   <li>Action history table
 * </ul>
 */
@Slf4j
@Component
...
public class RefactoredExecutionDashboardPanel extends BorderPane {
     ...
     */
    @Autowired
    public RefactoredExecutionDashboardPanel(
            EventBus eventBus,
    ...
    private void setupEventHandlers() {
        ...
        eventBus.subscribe(
                BrobotEvent.EventType.AUTOMATION_PROGRESS, this::handleExecutionProgress);
        eventBus.subscribe(
                BrobotEvent.EventType.AUTOMATION_COMPLETED, this::handleExecutionCompleted);
    ...
    /**
     * Updates memory usage display. This method is called by UIUpdateManager and is thread-safe.
     */
                ...
                () -> {
                    if (event instanceof ExecutionStatusEvent) {
                        ...
                        if (status.getCurrentState() != null) {
                            State state =
                                    allStatesInProjectService.getAllStates().stream()
                                            .filter(
                                                    s ->
                                                            s.getName()
                                                                    .equals(
        ...
        if (event instanceof LogEvent) {
            ...
            uiUpdateManager.queueUpdate(
                    ...
                    () -> {
                        ...
                        } else if (logEvent.getCategory() != null
                                && logEvent.getCategory().equals("ACTION")) {
    ...
    private void logPerformanceMetrics() {
        UIUpdateManager.UpdateMetrics memoryMetrics =
                uiUpdateManager.getMetrics(MEMORY_UPDATE_TASK_ID);
        if (memoryMetrics != null) {
            log.info(
                    "Memory update performance - Total: {}, Avg: {:.2f}ms",
                    memoryMetrics.getTotalUpdates(),
                    memoryMetrics.getAverageDurationMs());
        }

        UIUpdateManager.UpdateMetrics statusMetrics =
                uiUpdateManager.getMetrics(STATUS_UPDATE_TASK_ID);
        if (statusMetrics != null) {
            log.info(
                    "Status update performance - Total: {}, Avg: {:.2f}ms",
                    statusMetrics.getTotalUpdates(),
                    statusMetrics.getAverageDurationMs());
        }
    }
    ...
    public String getPerformanceSummary() {
        ...
        if (memoryMetrics != null) {
            summary.append(
                    String.format(
                            "  Memory Updates: %d total, %.2f ms avg\n",
        ...
        if (statusMetrics != null) {
            summary.append(
                    String.format(
                            "  Status Updates: %d total, %.2f ms avg\n",
