...
/**
 * JavaFX panel for controlling action recording. Provides start/stop controls, session management,
...
public class RecordingControlPanel extends VBox {
    ...
    @Autowired private PersistenceAdapterService recordingService;

    @Autowired private ActionHistoryExportService exportService;
                ...
                dialogButton -> {
                    if (dialogButton == startButtonType) {
                        return new SessionInfo(
                                nameField.getText(), appField.getText(), descField.getText());
                    }
                    return null;
                });
    ...
    private void stopRecording() {
        ...
        if (sessionId != null) {
            ...
            statusLabel.setText(
                    "Recording stopped - " + metadata.getTotalActions() + " actions recorded");
            ...
            if (file != null) {
                try {
                    ...
                    exportService.exportSessionToFile(
                            Long.parseLong(currentSessionId), file, format);
