...
public class StartupOptimizer {
    ...
    private void publishProgress(String taskName, int completed) {
        ...
        eventBus.publish(new StartupProgressEvent(taskName, completed, totalTasks, progress));
    }
    ...
    public static IStartupTask retryableTask(
            String name, Supplier<CompletableFuture<Void>> supplier) {
        return new IStartupTask() {
            @Override
            public CompletableFuture<Void> execute() {
                return supplier.get();
