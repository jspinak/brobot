 ...
 * 
 * This class monitors running executions and triggers timeout actions
 * when executions exceed their configured time limits.
 * 
...
public class ExecutionTimeoutManager implements DiagnosticCapable {
    ...
    public void monitor(ExecutionContext context, TimeoutHandler handler) {
        ...
        Duration timeout = context.getOptions() != null && context.getOptions().getTimeout() != null
        ...
        log.debug("Monitoring execution {} for timeout after {}",
                context.getTaskName(), timeout);
        
        if (diagnosticMode.get()) {
            log.info("[DIAGNOSTIC] Timeout monitor added - ID: {}, Task: {}, Timeout: {}",
                    context.getId(), context.getTaskName(), timeout);
        }
    }
    ...
    public DiagnosticInfo getDiagnosticInfo() {
        ...
        monitors.forEach((id, monitor) -> {
            ...
            states.put("execution." + id + ".elapsed", monitor.getElapsedTime().toSeconds() + "s");
            states.put("execution." + id + ".remaining", monitor.getRemainingTime().toSeconds() + "s");
    ...
    private void handleTimeout(String executionId, TimeoutMonitor monitor) {
        ...
        log.warn("Execution {} timed out after {}",
                monitor.getTaskName(), monitor.getElapsedTime());
        ...
        if (diagnosticMode.get()) {
            log.info("[DIAGNOSTIC] Timeout handled - ID: {}, Task: {}, Elapsed: {}",
                    executionId, monitor.getTaskName(), monitor.getElapsedTime());
        }
    }
    ...
    private static class TimeoutMonitor {
        ...
        public TimeoutMonitor(String executionId, String taskName, Instant startTime,
