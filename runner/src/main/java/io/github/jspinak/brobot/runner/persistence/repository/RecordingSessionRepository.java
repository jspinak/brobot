...
/**
 * Repository for RecordingSession entities with custom queries
 * for session management and analysis.
 */
@Repository
public interface RecordingSessionRepository extends JpaRepository<RecordingSessionEntity, Long> {
     ...
     */
    @Query("SELECT rs FROM RecordingSessionEntity rs WHERE " +
     ...
     */
    @Query("SELECT rs FROM RecordingSessionEntity rs WHERE rs.endTime < :cutoffDate " +
           "AND rs.status != 'ARCHIVED'")
    List<RecordingSessionEntity> findSessionsForCleanup(@Param("cutoffDate") LocalDateTime cutoffDate);
    ...
    @Query("SELECT " +
           ...
           "SUM(rs.totalActions) as totalActions, " +
           "SUM(rs.successfulActions) as successfulActions, " +
           "AVG(rs.successfulActions * 1.0 / NULLIF(rs.totalActions, 0)) as avgSuccessRate " +
           "FROM RecordingSessionEntity rs WHERE rs.application = :application")
    Object[] getStatisticsForApplication(@Param("application") String application);
    
    /**
     * Find longest sessions
     */
    @Query("SELECT rs FROM RecordingSessionEntity rs WHERE rs.endTime IS NOT NULL " +
     ...
     */
    @Query("UPDATE RecordingSessionEntity rs SET rs.status = 'ARCHIVED' " +
