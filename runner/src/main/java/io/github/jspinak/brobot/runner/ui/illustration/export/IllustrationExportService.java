 ...
 * <ul>
 * <li>Individual image export (PNG, JPEG)</li>
 * <li>Batch export as ZIP archive</li>
 * <li>PDF report generation</li>
 * <li>Video compilation of action sequences</li>
 * <li>Markdown documentation with embedded images</li>
 * </ul>
 *
...
public class IllustrationExportService {
    ...
    public Path exportSingle(Long illustrationId, ExportFormat format, String outputPath) {
        try {
            // Load illustration
            IllustrationEntity illustration = galleryService.getIllustrationById(illustrationId)
                .orElseThrow(() -> new IllegalArgumentException("Illustration not found"));
            ...
            switch (format) {
                ...
                default:
                    throw new UnsupportedOperationException("Format not supported for single export: " + format);
            }
        ...
        try {
            ...
            try (ZipOutputStream zos = new ZipOutputStream(Files.newOutputStream(targetPath))) {
                for (Long id : illustrationIds) {
                    galleryService.getIllustrationById(id).ifPresent(illustration -> {
                        try {
                            Path sourcePath = Paths.get(illustration.getFilePath());
                            if (Files.exists(sourcePath)) {
                                ZipEntry entry = new ZipEntry(
                                    String.format("%s_%s", 
                                        illustration.getTimestamp().toString().replace(":", "-"),
    ...
    public Path exportAsMarkdown(String sessionId, String outputPath, boolean includeImages) {
        try {
            List<IllustrationEntity> illustrations = galleryService.getSessionIllustrations(sessionId);
            ...
            // Summary statistics
            long successCount = illustrations.stream().filter(IllustrationEntity::isSuccess).count();
            markdown.append("**Success Rate:** ")
                .append(String.format("%.1f%%", (double) successCount / illustrations.size() * 100))
                .append("\n\n");
            ...
            for (IllustrationEntity illustration : illustrations) {
                ...
                markdown.append("**Status:** ").append(illustration.isSuccess() ? "✅ Success" : "❌ Failed").append("\n\n");
                ...
                if (includeImages) {
                    ...
                    if (Files.exists(sourcePath)) {
                        ...
                        markdown.append("![").append(illustration.getActionType())
                            .append("](images/").append(illustration.getFilename()).append(")\n\n");
                    }
                }
    ...
    /**
     * Creates a shareable link for an illustration or session.
     * This would typically upload to a cloud service or create a local server link.
     *
     ...
     */
    private void addMetadataToZip(ZipOutputStream zos, List<Long> illustrationIds) throws IOException {
        ...
        for (Long id : illustrationIds) {
            galleryService.getIllustrationById(id).ifPresent(ill -> {
                metadata.append("- ").append(ill.getFilename())
                    .append(" (").append(ill.getActionType())
                    .append(", ").append(ill.getTimestamp())
                    .append(")\n");
            });
        }
    ...
    private String generatePdfHtml(List<IllustrationEntity> illustrations) {
        ...
        for (IllustrationEntity illustration : illustrations) {
            ...
            html.append("<p><strong>Time:</strong> ").append(illustration.getTimestamp()).append("</p>");
            html.append("<p><strong>State:</strong> ").append(illustration.getStateName()).append("</p>");
            html.append("<p><strong>Status:</strong> <span class='")
                .append(illustration.isSuccess() ? "success" : "failure").append("'>")
