...
/**
 * Service for managing log detail display panels. Provides various detail views and export
 * capabilities.
 */
@Slf4j
@Service
...
public class LogDetailPanelService {
    ...
    public static class DetailPanel extends VBox {
        ...
        public DetailPanel(
                DetailPanelConfiguration config,
    ...
    public void updateDetails(DetailPanel panel, LogEntryViewModel entry) {
        ...
        // Update main details
        String detailText =
                viewModelFactory.formatDetailedText(
                        entry, LogEntryViewModelFactory.DetailFormat.DETAILED);
    ...
    private void updateStateVisualization(
            StateVisualizationService.StateVisualizationPanel statePanel, LogData logData) {
        if (logData == null) {
        ...
        if (logData.getType() == LogEventType.TRANSITION) {
            ...
            List<String> toStates =
                    logData.getToStateNames() != null
                            ? logData.getToStateNames()
                            : new ArrayList<>();

            stateVisualizationService.visualizeTransition(statePanel, fromStates, toStates);
        } else if (logData.getCurrentStateName() != null) {
            stateVisualizationService.visualizeCurrentState(
                    statePanel, logData.getCurrentStateName());
        } else {
            stateVisualizationService.clearVisualization(statePanel);
        }
    }
    ...
    private void updateRawDataTab(DetailPanel panel, LogEntryViewModel entry) {
        ...
        if (rawTab != null && rawTab.getContent() instanceof TextArea) {
            ...
            String rawText =
                    viewModelFactory.formatDetailedText(
                            entry, LogEntryViewModelFactory.DetailFormat.JSON);
        ...
        if (metadataTab != null && metadataTab.getContent() instanceof TreeView) {
            ...
            if (data != null) {
                ...
                if (data.getActionType() != null) {
                    logDataItem
                            .getChildren()
                            .add(new TreeItem<>("Action Type: " + data.getActionType()));
                }
                if (data.getCurrentStateName() != null) {
                    logDataItem
                            .getChildren()
                            .add(new TreeItem<>("Current State: " + data.getCurrentStateName()));
                }
                if (data.getPerformance() != null) {
                    TreeItem<String> perfItem = new TreeItem<>("Performance");
                    perfItem.getChildren()
                            .add(
                                    new TreeItem<>(
                                            "Duration: "
                                                    + data.getPerformance().getActionDuration()
                                                    + " ms"));
    ...
    public String exportDetails(DetailPanel panel, ExportFormat format) {
        ...
        switch (format) {
            ...
            case XML:
                return viewModelFactory.formatDetailedText(
                        entry, LogEntryViewModelFactory.DetailFormat.XML);
            case MARKDOWN:
