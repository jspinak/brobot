...
public class RecoveryManager {
    ...
    public CompletableFuture<RecoveryResult> attemptRecovery(
        ...
        // Execute recovery
        CompletableFuture<RecoveryResult> future = strategy.recover(
            error, 
            context, 
            retryOperation,
            state
    ...
    private void initializeDefaultStrategies() {
        ...
        recoveryStrategies.put(ErrorContext.ErrorCategory.NETWORK.name(), 
            new ExponentialBackoffStrategy(3, 1000, 30000));
        
        recoveryStrategies.put(ErrorContext.ErrorCategory.FILE_IO.name(), 
            new ImmediateRetryStrategy(2));
        
        recoveryStrategies.put(ErrorContext.ErrorCategory.DATABASE.name(), 
            new CircuitBreakerStrategy(5, 60000));
    }
    ...
    private void logRecoveryResult(String recoveryId, RecoveryResult result, Throwable error) {
        if (error != null) {
            ...
            log.info("Recovery [{}] succeeded after {} attempts", 
                recoveryId, result.getAttempts());
        } else {
            log.warn("Recovery [{}] failed after {} attempts: {}", 
                recoveryId, result.getAttempts(), result.getFailureReason());
        }
    }
            ...
            return CompletableFuture.supplyAsync(() -> {
                while (state.shouldContinue(maxAttempts)) {
                    ...
                    try {
                        ...
                        long delay = Math.min(
                            initialDelay * (long) Math.pow(2, state.attemptCount.get() - 1),
                            maxDelay
                        );
                        
                        if (state.attemptCount.get() > 1) {
                            log.debug("Waiting {} ms before retry attempt {}", 
                                delay, state.attemptCount.get());
                            Thread.sleep(delay);
                        }
                        ...
                        log.debug("Recovery attempt {} failed: {}", 
                            state.attemptCount.get(), e.getMessage());
