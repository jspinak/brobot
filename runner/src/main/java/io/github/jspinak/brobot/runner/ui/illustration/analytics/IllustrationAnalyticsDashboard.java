 ...
 *
 * <p>This dashboard provides real-time and historical analytics including:
 *
 * <ul>
 *   <li>Performance metrics (throughput, processing time, queue status)
 *   <li>Action distribution and success rates
 *   <li>Quality metrics and sampling effectiveness
 *   <li>Resource utilization and optimization metrics
 * </ul>
 *
...
public class IllustrationAnalyticsDashboard extends ScrollPane {
    ...
    @Autowired
    public IllustrationAnalyticsDashboard(
            IllustrationAnalyticsService analyticsService, UnifiedThemeManager themeService) {
    ...
    private void setupUI() {
        ...
        // Add all to content
        content.getChildren()
                .addAll(headerLabel, new Separator(), metricsRow, chartsGrid, tablesRow);
    ...
    private void startUpdates() {
        updateTimeline = new Timeline(new KeyFrame(Duration.seconds(2), e -> updateDashboard()));
    ...
    private void updateMetricPanels(AnalyticsSnapshot snapshot) {
        ...
        avgProcessingTimePanel.setValue(
                String.format("%.0fms", snapshot.getAverageProcessingTime()));
    ...
    private void updateSuccessRateChart(AnalyticsSnapshot snapshot) {
        ...
        snapshot.getSuccessRatesByAction()
                .forEach(
                        (action, rate) -> {
    ...
    private void updateActionStatsTable(AnalyticsSnapshot snapshot) {
        ...
        snapshot.getActionCounts()
                .forEach(
                        (action, count) -> {
                            double successRate =
                                    snapshot.getSuccessRatesByAction().getOrDefault(action, 0.0);
                            double avgTime =
                                    snapshot.getAverageTimeByAction().getOrDefault(action, 0.0);

                            items.add(
                                    new ActionStatistic(action, count.get(), successRate, avgTime));
                        });
