...
/**
 * Service responsible for automatic periodic saving of sessions.
...
public class SessionAutosaveService {
    ...
    @Getter
    private long autosaveIntervalMinutes = DEFAULT_AUTOSAVE_INTERVAL_MINUTES;
    
    @Getter
    private LocalDateTime lastAutosaveTime;
    
    @Getter
    private long autosaveCount = 0;
    ...
    public void initialize() {
        ...
        log.info("Session autosave service initialized with interval: {} minutes", 
    ...
    public void startAutosave() {
        ...
        eventBus.publish(LogEvent.info(this,
                "Session autosave started: " + autosaveIntervalMinutes + " minutes", "Session"));
    }
        ...
        try {
            if (lifecycleService.isSessionActive()) {
                ...
                eventBus.publish(LogEvent.debug(this,
                        "Session autosaved: " + lifecycleService.getCurrentSession().getId(), 
                        "Session"));
            }
        } catch (Exception e) {
    ...
    public void setAutosaveInterval(long minutes) {
        if (minutes < MIN_AUTOSAVE_INTERVAL_MINUTES || minutes > MAX_AUTOSAVE_INTERVAL_MINUTES) {
            throw new IllegalArgumentException(
                String.format("Autosave interval must be between %d and %d minutes",
        ...
        eventBus.publish(LogEvent.info(this,
                "Autosave interval changed to: " + minutes + " minutes", "Session"));
    }
