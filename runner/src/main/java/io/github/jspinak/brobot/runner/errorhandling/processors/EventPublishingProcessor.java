...
/**
 * Error processor that publishes error events to the event bus.
 * Enables reactive error handling throughout the application.
 */
@Slf4j
...
public class EventPublishingProcessor implements IErrorProcessor {
    ...
    public void process(Throwable error, ErrorContext context) {
        try {
            ...
            log.trace("Published error event for {} with severity {}", 
                     error.getClass().getSimpleName(), eventSeverity);
