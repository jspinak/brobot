 ...
 *
 * <p>This component implements a publish-subscribe pattern for decoupled communication between
 * different parts of the application. It manages event publishing and subscription, allowing
 * components to communicate without direct dependencies.
 *
 * <p>Key features:
 *
 * <ul>
 *   <li>Thread-safe event publishing and subscription
 *   <li>Asynchronous event delivery using a cached thread pool
 *   <li>Type-based event filtering
 *   <li>Support for multiple subscribers per event type
 *   <li>Automatic exception handling in event handlers
 * </ul>
 *
 * <p>Common event types include:
 *
 * <ul>
 *   <li>CONFIG_LOADED - Configuration file loaded
 *   <li>AUTOMATION_STARTED/STOPPED - Automation lifecycle events
 *   <li>STATE_CHANGED - State transition events
 *   <li>ERROR_OCCURRED - Error events
 *   <li>LOG_ENTRY - Logging events
 * </ul>
 *
 * @see BrobotEvent
...
public class EventBus {
    ...
    private final Map<BrobotEvent.EventType, Set<Consumer<BrobotEvent>>> subscribers =
    ...
    public void publish(BrobotEvent event) {
        ...
        if (queueSize > BACKPRESSURE_THRESHOLD) {
            logger.warn(
                    "EventBus queue size {} exceeds threshold {}, backpressure may occur",
                    queueSize,
     ...
     */
    public void subscribeToTypes(
            List<BrobotEvent.EventType> eventTypes, Consumer<BrobotEvent> handler) {
     ...
     * @param handler The handler to remove
     */
    public void unsubscribeFromTypes(
            List<BrobotEvent.EventType> eventTypes, Consumer<BrobotEvent> handler) {
        for (BrobotEvent.EventType eventType : eventTypes) {
    ...
    public EventBusMetrics getMetrics() {
        return EventBusMetrics.builder()
                ...
                .subscriberCount(subscribers.values().stream().mapToInt(Set::size).sum())
                .build();
    }
    ...
    private class BackpressureRejectionHandler
        ...
        public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
            ...
            logger.error(
                    "Event dropped due to backpressure - queue full ({} events)",
