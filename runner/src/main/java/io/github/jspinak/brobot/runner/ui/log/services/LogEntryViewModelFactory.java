...
/**
 * Factory service for creating and managing LogEntryViewModel instances.
 * Provides caching and batch creation capabilities.
 */
@Slf4j
@Service
public class LogEntryViewModelFactory {
    ...
    public static class ViewModelConfiguration {
        private DateTimeFormatter dateFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss.SSS");
    ...
    private String formatJsonText(LogEntryViewModel viewModel) {
        ...
        if (data != null) {
            if (data.getErrorMessage() != null) {
                json.append(",\n  \"error\": \"").append(escapeJson(data.getErrorMessage())).append("\"");
            }
            if (data.getCurrentStateName() != null) {
                json.append(",\n  \"state\": \"").append(escapeJson(data.getCurrentStateName())).append("\"");
            }
            if (data.getPerformance() != null && data.getPerformance().getActionDuration() > 0) {
                json.append(",\n  \"duration\": ").append(data.getPerformance().getActionDuration());
            }
        }
    ...
    private String formatXmlText(LogEntryViewModel viewModel) {
        ...
        if (data != null) {
            if (data.getErrorMessage() != null) {
                xml.append("  <error>").append(escapeXml(data.getErrorMessage())).append("</error>\n");
            }
            if (data.getCurrentStateName() != null) {
                xml.append("  <state>").append(escapeXml(data.getCurrentStateName())).append("</state>\n");
            }
            if (data.getPerformance() != null && data.getPerformance().getActionDuration() > 0) {
                xml.append("  <duration>").append(data.getPerformance().getActionDuration()).append("</duration>\n");
            }
        }
    ...
    private String createCacheKey(LogEvent logEvent) {
        return logEvent.getTimestamp() + "_" + 
               logEvent.getLevel() + "_" +
