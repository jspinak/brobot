...
public class StyleDivergenceAnalyzer {
    ...
    public static class StyleDivergenceIssue {
        ...
        public StyleDivergenceIssue(
                Node node, String issueType, String description, double severity) {
            this.node = node;
    ...
    public StyleDivergenceResult analyzeStyleDivergence(List<Node> nodes) {
            ...
            if (node instanceof Region) {
                ...
                if (region.getBorder() != null && !region.getBorder().getStrokes().isEmpty()) {
                    ...
                    for (var stroke : region.getBorder().getStrokes()) {
                        if (stroke.getTopStroke() != null
                                && stroke.getTopStroke() instanceof Color) {
                        ...
                        if (!isExpectedBorder) {
                            result.issues.add(
                                    new StyleDivergenceIssue(
                                            ...
                                            "Excessive Border",
                                            String.format(
                                                    "%s has unexpected border styling",
                ...
                if (style != null) {
                    if (style.contains("-fx-border-")
                            && !style.contains("-fx-border-color: transparent")) {
                        ...
                        if (!(fill.getFill() instanceof Color)) {
                            result.issues.add(
                                    new StyleDivergenceIssue(
                                            ...
                                            String.format(
                                                    "%s uses non-solid background fill",
                                                    NodeDescriptionUtil.getNodeDescription(node)),
                                            0.5));
                            totalSeverity += 0.5;
                            issueCount++;
        ...
        if (divergenceScore > 0.5) {
            logger.error(
                    "HIGH STYLE DIVERGENCE: Score {:.2f} with {} issues detected",
                    divergenceScore,
                    issueCount);
        } else if (divergenceScore > 0.3) {
            logger.warn(
                    "Moderate style divergence: Score {:.2f} with {} issues",
                    divergenceScore,
                    issueCount);
        }
    ...
    private boolean isExpectedToHaveBorder(Node node) {
        ...
        for (String styleClass : styleClasses) {
            if (styleClass.contains("button")
                    || styleClass.contains("field")
                    || styleClass.contains("input")
                    || styleClass.contains("bordered")) {
                return true;
            }
        }
    ...
    public void logResults(StyleDivergenceResult result) {
        if (!result.issues.isEmpty()) {
            ...
            logger.error(
                    "Overall divergence score: {:.2f} (0=perfect, 1=worst)",
                    result.overallDivergenceScore);
