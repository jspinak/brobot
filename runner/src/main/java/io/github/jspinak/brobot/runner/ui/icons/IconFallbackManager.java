...
/**
 * Manages icon fallbacks to prevent pi symbol display issues. This component ensures that when
 * icons cannot be loaded or displayed, appropriate text alternatives are shown instead of unicode
 * fallback characters.
 */
@Slf4j
@Component
public class IconFallbackManager {
    ...
    /**
     * Processes a node tree to replace icon placeholders with text alternatives. This prevents pi
     * symbols from appearing when icon fonts or resources fail to load.
     *
    ...
    private void processLabeled(Labeled labeled) {
        ...
        if (graphic != null && shouldRemoveGraphic(graphic)) {
            ...
            log.debug(
                    "Removed potentially problematic graphic from {}",
                    labeled.getClass().getSimpleName());
        }
    }
    ...
    private String cleanText(String text) {
        ...
        return text.replaceAll("[\\u03C0\\u03A0]", "") // Greek pi symbols
                .replaceAll("[\\u2190-\\u21FF]", "") // Arrows
