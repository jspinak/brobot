 ...
 *
 * <p>This service provides factory methods for creating different types of thread pools optimized
 * for various workloads. It handles thread factory creation and pool configuration.
 *
...
public class ThreadPoolFactoryService {
    ...
    public ForkJoinPool createWorkStealingPool(String name, int parallelism) {
        return new ForkJoinPool(
                ...
                (thread, exception) ->
                        log.error("Uncaught exception in pool '{}'", name, exception),
                true // async mode
                );
    }
    ...
    private RejectedExecutionHandler createRejectionHandler(String poolName) {
        return (r, executor) -> {
            log.warn(
                    "Task rejected from pool '{}' - Queue size: {}, Active threads: {}",
                    poolName,
...
class ManagedThreadPool extends ThreadPoolExecutor {
    ...
    public ThreadPoolHealth getHealth() {
        ...
        double utilization = poolSize > 0 ? (double) activeThreads / poolSize : 0.0;

        long avgDuration =
                completedTasks.get() > 0 ? totalTaskDuration.get() / completedTasks.get() : 0;
