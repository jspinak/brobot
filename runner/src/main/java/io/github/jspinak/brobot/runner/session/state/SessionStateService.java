 ...
 * 
 * This service is responsible for capturing the current application state,
 * storing it in sessions, and restoring state from saved sessions.
 * 
...
public class SessionStateService implements DiagnosticCapable {
    ...
    public ApplicationState captureCurrentState() {
        ...
        appState.setActiveStateIds(activeStates.stream()
                .map(State::getId)
                .collect(Collectors.toSet()));
        
        if (diagnosticMode.get()) {
            log.info("[DIAGNOSTIC] State captured - Active states: {}, Transitions: {}",
                    activeStates.size(), transitions.size());
        }
    ...
    public void restoreState(ApplicationState appState) {
        ...
        try {
            ...
            // Note: StateTransitionStore doesn't have a clear method, so we work with what's available
            ...
            if (appState.getActiveStateIds() != null && !appState.getActiveStateIds().isEmpty()) {
                ...
                log.info("Restored {} state transitions and {} active states",
                        appState.getStateTransitions() != null ? appState.getStateTransitions().size() : 0,
                        appState.getActiveStateIds().size());
            }
            
            if (diagnosticMode.get()) {
                log.info("[DIAGNOSTIC] State restored - Transitions: {}, Active states: {}",
                        appState.getStateTransitions() != null ? appState.getStateTransitions().size() : 0,
    ...
    public void updateSessionState(Session session) {
        ...
        session.addStateData("stateCount", currentState.getActiveStateIds() != null ? 
                currentState.getActiveStateIds().size() : 0);
    }
    ...
    public DiagnosticInfo getDiagnosticInfo() {
        ...
        states.put("stateTransitionCount", stateTransitionStore.getAllStateTransitionsAsCopy().size());
        
        // Memory usage estimate
        long memoryUsage = snapshots.values().stream()
