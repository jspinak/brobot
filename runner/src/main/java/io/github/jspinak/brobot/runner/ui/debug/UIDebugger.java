 ...
 * 
 * Features:
 * - Debug mode with visual borders
...
public class UIDebugger {
    ...
    // Debug border styles
    private static final Border DEBUG_BORDER = new Border(new BorderStroke(
        Color.RED, BorderStrokeStyle.DASHED, 
        new CornerRadii(2), new BorderWidths(2)
    ));
    
    private static final Border HIGHLIGHT_BORDER = new Border(new BorderStroke(
        ...
        traverseTree(root, node -> {
            if (!originalBorders.containsKey(node)) {
                ...
                if (node instanceof Parent) {
                    ...
                    parent.setStyle(parent.getStyle() + "; -fx-border-color: red; -fx-border-style: dashed; -fx-border-width: 1;");
                }
    ...
    private void removeAllDebugBorders() {
        originalBorders.forEach((node, originalStyle) -> {
            if (node instanceof Parent) {
    ...
    private void logComponentTreeRecursive(Node node, int depth) {
        ...
        String nodeInfo = String.format("%s%s [id=%s, classes=%s]",
            indent,
            node.getClass().getSimpleName(),
            node.getId(),
            String.join(",", node.getStyleClass())
    ...
    public void highlightComponent(String componentId) {
        componentRegistry.get(componentId).ifPresent(node -> {
    ...
    public Map<String, List<String>> detectDuplicateLabels(Node root) {
        ...
        traverseTree(root, node -> {
            if (node instanceof Label) {
                ...
                if (text != null && !text.trim().isEmpty()) {
                    ...
                    labelOccurrences.computeIfAbsent(text, k -> new ArrayList<>()).add(location);
                }
            }
        });
        ...
        labelOccurrences.forEach((text, locations) -> {
            if (locations.size() > 1) {
    ...
    public String createDebugReport(Node root) {
        ...
        duplicates.forEach((text, locations) -> {
            report.append("  '").append(text).append("' (").append(locations.size()).append(" occurrences)\n");
        });
