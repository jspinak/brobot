...
class PerformanceBenchmarkTest extends ImprovedJavaFXTestBase {
    ...
    void benchmarkLabelCreationOldVsNew() throws InterruptedException {
        ...
        System.out.println(
                "Old approach (direct creation): " + (oldApproachTime.get() / 1_000_000) + " ms");
        System.out.println(
                "New approach (LabelManager): " + (newApproachTime.get() / 1_000_000) + " ms");
        System.out.println(
                "LabelManager prevented duplicates: "
                        + (LABEL_COUNT - labelManager.getLabelCount())
                        + " labels");

        // LabelManager should be efficient
        assertTrue(
                newApproachTime.get() < oldApproachTime.get() * 2,
    ...
    void benchmarkUIUpdatesOldVsNew() throws InterruptedException {
        ...
        System.out.println(
                "Average update time: "
                        ...
                        + " ms");
        System.out.println(
                "Min update time: " + String.format("%.2f", metrics.getMinDurationMs()) + " ms");
        System.out.println(
                "Max update time: " + String.format("%.2f", metrics.getMaxDurationMs()) + " ms");
    ...
    void benchmarkDuplicateLabelPrevention() throws InterruptedException {
        ...
        System.out.println(
                "Memory saved: ~"
                        + ((oldMemoryAfter.get() - oldMemoryBefore.get())
                                        - (newMemoryAfter.get() - newMemoryBefore.get()))
                                / 1024
                        + " KB");
    ...
    void benchmarkConcurrentAccess() throws InterruptedException {
        ...
        System.out.println(
                "Average thread time: "
                        + (totalOperationTime.get() / THREAD_COUNT / 1_000_000)
                        + " ms");
        System.out.println(
                "Throughput: "
                        + ((THREAD_COUNT * OPERATIONS_PER_THREAD)
                                * 1000.0
                                / (overallTime / 1_000_000))
                        + " ops/sec");
