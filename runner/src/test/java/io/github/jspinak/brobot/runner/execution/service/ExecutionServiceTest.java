 ...
 *
 * <p>Tests core execution orchestration, status management, and component coordination.
 */
...
class ExecutionServiceTest {
    ...
    @Mock private ExecutionThreadManager threadManager;

    @Mock private ExecutionTimeoutManager timeoutManager;

    @Mock private ExecutionSafetyService safetyService;
    ...
    void setUp() {
        ...
        when(threadManager.submit(any(), any()))
    ...
    void shouldHandleSafetyCheckFailure() throws Exception {
        // Given
        when(safetyService.checkActionSafety(any(), eq("execution_start"))).thenReturn(false);

        Runnable task = () -> {};

        // When
        CompletableFuture<Void> future =
                executionService.execute(task, "BlockedTask", ExecutionOptions.defaultOptions());
    ...
    void shouldTrackStatusUpdates() throws Exception {
        ...
        // When
        CompletableFuture<Void> future =
                executionService.execute(task, "StatusTask", ExecutionOptions.defaultOptions());
    ...
    void shouldHandleTimeout() throws Exception {
        ...
        doAnswer(
                        invocation -> {
                            ExecutionTimeoutManager.TimeoutHandler handler =
        ...
        CompletableFuture<Void> future =
                executionService.execute(
                        task,
                        "TimeoutTask",
                        ExecutionOptions.builder().timeout(Duration.ofMillis(100)).build());
    ...
    void shouldRespectStartDelay() throws Exception {
        ...
        ExecutionOptions options =
                ExecutionOptions.builder().startDelay(Duration.ofMillis(startDelay)).build();
    ...
    void shouldCleanupAfterExecution() throws Exception {
        ...
        // When
        CompletableFuture<Void> future =
                executionService.execute(task, "CleanupTask", ExecutionOptions.defaultOptions());
