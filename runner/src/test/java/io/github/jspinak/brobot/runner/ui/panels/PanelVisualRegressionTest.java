...
class PanelVisualRegressionTest extends VisualRegressionTest {
    ...
    void testRefactoredResourceMonitorPanelVisual() throws Exception {
        runAndWait(() -> {
            ...
            ResourceMonitoringService.ResourceData testData = new ResourceMonitoringService.ResourceData();
    ...
    void testRefactoredConfigDetailsPanelVisual() throws Exception {
        runAndWait(() -> {
            RefactoredConfigDetailsPanel panel = new RefactoredConfigDetailsPanel(
                eventBus,
                labelManager,
                uiUpdateManager
            );
            ...
            testConfig.setProject("Test Project");
            ...
            testConfig.setDescription("This is a test configuration for visual regression testing");
    ...
    void testResponsiveLayout() throws Exception {
        runAndWait(() -> {
            ...
            // Set test data
            ConfigEntry testConfig = new ConfigEntry();
            testConfig.setName("Responsive Test");
            testConfig.setProject("Test Project");
            testConfig.setProjectConfigPath(Paths.get("/test/config.xml"));
            testConfig.setDslConfigPath(Paths.get("/test/config.dsl"));
            testConfig.setImagePath(Paths.get("/test/images"));
            testConfig.setLastModified(LocalDateTime.now());
    ...
    /**
     * Use this test to update baselines when UI changes are intentional.
