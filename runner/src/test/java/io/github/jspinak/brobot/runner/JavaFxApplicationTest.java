...
public class JavaFxApplicationTest extends ApplicationTest {
    ...
    public static void checkEnvironment() {
        assumeFalse(
                GraphicsEnvironment.isHeadless(), "Skipping JavaFX test in headless environment");
    }
    ...
    public void testJavaFxApplicationLaunches() throws InterruptedException {
        ...
        Platform.runLater(
                () -> {
                    ...
                    assertEquals(
                            "JavaFX Test Window", stage.getTitle(), "Window title should match");
                    latch.countDown();
                });

        assertTrue(
                latch.await(5, TimeUnit.SECONDS),
                "JavaFX operations should complete within timeout");
    }
    ...
    public void testSceneIsProperlyConfigured() throws InterruptedException {
        ...
        assertTrue(
                latch.await(5, TimeUnit.SECONDS),
                "Scene verification should complete within timeout");
    }
}
