...
class SearchServiceTest {
    ...
    void testSearchTreeWithMatch() {
        ...
        assertTrue(
                result.getMatchingItems().stream()
    ...
    void testResetTreeExpansion() {
        ...
        assertTrue(rootItem.getChildren().get(0).isExpanded()); // FOLDER should be expanded
        assertFalse(
                rootItem.getChildren().get(1).isExpanded()); // Non-folder should not be expanded
        assertFalse(
                rootItem.getChildren()
                        .get(0)
                        .getChildren()
                        .get(0)
                        .isExpanded()); // STATE should not be expanded
    }
    ...
    private TreeItem<ConfigBrowserPanel.ConfigItem> createTestTree() {
        ...
        TreeItem<ConfigBrowserPanel.ConfigItem> button =
                new TreeItem<>(
                        new ConfigBrowserPanel.ConfigItem(
                                "Button1", ConfigBrowserPanel.ConfigItemType.AUTOMATION_BUTTON));
