...
class LogDataRepositoryTest {
    ...
    void testQueryByLevel() {
        ...
        // When
        LogDataRepository.LogQuery query =
                LogDataRepository.LogQuery.builder().levels(LogEntry.LogLevel.ERROR).build();
    ...
    void testQueryByType() {
        ...
        // When
        LogDataRepository.LogQuery query =
                LogDataRepository.LogQuery.builder().types("ACTION").build();
        List<LogEntry> results = repository.queryLogs(query);
    ...
    void testQueryByTimeRange() {
        ...
        // When
        LogDataRepository.LogQuery query =
                LogDataRepository.LogQuery.builder()
    ...
    void testQueryWithSearchText() {
        ...
        // When
        LogDataRepository.LogQuery query =
                LogDataRepository.LogQuery.builder().searchText("keyword").build();
        List<LogEntry> results = repository.queryLogs(query);
    ...
    void testQueryLimit() {
        ...
        // When
        LogDataRepository.LogQuery query = LogDataRepository.LogQuery.builder().limit(3).build();
        List<LogEntry> results = repository.queryLogs(query);
                            ...
                            () -> {
                                for (int j = 0; j < entriesPerThread; j++) {
                                    repository.addLogEntry(
                                            createTestEntry(
                                                    threadId + "-" + j, LogEntry.LogLevel.INFO));
                                }
                            });
            threads[i].start();
        }
