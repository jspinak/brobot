...
class ErrorStatisticsServiceTest {
    ...
    void testRecordNullOperationResult() {
        ...
        assertThatNoException()
                .isThrownBy(
                        () ->
                                service.recordOperationResult(
                                        null, ErrorResult.handled("Test", "123")));
        assertThatNoException().isThrownBy(() -> service.recordOperationResult("TestOp", null));
    }
                        ...
                        () ->
                                service.recordError(
                                        ...
                                        ErrorContext.minimal(
                                                "Op", ErrorContext.ErrorCategory.SYSTEM)));
        assertThatNoException().isThrownBy(() -> service.recordError(new RuntimeException(), null));
    }
    ...
    void testGetTopErrorOperations() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            service.recordOperationResult("HighErrorOp", ErrorResult.unrecoverable("Failed", "id"));
        }

        for (int i = 0; i < 2; i++) {
            service.recordOperationStart("LowErrorOp");
            service.recordOperationResult("LowErrorOp", ErrorResult.unrecoverable("Failed", "id"));
        }

        service.recordOperationStart("NoErrorOp");
        service.recordOperationResult("NoErrorOp", ErrorResult.handled("Success", "id"));
