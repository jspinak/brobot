...
class ConfigOperationsServiceTest {
    ...
    @Mock private ApplicationConfig appConfig;
    ...
    void testLoadConfiguration() throws ExecutionException, InterruptedException {
        ...
        assertTrue(
                events.stream()
                        .anyMatch(
                                e -> e.getMessage().contains("Loading configuration: TestConfig")));
        assertTrue(
                events.stream()
                        .anyMatch(
                                e ->
        ...
        assertTrue(
                events.stream()
                        .anyMatch(
                                e ->
                                        e.getMessage()
                                                .contains("Deleting configuration: TestConfig")));
        assertTrue(
                events.stream()
                        .anyMatch(
                                e ->
    ...
    void testLoadRecentConfigurations() {
        ...
        assertTrue(
                events.stream()
                        .anyMatch(e -> e.getMessage().contains("Loading recent configurations")));
    }
        ...
        assertTrue(
                events.stream()
                        .anyMatch(
                                e ->
                                        e.getMessage()
                                                .contains("Exporting configuration: TestConfig")));
        assertTrue(
                events.stream()
                        .anyMatch(
                                e ->
