...
/**
 * Comprehensive tests for the StateStore class which manages the collection of states in the Brobot
 * framework.
 */
...
public class StateStoreTest extends BrobotTestBase {
    ...
    void testUpdateStateObjectIds() {
        // Given
        StateImage stateImage = new StateImage.Builder().setName("testImage").build();
        StateLocation stateLocation = new StateLocation.Builder().setName("testLocation").build();
        StateRegion stateRegion = new StateRegion.Builder().setName("testRegion").build();
        StateString stateString = new StateString.Builder().setName("testString").build();

        State state = new State.Builder("complexState").build();
    ...
    Stream<DynamicTest> testStateManagementScenarios() {
        return Stream.of(
                dynamicTest(
                        "Add and retrieve multiple states",
                        () -> {
    ...
    void testStateWithMatchHistory() {
        // Given
        StateLocation location = new StateLocation.Builder().setName("locationWithHistory").build();

        ActionHistory history = location.getMatchHistory();
        history.addSnapshot(new ActionRecord());

        State state = new State.Builder("stateWithHistory").build();
        ...
        history.getSnapshots()
                .forEach(snapshot -> assertEquals(state.getId(), snapshot.getStateId()));
    }
