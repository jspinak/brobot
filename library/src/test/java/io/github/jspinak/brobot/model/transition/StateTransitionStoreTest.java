...
/**
 * Comprehensive tests for the StateTransitionStore class which manages
 * the repository of state transitions in the Brobot framework.
 */
...
public class StateTransitionStoreTest extends BrobotTestBase {
    ...
    Stream<DynamicTest> testUsageScenarios() {
        return Stream.of(
            dynamicTest("Empty store operations", () -> {
                StateTransitionStore emptyStore = new StateTransitionStore(mockJointTable);
            ...
            dynamicTest("Single state transitions", () -> {
                StateTransitionStore singleStore = new StateTransitionStore(mockJointTable);
            ...
            dynamicTest("Multiple states with same ID", () -> {
                StateTransitionStore duplicateStore = new StateTransitionStore(mockJointTable);
            ...
            dynamicTest("Large repository performance", () -> {
                StateTransitionStore largeStore = new StateTransitionStore(mockJointTable);
    ...
    void testEmptyTransitionsList() {
        ...
        when(mockStateTransitions1.getTransitionFinish()).thenReturn(new TaskSequenceStateTransition());
    ...
    void testNullFinishTransition() {
        // Given
        when(mockStateTransitions1.getTransitions()).thenReturn(List.of(new TaskSequenceStateTransition()));
    ...
    void testFilteringOperations() {
        ...
        List<StateTransitions> activeOnly = store.getRepo().stream()
            .filter(st -> st.getStateId() != null && activeIds.contains(st.getStateId()))
            .toList();
