...
/**
 * Comprehensive test suite for SceneAnalysis. Tests scene analysis capabilities including color
 * analysis, pixel profiles, and various analysis types.
 */
...
public class SceneAnalysisTest extends BrobotTestBase {
    ...
    @Mock private PixelProfiles mockPixelProfiles1;
    @Mock private PixelProfiles mockPixelProfiles2;
    @Mock private StateImage mockStateImage1;
    @Mock private StateImage mockStateImage2;
    @Mock private ColorCluster mockColorCluster;
    @Mock private ColorSchema mockColorSchema;
    @Mock private ColorStatistics mockColorStatistics;
    ...
    void shouldGetColorStatProfiles() {
        ...
        when(mockColorSchema.getColorStatistics(ColorInfo.ColorStat.MEAN))
                .thenReturn(mockColorStatistics);

        sceneAnalysis = new SceneAnalysis(pixelAnalysisCollections, mockScene);

        List<ColorStatistics> profiles =
                sceneAnalysis.getColorStatProfiles(BGR, ColorInfo.ColorStat.MEAN);
    ...
    void shouldGetColorValues() {
        ...
        when(mockColorSchema.getColorStatistics(ColorInfo.ColorStat.MEAN))
                .thenReturn(mockColorStatistics);
        when(mockColorStatistics.getStat(ColorSchema.ColorValue.HUE))
                .thenReturn(120.5)
                .thenReturn(180.7);
        ...
        List<Integer> colorValues =
                sceneAnalysis.getColorValues(
                        HSV, ColorInfo.ColorStat.MEAN, ColorSchema.ColorValue.HUE);
    ...
    void shouldGetScoresForStateImage() {
        ...
        when(mockPixelProfiles1.getAnalysis(PixelProfiles.Analysis.SCORE, BGR))
                .thenReturn(scoresMat);
