...
/**
 * Comprehensive test suite for MatchCollection - manages collections of Match objects.
 * Tests add/remove operations, sorting, filtering, and statistical analysis.
 */
...
public class MatchCollectionTest extends BrobotTestBase {
    ...
    class FilteringOperations {
        ...
        public void testFilterByScoreRange() {
            MatchCollection filteredColl = collection.filter(m -> m.getScore() >= 0.86 && m.getScore() <= 0.92);
            List<Match> filtered = filteredColl.getMatches();
    ...
    class BestMatchSelection {
        ...
        public void testGetTopMatches() {
            collection.sortByScoreDescending();
            List<Match> top2 = collection.getMatches().size() > 2 ? 
                collection.getMatches().subList(0, 2) : collection.getMatches();
    ...
    class CollectionStatistics {
        ...
        public void testGetAverageScore() {
            double avgScore = collection.getMatches().stream()
        ...
        public void testGetMinMaxScores() {
            double minScore = collection.getMatches().stream()
                .mapToDouble(Match::getScore)
                .min().orElse(0.0);
            double maxScore = collection.getMatches().stream()
                .mapToDouble(Match::getScore)
                .max().orElse(0.0);
        ...
        public void testCountMatchesAboveThreshold() {
            long count = collection.getMatches().stream()
                .filter(m -> m.getScore() > 0.88)
                .count();
