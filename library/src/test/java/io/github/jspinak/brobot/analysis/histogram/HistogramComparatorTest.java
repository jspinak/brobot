...
public class HistogramComparatorTest extends BrobotTestBase {
    ...
    @Mock private HistogramExtractor histogramExtractor;

    @Mock private HistogramRegions histRegions1;

    @Mock private HistogramRegions histRegions2;

    @Mock private HistogramRegion topLeft1, topRight1, bottomLeft1, bottomRight1, ellipse1;
    @Mock private HistogramRegion topLeft2, topRight2, bottomLeft2, bottomRight2, ellipse2;

    @InjectMocks private HistogramComparator histogramComparator;
    ...
    void shouldCompareAllRegions() {
        ...
        when(region1.getJavaCVRect())
                .thenReturn(new org.bytedeco.opencv.opencv_core.Rect(0, 0, 100, 100));
        when(region2.getJavaCVRect())
                .thenReturn(new org.bytedeco.opencv.opencv_core.Rect(100, 100, 100, 100));
    ...
    void shouldProcessMultipleRegions() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            when(region.getJavaCVRect())
                    .thenReturn(new org.bytedeco.opencv.opencv_core.Rect(i * 10, i * 10, 100, 100));
            regions.add(region);
        }
    ...
    void shouldSortMatchesByScore() {
        ...
        when(region1.getJavaCVRect())
                .thenReturn(new org.bytedeco.opencv.opencv_core.Rect(0, 0, 100, 100));
        when(region2.getJavaCVRect())
                .thenReturn(new org.bytedeco.opencv.opencv_core.Rect(100, 0, 100, 100));
        when(region3.getJavaCVRect())
                .thenReturn(new org.bytedeco.opencv.opencv_core.Rect(200, 0, 100, 100));
            ...
            try {
                ...
                try {
                    ...
                    assertThrows(
                            RuntimeException.class,
                            () -> {
                                histogramComparator.compare(
                                        histRegions1, histRegions2, customIndexedColumn);
                            });
                } finally {
    ...
    class PerformanceConsiderations {
        ...
        void shouldHandleLargeRegionListsEfficiently() {
            ...
            for (int i = 0; i < 100; i++) {
                ...
                when(region.getJavaCVRect())
                        .thenReturn(new org.bytedeco.opencv.opencv_core.Rect(x, y, 50, 50));
                regions.add(region);
            }

            when(histogramExtractor.getHistogramsHSV(stateImage)).thenReturn(histRegions1);
            when(histogramExtractor.getHistogramFromRegion(any(Mat.class)))
                    .thenReturn(histRegions2);
            when(histogramExtractor.getHueBins()).thenReturn(256);
