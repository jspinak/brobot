...
/**
 * Test implementation of BrobotJsonModule with minimal serializers.
 * Used for testing BrobotObjectMapper configuration without complex dependencies.
 */
public class TestBrobotJsonModule extends BrobotJsonModule {
    ...
    private static MatchesSerializer createMatchesSerializer() {
        return new MatchesSerializer() {
            @Override
            public void serialize(ActionResult value, JsonGenerator gen, SerializerProvider provider) throws IOException {
    ...
    private static ObjectCollectionSerializer createObjectCollectionSerializer() {
        return new ObjectCollectionSerializer() {
            @Override
            public void serialize(ObjectCollection value, JsonGenerator gen, SerializerProvider provider) throws IOException {
                gen.writeStartObject();
                ...
                for (Scene scene : value.getScenes()) {
                    gen.writeStartObject();
                    if (scene.getPattern() != null && scene.getPattern().getName() != null) {
                        gen.writeStringField("name", scene.getPattern().getName());
                    }
                    gen.writeEndObject();
                }
                gen.writeEndArray();
    ...
    private static ActionConfigDeserializer createActionConfigDeserializer() {
        return new ActionConfigDeserializer() {
            @Override
            public ActionConfig deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
    ...
    private static SearchRegionsDeserializer createSearchRegionsDeserializer() {
        return new SearchRegionsDeserializer() {
            @Override
            public SearchRegions deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
