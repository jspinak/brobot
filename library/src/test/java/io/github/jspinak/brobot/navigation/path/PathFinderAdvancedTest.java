...
/**
 * Advanced test suite for PathFinder - finds navigation paths through state graphs. Tests path
 * algorithms, optimization strategies, caching, and complex graph scenarios.
 */
...
class PathFinderAdvancedTest extends BrobotTestBase {
    ...
    @Mock private StateTransitionService mockTransitionService;

    @Mock private State startState;

    @Mock private State targetState;

    @Mock private State intermediateState1;

    @Mock private State intermediateState2;
                            ...
                            try {
                                ...
                                Paths paths =
                                        pathFinder.getPathsToState(
                                                Set.of((long) threadId), (long) (10 + threadId));
                                assertNotNull(paths);
                            } finally {
                                latch.countDown();
                            }
    ...
    private void setupDisconnectedGraph() {
        ...
        when(mockJointTable.getStatesWithTransitionsTo(anyLong()))
