...
/**
 * Advanced test suite for StateMemory - manages active state tracking and history.
 * Tests state persistence, concurrent access, history management, and recovery.
 */
...
class StateMemoryAdvancedTest extends BrobotTestBase {
    ...
    class ActiveStateManagement {
        ...
        void testStateVisits() {
            ...
            verify(mockState1, times(2)).setProbabilityExists(100);
        }
    }
                ...
                executor.submit(() -> {
                    try {
                        for (int j = 0; j < statesPerThread; j++) {
                            ...
                            when(mockStateService.getState(stateId)).thenReturn(Optional.of(mockState));
                            when(mockStateService.getStateName(stateId)).thenReturn("State" + stateId);
    ...
    class StateRemovalAndCleanup {
        ...
        void testRemoveInactiveStates() {
            ...
            verify(mockState2).setProbabilityExists(0);
        }
    ...
    class MatchIntegration {
        ...
        void testIgnoreMatchesWithoutStateData() {
            ...
            ActionResult actionResult = mock(ActionResult.class);
            when(actionResult.getMatchList()).thenReturn(Arrays.asList(matchWithoutData, matchWithData));
