...
/**
 * Comprehensive test class for BrobotPatternCapture functionality. Tests pattern capture operations
...
public class BrobotPatternCaptureTest extends BrobotTestBase {
    ...
    void shouldCapturePatternWithMetadata() throws Exception {
        ...
        try (MockedStatic<GraphicsEnvironment> geMock = mockStatic(GraphicsEnvironment.class);
            ...
            try (MockedStatic<ImageIO> imageIOMock = mockStatic(ImageIO.class)) {
                ...
                imageIOMock
                        .when(() -> ImageIO.getImageWritersByFormatName("png"))
                        .thenReturn(mockIterator);
                imageIOMock
                        .when(() -> ImageIO.createImageOutputStream(any(File.class)))
                        .thenReturn(mockOutputStream);
                when(mockWriter.getDefaultImageMetadata(any(), any())).thenReturn(mockMetadata);

                // Capture pattern
                assertDoesNotThrow(() -> patternCapture.capturePattern(patternName, region));
    ...
    void shouldSavePatternWithMetadata() throws Exception {
        ...
        // Use reflection to test private method
        var method =
                BrobotPatternCapture.class.getDeclaredMethod(
                        "savePatternWithMetadata",
                        BufferedImage.class,
                        String.class,
    ...
    void shouldCreateScaledVersions() throws Exception {
        ...
        try (MockedStatic<ImageIO> imageIOMock = mockStatic(ImageIO.class)) {
            imageIOMock
                    .when(() -> ImageIO.write(any(BufferedImage.class), eq("png"), any(File.class)))
                    .thenReturn(true);
    ...
    void shouldHandleCaptureException() {
        ...
        try (MockedConstruction<Screen> screenMock =
                mockConstruction(
                        Screen.class,
                        (mock, context) -> {
                            when(mock.capture(any(Rectangle.class)))
    ...
    void shouldHandleNullRegion() {
        ...
        assertDoesNotThrow(() -> patternCapture.capturePattern(patternName, null));
    }
    ...
    void shouldHandleEmptyPatternName() {
        ...
        try (MockedConstruction<Screen> screenMock =
            ...
            assertDoesNotThrow(() -> patternCapture.capturePattern("", region));
        }
    }
    ...
    void shouldHandleInteractiveCapture() {
        ...
        try (MockedStatic<GraphicsEnvironment> geMock = mockStatic(GraphicsEnvironment.class)) {
            ...
            assertDoesNotThrow(() -> patternCapture.interactiveCapture(patternName));
        }
    }
