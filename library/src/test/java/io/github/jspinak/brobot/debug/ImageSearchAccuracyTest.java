...
/**
 * Test to verify if SikuliX image searches work correctly despite highlighting issues. This is
 * critical - if searches are also broken, the entire automation won't work.
 */
...
public class ImageSearchAccuracyTest extends DebugTestBase {
    ...
    public void testImageSearchAccuracy() throws Exception {
        ...
        System.out.printf(
                "Captured region at (%d,%d,%d,%d) and saved to %s%n",
        ...
        try {
            ...
            boolean correctLocation =
                    (Math.abs(match.x - testX) < 5) && (Math.abs(match.y - testY) < 5);
            ...
            if (correctLocation) {
                ...
                System.out.printf("  Offset: x=%d, y=%d%n", match.x - testX, match.y - testY);
            }
        ...
        boolean mouseCorrect =
                (Math.abs(mousePos.x - clickX) < 5) && (Math.abs(mousePos.y - clickY) < 5);
            ...
            for (int y : testYCoords) {
                ...
                try {
                    ...
                    if (Math.abs(m.x - x) < 5 && Math.abs(m.y - y) < 5) {
                        ...
                        System.out.printf(
                                "  ✗ (%d,%d) found at wrong location (%d,%d)%n", x, y, m.x, m.y);
                    }
        ...
        if (successCount == totalTests) {
            ...
            System.out.println(
                    "  Pattern matching has the same coordinate issues as highlighting.");
        }
