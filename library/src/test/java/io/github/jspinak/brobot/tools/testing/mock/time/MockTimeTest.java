...
/**
 * Comprehensive tests for MockTime virtual clock functionality. Tests time simulation, action
 * durations, and deterministic behavior.
 */
...
public class MockTimeTest extends BrobotTestBase {
    ...
    @Mock private ActionDurations mockActionDurations;
    ...
    class TimeInitialization {
        ...
        void shouldInitializeWithCurrentTime() {
            ...
            assertTrue(secondsDiff < 5, "Initial mock time should be close to actual current time");
        }
    ...
    class BasicWaitOperations {
        ...
        void shouldAdvanceTimeForVariousDurations(double seconds) {
            ...
            assertEquals(
                    seconds, actualSeconds, 0.000001, "Should advance by " + seconds + " seconds");
        }
        ...
        void shouldHandleZeroWaitTime() {
            ...
            assertEquals(before, after, "Zero wait should not advance time");
        }
        ...
        void shouldHandleNegativeWaitTime() {
            ...
            assertEquals(before, after, "Negative wait should not advance time");
        }
        ...
        void shouldAccumulateMultipleWaits() {
            ...
            assertEquals(6, totalSeconds, "Multiple waits should accumulate");
        }
    }
    ...
    class ActionTypeWaitOperations {
        ...
        void shouldWaitForEachActionTypeDuration(ActionType actionType) {
            ...
            when(mockActionDurations.getActionDuration(actionType)).thenReturn(expectedDuration);
            ...
            assertEquals(expectedNanos, nanosDiff, "Should wait for " + actionType + " duration");
        }

        @Test
        @DisplayName("Should use different durations for different actions")
        void shouldUseDifferentDurationsForDifferentActions() {
            when(mockActionDurations.getActionDuration(ActionType.CLICK)).thenReturn(0.5);
            when(mockActionDurations.getActionDuration(ActionType.TYPE)).thenReturn(2.0);
            when(mockActionDurations.getActionDuration(ActionType.DRAG)).thenReturn(3.0);
    ...
    class FindStrategyWaitOperations {
        ...
        void shouldWaitForEachFindStrategyDuration(PatternFindOptions.Strategy strategy) {
            ...
            assertEquals(expectedNanos, nanosDiff, "Should wait for " + strategy + " duration");
        }

        @Test
        @DisplayName("Should handle different find strategies")
        void shouldHandleDifferentFindStrategies() {
            when(mockActionDurations.getFindStrategyDuration(PatternFindOptions.Strategy.FIRST))
                    .thenReturn(0.3);
            when(mockActionDurations.getFindStrategyDuration(PatternFindOptions.Strategy.ALL))
                    .thenReturn(1.5);
    ...
    class ConsoleOutput {
        ...
        void shouldPrintWaitMessageWhenReportingHigh() {
            try (MockedStatic<ConsoleReporter> reporter = mockStatic(ConsoleReporter.class)) {
                reporter.when(
                                () ->
                                        ConsoleReporter.minReportingLevel(
                ...
                assertTrue(output.contains("wait-3.5"), "Should print wait message with duration");
            }
        }
        ...
        void shouldNotPrintWhenReportingNotHigh() {
            try (MockedStatic<ConsoleReporter> reporter = mockStatic(ConsoleReporter.class)) {
                ...
                String output = outputStream.toString();
                assertFalse(
                        output.contains("wait"), "Should not print when reporting level not HIGH");
            }
        }
    }
    ...
    class TimePrecision {
        ...
        void shouldMaintainNanosecondPrecision() {
            ...
            assertEquals(123456789, nanosDiff, "Should maintain nanosecond precision");
        }
    ...
    class IntegrationScenarios {
        ...
        void shouldSimulateTimeoutScenario() {
            ...
            assertTrue(elapsed > timeoutSeconds, "Should be able to simulate timeout scenarios");
        }

        @Test
        @DisplayName("Should simulate action sequence")
        void shouldSimulateActionSequence() {
            when(mockActionDurations.getActionDuration(any())).thenReturn(0.5, 1.0, 0.3, 2.0);

            // Simulate a sequence of actions
            ActionType[] sequence = {
                ActionType.FIND, ActionType.CLICK, ActionType.TYPE, ActionType.VANISH
            };
            ...
            assertEquals(3800, totalMillis, "Should correctly accumulate action sequence timing");
        }
        ...
        void shouldSupportPerformanceTestingPatterns() {
            ...
            LocalDateTime endTime = mockTime.now();
            double actualTotal = ChronoUnit.NANOS.between(startTime, endTime) / 1_000_000_000.0;
    ...
    class EdgeCases {
        ...
        void shouldHandleRapidSuccessiveWaits() {
            ...
            assertEquals(1, secondsDiff, "1000 waits of 1ms should equal 1 second");
        }
    }
}
