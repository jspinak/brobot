...
@DisabledIfEnvironmentVariable(
        named = "CI",
        matches = "true",
        disabledReason = "System stream manipulation unreliable in CI")
public class MessageRouterTest extends BrobotTestBase {
    ...
    @Mock private ConsoleFormatter consoleFormatter;

    @Mock private LoggingVerbosityConfig.NormalModeConfig normalConfig;

    @Mock private LoggingVerbosityConfig.VerboseModeConfig verboseConfig;
    ...
    public void setupTest() {
        ...
        lenient()
                .when(consoleFormatter.format(any(LogEvent.class)))
                .thenAnswer(
                        invocation -> {
    ...
    public void testRouteEvent_ActionEvent() {
        ...
        try (var mockStatic = mockStatic(ConsoleReporter.class)) {
            mockStatic.when(() -> ConsoleReporter.minReportingLevel(any())).thenReturn(true);
            ...
            // Assert
            verify(actionLogger)
                    .logAction(anyString(), any(ActionResult.class), any(ObjectCollection.class));
            verify(consoleFormatter).format(event);
            String output = outputStream.toString();
            assertTrue(
                    output.contains("[INFO]") || output.contains("Click action"),
    ...
    public void testRouteEvent_ObservationEvent() {
        ...
        // Assert
        verify(actionLogger)
                .logObservation(anyString(), eq("OBSERVATION"), eq("Test observation"), eq("INFO"));
        verify(consoleFormatter).format(event);
    }
    ...
    public void testDetermineConsoleLevel_ErrorType() {
        ...
        ConsoleReporter.OutputLevel level =
                invokePrivateMethod(messageRouter, "determineConsoleLevel", event);
    ...
    public void testRouteToConsole_WithLevelCheck() {
        ...
        try (var mockStatic = mockStatic(ConsoleReporter.class)) {
            ...
            String output = outputStream.toString();
            assertTrue(
                    output.contains("Test action") || output.contains("[INFO]"),
    ...
    public void testRouteToConsole_LevelNotAllowed() {
        ...
        try (var mockStatic = mockStatic(ConsoleReporter.class)) {
            mockStatic
                    .when(() -> ConsoleReporter.minReportingLevel(ConsoleReporter.OutputLevel.HIGH))
                    .thenReturn(false);
    ...
    public void testRouteToActionLogger_CreateActionResult() {
        ...
        verify(actionLogger)
                .logAction(eq("test-session"), resultCaptor.capture(), any(ObjectCollection.class));
    ...
    public void testFormatMessage_NormalMode() {
        ...
        assertTrue(
                formatted.contains("FIND")
                        && formatted.contains("TestObject")
    ...
    public void testConcurrentEventRouting() throws InterruptedException {
        ...
        // Assert
        verify(actionLogger, times(threadCount))
                .logObservation(anyString(), anyString(), anyString(), anyString());
