...
/**
 * Comprehensive test suite for StateNavigator. Tests high-level state navigation orchestration and
 * path finding.
 */
...
class StateNavigatorTest extends BrobotTestBase {
    ...
    class OpenStateByName {
        ...
        void testOpenStateByNameSuccess() {
            ...
            when(stateService.findSetById(any(Set.class)))
                    .thenReturn(new HashSet<>(Arrays.asList(mockState)));
            ...
            verify(actionLogger)
                    .logStateTransition(anyString(), any(), any(), any(), eq(true), anyLong());
        }
    ...
    class OpenStateById {
        ...
        void testOpenStateByIdSuccess() {
            ...
            when(pathFinder.getPathsToState(any(Set.class), eq(targetStateId)))
            ...
            when(stateService.findSetById(any(Set.class)))
                    .thenReturn(new HashSet<>(Arrays.asList(targetState)));
        ...
        void testAlreadyAtTargetState() {
            ...
            when(stateMemory.getActiveStates())
                    .thenReturn(new HashSet<>(Arrays.asList(targetStateId)));
        ...
        void testNoPathsAvailable() {
            ...
            verify(pathTraverser, never()).traverse(any());
            verify(actionLogger)
                    .logStateTransition(anyString(), any(), any(), any(), eq(false), anyLong());
        }
    }
    ...
    class PathTraversalAndRecovery {
        ...
        void testAlternativePathOnFailure() {
            ...
            when(pathManager.getCleanPaths(any(), eq(initialPaths), eq(1L)))
        ...
        void testUpdateActiveStatesAfterPartialTraversal() {
            ...
            when(pathFinder.getPathsToState(eq(initialActiveStates), eq(targetStateId)))
                    .thenReturn(initialPaths);
    ...
    class LoggingAndSessionManagement {
        ...
        void testLoggingTransitionStartAndEnd() {
            ...
            verify(actionLogger)
                    .logStateTransition(
                            eq("test-session-123"), any(), any(), any(), eq(true), anyLong());
        }
        ...
        void testUseCurrentSessionId() {
            ...
            // Assert
            verify(actionLogger)
                    .logObservation(eq(customSessionId), anyString(), anyString(), anyString());
            verify(actionLogger)
                    .logStateTransition(
                            eq(customSessionId), any(), any(), any(), anyBoolean(), anyLong());
        }
    }
}
