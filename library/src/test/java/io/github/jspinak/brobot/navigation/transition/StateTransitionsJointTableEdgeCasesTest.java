...
/**
 * Edge case and comprehensive tests for StateTransitionsJointTable.
 * Tests graph management, dynamic transitions, and complex scenarios.
 */
    ...
    class HiddenStateManagement {
        ...
        void testAddTransitionsToHiddenStates() {
            ...
            for (Long hiddenId : hiddenStates) {
                ...
                assertTrue(jointTable.getIncomingTransitionsToPREVIOUS().get(hiddenId).contains(100L));
            }
        }
        ...
        void testConcurrentModifications() throws InterruptedException {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                Thread thread = new Thread(() -> {
                    for (int j = 0; j < 100; j++) {
                        jointTable.add((long)(threadId * 100 + j), (long)threadId);
                    }
                });
                threads.add(thread);
                thread.start();
