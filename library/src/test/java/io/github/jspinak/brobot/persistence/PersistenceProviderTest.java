...
/**
 * Comprehensive test suite for PersistenceProvider interface implementations.
 * Tests session management, action recording, and data import/export functionality.
 */
...
public class PersistenceProviderTest extends BrobotTestBase {
    ...
    class SessionManagement {
        ...
        void shouldStartNewSession() {
            String sessionId = persistenceProvider.startSession(
                "TestSession", "TestApp", "metadata"
            );
        ...
        void shouldStopActiveSession() {
            String startId = persistenceProvider.startSession(
                "TestSession", "TestApp", null
            );
    ...
    class ActionRecording {
        ...
        void shouldRecordBatchActions() {
            ...
            ActionHistory history = persistenceProvider.exportSession(
                persistenceProvider.getCurrentSessionId()
            );
    ...
    class SessionImportExport {
        ...
        void shouldImportActionHistoryAsNewSession() {
            ...
            history.setSnapshots(Arrays.asList(
    ...
    class PerformanceScalability {
        ...
        void shouldHandleLargeBatchRecordings(int batchSize) {
            ...
            assertTrue(duration < batchSize * 10, // Allow 10ms per record
        ...
        void shouldHandleManySession() {
            ...
            for (int i = 0; i < 50; i++) {
                ...
                persistenceProvider.recordAction(
                    createActionRecord("ACTION", true), null
                );
                persistenceProvider.stopSession();
    ...
    private static class TestPersistenceProvider implements PersistenceProvider {
        ...
        public void recordAction(ActionRecord record, StateObject stateObject) {
            ...
            currentSession.metadata.setTotalActions(
                currentSession.metadata.getTotalActions() + 1
            );
        ...
        public String importSession(ActionHistory history, String sessionName) {
            ...
            session.metadata.setSuccessfulActions(
                (int) history.getSnapshots().stream()
