...
/**
 * Comprehensive tests for pattern matching edge cases and advanced scenarios. This test class
 * focuses on challenging situations that can occur during pattern matching operations.
 */
...
class PatternMatchingEdgeCasesTest extends BrobotTestBase {
    ...
    class SimilarityThresholdEdgeCases {
        ...
        void testExactMatchRequirement() {
            ...
            verify(findPipeline)
                    .execute(
                            argThat(opts -> ((PatternFindOptions) opts).getSimilarity() == 1.0),
                            eq(actionResult),
                            eq(objectCollection));
        }
        ...
        void testFuzzyMatching() {
            ...
            verify(findPipeline)
                    .execute(
                            argThat(opts -> ((PatternFindOptions) opts).getSimilarity() == 0.3),
                            eq(actionResult),
                            eq(objectCollection));
        }
    ...
    class ComplexSearchRegionScenarios {
        ...
        void testOverlappingSearchRegions() {
            ...
            verify(findPipeline)
                    .execute(
                            argThat(opts -> ((PatternFindOptions) opts).getSearchRegions() != null),
                            eq(actionResult),
                            eq(objectCollection));
        }
    ...
    class PerformanceAndTimeoutScenarios {
        ...
        void testSearchDurationTimeout() {
            ...
            verify(findPipeline)
                    .execute(
                            argThat(opts -> ((PatternFindOptions) opts).getSearchDuration() == 0.1),
                            eq(actionResult),
                            eq(objectCollection));
        }
        ...
        void testEarlyExitOptimization() {
            ...
            verify(findPipeline, times(1))
                    .execute(eq(options), eq(actionResult), eq(objectCollection));
        }
    }
    ...
    class MatchFusionEdgeCases {
        ...
        void testMatchFusionForOverlapping() {
            ...
            verify(findPipeline)
                    .execute(
                            argThat(
                                    opts ->
                                            ((PatternFindOptions) opts).getMatchFusionOptions()
                                                    != null),
                            eq(actionResult),
                            eq(objectCollection));
        }
        ...
        void testDifferentFusionMethods() {
            ...
            for (MatchFusionOptions.FusionMethod method :
                    MatchFusionOptions.FusionMethod.values()) {
                // Arrange
                MatchFusionOptions fusionOptions =
                        MatchFusionOptions.builder().setFusionMethod(method).build();

                PatternFindOptions options =
                        new PatternFindOptions.Builder().setMatchFusion(fusionOptions).build();
                ActionResult actionResult = new ActionResult(options);
    ...
    void testCombinedEdgeCases(
            double similarity, PatternFindOptions.Strategy strategy, int maxMatches) {
        // Arrange
        PatternFindOptions options =
                new PatternFindOptions.Builder()
