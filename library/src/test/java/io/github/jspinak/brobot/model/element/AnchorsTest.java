...
/**
 * Comprehensive tests for the Anchors class which manages collections of Anchor objects for complex
...
public class AnchorsTest extends BrobotTestBase {
    ...
    void testJsonIgnoreUnknownProperties() throws JsonProcessingException {
        ...
        String jsonWithUnknown =
                "{\"anchorList\":[{\"anchorInNewDefinedRegion\":\"TOPLEFT\","
                    + "\"positionInMatch\":{\"x\":10,\"y\":20,\"xOffsetPercent\":30,\"yOffsetPercent\":40}}],\"unknownProperty\":\"unknown"
                    + " value\"}";
        ...
        assertEquals(
                ...
                deserialized.getAnchorList().get(0).getAnchorInNewDefinedRegion());
    }
    ...
    Stream<DynamicTest> testComplexPositioningScenarios() {
        return Stream.of(
                dynamicTest(
                        "Form field bounded by multiple elements",
                        () -> {
                            Anchors formAnchors = new Anchors();
                            // Label's right edge defines field's left boundary
                            Anchor labelAnchor =
                                    new Anchor(Positions.Name.MIDDLELEFT, new Position(50, 50));
                            // Submit button's top edge defines field's bottom boundary
                            Anchor buttonAnchor =
                                    new Anchor(Positions.Name.BOTTOMMIDDLE, new Position(70, 100));
                ...
                dynamicTest(
                        ...
                        () -> {
                            ...
                            boundaryAnchors.add(
                                    new Anchor(Positions.Name.TOPLEFT, new Position(0, 0)));
                            boundaryAnchors.add(
                                    new Anchor(Positions.Name.TOPRIGHT, new Position(100, 0)));
                            boundaryAnchors.add(
                                    new Anchor(Positions.Name.BOTTOMLEFT, new Position(0, 100)));
                            boundaryAnchors.add(
                                    new Anchor(Positions.Name.BOTTOMRIGHT, new Position(100, 100)));
                            assertEquals(4, boundaryAnchors.size());
                        }),
                dynamicTest(
                        "Multiple fallback anchors",
                        () -> {
                            Anchors fallbackAnchors = new Anchors();
                            // Primary anchor
                            fallbackAnchors.add(
                                    new Anchor(Positions.Name.MIDDLEMIDDLE, new Position(50, 50)));
                            // Fallback anchors if primary is not found
                            fallbackAnchors.add(
                                    new Anchor(Positions.Name.TOPLEFT, new Position(0, 0)));
                            fallbackAnchors.add(
                                    new Anchor(Positions.Name.BOTTOMRIGHT, new Position(80, 80)));
