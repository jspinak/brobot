 ...
 * Tests image comparison and pattern matching functionality.
 */
...
public class ImageComparerTest extends BrobotTestBase {
    ...
    class PatternComparison {
        ...
        void shouldCompareTwoValidPatterns() {
            ...
            when(compareSize.getEnvelopedFirstOrNone(pattern1, pattern2)).thenReturn(sortedPatterns);
            when(mockOrLive.findAll(eq(pattern1), any(Scene.class))).thenReturn(Collections.singletonList(match1));
        ...
        void shouldReturnEmptyMatchWhenSizeComparisonFails() {
            when(compareSize.getEnvelopedFirstOrNone(pattern1, pattern2)).thenReturn(Collections.emptyList());
        ...
        void shouldCreateEmptyMatchWithMetadata() {
            List<Pattern> sortedPatterns = Arrays.asList(pattern1, pattern2);
            when(compareSize.getEnvelopedFirstOrNone(pattern1, pattern2)).thenReturn(sortedPatterns);
            when(mockOrLive.findAll(eq(pattern1), any(Scene.class))).thenReturn(Collections.emptyList());
    ...
    class StateImageListComparison {
        ...
        void shouldCompareListOfStateImages() {
            ...
            when(targetStateImage.getPatterns()).thenReturn(Collections.singletonList(targetPattern));
    ...
    class IntegrationScenarios {
        ...
        void shouldHandleComplexNestedComparison() {
            ...
            assertNotNull(result);
            verify(mockOrLive, times(9)).findAll(any(Pattern.class), any(Scene.class)); // 3x3 combinations
        }
