...
public class StringSimilarityTest extends BrobotTestBase {
    ...
    public void testConsensusScores() {
        ...
        // Each variation appears as many times as itself in the array [base, base, base, caps, lower]
        // Consensus score for each would be: sum of similarities to all variations
        double clickHereScore = 3 * StringSimilarity.similarity(base, base) + 
                                StringSimilarity.similarity(base, caps) +
                                StringSimilarity.similarity(base, lower);
        
        double clickCAPSScore = 3 * StringSimilarity.similarity(caps, base) + 
                               StringSimilarity.similarity(caps, caps) +
                               StringSimilarity.similarity(caps, lower);
        
        double clickLowerScore = 3 * StringSimilarity.similarity(lower, base) + 
        ...
        assertEquals(clickHereScore, clickCAPSScore, tolerance, 
                     "Consensus scores should be equal due to symmetry");
        assertEquals(clickHereScore, clickLowerScore, tolerance, 
                     "Consensus scores should be equal due to symmetry");
    }
