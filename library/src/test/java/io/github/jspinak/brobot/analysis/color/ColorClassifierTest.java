...
public class ColorClassifierTest extends BrobotTestBase {
    ...
    @Mock private ColorCluster colorCluster;

    @Mock private ColorSchema colorSchemaBGR;

    @Mock private ColorSchema colorSchemaHSV;

    @Mock private ColorStatistics colorStatistics;

    @InjectMocks private ColorClassifier colorClassifier;
    ...
    void shouldCreateSceneAnalysisWithIndices() {
        ...
        doNothing()
                .when(matOps3d)
                .minIndex(any(Mat.class), any(Mat.class), any(Mat.class), anyInt());

        Scene mockScene = MockSceneBuilder.createMockScene();
        SceneAnalysis result =
                colorClassifier.getSceneAnalysis(pixelAnalysisCollections, mockScene);

        assertNotNull(result);
        assertEquals(pixelAnalysisCollections, result.getPixelAnalysisCollections());
        assertEquals(mockScene, result.getScene());
        verify(matOps3d, atLeastOnce())
                .minIndex(any(Mat.class), any(Mat.class), any(Mat.class), eq(1));
    }
    ...
    void shouldReturnEmptyMatForEmptyAnalysis() {
        ...
        Mat result =
                colorClassifier.getImageIndices(sceneAnalysis, ColorCluster.ColorSchemaName.BGR);
    ...
    void shouldProcessBGRColorSchema() {
        ...
        SceneAnalysis sceneAnalysis =
                MockSceneBuilder.sceneAnalysis()
    ...
    void shouldProcessHSVColorSchema() {
        ...
        Mat result =
                colorClassifier.getImageIndices(sceneAnalysis, ColorCluster.ColorSchemaName.HSV);
