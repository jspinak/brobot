...
class ExplorationSessionRunnerTest extends BrobotTestBase {
    ...
    public void setupTest() {
        super.setupTest();
        sessionRunner = new ExplorationSessionRunner(
            stateNavigator,
            stateMemory,
            stateService,
            actionLogger,
    ...
    class TestSessionExecutionTests {
        ...
        void shouldExecuteSuccessfulTest() throws IOException, AWTException {
            ...
            verify(actionLogger).logObservation(anyString(), eq("TEST_START"), contains("Test started"), eq("INFO"));
            verify(actionLogger).logObservation(anyString(), eq("INITIAL_STATE"), contains("Initial states"), eq("INFO"));
            verify(actionLogger).logStateTransition(anyString(), isNull(), anySet(), anySet(), eq(true), anyLong());
            verify(actionLogger).logObservation(anyString(), eq("FINAL_STATE"), contains("Final states"), eq("INFO"));
            ...
            verify(actionLogger).logObservation(anyString(), eq("TEST_END"), contains("Test ended"), eq("INFO"));
        ...
        void shouldHandleTransitionFailure() throws IOException, AWTException {
            ...
            when(stateService.getState(destination))
                .thenReturn(Optional.of(unreachableState));
            ...
            verify(actionLogger).logError(anyString(), contains("Failed to transition"), eq(screenshotPath));
            verify(actionLogger).logStateTransition(anyString(), isNull(), anySet(), anySet(), eq(false), anyLong());
            
            // Should still complete the test
            verify(actionLogger).stopVideoRecording(anyString());
            verify(actionLogger).logObservation(anyString(), eq("TEST_END"), anyString(), eq("INFO"));
        }
        ...
        void shouldHandleDestinationNotFound() throws IOException, AWTException {
            ...
            // Assert
            verify(actionLogger).startVideoRecording(anyString());
            verify(actionLogger).logError(anyString(), contains("Destination state not found"), eq(screenshotPath));
    ...
    class PerformanceMetricsTests {
        ...
        void shouldMeasureTransitionDuration() {
            ...
            when(stateService.getState(destination))
                .thenReturn(Optional.of(slowState));
            
            when(stateMemory.getActiveStates()).thenReturn(Set.of(1L));
            when(stateService.findSetById(anySet())).thenReturn(new HashSet<>());
            
            // Simulate slow transition
            when(stateNavigator.openState(destinationId)).thenAnswer(invocation -> {
            ...
            verify(actionLogger).logPerformanceMetrics(
                anyString(),
        ...
        void shouldLogMetricsOnFailure() {
            ...
            when(stateService.getState(destination))
                .thenReturn(Optional.of(failState));
    ...
    class SessionManagementTests {
        ...
        void shouldMaintainSessionConsistency() throws IOException, AWTException {
            ...
            verify(actionLogger).logObservation(eq(sessionId), eq("TEST_START"), anyString(), anyString());
            verify(actionLogger).logObservation(eq(sessionId), eq("INITIAL_STATE"), anyString(), anyString());
            verify(actionLogger).logStateTransition(eq(sessionId), any(), anySet(), anySet(), anyBoolean(), anyLong());
            verify(actionLogger).logObservation(eq(sessionId), eq("FINAL_STATE"), anyString(), anyString());
            verify(actionLogger).logPerformanceMetrics(eq(sessionId), anyLong(), anyLong(), anyLong());
            verify(actionLogger).stopVideoRecording(eq(sessionId));
            verify(actionLogger).logObservation(eq(sessionId), eq("TEST_END"), anyString(), anyString());
        }
    }
    ...
    class ErrorHandlingTests {
        ...
        void shouldHandleVideoStartFailure() throws IOException, AWTException {
            ...
            doThrow(new RuntimeException("Video service unavailable"))
                .when(actionLogger).startVideoRecording(anyString());
            ...
            verify(actionLogger).logError(anyString(), contains("Video service unavailable"), anyString());
            verify(actionLogger).stopVideoRecording(anyString());
        }
        ...
        void shouldHandleVideoStopFailure() throws IOException, AWTException {
            ...
            doThrow(new RuntimeException("Cannot stop recording"))
                .when(actionLogger).stopVideoRecording(anyString());
        ...
        void shouldHandleTransitionException() throws IOException, AWTException {
            ...
            // Assert
            verify(screenshotCapture).captureScreenshot(contains("error"));
            verify(actionLogger).logError(anyString(), contains("Navigation error"), eq("/error_screenshot.png"));
            verify(actionLogger).stopVideoRecording(anyString());
            verify(actionLogger).logObservation(anyString(), eq("TEST_END"), anyString(), eq("INFO"));
        }
    }
    ...
    class StateServiceIntegrationTests {
        ...
        void shouldRetrieveAndNavigateToState() {
            ...
            when(stateService.findSetById(Set.of(1L, 2L)))
                .thenReturn(activeStates);
            
            when(stateService.findSetById(Set.of(destinationId)))
                .thenReturn(finalStates);
        ...
        void shouldHandleStateLookupFailure() throws IOException, AWTException {
            ...
            when(screenshotCapture.captureScreenshot(anyString()))
                .thenReturn("/error.png");
            
            // Act
            sessionRunner.runTest(destination);
            
            // Assert
            verify(actionLogger).logError(anyString(), contains("Database error"), eq("/error.png"));
            verify(stateNavigator, never()).openState(anyLong());
            verify(actionLogger).stopVideoRecording(anyString());
        }
    }
