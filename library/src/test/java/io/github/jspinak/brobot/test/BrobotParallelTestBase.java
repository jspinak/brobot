 ...
 * 
 * This class provides:
 * - Thread-safe test initialization
 * - Automatic mock mode enforcement
 * - Test isolation for parallel execution
 * - Resource tracking and cleanup
 * - Performance metrics collection
 * 
 * All Brobot tests should extend this class or BrobotTestBase.
 */
...
public abstract class BrobotParallelTestBase {
    ...
    public void teardownParallelTest(TestInfo testInfo) {
        try {
            ...
            if (startTime != null && log.isDebugEnabled()) {
                ...
                log.debug("[{}] Completed test: {} in {}ms (Active: {}, Total: {})",
                        testExecutionId, testKey, duration, active, completed);
            }
    ...
    /**
     * Enforces mock mode for all tests to ensure they don't interact with real
     * resources.
     */
    ...
    private void resetEnvironment() {
        try {
            ...
            ExecutionEnvironment defaultTestEnv = ExecutionEnvironment.builder()
                    .forceHeadless(true)
                    .mockMode(true)
                    .build();
    ...
    protected void assertMockModeEnabled() {
        if (!FrameworkSettings.mock) {
            throw new IllegalStateException("Mock mode is not enabled. All tests must run in mock mode.");
        }
    }
