...
/**
 * Comprehensive tests for MockModeManager.
 * Tests mock mode synchronization across all Brobot components.
 */
...
public class MockModeManagerTest extends BrobotTestBase {
    ...
    void testCheckMockModeFromExecutionEnvironment() {
        ...
        assertTrue(MockModeManager.isMockMode(), "Should detect mock mode from ExecutionEnvironment");
        
        // Disable mock mode
        ExecutionEnvironment realEnv = ExecutionEnvironment.builder()
            .mockMode(false)
            .build();
        ExecutionEnvironment.setInstance(realEnv);
        
        assertFalse(MockModeManager.isMockMode(), "Should detect real mode from ExecutionEnvironment");
    }
    ...
    void testCaseInsensitivePropertyValues() {
        ...
        for (String value : trueValues) {
            ...
            assertTrue(MockModeManager.isMockMode(), 
                "Should recognize '" + value + "' as true");
            System.clearProperty(MOCK_MODE_PROPERTY);
        }
        ...
        for (String value : falseValues) {
            ...
            assertFalse(MockModeManager.isMockMode(), 
                "Should recognize '" + value + "' as false");
            System.clearProperty(MOCK_MODE_PROPERTY);
        }
    }
    ...
    void testHandleExecutionEnvironmentUpdateFailure() {
        ...
        try (MockedStatic<ExecutionEnvironment> envMock = mockStatic(ExecutionEnvironment.class)) {
            envMock.when(ExecutionEnvironment::builder).thenThrow(new RuntimeException("Test exception"));
    ...
    void testInitializeFromAnyProperty(String property) {
        ...
        assertTrue(MockModeManager.isMockMode(), 
            "Should initialize mock mode from " + property);
    }
    ...
    void testRapidModeSwitches() {
        for (int i = 0; i < 10; i++) {
            ...
            assertEquals(enable, MockModeManager.isMockMode(), 
    ...
    void testExecutionEnvironmentException() {
        try (MockedStatic<ExecutionEnvironment> envMock = mockStatic(ExecutionEnvironment.class)) {
            envMock.when(ExecutionEnvironment::getInstance).thenThrow(new RuntimeException("Test exception"));
    ...
    @Order(23)
    void testVariousMockModeScenarios(String prop1, String prop2, String prop3, 
                                      Boolean envMock, boolean expected) {
        // Set properties
