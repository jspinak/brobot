...
/** Test suite for TransitionFetcher. Tests retrieval and packaging of transition components. */
...
class TransitionFetcherTest extends BrobotTestBase {
    ...
    @Mock private TransitionConditionPackager conditionPackager;

    @Mock private StateTransitions mockFromTransitions;

    @Mock private StateTransitions mockToTransitions;

    @Mock private StateTransition mockFromTransition;

    @Mock private StateTransition mockToTransition;

    @Mock private State mockFromState;

    @Mock private State mockToState;

    @Mock private BooleanSupplier mockBooleanSupplier;
    ...
    public void setupTest() {
        ...
        transitionFetcher =
                new TransitionFetcher(
                        stateMemory, stateService, stateTransitionService, conditionPackager);
    }
    ...
    class CompleteTransitionFetching {
        ...
        void testHandlePreviousStateTransition() {
            ...
            // Act
            Optional<TransitionFetcher> result =
                    transitionFetcher.getTransitions(fromId, previousId);
    ...
    class IncompleteTransitionFetching {
        ...
        void testMissingFromTransitions() {
            ...
            when(stateTransitionService.getTransitions(toId))
                    .thenReturn(Optional.of(mockToTransitions));
        ...
        void testMissingToTransitions() {
            ...
            when(stateTransitionService.getTransitions(fromId))
                    .thenReturn(Optional.of(mockFromTransitions));
            ...
            when(conditionPackager.toBooleanSupplier(mockFromTransition))
                    .thenReturn(mockBooleanSupplier);
        ...
        void testNullTransitionFinish() {
            ...
            when(stateTransitionService.getTransitions(toId))
                    .thenReturn(Optional.of(mockToTransitions));
            when(stateService.getState(toId)).thenReturn(Optional.of(mockToState));
            when(mockToTransitions.getTransitionFinish())
                    .thenReturn(null); // Null transition finish
        ...
        void testTransitionResolvesToNull() {
            ...
            // When transitionToEnum is null, getTransitionFunctionByActivatedStateId(null) returns
            // empty
