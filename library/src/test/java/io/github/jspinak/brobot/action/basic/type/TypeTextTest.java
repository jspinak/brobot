 ...
 * functionality, timing configuration, and batch processing.
 */
...
public class TypeTextTest extends BrobotTestBase {
    ...
    class BasicTypingOperations {
        ...
        public void testTypeMultipleStrings() {
            ...
            TypeOptions options = new TypeOptions.Builder().setPauseAfterEnd(0.5).build();
        ...
        public void testTypeSpecialCharacters() {
            StateString specialChars =
                    new StateString.Builder().setString("!@#$%^&*()_+-=[]{}|;':\",./<>?").build();
        ...
        public void testTypeWithWhitespace() {
            StateString withWhitespace =
                    new StateString.Builder().setString("Line1\nLine2\tTabbed").build();
    ...
    class TypeDelayConfiguration {
        ...
        public void testRestoreTypeDelay() {
            ...
            TypeOptions options = new TypeOptions.Builder().setTypeDelay(customDelay).build();
            when(mockActionResult.getActionConfig()).thenReturn(options);
    ...
    class PauseConfiguration {
        ...
        public void testCustomPauseBetweenStrings() {
            TypeOptions options = new TypeOptions.Builder().setPauseAfterEnd(1.0).build();
            when(mockActionResult.getActionConfig()).thenReturn(options);

            StateString str1 = new StateString.Builder().setString("First").build();
            StateString str2 = new StateString.Builder().setString("Second").build();
            when(mockObjectCollection.getStateStrings()).thenReturn(Arrays.asList(str1, str2));
        ...
        @ParameterizedTest
        @CsvSource({"2, 0.5, 1", "3, 1.0, 2", "5, 0.25, 4", "10, 0.1, 9"})
        @DisplayName("Multiple strings with various pauses")
        public void testMultipleStringsVariousPauses(
                int stringCount, double pause, int expectedPauses) {
            TypeOptions options = new TypeOptions.Builder().setPauseAfterEnd(pause).build();
            when(mockActionResult.getActionConfig()).thenReturn(options);
    ...
    class ActionType {
        ...
        public void testActionType() {
            assertEquals(
                    io.github.jspinak.brobot.action.ActionInterface.Type.TYPE,
    ...
    class IntegrationScenarios {
        ...
        public void testFormDataSequence() {
            ...
            StateString confirmPassword =
                    new StateString.Builder().setString("SecurePass123!").build();

            when(mockObjectCollection.getStateStrings())
                    .thenReturn(Arrays.asList(username, password, confirmPassword));

            TypeOptions options =
                    new TypeOptions.Builder().setTypeDelay(0.05).setPauseAfterEnd(0.3).build();
            when(mockActionResult.getActionConfig()).thenReturn(options);
    ...
    class PerformanceConsiderations {
        ...
        public void testFastTyping() {
            TypeOptions options =
                    new TypeOptions.Builder().setTypeDelay(0.0).setPauseAfterEnd(0.0).build();
            when(mockActionResult.getActionConfig()).thenReturn(options);
        ...
        public void testSlowTyping() {
            TypeOptions options =
                    new TypeOptions.Builder().setTypeDelay(0.2).setPauseAfterEnd(2.0).build();
            when(mockActionResult.getActionConfig()).thenReturn(options);
