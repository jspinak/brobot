...
/**
 * Comprehensive test suite for MockBehaviorVerifier. Tests the core orchestration component
 * responsible for managing verification workflows.
 */
...
class MockBehaviorVerifierTest extends BrobotTestBase {
    ...
    class ActionRecordingTests {
        ...
        void shouldRecordMultipleActionsInOrder() {
            ...
            for (MockBehaviorVerifier.ExecutionEvent event : history) {
                assertTrue(
                        event.getTimestamp().isAfter(previousTimestamp)
        ...
        void shouldHandleConcurrentActionRecording() throws InterruptedException {
            ...
            assertTrue(
                    history.stream().allMatch(MockBehaviorVerifier.ExecutionEvent::isActionEvent));
        }
    }
    ...
    class EventFilteringTests {
        ...
        void shouldFilterActionEvents() {
            ...
            assertTrue(
                    actionEvents.stream()
                            .allMatch(MockBehaviorVerifier.ExecutionEvent::isActionEvent));
        }
        ...
        void shouldFilterStateTransitionEvents() {
            ...
            assertTrue(
                    transitionEvents.stream()
                            .allMatch(MockBehaviorVerifier.ExecutionEvent::isStateTransitionEvent));
        }
        ...
        void shouldFilterSuccessfulActions() {
            ...
            List<MockBehaviorVerifier.ExecutionEvent> successEvents =
                    verifier.getEvents(successfulActions);

            // Assert
            assertEquals(2, successEvents.size()); // FIND and TYPE were successful
            assertTrue(successEvents.stream().allMatch(e -> e.getResult().isSuccess()));
        }
        ...
        void shouldFilterByTransitionPath() {
            ...
            List<MockBehaviorVerifier.ExecutionEvent> fromBTransitions =
                    verifier.getEvents(fromStateB);
            ...
            Thread recordingThread =
                    new Thread(
                            () -> {
                                for (int i = 0; i < 100; i++) {
                                    verifier.recordAction(
                                            ActionType.CLICK, successResult, mockContext);
                                    try {
                                        Thread.sleep(1);
    ...
    class ExecutionEventTests {
        ...
        void shouldIdentifyActionEventsCorrectly() {
            // Arrange
            MockBehaviorVerifier.ExecutionEvent actionEvent =
                    MockBehaviorVerifier.ExecutionEvent.builder()
            ...
            MockBehaviorVerifier.ExecutionEvent transitionEvent =
                    MockBehaviorVerifier.ExecutionEvent.builder()
                            .timestamp(LocalDateTime.now())
                            .fromState("A")
                            .toState("B")
        ...
        void shouldHandleMetadataCorrectly() {
            // Arrange
            Map<String, Object> metadata = Map.of("key1", "value1", "key2", 42, "key3", true);
    ...
    class ActiveVerificationIntegrationTests {
        ...
        void shouldTriggerActiveVerificationsOnNewEvents() {
            ...
            // Assert
            verify(mockTransition, times(1))
                    .checkEvent(any(MockBehaviorVerifier.ExecutionEvent.class));
            verify(mockPattern, times(1))
                    .checkEvent(any(MockBehaviorVerifier.ExecutionEvent.class));
        }
