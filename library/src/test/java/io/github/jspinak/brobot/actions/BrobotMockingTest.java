 ...
 * - Mock mode is properly enabled from BrobotTestBase
 * - Real mode can be enabled but handles headless environments gracefully
 * - Pattern match history is preserved and accessible
...
class BrobotMockingTest extends BrobotTestBase {
        ...
        void testPatternMatchHistory() {
                ...
                assertFalse(patternWithHistory.getMatchHistory().isEmpty(),
                ...
                assertTrue(patternWithoutHistory.getMatchHistory().isEmpty(),
        ...
        void testObjectCollectionBuilding() {
                ...
                ObjectCollection collectionWithHistory = new ObjectCollection.Builder()
                                .withImages(stateImageWithHistory)
                                .build();

                assertNotNull(collectionWithHistory, "Collection should not be null");
                assertEquals(1, collectionWithHistory.getStateImages().size(),
                                "Should have one state image");

                // Test building collection with images without history
                ObjectCollection collectionWithoutHistory = new ObjectCollection.Builder()
                                .withImages(stateImageWithoutHistory)
                                .build();

                assertNotNull(collectionWithoutHistory, "Collection should not be null");
                assertEquals(1, collectionWithoutHistory.getStateImages().size(),
                                "Should have one state image");

                // Test building collection with multiple images
                ObjectCollection combinedCollection = new ObjectCollection.Builder()
                ...
                assertEquals(2, combinedCollection.getStateImages().size(),
                                "Should have two state images");
        }
}