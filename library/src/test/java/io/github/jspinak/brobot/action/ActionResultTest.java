...
/**
 * Comprehensive test suite for ActionResult. Tests the unified results container for all action
 * executions.
 */
...
public class ActionResultTest extends BrobotTestBase {
    ...
    class MatchManagement {
        ...
        void shouldSetMatchListDirectly() {
            List<Match> matches =
                    Arrays.asList(createMockMatch(0.9), createMockMatch(0.8), createMockMatch(0.7));
        ...
        void shouldTrackInitialMatches() {
            List<Match> initialMatches = Arrays.asList(createMockMatch(0.9), createMockMatch(0.8));
    ...
    class MatchSorting {
        ...
        void shouldSortMatchesBySizeDescending() {
            ...
            assertTrue(
                    sorted.get(0).getRegion().size() >= sorted.get(1).getRegion().size(),
                    "First should be >= second: "
                            + sorted.get(0).getRegion().size()
                            + " >= "
                            + sorted.get(1).getRegion().size());
            assertTrue(
                    sorted.get(1).getRegion().size() >= sorted.get(2).getRegion().size(),
                    "Second should be >= third: "
                            + sorted.get(1).getRegion().size()
                            + " >= "
                            + sorted.get(2).getRegion().size());
            assertTrue(
                    sorted.get(2).getRegion().size() >= sorted.get(3).getRegion().size(),
                    "Third should be >= fourth: "
                            + sorted.get(2).getRegion().size()
                            + " >= "
                            + sorted.get(3).getRegion().size());
        }
    }
    ...
    class BestMatchOperations {
        ...
        void shouldFindBestMatch() {
            ...
            actionResult.add(createMockMatch(0.7), bestMatch, createMockMatch(0.8));
    ...
    class MatchExtraction {
        ...
        void shouldExtractRegionsFromMatches() {
            ...
            actionResult.add(
                    createMockMatchWithRegion(0.8, r1), createMockMatchWithRegion(0.9, r2));
        ...
        void shouldExtractLocationsFromMatches() {
            ...
            actionResult.add(
                    createMockMatchWithLocation(0.8, l1), createMockMatchWithLocation(0.9, l2));
    ...
    class EdgeCases {
        ...
        void shouldHandleMatchesWithIdenticalScores() {
            actionResult.add(createMockMatch(0.8), createMockMatch(0.8), createMockMatch(0.8));
    ...
    private Match createMockMatchWithLocation(double score, Location location) {
        ...
        when(match.getRegion())
                .thenReturn(new Region(location.getX() - 25, location.getY() - 25, 50, 50));
        return match;
    }
}
