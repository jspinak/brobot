...
/**
 * Comprehensive test suite for StateTransitionsJointTable. Tests the state transition graph
 * management and navigation queries.
 */
...
class StateTransitionsJointTableTest extends BrobotTestBase {
    ...
    class BasicTransitionManagement {
        ...
        void testPreviousStateTransitions() {
            ...
            assertFalse(
                    jointTable.getIncomingTransitions().containsKey(2L)
                            && jointTable
                                    .getIncomingTransitions()
                                    .get(2L)
                                    .contains(SpecialStateType.PREVIOUS.getId()));

            // But should appear in outgoing transitions
            assertTrue(
                    jointTable
                            .getOutgoingTransitions()
                            .containsKey(SpecialStateType.PREVIOUS.getId()));
            assertTrue(
                    jointTable
                            .getOutgoingTransitions()
                            .get(SpecialStateType.PREVIOUS.getId())
                            .contains(2L));
        }
    ...
    class HiddenStateManagement {
        ...
        void testAddTransitionsToHiddenStates() {
            ...
            // Assert
            Map<Long, Set<Long>> previousTransitions =
                    jointTable.getIncomingTransitionsToPREVIOUS();
    ...
    class QueryOperations {
        ...
        void testGetStatesWithTransitionsFromMultiple() {
            // Act
            Set<Long> children =
                    jointTable.getStatesWithTransitionsFrom(new HashSet<>(Arrays.asList(1L, 2L)));
        ...
        void testQueryWithPreviousTransitions() {
            ...
            assertTrue(
                    parentsOf2.containsAll(
                            Arrays.asList(1L, 5L))); // 1L from regular, 5L from PREVIOUS
        }
    }
    ...
    class CombinedTransitions {
        ...
        void testGetIncomingTransitionsWithHiddenTransitions() {
            ...
            // Act
            Map<Long, Set<Long>> combined =
                    jointTable.getIncomingTransitionsWithHiddenTransitions();
