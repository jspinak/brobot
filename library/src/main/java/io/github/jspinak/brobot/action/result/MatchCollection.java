...
/**
 * Manages a collection of Match objects found during action execution. Provides operations for
 * adding, sorting, filtering, and analyzing matches.
 *
 * <p>This class encapsulates all match-related operations that were previously scattered throughout
 * ActionResult, following the Single Responsibility Principle.
 *
 * @since 2.0
 */
@Data
public class MatchCollection {
    ...
    public Optional<Match> getBest() {
        return matches.stream().max(Comparator.comparingDouble(Match::getScore));
    }
    ...
    public Optional<Match> getClosestTo(Location location) {
        ...
        return matches.stream().min(Comparator.comparingDouble(m -> getDistance(m, location)));
    }
    ...
    public MatchCollection filter(Predicate<Match> predicate) {
        ...
        filtered.matches = matches.stream().filter(predicate).collect(Collectors.toList());
        return filtered;
    }
                ...
                .filter(
                        m ->
                                m.getStateObjectData() != null
                                        && Objects.equals(
                                                m.getStateObjectData().getStateObjectName(),
                                                stateObjectId))
    ...
    public List<Region> getRegions() {
        return matches.stream().map(Match::getRegion).collect(Collectors.toList());
    }
    ...
    public List<Location> getLocations() {
        return matches.stream().map(Match::getTarget).collect(Collectors.toList());
    }
