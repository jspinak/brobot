 ...
 *
 * <p>This wrapper provides a stable API for time-related operations while allowing the underlying
 * implementation to switch between mock and live modes.
 */
@Slf4j
@Component
    ...
    public void wait(double seconds) {
        ...
        if (executionMode.isMock() || env.shouldSkipSikuliX()) {
            ...
            try {
                ...
                log.warn(
                        "SikuliX failed in wait operation ({}), falling back to Thread.sleep",
                        e.getMessage());
                try {
                    Thread.sleep((long) (seconds * 1000));
