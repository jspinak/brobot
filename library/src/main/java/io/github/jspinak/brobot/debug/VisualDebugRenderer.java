 ...
 * Highlights search regions, matches, and provides visual comparison grids.
 */
@Slf4j
@Component
...
public class VisualDebugRenderer {
    ...
    private static final Color COLOR_SEARCH_REGION = new Color(0, 128, 255, 100); // Blue semi-transparent
    private static final Color COLOR_MATCH_SUCCESS = new Color(0, 255, 0, 150);   // Green semi-transparent
    private static final Color COLOR_MATCH_FAILED = new Color(255, 0, 0, 100);    // Red semi-transparent
    private static final Color COLOR_BEST_MATCH = new Color(255, 255, 0, 200);    // Yellow
    private static final Color TEXT_BACKGROUND = new Color(0, 0, 0, 180);         // Dark background for text
    ...
    public BufferedImage createComparisonGrid(
        ...
        for (int i = 0; i < matches.size(); i++) {
            ...
            if (matchRegion != null) {
                ...
                g2d.drawString(String.format("Match %d (%.1f%%)", i + 1, match.getScore() * 100),
                    x, y - 5);
    ...
    private void drawSearchRegion(Graphics2D g2d, Region region) {
        ...
        g2d.fillRect(region.getX(), region.getY() - labelHeight - 2, labelWidth + 4, labelHeight + 2);
