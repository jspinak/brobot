 ...
 * This enables flexible region definitions that adapt to GUI changes by using reference points from
 * any state object in the model.
 */
@Component
public class CrossStateAnchorProcessor {
    ...
    public CrossStateAnchorProcessor(
    ...
    public void processAnchors(
            ...
            List<CrossStateAnchor> crossStateAnchors,
            ActionResult actionResult) {
        if (crossStateAnchors == null || crossStateAnchors.isEmpty()) return;

        // Collect anchor locations from cross-state matches
        List<LocationWithAnchor> anchorPoints =
                collectAnchorPoints(crossStateAnchors, actionResult);
        ...
        Optional<Region> definedRegion =
                applyStrategy(strategy, anchorPoints, stateRegion.getSearchRegion());

        definedRegion.ifPresent(stateRegion::setSearchRegion);
    }

    /** Collects anchor points from cross-state matches. */
    private List<LocationWithAnchor> collectAnchorPoints(
            List<CrossStateAnchor> anchors, ActionResult actionResult) {
    ...
    private Optional<Match> findHistoricalMatch(
            State state, String objectName, StateObject.Type type) {
        switch (type) {
            case IMAGE:
    ...
    private Match createMatchFromLocation(StateLocation stateLocation) {
        ...
        return new Match.Builder()
                .setRegion(
                        stateLocation.getLocation().getCalculatedX(),
                        stateLocation.getLocation().getCalculatedY(),
                        1,
                        1)
    ...
    private Optional<Region> applyStrategy(
            DefineAs strategy, List<LocationWithAnchor> anchorPoints, Region currentRegion) {
        switch (strategy) {
            case INSIDE_ANCHORS:
