 ...
 *
 * <p>This class provides convenience methods for constructing drag operations using the
 * ActionConfig chaining mechanism. A drag is composed of six actions: 1. Find the source location
 * 2. Find the destination location 3. Move mouse to source 4. Press mouse button down 5. Move mouse
 * to destination (while holding) 6. Release mouse button
 *
 * <p><b>Example usage:</b>
 *
 * <pre>{@code
...
public class DragBuilder {
    ...
    public DragBuilder withMoveToSourceSpeed(float delay) {
        this.moveToSourceOptions = new MouseMoveOptions.Builder().setMoveMouseDelay(delay).build();
        return this;
    }
    ...
    public DragBuilder withDragSpeed(float delay) {
        this.dragMoveOptions = new MouseMoveOptions.Builder().setMoveMouseDelay(delay).build();
        return this;
    }
    ...
    public DragBuilder withButton(MouseButton button) {
        this.mouseDownOptions =
                new MouseDownOptions.Builder()
                        .setPressOptions(MousePressOptions.builder().setButton(button).build())
                        .build();
        this.mouseUpOptions =
                new MouseUpOptions.Builder()
                        .setPressOptions(MousePressOptions.builder().setButton(button).build())
    ...
    public DragBuilder withDestinationOffset(int offsetX, int offsetY) {
        this.toOptions =
                new PatternFindOptions.Builder(toOptions)
                        .setMatchAdjustment(
    ...
    public DragBuilder withMouseDownPauses(double pauseBefore, double pauseAfter) {
        this.mouseDownOptions =
                new MouseDownOptions.Builder(mouseDownOptions)
                        .setPressOptions(
    ...
    public DragBuilder withMouseUpPauses(double pauseBefore, double pauseAfter) {
        this.mouseUpOptions =
                new MouseUpOptions.Builder(mouseUpOptions)
                        .setPressOptions(
                                MousePressOptions.builder()
    ...
    public ActionConfig build() {
        // Chain the actions: Find source -> Find destination -> Move to source -> Mouse down ->
        // Drag move -> Mouse up
