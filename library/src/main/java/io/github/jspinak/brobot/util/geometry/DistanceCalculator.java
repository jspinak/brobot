 ...
 *
 * <p>This utility class provides essential geometric operations for Brobot's spatial reasoning,
 * including distance measurements, angle calculations, and location transformations. It handles the
 * screen coordinate system where Y increases downward, unlike traditional Cartesian coordinates.
 *
 * <p>Key capabilities:
 *
 * <ul>
 *   <li>Euclidean distance calculations between points and matches
 *   <li>Angle measurements in degrees (-180 to 180)
 *   <li>Angle normalization and scale conversion
 *   <li>Vector operations and location transformations
 *   <li>Sector analysis for angle clustering
 * </ul>
 *
 * <p>Coordinate system notes:
 *
 * <ul>
 *   <li>Screen coordinates: Y increases downward
 *   <li>Angles: 0° is East, positive counterclockwise
 *   <li>Range: -180° to 180° (standard mathematical convention)
 *   <li>Conversions handle the inverted Y-axis automatically
 * </ul>
 *
 * <p>Angle operations:
 *
 * <ul>
 *   <li>Median angle: Vector sum approach for averaging angles
 *   <li>Scale conversion: Handles circular wraparound at ±180°
 *   <li>Sector analysis: Finds gaps in angle distributions
 *   <li>Span calculation: Measures angular coverage
 * </ul>
 *
 * <p>Common use cases:
 *
 * <ul>
 *   <li>Calculating distances between UI elements
 *   <li>Determining relative positions and directions
 *   <li>Gesture path analysis and validation
 *   <li>Spatial clustering and grouping
 *   <li>Navigation and movement planning
 * </ul>
 *
...
public class DistanceCalculator {
     ...
     *
     * <p>Handles the screen coordinate system where Y increases downward by inverting the Y
     * difference to match mathematical conventions.
     *
     ...
     *
     * <p>Uses atan2 for proper quadrant handling. Returns angles in standard mathematical
     * convention: 0° = East, positive counterclockwise, range -180° to 180°.
     *
     * @param distPair [deltaX, deltaY] components
     ...
     *
     * <p>Delegates to euclidean method for consistency.
     *
    ...
    public double getDistance(Location loc1, Location loc2) {
        return euclidean(loc1, loc2);
    }

    /**
     * Calculates distance between centers of two matches.
     *
     * <p>Uses the center points of match regions for distance calculation, appropriate for
     * measuring spacing between UI elements.
     *
    ...
    public double getDistance(Match match1, Match match2) {
        return Math.sqrt(
                Math.pow(
                                match1.getRegion().sikuli().getCenter().getX()
                                        - match2.getRegion().sikuli().getCenter().getX(),
                                2)
                        + Math.pow(
                                match1.getRegion().sikuli().getCenter().getY()
                                        - match2.getRegion().sikuli().getCenter().getY(),
                                2));
    }
     ...
     *
     * <p>Returns the direction angle in degrees, where 0° points East, 90° points North (due to
     * inverted Y), -90° points South, and ±180° points West.
     *
     * @param loc1 starting location
     ...
     *
     * <p>Useful for determining absolute screen positions and quadrant analysis.
     *
     ...
     *
     * <p>Returns the smallest angle between the vectors (acute angle), always positive. For
     * example, returns 30° instead of 330°.
     *
     * <p>Vector definitions:
     *
     * <ul>
     *   <li>Vector 1: from center to loc1
     *   <li>Vector 2: from center to loc2
     * </ul>
     *
     ...
     *
     * <p>Convenience method using match centers as vector endpoints.
     *
     ...
     *
     * <p>Handles circular wraparound to ensure the result represents the shortest angular distance.
     *
     ...
     *
     * <p>On a circular scale, there's a discontinuity at ±180°. This method adjusts angle values so
     * that clustered angles don't span this break, enabling proper averaging and comparison.
     *
     * <p>Algorithm:
     *
     * <ol>
     *   <li>Find median angle of the cluster
     *   <li>Identify the opposite angle (180° away)
     *   <li>Adjust angles near the break to maintain continuity
     * </ol>
     *
     * <p>Example: [170°, -170°, -160°] becomes [170°, 190°, 200°]
     *
     ...
     *
     * <p>Maintains the -180° to 180° range convention.
     *
     ...
     *
     * <p>Converts angles to unit vectors, sums them, and finds the resulting angle. This method
     * properly handles angle averaging on a circular scale.
     *
     ...
     *
     * <p>This method finds the average direction by:
     *
     * <ol>
     *   <li>Summing all X coordinates
     *   <li>Summing all Y coordinates
     *   <li>Calculating angle to the summed point
     * </ol>
     *
     ...
     *
     * <p>Generates a location 1000 pixels from origin at the specified angle. Used for angle
     * calculations and normalization where actual distance doesn't matter.
     *
     ...
     *
     * <p>Performs polar to Cartesian conversion, accounting for the inverted Y-axis in screen
     * coordinates. Results are rounded to nearest pixel.
     *
     * <p>Coordinate transformation:
     *
     * <ul>
     *   <li>X = distance × cos(angle)
     *   <li>Y = -distance × sin(angle) (inverted for screen)
     * </ul>
     *
    ...
    public Location getLocation(double angle, double distance, Location startingLocation) {
        ...
        return new Location(
                (int) Math.round(startingLocation.getCalculatedX() + addX),
     ...
     *
     * <p>Returns a Location object representing the vector from location1 to location2. The result
     * can be used for relative positioning and movement calculations.
     *
    ...
    public Location getLocation(Location location1, Location location2) {
        return new Location(
                location2.getCalculatedX() - location1.getCalculatedX(),
                location2.getCalculatedY() - location1.getCalculatedY());
    }

    /**
     * Reverses angle scale normalization for a list.
     *
     * <p>Converts angles back to standard -180° to 180° range after scale conversion operations.
     *
     ...
     *
     * <p>Adjusts angles outside the standard range by adding or subtracting 360° as needed.
     *
     ...
     *
     * <p>Identifies the angle that borders the largest gap when moving counterclockwise. This
     * represents the left edge of the main cluster of angles.
     *
     * <p>Algorithm:
     *
     * <ol>
     *   <li>Find the largest angular gap between consecutive angles
     *   <li>Return the angle on the right side of this gap
     * </ol>
     *
     ...
     *
     * <p>Identifies the angle that borders the largest gap when moving clockwise. This represents
     * the right edge of the main cluster.
     *
     * @param angles list of angles to analyze
     ...
     *
     * <p>Returns the total angular coverage, which is 360° minus the largest gap. This indicates
     * how spread out the angles are around the circle.
     *
     ...
     *
     * <p>Calculates the angle that best represents the center of the angle cluster, accounting for
     * the largest gap. The result points away from the gap toward the cluster center.
     *
     * @param angles list of angles to analyze
    ...
    public double getMiddleAngle(List<Double> angles) {
        Sector largestSector = getLargestSector(angles);
        double angleBetween =
                getMedianAngle(largestSector.getLeftAngle(), largestSector.getRightAngle());
     ...
     *
     * <p>Finds the sector (angular range) with the maximum span between consecutive angles. This is
     * useful for identifying the main cluster of angles and its boundaries.
     *
     * <p>Side effects: Modifies the input list by adding first element to the end for circular
     * comparison.
     *
