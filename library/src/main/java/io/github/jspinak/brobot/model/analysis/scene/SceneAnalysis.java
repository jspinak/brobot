 ...
 *
 * <p>SceneAnalysis aggregates all pixel-level analyses for a scene, organizing results by state
 * image and color space. It serves as the central data structure for color-based matching
 * operations, containing classification indices, scores, visualizations, and extracted matches.
 *
 * <p>Key components:
 *
 * <ul>
 *   <li><b>PixelAnalysisCollections</b>: One per state image, containing detailed pixel scores
 *   <li><b>Classification indices</b>: Which state image best matches each pixel
 *   <li><b>Visualizations</b>: BGR representations for display and debugging
 *   <li><b>Contours</b>: Extracted regions from classification results
 *   <li><b>Matches</b>: Concrete match objects for found patterns
 * </ul>
 *
 * <p>Analysis types stored:
 *
 * <ul>
 *   <li><b>SCENE</b>: Original scene in BGR and HSV formats
 *   <li><b>INDICES_3D</b>: Per-channel classification results
 *   <li><b>INDICES_3D_TARGETS</b>: Filtered to target images only
 *   <li><b>INDICES_2D</b>: Flattened classification (HSV H-channel)
 *   <li><b>BGR_FROM_INDICES_2D</b>: Colored visualization of results
 *   <li><b>BGR_FROM_INDICES_2D_TARGETS</b>: Visualization of targets only
 * </ul>
 *
 * @see PixelProfiles
 * @see Scene
...
public class SceneAnalysis {
    ...
    */
    ...
    // these matches are scene specific. for example, the matches from motion from the previous
    // scene to this one.
    ...
    public SceneAnalysis(List<PixelProfiles> pixelAnalysisCollections, Scene scene) {
        ...
        log.debug(
                "[SCENE_ANALYSIS] Creating with scene BGR Mat: {}x{}",
                sceneBGR.cols(),
                sceneBGR.rows());
     ...
     *
     * <p>This constructor is used when color analysis is not required, typically for
     * non-color-based operations. The scene is stored in both BGR and HSV formats for potential
     * visualization, but no pixel analysis collections are created.
     *
     * <p>Side effects: Initializes illustrations with the scene image
     *
    ...
    public SceneAnalysis(Scene scene) {
        ...
        Mat sceneHSV = scene.getPattern().getImage().getMatHSV();
        log.debug(
                "[SCENE_ANALYSIS] Creating minimal with scene BGR Mat: {}x{}",
                sceneBGR.cols(),
                sceneBGR.rows());
        addAnalysis(BGR, SCENE, sceneBGR);
        addAnalysis(HSV, SCENE, sceneHSV);
     ...
     *
     * <p>Stores the provided matrix in the internal analysis maps organized by color space and
     * analysis type. When adding BGR visualization data, also updates the illustrations for display
     * purposes.
     *
     * <p>Side effects: Updates illustrations if analysis type is BGR_FROM_INDICES_2D
     *
    ...
    public void addAnalysis(
            ColorCluster.ColorSchemaName colorSchemaName, Analysis analysis, Mat mat) {
    ...
    public List<StateImage> getStateImageObjects() {
        return pixelAnalysisCollections.stream()
                .map(PixelProfiles::getStateImage)
    ...
    public String getImageNames() {
        return pixelAnalysisCollections.stream()
                .map(PixelProfiles::getImageName)
    ...
    public int getLastIndex() {
        OptionalInt maxOpt =
                pixelAnalysisCollections.stream()
                        .mapToInt(pic -> pic.getStateImage().getIndex())
                        .max();
     ...
     *
     * <p>Retrieves the specified color statistic (mean, stddev, etc.) profiles from each state
     * image's color cluster for the given color space.
     *
     * @param colorSchemaName the color space (BGR or HSV)
     * @param colorStat the type of statistic to retrieve
     * @return list of color stat profiles from all state images
     */
    public List<ColorStatistics> getColorStatProfiles(
            ColorCluster.ColorSchemaName colorSchemaName, ColorInfo.ColorStat colorStat) {
        ...
        for (PixelProfiles pixelAnalysisCollection : pixelAnalysisCollections) {
            ...
            ColorStatistics colorStatProfile =
                    colorCluster.getSchema(colorSchemaName).getColorStatistics(colorStat);
     ...
     *
     * <p>Retrieves a single color channel value (e.g., HUE) from the specified statistic type for
     * all state images, useful for visualization and debugging.
     *
     * @param colorSchemaName the color space (BGR or HSV)
    ...
    public List<Integer> getColorValues(
        ...
        return colorStatProfiles.stream()
                .mapToDouble(profile -> profile.getStat(colorValue))
                .boxed()
