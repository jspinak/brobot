...
/**
 * Provides concise logging for find operations with intelligent deduplication
 * and summarization to reduce repetitive log output.
 * 
 * This logger tracks patterns that have already been logged in a search session
 * and provides summarized output for repeated searches, significantly reducing
 * log verbosity while maintaining useful diagnostic information.
 */
@Component
public class ConciseFindLogger {
        ...
        if (isSameSession(sessionId)) {
            ...
            if (totalSearchAttempts > 1) {
                ...
                String summary = String.format(
                    ...
                    status, sessionPatterns.size(), totalSearchAttempts, duration, totalMatches
                );
     ...
     * First occurrence shows full details, subsequent show minimal info.
     */
    public void logPatternSearch(Pattern pattern, Scene scene, double similarity) {
        ...
        if (info == null) {
            ...
            msg.append(String.format("[SEARCH] %s (%dx%d) sim=%.2f",
                pattern.getName(),
                pattern.w(), pattern.h(),
                similarity));
            ...
            if (pattern.getRegions() != null && !pattern.getRegions().isEmpty()) {
                ...
                if (region.w() < scene.getPattern().w() || region.h() < scene.getPattern().h()) {
                    ...
                    msg.append(String.format(" in [%d,%d %dx%d]", 
                        region.x(), region.y(), region.w(), region.h()));
                }
            }
            
            if (!hasConstrainedRegion) {
                msg.append(String.format(" scene=%dx%d",
                    scene.getPattern().w(), scene.getPattern().h()));
            }
            ...
            if (Math.abs(info.lastSimilarity - similarity) > 0.01) {
                ConsoleReporter.println(String.format(
                    ...
                    info.searchCount, pattern.getName(), info.lastSimilarity, similarity
                ));
                ...
                ConsoleReporter.println(String.format(
                    "[RE-SEARCH #%d] %s",
                    info.searchCount, pattern.getName()
                ));
            }
        }
    }
