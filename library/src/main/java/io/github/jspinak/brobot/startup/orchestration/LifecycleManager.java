...
/**
 * Manages Brobot's initialization lifecycle to ensure proper component startup order. This replaces
 * the need for multiple InitializingBean implementations and guarantees that all components are
 * initialized in the correct sequence.
 *
 * <p>Initialization Order:
 *
 * <ol>
 *   <li>Environment detection and configuration
 *   <li>Image path resolution and SikuliX setup
 *   <li>Resource verification and diagnostics
 *   <li>Framework readiness check
 * </ol>
 */
@Slf4j
@Component
public class LifecycleManager implements SmartLifecycle, Ordered {

    private final AtomicBoolean running = new AtomicBoolean(false);

    private final io.github.jspinak.brobot.config.core.BrobotConfiguration configuration;
    private final ExecutionEnvironment environment;
    private final io.github.jspinak.brobot.config.core.ImagePathManager pathManager;
    private final io.github.jspinak.brobot.config.core.SmartImageLoader imageLoader;
    private final ConfigurationDiagnostics diagnostics;

    @Autowired
    public LifecycleManager(
            io.github.jspinak.brobot.config.core.BrobotConfiguration configuration,
            ExecutionEnvironment environment,
            io.github.jspinak.brobot.config.core.ImagePathManager pathManager,
            io.github.jspinak.brobot.config.core.SmartImageLoader imageLoader,
    ...
    private void logFrameworkReady() {
        ...
        status.append("  - Environment: ")
                .append(configuration.getEnvironment().getProfile())
                .append("\n");
        ...
        status.append("  - Image Paths: ")
                .append(pathManager.getConfiguredPaths().size())
                .append(" configured\n");
        status.append("  - SikuliX: ")
                .append(!environment.shouldSkipSikuliX() ? "Active" : "Skipped")
                .append("\n");
    ...
    public static class FrameworkStatus {
        ...
        public int getImagePathsConfigured() {
            return imagePathsConfigured;
        }
        ...
        public boolean isSikulixActive() {
            return sikulixActive;
        }

        public void setSikulixActive(boolean sikulixActive) {
            this.sikulixActive = sikulixActive;
        }

        @Override
        public String toString() {
            return String.format(
                    "FrameworkStatus[running=%s, profile=%s, mockMode=%s, display=%s, capture=%s,"
                            + " paths=%d, sikuli=%s]",
                    running,
                    profile,
                    mockMode,
