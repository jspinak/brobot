 ...
 * <p>
 * This utility performs efficient clustering of 2D points by dividing space into
 * grid cells and counting point frequencies. It uses overlapping grids to avoid
 * edge effects where clusters might be split across grid boundaries. The algorithm
 * is particularly effective for finding dense regions of points without the
 * computational overhead of traditional clustering algorithms.
 * <p>
 * Algorithm overview:
 * <ol>
 * <li>Divide region into a 3x3 grid</li>
 * <li>Create overlapping grid offset by half cell size</li>
 * <li>Count points in each cell of both grids</li>
 * <li>Sort cells by point frequency</li>
 * <li>Return top N densest regions</li>
 * </ol>
 * <p>
 * Key features:
 * <ul>
 * <li>O(n) time complexity for n points</li>
 * <li>Overlapping grids prevent cluster splitting</li>
 * <li>No need to specify cluster centers</li>
 * <li>Works well for rectangular regions</li>
 * <li>Handles variable cluster sizes</li>
 * </ul>
 * <p>
 * Use cases:
 * <ul>
 * <li>Finding dense UI element groups</li>
 * <li>Identifying click/interaction hotspots</li>
 * <li>Spatial data preprocessing</li>
 * <li>Quick density estimation</li>
 * <li>Region of interest detection</li>
 * </ul>
 * <p>
 * Limitations:
 * <ul>
 * <li>Fixed 3x3 grid may miss fine details</li>
 * <li>Rectangular cells don't match all cluster shapes</li>
 * <li>No iterative refinement in current implementation</li>
 * <li>Equal weight to all points</li>
 * </ul>
 * <p>
 * For more accuracy, the process can be run iteratively, with each iteration
 * focusing on one of the high-density cells from the previous iteration.
 *
 * @see Grid
...
public class GridBasedClusterer {
     ...
     * <p>
     * Creates two overlapping 3x3 grids over the region and counts points in each
     * cell. The overlapping approach ensures clusters near cell boundaries are
     * properly detected. Returns up to maxClusters regions ordered by point density.
     * <p>
     * Grid configuration:
     * <ul>
     * <li>Primary grid: 3x3 cells covering the entire region</li>
     * <li>Overlapping grid: Offset by half cell size</li>
     * <li>Both grids analyzed independently</li>
     * <li>Results merged and sorted by density</li>
     * </ul>
     * <p>
     * Return value:
     * <ul>
     * <li>Key: Region representing a grid cell</li>
     * <li>Value: Number of points in that cell</li>
     * <li>Ordered by descending point count</li>
     * <li>Limited to maxClusters entries</li>
     * </ul>
     *
     ...
     * <p>
     * This method combines results from overlapping grids to find the
     * overall densest regions. It prevents the same cluster from being
     * split across grid boundaries by considering both grid systems.
     * <p>
     * Processing steps:
     * <ol>
     * <li>Sort both frequency maps by density (descending)</li>
     * <li>Take top N regions from each grid</li>
     * <li>Merge results, keeping all unique regions</li>
     * <li>Re-sort combined results by density</li>
     * <li>Return top maxClusters regions</li>
     * </ol>
     * <p>
     * Bug note: The current implementation has a bug where it uses index i
     * instead of accessing map entries properly, potentially causing incorrect
     * grid number retrieval.
     *
     ...
     * <p>
     * Determines which grid cell contains the given location and increments
     * its frequency count. Uses the merge operation for thread-safe counting
     * if this method is called concurrently.
     * <p>
     * Grid numbering is row-major:
     * <pre>
     * 0 1 2
     * 3 4 5
     * 6 7 8
     * </pre>
     * <p>
     * Side effects: Modifies the frequency map by incrementing counts.
     *
