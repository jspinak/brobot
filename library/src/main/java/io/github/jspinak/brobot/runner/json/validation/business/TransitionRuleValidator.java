 ...
 *
 * <p>This validator ensures that state transitions form a coherent, efficient, and maintainable
 * state machine. It goes beyond basic reference validation to analyze the transition graph
 * structure, identifying potential issues that could lead to deadlocks, unreachable states, or
 * inefficient automation flows.
 *
 * <h2>Validation Categories:</h2>
 *
 * <ul>
 *   <li><b>Cycle Detection</b> - Identifies potentially problematic loops in state transitions
 *   <li><b>Reachability Analysis</b> - Ensures all states can be reached from the start
 *   <li><b>Efficiency Checks</b> - Detects redundant or inefficient transition patterns
 *   <li><b>Concurrency Analysis</b> - Identifies potential race conditions in transitions
 * </ul>
 *
 * <h2>Why Transition Rules Matter:</h2>
 *
 * <p>State transitions define the flow of automation. Poorly designed transitions can lead to:
 *
 * <ul>
 *   <li>Automation getting stuck in infinite loops
 *   <li>States that can never be reached (dead code)
 *   <li>Race conditions when multiple transitions compete
 *   <li>Inefficient paths that slow down automation
 * </ul>
 *
 * <h2>Graph Theory Application:</h2>
 *
 * <p>This validator treats transitions as a directed graph and applies graph algorithms to detect
 * structural issues. Understanding the graph structure helps identify systemic problems early.
 *
 * <h2>Usage Example:</h2>
 *
 * <pre>{@code
...
public class TransitionRuleValidator {
     ...
     *
     * <p>This method performs comprehensive analysis of the state transition graph, applying
     * multiple validation strategies to ensure the state machine is well-formed and efficient. Each
     * validation check is independent, allowing all issues to be discovered in a single pass.
     *
     * <h3>Validation Sequence:</h3>
     *
     * <ol>
     *   <li><b>Cycle Analysis</b> - Detects loops that might cause infinite execution
     *   <li><b>Reachability Check</b> - Ensures no states are orphaned or unreachable
     *   <li><b>Efficiency Analysis</b> - Identifies redundant or suboptimal transitions
     *   <li><b>Concurrency Check</b> - Warns about potential race conditions
     * </ol>
     *
     * @param projectModel Parsed project model containing states and transitions. Expected to be a
     *     Map with "states" and "stateTransitions" arrays
     * @return ValidationResult containing discovered issues:
     *     <ul>
     *       <li>CRITICAL - Invalid model structure preventing analysis
     *       <li>ERROR - Serious issues like invalid state references
     *       <li>WARNING - Quality issues like unreachable states or cycles
     *     </ul>
     */
                ...
                if (value != null && !(value instanceof Number)) {
                    result.addError(
                            new ValidationError(
                                    ...
                                    "Error validating transition: sourceStateId is not a valid"
                                            + " number type",
                                    ValidationSeverity.ERROR));
                }
            }
            ...
            if (transition.containsKey("fromState")) {
                ...
                if (value != null && !(value instanceof Number)) {
                    result.addError(
                            new ValidationError(
                                    "Malformed transition",
                                    "Error validating transition: fromState is not a valid number"
                                            + " type",
                                    ValidationSeverity.ERROR));
                }
            }
            ...
            if (transition.containsKey("statesToEnter")) {
                ...
                if (value != null && !(value instanceof List)) {
                    result.addError(
                            new ValidationError(
                                    "Malformed transition",
                                    "Error validating transition: statesToEnter is not a valid"
                                            + " list",
                                    ValidationSeverity.ERROR));
                }
            }
     ...
     *
     * <p>Not all cycles are problematic - many state machines have legitimate loops for retry logic
     * or recurring processes. This method identifies cycles that are likely to cause issues based
     * on their structure and the types of states involved.
     *
     * <h3>Problematic Cycle Patterns:</h3>
     *
     * <ul>
     *   <li><b>Tight Loops</b> - 2-state cycles that might execute too rapidly
     *   <li><b>No-Exit Cycles</b> - Cycles with no conditional exits
     *   <li><b>Resource-Intensive Cycles</b> - Loops containing heavy operations
     * </ul>
     *
     * <h3>Algorithm:</h3>
     *
     * <p>Uses depth-first search (DFS) with a recursion stack to detect cycles, then analyzes each
     * cycle's characteristics to determine if it's problematic.
     *
                ...
                if (isPotentiallyProblematicCycle(cycle, project)) {
                    result.addError(
                            new ValidationError(
                                    ...
                                    "Found a transition cycle that may cause issues: "
                                            + formatCycle(cycle, project),
     ...
     *
     * <p>This method ensures that all defined states can actually be reached during automation
     * execution. Unreachable states represent dead code that wastes resources and indicates
     * potential design flaws in the state machine.
     *
     * <h3>Reachability Analysis:</h3>
     *
     * <ul>
     *   <li>Assumes the first state is the entry point (or finds designated start state)
     *   <li>Performs graph traversal to find all reachable states
     *   <li>Reports any states that cannot be reached from the start
     * </ul>
     *
     * <h3>Common Causes of Unreachable States:</h3>
     *
     * <ul>
     *   <li>States added during development but never connected
     *   <li>Transitions removed without updating target states
     *   <li>Copy-paste errors in configuration
     * </ul>
     *
                ...
                if (!reachableStates.contains(stateId)) {
                    result.addError(
                            new ValidationError(
                                    ...
                                    "State with ID "
                                            + stateId
                                            + " appears to be unreachable from other states",
                                    ValidationSeverity.WARNING));
                }
            }
     ...
     *
     * <p>This method checks for inefficient transition patterns that could be simplified or
     * optimized. Redundant transitions make the state machine harder to understand and maintain
     * while potentially causing unexpected behavior.
     *
     * <h3>Efficiency Checks:</h3>
     *
     * <ul>
     *   <li><b>Duplicate Transitions</b> - Multiple transitions between same states
     *   <li><b>Redundant Paths</b> - Different routes achieving the same result
     *   <li><b>Unnecessary Complexity</b> - Overly complex transition conditions
     * </ul>
     *
     * <h3>Impact of Inefficiency:</h3>
     *
     * <p>Redundant transitions can cause confusion about which transition will execute, make
     * debugging difficult, and indicate poor state machine design that should be refactored.
     *
     * @param project The project model containing transition definitions
     * @param result ValidationResult to add efficiency warnings to
     */
    private void validateTransitionEfficiency(
        ...
        try {
            ...
            for (int i = 0; i < transitions.size(); i++) {
                ...
                if (transition.containsKey("sourceStateId")
                        && transition.containsKey("statesToEnter")
                    ...
                    for (Integer targetId : targetIds) {
                        ...
                        stateToStateTransitions
                                .computeIfAbsent(key, k -> new ArrayList<>())
                                .add(
                                        transition.containsKey("id")
                                                ? (Integer) transition.get("id")
                                                : i);
                    }
                }
            }
     ...
     *
     * <p>This method identifies patterns that might lead to race conditions or unpredictable
     * behavior when multiple transitions could execute simultaneously. While Brobot typically
     * executes transitions sequentially, certain patterns can still cause logical race conditions.
     *
     * <h3>Concurrency Concerns:</h3>
     *
     * <ul>
     *   <li><b>Multiple Activators</b> - States triggered by many different transitions
     *   <li><b>Competing Transitions</b> - Multiple transitions with similar triggers
     *   <li><b>State Conflicts</b> - Transitions that might interfere with each other
     * </ul>
     *
     * <h3>Why This Matters:</h3>
     *
     * <p>When a state can be activated by many transitions, it becomes difficult to predict the
     * system's behavior and ensure consistent state. This often indicates a need to refactor the
     * state machine design.
     *
     * @param project The project model containing transition definitions
     * @param result ValidationResult to add concurrency warnings to
     */
    private void validateTransitionConcurrency(
            Map<String, Object> project, ValidationResult result) {
        if (!project.containsKey("stateTransitions")) {
            return;
        }
        ...
        try {
                ...
                if (entry.getValue().size() > 1) {
                    result.addError(
                            new ValidationError(
                                    ...
                                    "State "
                                            + entry.getKey()
                                            + " is targeted by multiple async transitions. "
            ...
            for (Map.Entry<Integer, List<Map<String, Object>>> entry : stateActivators.entrySet()) {
                if (entry.getValue().size()
                        > 3) { // More than 3 transitions to same state might be a concern
                    result.addError(
                            new ValidationError(
                                    "Potential concurrency issue",
                                    "State "
                                            + entry.getKey()
                                            + " is activated by many transitions ("
                                            + entry.getValue().size()
        ...
        for (Map<String, Object> transition : transitions) {
            try {
                ...
                if (sourceId != null && targetIds != null && !targetIds.isEmpty()) {
                    Set<Integer> outgoingStates =
                            graph.computeIfAbsent(sourceId, k -> new HashSet<>());
    ...
    /**
     * Finds ALL cycles in the transition graph, including multiple independent cycles. Uses
    ...
    private void findCyclesInSCC(
            Set<Integer> scc, Map<Integer, Set<Integer>> graph, Set<List<Integer>> cycles) {
    ...
    private List<Integer> findSimpleCycleFromNode(
            Integer start, Set<Integer> scc, Map<Integer, Set<Integer>> graph) {
    ...
    private void findReachableStates(
            Integer startState, Map<Integer, Set<Integer>> graph, Set<Integer> reachableStates) {
    ...
    private boolean isPotentiallyProblematicCycle(
            List<Integer> cycle, Map<String, Object> project) {
                ...
                if (entry.getValue().size() > 1) {
                            ...
                            if (!conditions.add(condition)) {
                                ...
                                result.addError(
                                        new ValidationError(
                                                ...
                                                "Duplicate transitions found for "
                                                        + entry.getKey()
                                                        + " with same condition",
                                                ValidationSeverity.ERROR));
                            } else {
                    ...
                    if (hasDifferentConditions) {
                        ...
                        result.addError(
                                new ValidationError(
                                        "Redundant transitions",
                                        "Multiple redundant transitions between same states: "
                                                + entry.getKey()
                        ...
                        result.addError(
                                new ValidationError(
                                        "Duplicate transition",
                                        "Multiple unconditional transitions found for "
                                                + entry.getKey(),
                                        ValidationSeverity.ERROR));
                    }
                }
        ...
        try {
            ...
            for (Map<String, Object> transition : transitions) {
                ...
                if (sourceId != null && transition.containsKey("probability")) {
                    ...
                    stateProbabilities.put(
                            sourceId, stateProbabilities.getOrDefault(sourceId, 0.0) + prob);
                }
            }
