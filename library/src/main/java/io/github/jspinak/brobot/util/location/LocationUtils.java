 ...
 * supporting both absolute (x,y) and relative (region-based) location representations. These
 * utilities handle conversions, calculations, and transformations while maintaining the dual nature
 * of Location objects in the framework.
 *
 * <p>Operation categories:
 *
 * <ul>
 *   <li><b>Conversions</b>: Transform between Brobot and SikuliX location representations
 *   <li><b>Type Checking</b>: Determine if locations are absolute or relative
 *   <li><b>Calculations</b>: Geometric operations like opposites, additions, angles
 *   <li><b>Transformations</b>: Convert locations to matches, state objects, collections
 *   <li><b>Utilities</b>: Validation, comparison, string representation
 * </ul>
 *
 * <p>Location type handling:
 *
 * <ul>
 *   <li><b>Absolute (XY)</b>: Defined by x,y coordinates directly
 *   <li><b>Relative (Region)</b>: Defined by position within a region
 *   <li>Methods adapt behavior based on location type
 *   <li>Supports conversion between types when needed
 * </ul>
 *
 * <p>Key operations:
 *
 * <ul>
 *   <li>{@code getSikuliLocation()}: Convert to SikuliX format with offset support
 *   <li>{@code getOpposite()}: Find opposite point within region
 *   <li>{@code setFromCenter()}: Position based on angle and distance
 *   <li>{@code adjustToRegion()}: Ensure location stays within bounds
 *   <li>{@code add()}: Combine locations with type awareness
 * </ul>
 *
 * <p>Integration features:
 *
 * <ul>
 *   <li>Creates StateLocation objects for state-based operations
 *   <li>Generates ObjectCollections for action execution
 *   <li>Converts to Match objects for result compatibility
 *   <li>Supports null state operations for stateless contexts
 * </ul>
 *
 * <p>Geometric calculations:
 *
 * <ul>
 *   <li>Polar coordinate support (angle/distance from center)
 *   <li>Opposite point calculations (reflection)
 *   <li>Vector addition with type-aware semantics
 *   <li>Boundary enforcement for relative locations
 * </ul>
 *
 * <p>In the model-based approach, LocationUtils enables sophisticated spatial reasoning about point
 * locations while abstracting the complexity of dual representation modes. This flexibility allows
 * the framework to work efficiently with both fixed screen coordinates and adaptive region-based
 * positions.
 *
 * @since 1.0
 * @see Location
...
public class LocationUtils {
    ...
    public static org.sikuli.script.Location getSikuliLocationFromXY(
    ...
    public static org.sikuli.script.Location getSikuliLocationFromRegion(
            Region region, Position position, int offsetX, int offsetY) {
    ...
    public static org.sikuli.script.Location getSikuliLocation(Location location) {
        if (isDefinedByXY(location)) {
            ...
            return new org.sikuli.script.Location(
    ...
    public static boolean isDefined(Location location) {
        return !isDefinedByXY(location)
                || location.getCalculatedX() >= 0
                || location.getCalculatedY() >= 0;
    }
    ...
    public static StateLocation asStateLocationInNullState(Location location) {
        return new StateLocation.Builder().setOwnerStateName("null").setLocation(location).build();
    }
    ...
    public static ObjectCollection asObjectCollection(Location location) {
        StateLocation stateLocation =
                new StateLocation.Builder()
                        ...
                        .build();
        return new ObjectCollection.Builder().withLocations(stateLocation).build();
    }
        ...
        if (isDefinedByXY(location)) {
            location.setX(
                    location.getCalculatedX()
                            + (int)
                                    (locationToAdd.getPosition().getPercentW()
                                            * (double) location.getRegion().w()));
            location.setY(
                    location.getCalculatedY()
                            + (int)
                                    (locationToAdd.getPosition().getPercentH()
