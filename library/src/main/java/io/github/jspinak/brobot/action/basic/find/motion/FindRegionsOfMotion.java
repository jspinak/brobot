 ...
 * collection of scenes to find areas where any pixel movement occurs, creating a comprehensive map
 * of all motion regions.
 *
 * <p>Unlike {@link FindMotion} which tracks specific moving objects, this class identifies any
 * region that experiences movement across the entire observation period. This makes it useful for:
 *
 * <ul>
 *   <li>Detecting areas of activity in user interfaces
 *   <li>Identifying animated or dynamic screen regions
 *   <li>Monitoring for any changes in specific areas
 *   <li>Creating heat maps of screen activity
 * </ul>
 *
 * <p>The number of scenes and pause between captures are controlled by:
 *
 * <ul>
 *   <li>TimesToRepeatIndividualAction: number of scenes to capture
 *   <li>PauseBetweenIndividualActions: delay between captures (seconds)
 * </ul>
 *
...
public class FindRegionsOfMotion {
    ...
    /**
     * Finds all regions containing motion across multiple scene captures. Captures scenes at
     * specified intervals and identifies any areas where pixels change between captures.
     *
     * <p>The process involves:
     *
     * <ul>
     *   <li>Capturing multiple scenes based on action options
     *   <li>Detecting all pixels that change across any scenes
     *   <li>Finding contours around dynamic pixel regions
     *   <li>Creating matches for each motion region found
     * </ul>
     *
     * <p>Side effects:
     *
     ...
     *   <li>Populates matches with all motion regions found
     *   <li>Creates motion illustrations for each scene
     *   <li>Sets the dynamic pixel mask in matches
     *   <li>Limits matches based on action options
     * </ul>
     *
    ...
    public void find(ActionResult matches, List<ObjectCollection> objectCollections) {
        ...
        // sceneAnalysisCollection.getSceneAnalyses().size());
        ...
        List<Match> dynamicPixelRegions =
                getDynamicRegions(sceneAnalysisCollection, actionConfig, searchRegions);
    ...
    /**
     * Identifies regions containing dynamic pixels across all scenes. Creates a cumulative mask
     * showing all areas that experienced motion at any point during the observation period.
     *
     * <p>The method:
     *
     * <ul>
     *   <li>Combines all scenes to find any pixels that changed
     *   <li>Creates contours around dynamic pixel regions
     *   <li>Filters contours by area and search region constraints
     *   <li>Saves dynamic pixel visualization to history
     * </ul>
     *
     * <p>Side effects:
     *
     * <ul>
     *   <li>Sets results (dynamic pixel mask) in scene analysis collection
     *   <li>Sets contours in scene analysis collection
     *   <li>Writes dynamic pixel mask to history folder
     * </ul>
     *
     ...
     */
    public List<Match> getDynamicRegions(
            SceneAnalyses sceneAnalysisCollection,
        ...
        ContourExtractor contours =
                new ContourExtractor.Builder()
                        ...
                        .build();
        // System.out.println("FindRegionsOfMotion: # of contours = " +
        // contours.getContours().size());
