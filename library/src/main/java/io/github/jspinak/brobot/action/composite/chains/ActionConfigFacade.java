...
public class ActionConfigFacade {
     ...
     * @param stateImages Variable number of state images to search for. The first found image will
     *     be clicked.
    ...
    public boolean click(double maxWait, StateImage... stateImages) {
        ...
        PatternFindOptions findOptions =
                new PatternFindOptions.Builder().setPauseAfterEnd(0.1).build();

        // Chain find and click
        ActionChainOptions chain =
                new ActionChainOptions.Builder(findOptions).then(clickOptions).build();

        return action.perform(chain, new ObjectCollection.Builder().withImages(stateImages).build())
                .isSuccess();
    }
     ...
     *
     * <p>This method bypasses visual search and clicks directly at the given coordinates, useful
     * for clicking at calculated positions or known screen locations.
     *
    ...
    public boolean doubleClick(double maxWait, StateImage... stateImages) {
        ClickOptions doubleClickOptions = new ClickOptions.Builder().setNumberOfClicks(2).build();
        ...
        ActionChainOptions chain =
                new ActionChainOptions.Builder(findOptions).then(doubleClickOptions).build();
    ...
    public boolean rightClick(double maxWait, StateImage... stateImages) {
        ClickOptions rightClickOptions =
                new ClickOptions.Builder()
                        .setPressOptions(
        ...
        ActionChainOptions chain =
                new ActionChainOptions.Builder(findOptions).then(rightClickOptions).build();
    ...
    public boolean clickBest(double similarity, StateImage... stateImages) {
        ...
        ActionChainOptions chain = new ActionChainOptions.Builder(findBest).then(click).build();
    ...
    public boolean rightClickBest(double similarity, StateImage... stateImages) {
        ...
        ActionChainOptions chain =
                new ActionChainOptions.Builder(findBest).then(rightClick).build();
     ...
     *
     * <p>This method is useful for dismissing dialogs, clearing notifications, or interacting with
     * UI elements that require multiple clicks to disappear.
     *
    ...
    public boolean clickUntilVanished(double timeout, StateImage... stateImages) {
        ...
        ClickOptions clickOptions =
                new ClickOptions.Builder()
                        .setSuccessCriteria(
                                matches -> matches.isEmpty()) // Success when images vanish
                        .setPauseAfterEnd(0.5)
                        .setRepetition(
                                RepetitionOptions.builder()
        ...
        return action.perform(
                        clickOptions,
                        new ObjectCollection.Builder().withImages(stateImages).build())
                .isSuccess();
    }
    ...
    public boolean type(String text) {
        ...
        return action.perform(typeOptions, new ObjectCollection.Builder().withStrings(text).build())
                .isSuccess();
    }
    ...
    public boolean typeWithModifiers(String text, String modifiers) {
        TypeOptions typeOptions = new TypeOptions.Builder().setModifiers(modifiers).build();
    ...
    public boolean waitFor(StateImage... stateImages) {
        ...
        return action.perform(
                        findOptions, new ObjectCollection.Builder().withImages(stateImages).build())
                .isSuccess();
    }
    ...
    public boolean waitToVanish(double maxWait, StateImage... stateImages) {
        VanishOptions vanishOptions = new VanishOptions.Builder().setTimeout(maxWait).build();
    ...
    public boolean clickRegion(StateRegion stateRegion) {
        ...
        return action.perform(
                        click, new ObjectCollection.Builder().withRegions(stateRegion).build())
                .isSuccess();
    }
