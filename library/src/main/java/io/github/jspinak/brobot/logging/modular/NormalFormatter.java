 ...
 * 
 * Produces balanced output with timestamps, key info, and success indicators.
 * Format examples:
 * [12:34:56] ✓ Find Working.ClaudeIcon completed in 234ms (1 match)
 * [12:34:56] ✗ Click Button.Submit failed in 156ms (No matches found)
 */
@Component
public class NormalFormatter implements ActionLogFormatter {
    ...
    public String format(ActionResult actionResult) {
        ...
        if (context.getEndTime() != null) {
            formatted.append("[").append(context.getEndTime().atZone(java.time.ZoneId.systemDefault()).format(TIME_FORMATTER)).append("] ");
        }
        ...
        if (!context.getResultMatches().isEmpty()) {
            ...
            formatted.append(" (").append(matchCount).append(matchCount == 1 ? " match)" : " matches)");
    ...
    private String buildTargetInfo(ActionResult.ActionExecutionContext context) {
        ...
        } else if (context.getPrimaryTargetName() != null && !context.getPrimaryTargetName().isEmpty()) {
