 ...
 * This class divides images into five distinct regions to enable
 * spatially-aware histogram comparisons and color-based matching.
 * 
 * <p>The five regions consist of:
 * <ul>
 * <li>An elliptical region at the center (covering ~75% of image dimensions)</li>
 * <li>Four corner regions (top-left, top-right, bottom-left, bottom-right)</li>
 * </ul></p>
 * 
 * <p>This spatial division allows histogram comparisons to capture not just
 * color distribution but also the spatial arrangement of colors, improving
 * matching accuracy for images with similar colors but different layouts.</p>
 * 
 * <p>For Brobot images containing multiple patterns, each pattern is
 * independently divided into these five regions, with histograms
 * computed and combined across all patterns.</p>
 * 
 * @see HistogramRegion
...
public class HistogramRegions {
     ...
     * Initializes the five regions and prepares masks for histogram calculation.
     * 
     ...
     * Each image is divided into five regions with corresponding masks.
     * 
    ...
    /**
     * Initializes all regions by creating grids, masks, and histogram containers.
     * This method orchestrates the complete setup process for spatial regions.
     */
    private void setAll() {
    ...
    /**
     * Initializes empty histogram containers for all five regions.
     * These will be populated during histogram calculation.
     */
    ...
    /**
     * Creates a 2x2 grid for each image to define the four corner regions.
     * Each grid divides the image into quadrants for spatial analysis.
     */
    ...
    /**
     * Creates elliptical masks for the center region of all images.
     * Each mask defines the central area for histogram calculation.
     */
    ...
    /**
     * Creates an elliptical mask for the center region of an image.
     * The ellipse covers 75% of the image dimensions, centered at the image center.
     * 
    ...
    /**
     * Creates corner masks for all images by subtracting the ellipse
     * from each quadrant, ensuring no overlap between regions.
     */
    ...
    /**
     * Creates masks for the four corner regions of a single image.
     * Each corner mask is created by drawing a rectangle for the quadrant
     * and subtracting the ellipse mask to avoid overlap.
     * 
     ...
     * This method creates:
     * <ul>
     * <li>A combined histogram for each image (sum of its five regions)</li>
     * <li>A total combined histogram (sum across all images)</li>
     * </ul>
     * The combined histograms provide overall color distributions while
     * maintaining the spatial information through the region-based approach.
     */
