 ...
 * intelligent filtering to avoid creating duplicate or unnecessary
 * illustrations.
 ...
 * <ul>
 * <li>Action permissions: Per-action type enablement via
 * {@link FrameworkSettings}</li>
 * <li>Illustration directives: Explicit YES/NO overrides in
 * {@link ActionConfig}</li>
 * <li>Repetition detection: Suppresses duplicate illustrations for same
 * action/objects</li>
 ...
 * <ul>
 * <li>Actions are considered repeated if they have the same type, find method,
 * and object collections</li>
 * <li>Repeated FIND operations are particularly common and filtered by
 * default</li>
 ...
 * </ul>
 * <p>
 * State management: Maintains last action details to enable repetition
 * detection
 * across consecutive operations. This state is instance-level and not
 * thread-safe.
 *
...
public class IllustrationController {
    ...
    /**
     * Determines whether an action should be illustrated based on multiple
     * criteria.
     * <p>
     ...
     *
     * @param actionConfig      configuration including action type and illustration
     *                          directive
     ...
     *
     * @param objectCollections current collections to compare against stored last
     *                          collections
    ...
    private boolean sameCollections(List<ObjectCollection> objectCollections) {
        if (objectCollections.size() != lastCollections.size())
            return false;
        for (int i = 0; i < objectCollections.size(); i++) {
            if (!objectCollections.get(i).equals(lastCollections.get(i)))
                return false;
        }
        return true;
    }
    ...
    /**
     * Conditionally creates illustrations based on permission and repetition
     * checks.
     * <p>
    ...
    private ActionType getActionType(ActionConfig config) {
        ...
        if (config instanceof TypeOptions)
            return ActionType.TYPE;
        if (config instanceof DragOptions)
            return ActionType.DRAG;
        if (config instanceof MouseMoveOptions)
