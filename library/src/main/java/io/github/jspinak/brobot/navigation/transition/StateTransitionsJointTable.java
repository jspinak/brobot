 ...
 *
 * <p>StateTransitionsJointTable serves as a centralized index of all state relationships in the
 * Brobot framework, maintaining both static transitions defined in the state structure and dynamic
 * transitions to hidden states. This joint table enables efficient pathfinding and state
 * relationship queries without traversing individual StateTransitions objects.
 *
 * <p>Table structure:
 *
 * <ul>
 *   <li><b>Incoming Transitions</b>: Map of state ID to set of states that can navigate to it
 *   <li><b>Outgoing Transitions</b>: Map of state ID to set of states it can navigate to
 *   <li><b>Hidden State Transitions</b>: Dynamic map of hidden states to their hiding states
 * </ul>
 *
 * <p>Key features:
 *
 * <ul>
 *   <li>Bidirectional navigation queries (who can reach X, where can Y go)
 *   <li>Dynamic hidden state management based on active states
 *   <li>Support for multi-target transitions
 *   <li>Efficient O(1) lookup for state relationships
 * </ul>
 *
 * <p>Hidden state handling:
 *
 * <ul>
 *   <li>When a state becomes active, its hidden states become accessible via PREVIOUS
 *   <li>These dynamic transitions are added/removed as states activate/deactivate
 *   <li>Enables navigation back to states that were covered by popups or overlays
 *   <li>Tracked separately in incomingTransitionsToPREVIOUS map
 * </ul>
 *
 * <p>Common queries:
 *
 * <ul>
 *   <li>"Which states can navigate to state X?" - getStatesWithTransitionsTo()
 *   <li>"Where can state Y navigate to?" - getStatesWithTransitionsFrom()
 *   <li>"What hidden states are currently accessible?" - check incomingTransitionsToPREVIOUS
 * </ul>
 *
 * <p>Integration points:
 *
 * <ul>
 *   <li>Populated during initialization from StateTransitions objects
 *   <li>Updated dynamically as states activate/deactivate
 *   <li>Used by PathFinder for efficient path discovery
 *   <li>Consulted during state navigation decisions
 * </ul>
 *
 * <p>In the model-based approach, StateTransitionsJointTable provides the indexed view of the state
 * graph that enables efficient navigation and pathfinding. By maintaining this pre-computed index,
 * the framework can quickly answer reachability questions and find navigation paths without
 * expensive graph traversals during automation execution.
 *
 * @since 1.0
...
public class StateTransitionsJointTable {
     ...
     *
     * <p>Resets both static transition maps and dynamic hidden state transitions. Used when
     * rebuilding the state graph or during testing.
     */
     ...
     *
     * <p>When a state becomes active and hides other states, those hidden states become accessible
     * via the PREVIOUS mechanism. This method creates temporary transitions from the active state
     * to its hidden states, enabling back navigation.
     *
     * <p>Side effects:
     *
     * <ul>
     *   <li>Updates incomingTransitionsToPREVIOUS map
     *   <li>Creates entries for new hidden states if needed
     * </ul>
     *
     ...
     *
     * <p>When a state exits, its hidden states are no longer accessible via PREVIOUS from that
     * state. This cleanup maintains accurate navigation possibilities.
     *
     * <p>Note: May leave empty sets in the map which could be cleaned up periodically.
     *
    ...
    public void removeTransitionsToHiddenStates(State exitedState) {
        exitedState
                ...
                .forEach(
                        hiddenState -> {
                            if (incomingTransitionsToPREVIOUS.containsKey(hiddenState))
                                incomingTransitionsToPREVIOUS
                                        .get(hiddenState)
                                        .remove(exitedState.getId());
                        });
    }
     ...
     *
     * <p>Processes each transition to extract all state activation relationships, creating entries
     * for every (source state, target state) pair. This builds the comprehensive navigation graph
     * used for pathfinding.
     *
     * <p>Processing:
     *
     * <ul>
     *   <li>Iterates through all transitions in the StateTransitions
     *   <li>For each activated state, creates bidirectional entries
     *   <li>Handles multi-target transitions correctly
     * </ul>
     *
     ...
     *
     * <p>Creates both incoming and outgoing entries for efficient bidirectional queries. PREVIOUS
     * state is handled specially as it doesn't have incoming transitions in the static table.
     *
     * @param to Target state ID
     * @param from Source state ID
     */
    public void add(Long to, Long from) {
        if (!from.equals(SpecialStateType.PREVIOUS.getId())) addIncomingTransition(to, from);
    ...
    private void addOutgoingTransition(Long childToAdd, Long parent) {
        if (outgoingTransitions.containsKey(parent))
            outgoingTransitions.get(parent).add(childToAdd);
        else {
            Set<Long> stateNames = new HashSet<>();
     ...
     *
     * <p>Queries both static transitions and dynamic hidden state transitions. Includes detailed
     * logging for debugging pathfinding issues.
     *
    ...
    public Set<Long> getStatesWithTransitionsTo(Long... children) {
        ...
        for (Long child : children) {
            ...
            if (incomingTransitions.containsKey(child)) {
                ...
                String parentsStr =
                        incomingTransitions.get(child).stream()
                                .map(id -> formatStateNameAndId(id))
                                .reduce((a, b) -> a + ", " + b)
                                .orElse("");
            ...
            if (incomingTransitionsToPREVIOUS.containsKey(child)) {
                ...
                String prevParentsStr =
                        incomingTransitionsToPREVIOUS.get(child).stream()
                                .map(id -> formatStateNameAndId(id))
                                .reduce((a, b) -> a + ", " + b)
                                .orElse("");
                log.debug(
                        "Found incoming PREVIOUS transitions for {}: [{}]",
                        childStr,
                        prevParentsStr);
            } else {
     ...
     * Finds all states that can navigate to any of the specified target states.
     *
     * <p>Set-based version for efficient bulk queries.
     *
    ...
    public Set<Long> getStatesWithTransitionsFrom(Set<Long> parents) {
        ...
        parents.forEach(
                parent -> {
                    if (outgoingTransitions.get(parent) != null)
                        children.addAll(outgoingTransitions.get(parent));
                });
        return children;
    }
     ...
     *
     * <p>Combines the permanent transitions defined in state structure with temporary transitions
     * to hidden states, providing a complete picture of current navigation possibilities.
     *
    ...
    public Map<Long, Set<Long>> getIncomingTransitionsWithHiddenTransitions() {
        ...
        incomingTransitionsToPREVIOUS.forEach(
                (stateName, stateNames) -> {
                    if (allIncoming.containsKey(stateName))
                        allIncoming.get(stateName).addAll(stateNames);
     ...
     *
     * <p>Useful for debugging and visualizing the current state graph structure including both
    ...
    public String print() {
        ...
        for (Long incomingStateId : incomingTransitions.keySet()) {
            stringBuilder
                    .append("incoming transitions to state ")
                    .append(incomingStateId.toString())
                    .append(": ");
            incomingTransitions
                    .get(incomingStateId)
                    .forEach(fromStateId -> stringBuilder.append(fromStateId).append(" "));
        }
        stringBuilder.append("\n");
        for (Long outgoingStateId : outgoingTransitions.keySet()) {
            stringBuilder
                    .append("outgoing transitions to state ")
                    .append(outgoingStateId.toString())
                    .append(": ");
            outgoingTransitions
                    .get(outgoingStateId)
                    .forEach(fromStateId -> stringBuilder.append(fromStateId).append(" "));
        ...
        for (Long previousStateId : incomingTransitionsToPREVIOUS.keySet()) {
            stringBuilder
                    .append("incoming transitions to PREVIOUS ")
                    .append(previousStateId.toString())
                    .append(": ");
            incomingTransitionsToPREVIOUS
                    .get(previousStateId)
                    .forEach(fromStateId -> stringBuilder.append(fromStateId).append(" "));
