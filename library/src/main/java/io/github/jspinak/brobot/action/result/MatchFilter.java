...
/**
 * Provides filtering operations for match collections. Static utility methods for common filtering
 * patterns.
 *
 * <p>This class extracts filtering operations that were previously embedded in ActionResult.
 *
 * @since 2.0
 */
public class MatchFilter {
                ...
                .filter(
                        m ->
                                m.getStateObjectData() != null
                                        && Objects.equals(
                                                m.getStateObjectData().getStateObjectId(),
                                                objectId))
    ...
    public static List<Match> byMinScore(List<Match> matches, double minScore) {
        ...
        return matches.stream().filter(m -> m.getScore() >= minScore).collect(Collectors.toList());
    }
    ...
    public static List<Match> byMaxScore(List<Match> matches, double maxScore) {
        ...
        return matches.stream().filter(m -> m.getScore() <= maxScore).collect(Collectors.toList());
    }
    ...
    public static List<Match> containing(List<Match> matches, Match target) {
        ...
        return matches.stream().filter(m -> contains(m, target)).collect(Collectors.toList());
    }
    ...
    public static List<Match> withinRegion(List<Match> matches, Region region) {
        ...
        return matches.stream().filter(m -> isWithin(m, region)).collect(Collectors.toList());
    }
    ...
    public static List<Match> overlapping(List<Match> matches, Region region) {
        ...
        return matches.stream().filter(m -> overlaps(m, region)).collect(Collectors.toList());
    }
     ...
     */
    public static List<Match> nearLocation(
            List<Match> matches, Location location, double maxDistance) {
    ...
    public static List<Match> byMinArea(List<Match> matches, int minArea) {
        ...
        return matches.stream().filter(m -> m.w() * m.h() >= minArea).collect(Collectors.toList());
    }
    ...
    public static List<Match> byMaxArea(List<Match> matches, int maxArea) {
        ...
        return matches.stream().filter(m -> m.w() * m.h() <= maxArea).collect(Collectors.toList());
    }
    ...
    public static List<Match> limit(List<Match> matches, int count) {
        ...
        return matches.stream().limit(count).collect(Collectors.toList());
    }

    /**
     * Gets unique matches by state object ID. Returns one match per unique state object.
     *
     * @param matches List of matches
    ...
    /**
     * Removes duplicate matches at the same location. Keeps the best scoring match at each
     * location.
     *
     * @param matches List of matches
    ...
    private static boolean isNearby(Match m1, Match m2, int tolerance) {
        return Math.abs(m1.x() - m2.x()) <= tolerance && Math.abs(m1.y() - m2.y()) <= tolerance;
    }
}
