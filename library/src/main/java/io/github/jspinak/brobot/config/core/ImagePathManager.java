...
/**
 * Centralized image path management system that handles all path resolution strategies. This class
 * provides intelligent path resolution, JAR extraction, and SikuliX integration.
 */
@Slf4j
@Component
public class ImagePathManager {
    ...
    public synchronized void initialize(String basePath) {
        ...
        if (initialized
                && basePath.equals(primaryImagePath != null ? primaryImagePath.toString() : null)) {
    ...
    public Map<String, Object> getDiagnostics() {
        ...
        diagnostics.put(
                "primaryPath", primaryImagePath != null ? primaryImagePath.toString() : null);
        ...
        diagnostics.put(
                "sikulixConfigured", !ExecutionEnvironment.getInstance().shouldSkipSikuliX());
        return diagnostics;
    }

    /**
     * Verify image setup and provide detailed diagnostics. This method checks for expected
     * directories and images, logging helpful information for troubleshooting missing resources.
     *
    ...
    private Optional<Path> createFallbackPath(String basePath) {
        ...
        try {
            ...
            Path fallback =
                    Paths.get(System.getProperty("java.io.tmpdir"), "brobot-images", basePath);
    ...
    private static class CommonLocationResolver implements PathResolver {
        private final List<String> commonLocations =
                Arrays.asList(
