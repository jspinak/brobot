 ...
 * When an object A is found, this registry helps identify all objects that depend on A
 * for their search region definition.
 */
@Component
@Slf4j
public class SearchRegionDependencyRegistry {
    ...
    /**
     * Registers a dependency where the dependent object's search region 
     * depends on the source object's location.
     * 
    ...
    public void registerDependency(StateObject dependent, SearchRegionOnObject searchRegionConfig) {
        ...
        String sourceKey = buildKey(searchRegionConfig.getTargetStateName(), 
                                   searchRegionConfig.getTargetObjectName());
        
        dependencies.computeIfAbsent(sourceKey, k -> ConcurrentHashMap.newKeySet())
                   .add(new DependentObject(dependent, searchRegionConfig));
        
        log.info("Registered search region dependency: {} depends on {}", 
    ...
    public Set<DependentObject> getDependents(String sourceStateName, String sourceObjectName) {
        ...
        Set<DependentObject> dependents = dependencies.getOrDefault(sourceKey, Collections.emptySet());
