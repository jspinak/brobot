 ...
 * Calculates medians, averages, distributions, and other statistical metrics.
 * 
 * This class extracts statistical operations that were previously
...
public class MatchStatistics {
     ...
     * Creates a region at the average position with average dimensions.
     * 
    ...
    public Optional<Location> getMedianLocation() {
        return getMedianRegion()
                .map(region -> new Location(region, Positions.Name.MIDDLEMIDDLE));
    }
    ...
    private void calculateStatistics() {
        if (matches != null && !matches.isEmpty()) {
            scoreStats = matches.stream()
                    .collect(Collectors.summarizingDouble(Match::getScore));
            
            sizeStats = matches.stream()
                    .collect(Collectors.summarizingDouble(m -> m.w() * m.h()));
        }
    }
