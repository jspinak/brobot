 ...
 * 
 * <p>LoggingContext maintains contextual information that should be automatically
 * included in all log entries. This includes session IDs, current state, active
 * operations, and custom metadata. The context is thread-local, ensuring that
 * concurrent automation runs don't interfere with each other.</p>
 * 
 * <p>Key features:
 * <ul>
 *   <li>Thread-local storage for isolation between concurrent executions</li>
 *   <li>Hierarchical operation tracking (stack-based)</li>
 *   <li>Custom metadata support</li>
 *   <li>Context snapshots for async operations</li>
 *   <li>Automatic cleanup to prevent memory leaks</li>
 * </ul>
 * </p>
 * 
 * <p>The context automatically propagates to all log entries created in the same
 * thread, eliminating the need to manually pass context information through
 * method calls.</p>
 * 
 * @since 2.0
 * @see BrobotLogger
...
public class LoggingContext {
    ...
    static class Context {
         ...
         * Used for capturing snapshots for async operations.
         */
        Context snapshot() {
    ...
    /**
     * Pushes an operation onto the operation stack.
     * Operations are tracked hierarchically for nested operations.
     * 
     ...
     * Useful for capturing context before async operations.
     * 
     ...
     * Should be called at the end of an automation run to prevent memory leaks.
     */
    public void clear() {
    ...
    /**
     * Executes a runnable with a temporary context.
     * The original context is restored after execution.
     * 
    ...
    public String toString() {
        Context ctx = contextHolder.get();
        return String.format("LoggingContext[session=%s, state=%s, action=%s, operations=%s, metadata=%s]",
                ctx.sessionId,
