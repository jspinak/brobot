 ...
 *
 * <p>MessageRouter is the central dispatching mechanism in the unified logging system. It receives
 * LogEvent instances and routes them to the appropriate existing logging systems (SLF4J,
 * ActionLogger, ConsoleReporter) based on the event type and current configuration.
 *
 * <p>This component acts as an adapter layer during the migration period, allowing the new unified
 * API to work with existing logging infrastructure. Over time, the routing logic can be enhanced to
 * support new handlers and sinks.
 *
 * <p>Routing rules:
 *
 * <ul>
 *   <li>All events go to SLF4J for traditional logging
 *   <li>ACTION events go to ActionLogger for structured persistence
 *   <li>All events go to ConsoleReporter based on output level
 *   <li>Structured logging can be enabled/disabled dynamically
 * </ul>
 *
 * @since 2.0
 * @see LogEvent
...
public class MessageRouter {
        ...
        switch (event.getType()) {
            case ACTION:
                ...
                if (verbosityConfig.getNormal().isShowMatchCoordinates()
                    ...
                    sb.append(" @(")
                            .append(event.getMetadataValue("matchX"))
                            .append(",")
                            .append(event.getMetadataValue("matchY"))
                            .append(")");
                }
            ...
            case TRANSITION:
                ...
                sb.append("STATE: ")
                        .append(event.getFromState())
                        .append(" -> ")
                        .append(event.getToState())
                        .append(" [")
                        .append(event.isSuccess() ? "SUCCESS" : "FAILED")
                        .append("]");
                break;
    ...
    private String formatVerboseMessage(LogEvent event) {
        ...
        switch (event.getType()) {
            case ACTION:
                ...
                if (verbosityConfig.getVerbose().isShowMetadata()
                        && !event.getMetadata().isEmpty()) {
                    sb.append(" {");
                    event.getMetadata()
    ...
    private ConsoleReporter.OutputLevel determineConsoleLevel(LogEvent event) {
        ...
        if (event.getType() == LogEvent.Type.ACTION
                || event.getType() == LogEvent.Type.TRANSITION) {
            return ConsoleReporter.OutputLevel.LOW;
        }
