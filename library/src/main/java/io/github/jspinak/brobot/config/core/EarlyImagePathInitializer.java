 ...
 * This component runs before state processing to ensure images can be found.
 */
@Slf4j
@Component
...
public class EarlyImagePathInitializer {
     ...
     * This runs after @Value injection but before most other beans are created.
     */
    @PostConstruct
    public void initializeImagePaths() {
        ...
        try {
            ...
            if (!imageDir.exists()) {
                log.warn("Image directory does not exist: {}. Creating it...", imageDir.getAbsolutePath());
                imageDir.mkdirs();
            }
    ...
    /**
     * Additional initialization on context refresh to ensure paths are set
     * even if the initial attempt failed.
     */
