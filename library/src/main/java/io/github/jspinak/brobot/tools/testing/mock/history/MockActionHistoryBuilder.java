 ...
 * generating realistic test data for mock mode execution.
 *
 * <p>Example usage:
 *
 * <pre>{@code
...
public class MockActionHistoryBuilder {
    ...
    @lombok.Builder.Default private int recordCount = 10;

    @lombok.Builder.Default private double successRate = 1.0;

    @lombok.Builder.Default private Region matchRegion = new Region(0, 0, 100, 100);

    @lombok.Builder.Default private double minSimilarity = 0.90;

    @lombok.Builder.Default private double maxSimilarity = 1.0;

    @lombok.Builder.Default private int minDuration = 50;

    @lombok.Builder.Default private int maxDuration = 150;

    @lombok.Builder.Default
    private PatternFindOptions.Strategy strategy = PatternFindOptions.Strategy.BEST;

    @lombok.Builder.Default private double searchSimilarity = 0.85;

    @Setter private Supplier<Match> customMatchSupplier;

    @Setter private Supplier<ActionRecord> customRecordSupplier;
    ...
    private ActionRecord createRecord(boolean success) {
        ActionRecord.Builder recordBuilder =
        ...
        if (success) {
            Match match = customMatchSupplier != null ? customMatchSupplier.get() : createMatch();
    ...
    private Match createMatch() {
        return new Match.Builder()
                .setRegion(matchRegion.x(), matchRegion.y(), matchRegion.w(), matchRegion.h())
