 ...
 * architecture instead of the deprecated ActionOptions.
 *
 * <p>Predefined configurations:
 *
 * <ul>
 *   <li><b>Click</b>: Various click configurations (single, double, right-click)
 *   <li><b>Find</b>: Pattern finding with configurable similarity and strategy
 *   <li><b>Type</b>: Keyboard input configurations with optional modifier keys
 *   <li><b>Vanish</b>: Wait for elements to disappear
 * </ul>
 *
 * <p>Benefits:
 *
 ...
 *   <li>Easier to extend with new configuration types
 * </ul>
 *
 * <p>Usage examples:
 *
...
public class ActionConfigShortcuts {
    ...
    public ClickOptions rightClick() {
        return new ClickOptions.Builder()
                .setPressOptions(MousePressOptions.builder().setButton(MouseButton.RIGHT).build())
                .build();
    }
     ...
     *
     * <p>Useful for scenarios like triple-click to select all text, or repeated clicks on stubborn
     * UI elements.
     *
    ...
    public ClickOptions multipleClicks(int numberOfClicks) {
        return new ClickOptions.Builder().setNumberOfClicks(numberOfClicks).build();
    }
    ...
    public PatternFindOptions find(double similarity) {
        return new PatternFindOptions.Builder().setSimilarity(similarity).build();
    }
     ...
     *
     * <p>Used for keyboard shortcuts and modified key presses. The modifier string follows standard
     * conventions: "CTRL", "ALT", "SHIFT", "CMD". Multiple modifiers can be combined: "CTRL+SHIFT".
     *
    ...
    public TypeOptions type(String modifiers) {
        return new TypeOptions.Builder().setModifiers(modifiers).build();
    }
     ...
     *
     * <p>Useful for applications that need slower typing to process input correctly.
     *
    ...
    public TypeOptions typeSlowly(double typeDelay) {
        return new TypeOptions.Builder().setTypeDelay(typeDelay).build();
    }

    /**
     * Creates a vanish action configuration.
     *
     * <p>Used to wait for elements to disappear from the screen.
     *
    ...
    public VanishOptions vanish(double timeout) {
        return new VanishOptions.Builder().setTimeout(timeout).build();
    }

    /**
     * Creates a standard ActionConfig based on action type.
     *
     * <p>This method provides a generic way to create appropriate ActionConfig instances based on
     * the ActionType. It's useful when the specific action type is determined at runtime.
     *
    ...
    public ActionConfig standard(ActionType actionType, double maxWait) {
        switch (actionType) {
            case CLICK:
                return click();
            case FIND:
                return new PatternFindOptions.Builder().setSearchDuration(maxWait).build();
            case TYPE:
                return type();
            case VANISH:
