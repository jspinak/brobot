 ...
 * 
 * This aspect tracks execution times, identifies bottlenecks, and provides
 * performance analytics without requiring any changes to existing code.
 * 
 * Features:
 ...
 * 
 * The aspect can be configured to monitor specific packages or methods
 * and provides both real-time alerts and aggregated reports.
 */
@Aspect
@Component
@Slf4j
...
public class PerformanceMonitoringAspect {
     ...
     */
    private void recordPerformanceData(String method, long executionTime, boolean success, long memoryDelta) {
        // Update statistics
        performanceStats.compute(method, (k, stats) -> {
            if (stats == null) {
        ...
        recentExecutions.compute(method, (k, list) -> {
            if (list == null) {
    ...
    public void generatePerformanceReport() {
        ...
        List<MethodPerformanceStats> slowestMethods = performanceStats.values().stream()
            .sorted((a, b) -> Long.compare(b.getAverageTime(), a.getAverageTime()))
            .limit(10)
            .collect(Collectors.toList());
        
        // Log detailed stats for slowest methods
        slowestMethods.forEach(stats -> {
            brobotLogger.log()
                .type(LogEvent.Type.PERFORMANCE)
                ...
                .metadata("successRate", String.format("%.2f%%", stats.getSuccessRate()))
    ...
    private void detectPerformanceTrends() {
        recentExecutions.forEach((method, executions) -> {
            if (executions.size() < 20) {
                return; // Not enough data for trend analysis
            }
            
            // Calculate moving averages
            List<Long> recent = executions.subList(Math.max(0, executions.size() - 10), executions.size());
            List<Long> previous = executions.subList(Math.max(0, executions.size() - 20), executions.size() - 10);
            
            double recentAvg = recent.stream().mapToLong(Long::longValue).average().orElse(0);
            double previousAvg = previous.stream().mapToLong(Long::longValue).average().orElse(0);
            ...
            if (recentAvg > previousAvg * 1.2) {
                brobotLogger.log()
                    ...
                    .metadata("degradation", String.format("%.1f%%", (recentAvg - previousAvg) / previousAvg * 100))
    ...
    public static class MethodPerformanceStats {
        ...
        public synchronized void recordExecution(long executionTime, boolean success, long memoryDelta) {
        ...
        public long getSuccessfulCalls() { return successfulCalls.sum(); }
