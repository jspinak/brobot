 ...
 *
 * <p>This service acts as the single point of entry for action logging, routing ActionResult
 * objects to the appropriate formatter based on configured verbosity level and outputting the
 * formatted messages.
 *
 * <p>Design principles: - Single responsibility: Only handles logging coordination - Configurable:
 * Verbosity level driven by configuration - Extensible: Easy to add new formatters or output
 * destinations - Testable: Clear separation of concerns
 */
@Service
@Slf4j
public class ActionLoggingService {
    ...
    public ActionLoggingService(List<ActionLogFormatter> formatterList) {
        ...
        log.debug(
                "ActionLoggingService initialized with {} formatters: {}",
                formatters.size(),
                formatters.keySet().stream().map(Enum::name).collect(Collectors.joining(", ")));
    }
    ...
    public void logAction(ActionResult actionResult) {
        ...
        try {
            if (!formatter.shouldLog(actionResult)) {
                log.debug(
                        "Formatter {} decided not to log this action",
            ...
            log.error(
                    "Error formatting action log with {}: {}",
                    formatter.getClass().getSimpleName(),
                    e.getMessage(),
                    e);
        }
    ...
    public void logAction(
            ActionResult actionResult, ActionLogFormatter.VerbosityLevel verbosityLevel) {
        if (!isLoggingEnabled() || actionResult == null) {
            return;
        }
