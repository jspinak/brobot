 ...
 * <p>
 * This class provides functionality to compare StateImage objects and their underlying
 * Pattern objects to determine how similar they are. It implements a hierarchical
 * comparison approach that can handle comparisons between single images, multiple
 * patterns within StateImages, and lists of StateImages.
 * <p>
 * The comparison strategy involves finding the smaller image within the larger one,
 * treating the larger image as a scene and the smaller as a search target. This
 * approach is particularly useful for GUI automation where UI elements need to be
 * located within screenshots.
 *
 * @see StateImage
 * @see Pattern
 * @see Match
...
public class ImageComparer {
     ...
     * <p>
     * This method iterates through all StateImage objects in the provided list and
     * compares each one with the target image. It returns the comparison result
     ...
     * @return The Match object with the highest similarity score, or a NoMatch if no
     ...
     * <p>
     * This method performs a comprehensive comparison by testing all Pattern objects
     * from img1 against all Pattern objects from img2. The larger image automatically
     * becomes the scene, while the smaller becomes the search target. The returned
     * Match contains details about img2 and its similarity score relative to img1.
     * <p>
     * This approach handles StateImages with multiple Pattern variations (e.g., different
     * scales or visual states of the same UI element).
     *
     ...
     * @return A Match object containing img2's details and the best similarity score
     ...
     * <p>
     * This is the core comparison method that implements the actual matching logic.
     * It automatically determines which Pattern is larger and treats it as the scene,
     * while the smaller Pattern becomes the search target. The method uses the
     * configured matcher (mock or live) to find all occurrences of the smaller
     * pattern within the larger one.
     * <p>
     * The comparison is currently optimized for simplicity, returning the best
     * scoring match when multiple instances are found. Future enhancements may
     * include ActionOptions to customize comparison strategies (e.g., weighting
     * multiple matches vs. single high-quality matches).
     * <p>
     ...
     * @return A Match object where the smaller Pattern is the searchImage and the
     *         larger Pattern is the Scene. Returns NoMatch if either pattern is
     *         null, has null BufferedImage, or if size comparison fails.
     */
    public Match compare(Pattern p1, Pattern p2) {
        if (p1 == null || p2 == null || p1.getBImage() == null || p2.getBImage() == null) return new EmptyMatch();
