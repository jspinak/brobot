 ...
 * Provides comprehensive reports and suggestions for common problems.
 */
@Slf4j
@Component
public class ConfigurationDiagnostics {
    ...
    private Map<String, Object> checkCommonIssues() {
        ...
        if (environment.isMockMode() && !configuration.getCore().isMockMode()) {
            detectedIssues.add("Environment in mock mode but configuration expects real operations");
        }
    ...
    private PathStatus checkPath(String pathStr) {
        ...
        try {
            ...
            if (status.exists && status.isDirectory) {
                long imageCount = Files.walk(path, 1)
    ...
    private static class PathStatus {
        ...
        public String toString() {
            ...
            return String.format("exists=%s, directory=%s, readable=%s, images=%d", 
