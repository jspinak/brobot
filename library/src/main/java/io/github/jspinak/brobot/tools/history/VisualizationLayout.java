 ...
 *
 * <p>This utility class handles the complex task of organizing multiple OpenCV Mat images into
 * structured column layouts. It supports both single-column and multi-column arrangements with
 * configurable spacing, making it essential for creating organized visual reports and sidebars in
 * the Brobot history visualization system.
 *
 * <p>Key features:
 *
 * <ul>
 *   <li>Multi-column layout generation from lists of images
 *   <li>Configurable spacing between entries
 *   <li>Automatic distribution of images across columns
 *   <li>Support for both vertical and horizontal merging
 *   <li>Flexible column extraction for partial layouts
 * </ul>
 *
 * <p>Layout structure:
 *
 ...
 *
 * <ul>
 *   <li>Creating sidebar visualizations for match results
 *   <li>Organizing classification results in grid layouts
 *   <li>Building comparison views of multiple images
 *   <li>Generating thumbnail galleries for reports
 * </ul>
 *
 * @see MatBuilder
 * @see AnalysisSidebar
...
public class VisualizationLayout {
     ...
     *
     * <p>This method organizes images into a grid layout with the specified number of columns and
     * rows. Images are distributed column-by-column (top to bottom, then left to right). Each
     * column is first created separately, then all columns are merged horizontally.
     *
     * <p>Distribution example with 9 images, 3 columns, 3 per column:
     *
     * <ul>
     *   <li>Column 1: images 0, 1, 2
     *   <li>Column 2: images 3, 4, 5
     *   <li>Column 3: images 6, 7, 8
     * </ul>
     *
     ...
     */
    public Mat getColumnMat(
            List<Mat> imagesMats, int matchesPerColumn, int columns, int spacesBetweenEntries) {
     ...
     *
     * <p>This method takes pre-built column matrices and combines them side-by-side with the
     * specified spacing. It's used both for final layout assembly and for merging individual
     * columns.
     *
     ...
     *
     * <p>This method calculates which images belong to the specified column based on the column
     * index and matches per column. It handles cases where the last column may have fewer entries
     * than the maximum.
     *
     * <p>Index calculation: startIndex = column * matchesPerColumn
     *
     ...
     *
     * <p>This convenience method combines entry extraction and column building. It first extracts
     * the appropriate entries for the specified column, then builds them into a vertical column
     * layout.
     *
     ...
     *
     * <p>This method arranges the provided images vertically with a fixed 4-pixel spacing between
     * them. It's the core building block for creating column layouts, used by other methods to
     * build individual columns before merging.
     *
