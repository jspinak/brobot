...
/**
 * BasicActions, which run for 1 iteration, require 1 or no Find operations. They can be used as
 * building blocks for CompositeActions.
 */
@Component
public class BasicActionRegistry {

    /**
     * Registry mapping action types to their implementations. Provides O(1) lookup for action
     ...
     *
     * <p>This constructor uses dependency injection to wire all basic action types into a
     * centralized registry. Each action implementation handles a specific GUI interaction primitive
     * that can be executed independently or composed into more complex operations.
     *
     * <p>The registered actions include:
     *
     * <ul>
     *   <li><strong>Input actions:</strong> Click, MouseDown, MouseUp, TypeText, KeyDown, KeyUp
     *   <li><strong>Movement actions:</strong> MoveMouse, ScrollMouseWheel
     *   <li><strong>Search actions:</strong> Find, WaitVanish
     *   <li><strong>Analysis actions:</strong> DefineRegion, Classify
     *   <li><strong>Visual actions:</strong> Highlight
     * </ul>
     *
     ...
     *
     * <p>This factory method provides type-safe access to action implementations. Returns an empty
     * Optional if the requested action type is not registered, allowing callers to handle missing
     * actions gracefully.
     *
     * <p>This method supports the framework's ability to dynamically resolve and execute actions
     * based on ActionConfig configuration.
     *
