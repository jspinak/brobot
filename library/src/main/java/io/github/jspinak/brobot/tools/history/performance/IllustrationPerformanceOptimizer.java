 ...
 *
 * <p>This component implements intelligent performance optimization strategies to maintain system
 * responsiveness while providing valuable visual feedback:
 *
 * <ul>
 *   <li>Adaptive sampling based on system load and action frequency
 *   <li>Smart batching to reduce I/O overhead
 *   <li>Quality-based prioritization to focus on meaningful illustrations
 *   <li>Resource monitoring and automatic throttling
 * </ul>
 *
 * <p>The optimizer continuously monitors system performance and automatically adjusts illustration
 * generation to maintain optimal balance between visual feedback and system performance.
 *
...
public class IllustrationPerformanceOptimizer {
    ...
    private final Map<ActionType, ActionFrequencyTracker> frequencyTrackers =
    ...
    private double calculateQualityScore(
            ActionResult result, IllustrationConfig.QualityMetrics qualityMetrics) {
        ...
        if (qualityMetrics.isUseRegionSize() && !result.getMatchList().isEmpty()) {
            int totalArea =
                    result.getMatchList().stream().mapToInt(match -> match.w() * match.h()).sum();
    ...
    private void updateFrequencyTracker(IllustrationContext context) {
        frequencyTrackers
                .computeIfAbsent(context.getCurrentAction(), k -> new ActionFrequencyTracker())
                .recordAction();
    }
    ...
    private static class ActionFrequencyTracker {
        ...
        public void recordAction() {
            ...
            while (!recentTimestamps.isEmpty() && now - recentTimestamps.peek() > WINDOW_SIZE_MS) {
        ...
        public double getActionsPerSecond() {
            ...
            long timeSpan =
                    recentTimestamps.stream().mapToLong(Long::longValue).max().orElse(0L)
                            - recentTimestamps.stream().mapToLong(Long::longValue).min().orElse(0L);
        ...
        public boolean shouldFlush(IllustrationConfig.BatchConfig batchConfig) {
            return contexts.size() >= batchConfig.getMaxBatchSize()
                    || Duration.between(createdAt, LocalDateTime.now())
                                    .compareTo(batchConfig.getFlushInterval())
                            >= 0;
        }
    }
}
