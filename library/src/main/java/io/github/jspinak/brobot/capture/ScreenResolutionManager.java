...
/**
 * Manages screen resolution information based on the active capture provider. This ensures all
 * components use consistent coordinate systems.
 *
 * <p>Key principle: Regions should be defined in the same coordinate space as the images being
...
public class ScreenResolutionManager {
    ...
    public void initialize() {
        ...
        if (!ScreenDimensions.isInitialized()) {
            ...
            if (ScreenDimensions.getWidth() != screenWidth
                    || ScreenDimensions.getHeight() != screenHeight) {
                log.warn(
                        "ScreenDimensions mismatch! Already initialized as {}x{}, but detected"
                                + " {}x{}",
    ...
    private void detectPhysicalResolution() {
        ...
        if (screenSize.width == LOGICAL_WIDTH_125_PERCENT
            ...
            log.debug(
                    "Detected 125% DPI scaling, using physical resolution: {}x{}",
            ...
            try {
                ...
                if (scaleX > 1.0 || scaleY > 1.0) {
                    ...
                    dpiScaleY = scaleY;
                    log.debug(
                            "Detected DPI scale: {}x{}, adjusted to physical: {}x{}",
                            scaleX,
                            scaleY,
                            screenWidth,
                            screenHeight);
    ...
    /**
     * Creates a region in the current capture coordinate space. This ensures regions are always
     * defined correctly for the active capture provider.
     */
    ...
    /**
     * Creates a region for the lower-left quarter of the screen. Commonly used for finding UI
     * elements like prompts.
     */
