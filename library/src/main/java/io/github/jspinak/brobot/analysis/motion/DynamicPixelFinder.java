...
/**
 * Finds dynamic and fixed pixels by comparing pixel values across multiple images. This
 * implementation of {@link FindDynamicPixels} uses direct pixel comparison to identify changes,
 * making it suitable for detecting any pixel-level differences.
 *
 * <p>Key features:
 *
 * <ul>
 *   <li>Compares pixel values directly using OpenCV's compare function
 *   <li>Combines multiple comparisons with bitwise OR for comprehensive detection
 *   <li>Supports time-based capture for real-time motion analysis
 *   <li>Can capture screenshots at specified intervals for temporal analysis
 * </ul>
 *
 * <p>This approach is particularly effective for:
 *
 * <ul>
 *   <li>Detecting UI element changes
 *   <li>Monitoring screen regions for activity
 *   <li>Identifying animated or dynamic content
 * </ul>
 *
...
public class DynamicPixelFinder implements FindDynamicPixels {
    ...
    /**
     * Creates a mask identifying pixels that change across a collection of images. Compares each
     * image to the first image using pixel-wise inequality comparison and combines all differences
     * with bitwise OR.
     *
        ...
        for (int i = 1; i < size; i++) {
            Mat dynamicPixels =
                    matOps3d.cOmpare(
                            firstMat,
                            matVector.get(i),
                            CMP_NE); // find pixels with different values
    ...
    /**
     * Creates a mask identifying pixels that remain unchanged across all images. This is the
     * inverse of the dynamic pixel mask.
     *
    ...
    /**
     * Captures screenshots of a region over time and identifies changing pixels. This method
     * automates the process of collecting temporal data by taking periodic screenshots and
     * analyzing them for motion.
     *
     ...
     */
    public Mat getDynamicPixelMask(
            Region region, double intervalSeconds, double totalSecondsToRun) {
        MatVector matVector =
                imageLoader.getMatsFromScreen(region, intervalSeconds, totalSecondsToRun);
        return getDynamicPixelMask(matVector);
    }

    /**
     * Captures screenshots of a region over time and identifies unchanging pixels. This method is
     * useful for finding static UI elements or background areas that remain constant while other
     * parts of the screen change.
     *
     * @param region the screen {@link Region} to monitor
     * @param intervalSeconds time between consecutive screenshots
     * @param totalSecondsToRun total duration to monitor the region
