 ...
 * This enables legacy code using ConsoleReporter to benefit from the new features like structured
 * logging, context propagation, and visual feedback.
 *
 * <p>The adapter maintains backward compatibility while adding:
 *
 * <ul>
 *   <li>Automatic context inclusion from thread-local storage
 *   <li>Structured metadata extraction from log messages
 *   <li>Integration with console action reporting
 *   <li>Proper log level mapping
 * </ul>
 *
...
public class ConsoleReporterAdapter {
    ...
    private static final Pattern PATH_PATTERN =
            Pattern.compile("^Find path:\\s+(.+?)\\s*->\\s*(.+)$");
    private static final Pattern TRANSITION_PATTERN =
            Pattern.compile("^(.+?)\\s*->\\s*(.+?)\\s*\\[(\\d+)ms\\]");
    ...
    public static boolean routeToUnified(String message, OutputLevel level) {
        ...
        var logBuilder = unifiedLogger.log().type(type).level(logLevel).observation(message.trim());
    ...
    /**
     * Static initialization block to patch ConsoleReporter methods. This is called when the adapter
     * is loaded to redirect all ConsoleReporter calls through the unified logging system.
     */
