 ...
 * allowing the framework to analyze images using different color representations and choose the
 * most appropriate one for specific automation tasks.
 *
 * <p>The class supports two primary color schemas:
 *
 * <ul>
 *   <li><b>BGR</b>: Blue-Green-Red color space, standard for OpenCV operations
 *   <li><b>HSV</b>: Hue-Saturation-Value color space, often better for color-based segmentation
 * </ul>
 *
 * <p>Key functionality includes:
 *
 * <ul>
 *   <li>Storage and retrieval of K-means profiles for different numbers of clusters
 *   <li>Generation of {@link ColorCluster} objects that combine both BGR and HSV representations
 *   <li>Creation of visualization matrices for color profiles
 *   <li>Validation of profile availability across color schemas
 * </ul>
 *
 * @see KmeansProfiles
 * @see KmeansProfile
...
public class KmeansProfilesAllSchemas {
    ...
    private Map<ColorCluster.ColorSchemaName, KmeansProfiles> kmeansProfiles =
            Map.of(
                    BGR,
                    new KmeansProfiles(),
     ...
     */
    public void addKmeansProfile(
            ColorCluster.ColorSchemaName colorSchemaName, KmeansProfile kmeansProfile) {
     ...
     */
    public KmeansProfile getKmeansProfile(
            ColorCluster.ColorSchemaName colorSchemaName, int numberOfCenters) {
     ...
     *
     * <p>Note: The method name contains a typo (missing 's' in 'Kmeans'). Consider renaming to
     * 'addKmeansProfiles' for consistency.
     *
     ...
     */
    public void addKmeanProfiles(
            ColorCluster.ColorSchemaName colorSchemaName, KmeansProfiles kmeansProfiles) {
    ...
    /**
     * Checks if K-means profiles exist for both BGR and HSV color schemas with the specified number
     * of centers.
     *
     * <p>This method is useful for ensuring that complete color analysis is available before
     * attempting to generate combined color clusters.
     *
    ...
    public boolean containsAll(int numberOfCenters) {
        return kmeansProfiles.get(BGR).getProfiles().containsKey(numberOfCenters)
                && kmeansProfiles
                        .get(ColorCluster.ColorSchemaName.HSV)
                        .getProfiles()
                        .containsKey(numberOfCenters);
    }

    /**
     * Creates ColorClusters from the ColorSchemas from the BGR and HSV KmeansProfile objects. Each
     * KmeansProfile has only one ColorSchema and not a full ColorCluster with both BGR and HSV.
     *
    ...
    public List<ColorCluster> getColorProfiles(int means) {
        ...
        List<ColorSchema> bgrSchemas =
                kmeansProfiles.get(BGR).get(means).get().getClusters().stream()
                        .map(KmeansCluster::getColorSchema)
                        .toList();
        List<ColorSchema> hsvSchemas =
                kmeansProfiles
                        .get(ColorCluster.ColorSchemaName.HSV)
                        .get(means)
                        .get()
     ...
     *
     * <p>This method creates OpenCV Mat objects that visually represent each color cluster's mean
     * color in BGR format. These matrices are useful for displaying color profiles in the user
     * interface or for debugging color detection algorithms.
     *
     * @param means The number of cluster centers to visualize
     * @param size The desired size for each color visualization matrix
     * @return A list of Mat objects, each representing a color cluster's mean BGR value, or an
     *     empty list if profiles for the specified means don't exist
     */
    public List<Mat> getColorProfileMats(int means, Size size) {
        ...
        colorClusters.forEach(
                colorProfile -> colorProfileMats.add(colorProfile.getMat(BGR, MEAN, size)));
