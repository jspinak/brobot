...
/**
 * Calculates and manages histograms for image regions in HSV color space. This class provides
 * functionality to compute histograms for different regions of an image (corners and center) and
 * combine them for analysis.
 *
 * <p>The class supports configurable histogram bins for Hue, Saturation, and Value channels,
 * allowing for flexible histogram resolution based on analysis needs.
 *
 * <p>Key features:
 *
 * <ul>
 *   <li>Computes HSV histograms for image regions
 *   <li>Supports multiple image processing with combined histograms
 *   <li>Configurable bin sizes for each HSV channel
 *   <li>Normalizes histograms for consistent comparison
 * </ul>
 *
...
public class HistogramExtractor {
    ...
    /**
     * HSV channel ranges: Hue [0-180], Saturation [0-256], Value [0-256]. These ranges define the
     * boundaries for histogram calculation in HSV color space.
     */
    ...
    /**
     * Sets the number of bins for each HSV channel in the histogram. This method modifies the
     * histogram resolution and affects all subsequent histogram calculations.
     *
    ...
    /**
     * Computes histograms for a single masked region of an image. The mask defines the area of
     * interest for histogram calculation.
     *
    ...
    /**
     * Computes HSV histograms for all regions of a StateImage. This method processes the image in
     * HSV color space and calculates histograms for five regions: four corners and a central
     * ellipse.
     *
    ...
    /**
     * Calculates histograms for all regions across multiple images. This method processes each
     * image in the collection and accumulates histograms for each region. After processing all
     * images, it combines the individual histograms.
     *
     ...
     */
    public void getHistogramRegions(
            HistogramRegions histRegs,
            IntPointer channelsUsed,
            IntPointer bins,
            PointerPointer<FloatPointer> ranges) {
        for (int i = 0; i < histRegs.getImages().size(); i++) {
            setHistogramRegions(
                    histRegs, i, 1, channelsUsed, histRegs.getImages().get(i), bins, ranges);
        }
     ...
     */
    private void setHistogramRegions(
            HistogramRegions histRegs,
            int index,
            int numberOfImages,
            IntPointer channelsUsed,
        ...
        setHistogramRegion(
                ...
                numberOfImages,
                channelsUsed,
                histRegs.getTopLeft().getMasks().get(index),
                img,
                bins,
                ranges);
        setHistogramRegion(
                histRegs.getTopRight(),
                numberOfImages,
                channelsUsed,
                histRegs.getTopRight().getMasks().get(index),
                img,
                bins,
                ranges);
        setHistogramRegion(
                histRegs.getBottomLeft(),
                numberOfImages,
                channelsUsed,
                histRegs.getBottomLeft().getMasks().get(index),
                img,
                bins,
                ranges);
        setHistogramRegion(
                histRegs.getBottomRight(),
                numberOfImages,
                channelsUsed,
                histRegs.getBottomRight().getMasks().get(index),
                img,
                bins,
                ranges);
        setHistogramRegion(
                ...
                numberOfImages,
                channelsUsed,
                histRegs.getEllipse().getMasks().get(index),
                img,
                bins,
                ranges);
    }
    ...
    /**
     * Calculates and sets the histogram for a specific region. If the region already has a
     * histogram, the new histogram is added to it, allowing for accumulation across multiple
     * images.
     *
     ...
     * @param ranges pointer to value ranges
     */
    private void setHistogramRegion(
            HistogramRegion histReg,
            int numberOfImages,
            IntPointer channelsUsed,
            Mat mask,
            Mat img,
    ...
    /**
     * Calculates a normalized histogram for the specified image region. The histogram is computed
     * using OpenCV's calcHist function and then normalized to ensure consistent comparison between
     * histograms.
     *
    ...
    /**
     * Calculates a histogram from multiple images simultaneously. This method is currently not used
     * since patterns in Brobot images typically have different sizes, making multi-image histogram
