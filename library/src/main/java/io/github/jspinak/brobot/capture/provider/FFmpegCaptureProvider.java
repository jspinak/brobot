 ...
 *
 * <p>Captures screens at physical resolution, bypassing Java's DPI awareness. This ensures
 * consistent captures regardless of Java version or DPI settings.
 *
 * @since 1.1.0
 */
@Component
public class FFmpegCaptureProvider implements CaptureProvider {
    ...
    private void addMacInput(List<String> command, int screenId, Rectangle region) {
        ...
        if (region != null) {
            ...
            command.add(
                    String.format(
                            "crop=%d:%d:%d:%d", region.width, region.height, region.x, region.y));
        }
    }
        ...
        try {
            ...
            if (!completed) {
                ...
                throw new IOException(
                        "FFmpeg capture timed out after " + captureTimeout + " seconds");
            }
    ...
    private void logCapture(String target, int width, int height) {
        System.out.println(
                String.format(
                        "[FFmpeg] Captured %s at %dx%d (physical resolution)",
                        target, width, height));
    }
