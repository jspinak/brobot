 ...
 * 
 * This aspect intercepts methods annotated with @Recoverable and implements
 * sophisticated retry logic including:
 ...
 * - Timeout handling
 * 
 * The aspect helps improve system resilience by automatically handling
 * transient failures without requiring boilerplate retry code.
 */
@Aspect
@Component
@Slf4j
...
public class ErrorRecoveryAspect {
    ...
    public Object handleRecoverable(ProceedingJoinPoint joinPoint, Recoverable recoverable) throws Throwable {
        String methodName = joinPoint.getSignature().toShortString();
        
        // Check circuit breaker
        CircuitBreaker breaker = circuitBreakers.computeIfAbsent(methodName, k -> new CircuitBreaker(methodName));
        if (breaker.isOpen()) {
     ...
     */
    private Object executeWithRetry(ProceedingJoinPoint joinPoint, Recoverable recoverable, CircuitBreaker breaker) throws Throwable {
        String methodName = joinPoint.getSignature().toShortString();
        ...
        while (attempt <= recoverable.maxRetries()) {
            // Check timeout
            if (recoverable.timeout() > 0 && System.currentTimeMillis() - startTime > recoverable.timeout()) {
                throw new RetryTimeoutException("Retry timeout exceeded for " + methodName, lastException);
            }
     ...
     */
    private Object handleRetryExhaustion(ProceedingJoinPoint joinPoint, Recoverable recoverable, Throwable lastException) throws Throwable {
        String methodName = joinPoint.getSignature().toShortString();
        ...
        if (!recoverable.fallbackMethod().isEmpty()) {
            try {
                ...
                throw new RetryExhaustedException("All retries and fallback failed for " + methodName, lastException);
            }
        }
     ...
     */
    private Object invokeFallbackMethod(ProceedingJoinPoint joinPoint, String fallbackMethodName) throws Exception {
