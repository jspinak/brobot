 ...
 *
 * <p>SceneAnalysisCollection aggregates analyses from multiple scenes, enabling operations that
 * span across time or multiple images. This includes motion detection, temporal pattern analysis,
 * and batch processing of related scenes.
 *
 * <p>Key features:
 *
 * <ul>
 *   <li>Ordered collection of scene analyses
 *   <li>Support for time-based analysis (motion detection)
 *   <li>Aggregated results across all scenes
 *   <li>Convenient access to first/last scene data
 * </ul>
 *
 * <p>Common use cases:
 *
 * <ul>
 *   <li>Motion analysis: Compare consecutive scenes
 *   <li>Batch processing: Analyze multiple screenshots
 *   <li>Temporal patterns: Detect changes over time
 *   <li>Multi-state detection: Analyze scene transitions
 * </ul>
 *
 * @see SceneAnalysis
 * @see Scene
...
public class SceneAnalyses {
    ...
    /**
     * The results of a multi-scene analysis are stored here. This can be for actions such as motion
     * analysis, or for aggregating matches across multiple scenes.
     */
    @com.fasterxml.jackson.annotation.JsonIgnore private Mat results;

    @com.fasterxml.jackson.annotation.JsonIgnore private ContourExtractor contours;

    /**
     * Merges another collection into this one, combining scene analyses.
     *
     * <p>Adds all scene analyses from the provided collection to this one. The results and contours
     * from the merged collection replace the existing values in this collection.
     *
     * <p>Side effects: Overwrites results and contours fields
     *
     ...
     *
     * <p>The last scene typically represents the current state and is used for illustration
     * purposes. In motion analysis, this is the most recent frame analyzed.
     *
    ...
    public Optional<Mat> getLastResultsBGR() {
        return getLastSceneAnalysis()
                .map(sceneAnalysis -> sceneAnalysis.getAnalysis(BGR, BGR_FROM_INDICES_2D));
    }
