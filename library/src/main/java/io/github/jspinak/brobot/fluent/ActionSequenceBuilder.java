 ...
 *
 * <p>This builder allows developers to programmatically create automation sequences that are fully
 * compatible with the Brobot DSL. Each method call creates ActionStep objects that are added to an
...
public class ActionSequenceBuilder {
    ...
    /**
     * Adds a find action to the sequence. The found object becomes the target for subsequent
     * actions.
     */
    ...
    public ActionSequenceBuilder thenClick() {
        ...
        ObjectCollection objects =
                new ObjectCollection.Builder().withImages(lastFindTarget).build();
    ...
    public ActionSequenceBuilder thenType(StateString stateString) {
        ...
        ObjectCollection objects = new ObjectCollection.Builder().withStrings(stateString).build();
    ...
    /**
     * Adds a drag action from the last found object to a target. This creates the 6 chained
     * actions: Find→Find→MouseMove→MouseDown→MouseMove→MouseUp
     */
    public ActionSequenceBuilder thenDragTo(StateImage target) {
        ...
        // Step 2: Find target
        PatternFindOptions findTargetOptions = new PatternFindOptions.Builder().build();
        ObjectCollection findTargetObjects =
        ...
        MouseMoveOptions moveToSourceOptions = new MouseMoveOptions.Builder().build();
        ObjectCollection moveToSourceObjects =
                new ObjectCollection.Builder().withImages(lastFindTarget).build();
        steps.add(new ActionStep(moveToSourceOptions, moveToSourceObjects));

        // Step 4: Mouse down
        MouseDownOptions mouseDownOptions = new MouseDownOptions.Builder().build();
        ObjectCollection mouseDownObjects = new ObjectCollection.Builder().build();
        steps.add(new ActionStep(mouseDownOptions, mouseDownObjects));

        // Step 5: Move to target
        MouseMoveOptions moveToTargetOptions = new MouseMoveOptions.Builder().build();
        ObjectCollection moveToTargetObjects =
                new ObjectCollection.Builder().withImages(target).build();
        steps.add(new ActionStep(moveToTargetOptions, moveToTargetObjects));

        // Step 6: Mouse up
        MouseUpOptions mouseUpOptions = new MouseUpOptions.Builder().build();
        ObjectCollection mouseUpObjects = new ObjectCollection.Builder().build();
    ...
    public ActionSequenceBuilder thenHighlight(StateImage target, double seconds) {
        HighlightOptions options =
                new HighlightOptions.Builder().setHighlightSeconds(seconds).build();
    ...
    public ActionSequenceBuilder thenWaitVanish(StateImage target) {
        VanishOptions options = new VanishOptions.Builder().build();
    ...
    public ActionSequenceBuilder thenScroll(ScrollOptions.Direction direction, int times) {
        ...
        ScrollOptions options = new ScrollOptions.Builder().setDirection(direction).build();
