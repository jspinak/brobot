 ...
 *
 * <p>This class implements a specialized histogram matching strategy that divides search regions
 * into overlapping grids of cells. Each cell is approximately the size of the target image, and
 * histogram comparison is performed on each cell to find the best matching locations.
 *
 * <p>The overlapping grid approach ensures that matches are not missed due to arbitrary grid
 * boundaries. This is particularly effective when:
 *
 * <ul>
 *   <li>The target object's exact location is unknown
 *   <li>Multiple instances of similar objects may exist
 *   <li>Precise template matching is too strict due to variations
 * </ul>
 *
...
public class SingleRegionHistogramExtractor {
     ...
     *
     * <p>This method implements a sliding window search strategy:
     *
     * <ol>
     *   <li>Adjusts the search region to fit within scene boundaries
     *   <li>Creates overlapping grids with cells sized to match the target image
     *   <li>Performs histogram comparison on each grid cell
     *   <li>Returns all matches found across the overlapping grids
     * </ol>
     *
     * <p>The overlapping grids ensure that objects positioned at grid boundaries are not missed.
     * Cell dimensions are automatically determined based on the average dimensions of the target
     * StateImage.
     *
     * <p>Side effects: The input region's dimensions may be adjusted to prevent out-of-bounds
     * access to the scene.
     *
    ...
    public List<Match> find(Region region, StateImage stateImage, Mat sceneHSV) {
        ...
        OverlappingGrids overlappingGrids =
                new OverlappingGrids(
                        new Grid.Builder()
                                .setRegion(region)
     ...
     *
     * <p>This convenience method applies the single-region histogram search to multiple regions,
     * aggregating all matches found. This is useful when searching in specific areas of interest
     * rather than the entire scene.
     *
     * <p>Each region is processed independently using {@link #find}, and all resulting matches are
     * combined into a single list.
     *
     * @param regions List of search regions to examine. Each region will be adjusted to scene
     *     bounds.
