 ...
 *
 * <p>The core problem: Even if we capture at physical resolution, applications can have their own
 * scaling that we cannot detect. This class provides strategies to handle this.
 *
 * @since 1.1.0
 */
@Component
    ...
    private BufferedImage captureExternal() {
        ...
        try {
            ...
            if (os.contains("win")) {
                ...
                process =
                        Runtime.getRuntime()
                                .exec(new String[] {"screencapture", "-x", "-T", "0", tmpFile});
            } else {
                // Linux - try gnome-screenshot first, then scrot
                process =
                        Runtime.getRuntime().exec(new String[] {"gnome-screenshot", "-f", tmpFile});
            }
