 ...
 *
 * <p>FFmpeg captures at physical resolution on all platforms, bypassing Java's DPI awareness.
 *
 * <p>Requirements: - FFmpeg must be installed and in PATH - Windows: Download from
 * https://ffmpeg.org/download.html - macOS: brew install ffmpeg - Linux: apt-get install ffmpeg or
 * yum install ffmpeg
 *
 * @since 1.1.0
 */
    ...
    private static String buildFFmpegCommand(String outputFile) {
        if (IS_WINDOWS) {
            ...
            return String.format(
                    "ffmpeg -f gdigrab -framerate 1 -frames:v 1 -i desktop -y \"%s\"", outputFile);
        } else if (IS_MAC) {
            // macOS: Use AVFoundation (captures physical/Retina resolution)
            // List devices: ffmpeg -f avfoundation -list_devices true -i ""
            return String.format(
                    "ffmpeg -f avfoundation -video_size 1920x1080 -framerate 1 -frames:v 1 -i"
                            + " \"1:\" -y \"%s\"",
                    outputFile);
        } else if (IS_LINUX) {
            ...
            return String.format(
                    "ffmpeg -f x11grab -video_size 1920x1080 -framerate 1 -frames:v 1 -i %s -y"
                            + " \"%s\"",
                    display, outputFile);
        } else {
