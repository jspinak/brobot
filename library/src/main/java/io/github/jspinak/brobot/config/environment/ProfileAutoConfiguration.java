...
/**
 * Auto-configuration for Brobot that applies profile-based settings. This configuration ensures
 * proper mock/live mode setup based on active profiles.
 */
@Configuration
...
public class ProfileAutoConfiguration {
    ...
    public static class ProfileValidator {
        ...
        public void validateProfileConfiguration() {
            ...
            if (!isTestProfile && mockEnabled) {
                log.info(
                        "ℹ️ Mock mode enabled in non-test profile - this is valid for development");
            }

            log.info(
                    "Profile validation complete: profiles={}, mock={}",
    ...
    public void logConfigurationSummary() {
        ...
        log.info(
                "║ Active Profiles: {}",
                String.format("%-45s", java.util.Arrays.toString(environment.getActiveProfiles())));
        log.info("║ Mock Mode: {}", String.format("%-52s", FrameworkSettings.mock));
        log.info(
                "║ Image Path: {}",
                String.format(
                        "%-51s", environment.getProperty("brobot.core.image-path", "images")));
        log.info(
                "║ Similarity: {}",
                String.format(
                        "%-51s", environment.getProperty("brobot.action.similarity", "0.85")));
        log.info("╚══════════════════════════════════════════════════════════════╝");
    }
}
