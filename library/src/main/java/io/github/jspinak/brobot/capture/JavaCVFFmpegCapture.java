 ...
 *
 * <p>This implementation uses the FFmpeg libraries bundled with JavaCV
 * (org.bytedeco:ffmpeg-platform) so users don't need to install FFmpeg separately. JavaCV provides
 * native FFmpeg bindings that work across platforms.
 *
 * <p>Benefits: - No external FFmpeg installation required - Cross-platform support through JavaCV -
 * Captures at physical resolution, bypassing DPI scaling - Uses native performance through JNI
 * bindings
 *
 * @since 1.1.0
 */
public class JavaCVFFmpegCapture {
    ...
    static {
        ...
        try {
            ...
            System.err.println(
                    "[JavaCVFFmpeg] Failed to initialize FFmpeg devices: " + e.getMessage());
        }
    }
    ...
    public static BufferedImage capture() throws IOException {
        ...
        try {
            ...
            try {
                if (IS_WINDOWS
                        && grabber.getFormat() != null
                        && grabber.getFormat().equals("gdigrab")) {
    ...
    /**
     * Alternative capture method using FFmpegFrameRecorder for fallback. This creates a screen
     * recording and extracts a frame.
     */
