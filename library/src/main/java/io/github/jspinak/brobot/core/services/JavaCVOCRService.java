...
// import static org.bytedeco.leptonica.global.leptonica.*; // TODO: Fix leptonica dependency
 ...
 * 
 * This implementation bypasses SikuliX's OCR wrapper and uses JavaCV's Tesseract
 * bindings directly. This provides the same OCR functionality but with better
 * control and consistency with the rest of the JavaCV-based components.
 * 
 * Benefits over SikuliX OCR:
 * - Direct Tesseract control
 * - Better language and configuration management
 * - Consistent with JavaCV ecosystem
 * - More OCR options (confidence scores, word boxes, etc.)
 * 
 * @since 2.0.0
 */
@Component
public class JavaCVOCRService {
    ...
    public List<WordMatch> findWords(BufferedImage image) {
        ...
        try {
            ...
            if (ri != null) {
                ...
                // ri.delete(); // ResultIterator doesn't have delete method, handled by garbage collection
            }
    ...
    private PIX convertToPix(BufferedImage image) {
        ...
        try {
            ...
            // PIX pix = leptonica.pixRead(tempFile.getAbsolutePath()); // TODO: Fix leptonica dependency
    ...
    public static class WordMatch {
        ...
        public String toString() {
            return String.format("Word['%s' at %d,%d %dx%d conf=%.1f%%]", 
