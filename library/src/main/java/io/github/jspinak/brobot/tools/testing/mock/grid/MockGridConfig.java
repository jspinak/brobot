 ...
 * Provides default grid dimensions when SikuliX is not available.
 * 
 * The default 3x3 grid matches SikuliX's default raster configuration.
 * Tests can override these values for specific test scenarios.
 */
public class MockGridConfig {
     ...
     * These dimensions are used when calculating grid operations
     * without SikuliX support.
     * 
    ...
    public static void setDefaultGrid(int rows, int cols) {
        if (rows < 1 || cols < 1) {
            throw new IllegalArgumentException(
                String.format("Grid dimensions must be positive: rows=%d, cols=%d", rows, cols));
        }
        defaultRows = rows;
        defaultCols = cols;
    }
