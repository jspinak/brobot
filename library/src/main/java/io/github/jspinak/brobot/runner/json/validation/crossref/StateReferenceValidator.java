 ...
 *
 * <p>This validator ensures that all state-related references are valid and consistent throughout
 * the configuration. States are central to Brobot's automation model, and invalid state references
 * are a common source of runtime failures. This validator catches these issues early in the
 * configuration loading process.
 *
 * <h2>State Reference Types:</h2>
 *
 * <ul>
 *   <li><b>State IDs</b> - Integer identifiers used in transitions and functions
 *   <li><b>StateImages</b> - Visual elements that identify states on screen
 *   <li><b>StateRegions</b> - Defined areas within states for targeted actions
 *   <li><b>StateLocations</b> - Specific coordinates within states
 *   <li><b>CanHide References</b> - States that can obscure other states
 * </ul>
 *
 * <h2>Validation Scope:</h2>
 *
 * <p>This validator performs both internal validation (within the project model) and cross-model
 * validation (state references from DSL functions). It ensures that every state reference points to
 * an actually defined state or state element.
 *
 * <h2>Common Issues Detected:</h2>
 *
 * <ul>
 *   <li>Transitions referencing non-existent states
 *   <li>Functions using invalid state IDs
 *   <li>Actions targeting missing StateImages or Regions
 *   <li>CanHide lists containing invalid state references
 * </ul>
 *
 * <h2>Usage Example:</h2>
 *
...
public class StateReferenceValidator {
     ...
     *
     * <p>This method performs comprehensive validation of state-related references that exist
     * entirely within the project configuration. It ensures that all state elements referenced in
     * various contexts actually exist and are properly defined.
     *
     * <h3>Validation Checks Performed:</h3>
     *
     * <ol>
     *   <li><b>StateImage References</b> - Validates images referenced in transitions
     *   <li><b>Transition State References</b> - Checks sourceStateId, statesToEnter, statesToExit
     *   <li><b>StateRegion References</b> - Validates regions used in action steps
     *   <li><b>StateLocation References</b> - Checks location references in actions
     *   <li><b>CanHide References</b> - Ensures canHide lists contain valid state IDs
     * </ol>
     *
     * <h3>Error Severity:</h3>
     *
     * <ul>
     *   <li>CRITICAL - Null or invalid model structure
     *   <li>ERROR - Invalid references that will cause runtime failures
     * </ul>
     *
     * @param projectModel Parsed project model containing state definitions. Expected to be a Map
     *     with "states" and "stateTransitions" arrays
     ...
     *
     * <p>This method checks that all state IDs referenced in DSL functions correspond to actual
     * states defined in the project model. This cross-model validation is crucial because functions
     * and states are defined separately but must work together at runtime.
     *
     * <h3>State References in Functions:</h3>
     *
     * <ul>
     *   <li><b>openState(stateId)</b> - Opens/activates a specific state
     *   <li><b>isStateActive(stateId)</b> - Checks if a state is currently active
     *   <li>Direct state ID usage in conditions and expressions
     * </ul>
     *
     * <h3>Validation Process:</h3>
     *
     * <ol>
     *   <li>Extract all valid state IDs from the project model
     *   <li>Recursively scan function statements for state references
     *   <li>Validate each reference against the valid state ID set
     *   <li>Report errors with function name and invalid state ID
     * </ol>
     *
     ...
     */
    public ValidationResult validateStateReferencesInFunctions(
            Object projectModel, Object dslModel) {
        ValidationResult result = new ValidationResult();
                        ...
                        if (statement.containsKey("arguments")) {
                            ...
                            for (Map<String, Object> arg : args) {
                                validateExpressionForStateIds(
                                        arg, validStateIds, functionName, result);
                            }
                        }
                    }
                    break;
    ...
    private void validateExpressionForStateIds(
        ...
        switch (expressionType) {
            case "literal":
                String valueType =
                        expression.containsKey("valueType")
                    ...
                    if (!validStateIds.contains(stateId)) {
                        result.addError(
                                new ValidationError(
                                        ...
                                        String.format(
                                                "Function '%s' references non-existent state ID %d",
     ...
     *
     * <p>StateImages are visual patterns used to identify states on screen. This method ensures
     * that all StateImage IDs referenced in transitions and actions correspond to actually defined
     * StateImages within their parent states.
     *
     * <h3>StateImage Reference Contexts:</h3>
     *
     * <ul>
     *   <li><b>Transitions</b> - May specify a stateImageId to click
     *   <li><b>Action Steps</b> - ObjectCollections can reference multiple StateImages
     * </ul>
     *
     * <h3>Validation Logic:</h3>
     *
     * <ol>
     *   <li>Build a map of all StateImage IDs to their names
     *   <li>Check transition.stateImageId references
     *   <li>Check action step ObjectCollection.stateImages arrays
     *   <li>Report errors with transition ID and invalid image ID
     * </ol>
     *
     ...
     */
    private void validateStateImageReferences(
            Map<String, Object> project, ValidationResult result) {
        ...
        for (Map<String, Object> state : states) {
            if (state.containsKey("stateImages")) {
                List<Map<String, Object>> images =
                        (List<Map<String, Object>>) state.get("stateImages");

                for (Map<String, Object> image : images) {
                    if (image.containsKey("id") && image.containsKey("name")) {
                        stateImageNames.put((Integer) image.get("id"), (String) image.get("name"));
                    }
                }
            }
        }
        ...
        for (Map<String, Object> transition : transitions) {
                ...
                if (!stateImageNames.containsKey(imageId)) {
                    result.addError(
                            new ValidationError(
                                    ...
                                    String.format(
                                            "Transition #%s references non-existent stateImage ID:"
                                                    + " %d",
                                            transition.containsKey("id")
            ...
            if (transition.containsKey("actionDefinition")) {
                Map<String, Object> actionDef =
                        (Map<String, Object>) transition.get("actionDefinition");

                if (actionDef.containsKey("steps")) {
                    List<Map<String, Object>> steps =
                            (List<Map<String, Object>>) actionDef.get("steps");

                    for (Map<String, Object> step : steps) {
                        if (step.containsKey("objectCollection")) {
                            Map<String, Object> objColl =
                                    (Map<String, Object>) step.get("objectCollection");
     ...
     *
     * <p>Transitions define how the automation moves between states. This method ensures that all
     * state IDs referenced in transition definitions correspond to actual states defined in the
     * project. Invalid state references in transitions will cause runtime failures.
     *
     * <h3>State References in Transitions:</h3>
     *
     * <ul>
     *   <li><b>sourceStateId</b> - The state that must be active for transition to execute
     *   <li><b>statesToEnter</b> - States to activate when transition completes
     *   <li><b>statesToExit</b> - States to deactivate when transition completes
     * </ul>
     *
     * <h3>Common Issues:</h3>
     *
     * <ul>
     *   <li>Referencing states that were deleted or renamed
     *   <li>Copy-paste errors with state IDs from other projects
     *   <li>Typos in manually edited configuration files
     * </ul>
     *
     ...
     */
    private void validateStateTransitionReferences(
            Map<String, Object> project, ValidationResult result) {
        if (!project.containsKey("states") || !project.containsKey("stateTransitions")) {
            return;
        }
                ...
                if (!stateIds.contains(sourceId)) {
                    result.addError(
                            new ValidationError(
                                    "Invalid state reference",
                                    String.format(
                                            "Transition #%s references non-existent state ID %d in"
                                                    + " sourceStateId",
                    ...
                    if (!stateIds.contains(stateId)) {
                        result.addError(
                                new ValidationError(
                                        "Invalid state reference",
                                        String.format(
                                                "Transition #%s references non-existent state ID %d"
                                                        + " in statesToEnter",
     ...
     * @param result Validation result to update
     */
    private void validateStateRegionReferences(
            Map<String, Object> project, ValidationResult result) {
        if (!project.containsKey("states") || !project.containsKey("stateTransitions")) {
            return;
        }
        ...
        for (Map<String, Object> state : states) {
            ...
            if (state.containsKey("stateRegions")) {
                List<Map<String, Object>> regions =
                        (List<Map<String, Object>>) state.get("stateRegions");
                ...
                for (Map<String, Object> region : regions) {
                    ...
                    if (region.containsKey("id") && region.containsKey("name")) {
                        regionNames.put((Integer) region.get("id"), (String) region.get("name"));
                    }
                }
                        ...
                        if (step.containsKey("objectCollection")) {
                            ...
                            if (objColl.containsKey("stateRegions")) {
                                List<Integer> regionIds =
                                        (List<Integer>) objColl.get("stateRegions");
     ...
     * @param result Validation result to update
     */
    private void validateStateLocationReferences(
            Map<String, Object> project, ValidationResult result) {
        if (!project.containsKey("states") || !project.containsKey("stateTransitions")) {
            return;
        }
        ...
        for (Map<String, Object> state : states) {
            if (state.containsKey("stateLocations")) {
                List<Map<String, Object>> locations =
                        (List<Map<String, Object>>) state.get("stateLocations");

                for (Map<String, Object> location : locations) {
                    if (location.containsKey("id") && location.containsKey("name")) {
                        locationNames.put(
                                (Integer) location.get("id"), (String) location.get("name"));
                    }
                }
            }
        }
                        ...
                        if (step.containsKey("objectCollection")) {
                            ...
                            if (objColl.containsKey("stateLocations")) {
                                List<Integer> locationIds =
                                        (List<Integer>) objColl.get("stateLocations");
     ...
     *
     * <p>The canHide property lists states that can potentially obscure or hide the current state.
     * This is crucial for robust automation that can handle overlapping UI elements like popups,
     * dialogs, or tooltips. This method ensures all states in canHide lists actually exist.
     *
     * <h3>CanHide Usage:</h3>
     *
     * <p>When Brobot searches for a state, it first checks if any states in the canHide list are
     * present. If found, it attempts to close them before searching for the target state. This
     * prevents false negatives caused by UI occlusion.
     *
     * <h3>Common CanHide Scenarios:</h3>
     *
     * <ul>
     *   <li>Modal dialogs that block underlying content
     *   <li>Tooltips that appear over buttons
     *   <li>Loading screens that temporarily hide the UI
     *   <li>Error messages that overlay form fields
     * </ul>
     *
    ...
    private void validateCanHideReferences(Map<String, Object> project, ValidationResult result) {
        ...
        for (Map<String, Object> state : states) {
            String stateName = state.containsKey("name") ? (String) state.get("name") : "unknown";
                    ...
                    if (!stateIds.contains(stateId)) {
                        result.addError(
                                new ValidationError(
                                        ...
                                        String.format(
                                                "State '%s' references non-existent state ID %d in"
                                                        + " canHide",
                                                stateName, stateId),
                                        ValidationSeverity.ERROR));
                    }
                    ...
                    if (stateId.equals(state.get("id"))) {
                        result.addError(
                                new ValidationError(
                                        ...
                                        String.format(
                                                "State '%s' references itself in canHide list",
                                                stateName),
    ...
    private void validateRegionDimensions(
        ...
        if (!hasShortNames && !hasLongNames) {
            result.addError(
                    new ValidationError(
                            ...
                            String.format(
                                    "StateRegion '%s' in state '%s' is missing width or height",
        ...
        try {
            ...
            if (width <= 0 || height <= 0) {
                result.addError(
                        new ValidationError(
                                ...
                                String.format(
                                        "StateRegion '%s' in state '%s' has invalid dimensions:"
                                                + " %dx%d",
            ...
            if (width > 10000 || height > 10000) {
                result.addError(
                        new ValidationError(
                                ...
                                String.format(
                                        "StateRegion '%s' in state '%s' has unreasonably large"
                                                + " dimensions: %dx%d",
                                        regionName, stateName, width, height),
                                ValidationSeverity.WARNING));
            }
            ...
            if (aspectRatio < 0.01 || aspectRatio > 100) {
                result.addError(
                        new ValidationError(
                                ...
                                String.format(
                                        "StateRegion '%s' in state '%s' has extreme aspect ratio:"
                                                + " %.2f",
            ...
            result.addError(
                    new ValidationError(
                            "Invalid region data",
                            String.format(
                                    "StateRegion '%s' in state '%s' has invalid dimension data",
                                    regionName, stateName),
                            ValidationSeverity.ERROR));
        }
    }
    ...
    private void validateMultipleFunctionReferences(
            List<Map<String, Object>> functions,
            Set<Integer> validStateIds,
            ValidationResult result) {
                    ...
                    if (!validStateIds.contains(stateId)) {
                        result.addError(
                                new ValidationError(
                                        ...
                                        String.format(
                                                "Function '%s' references invalid state ID: %d",
                                                functionName, stateId),
                                        ValidationSeverity.ERROR));
                    }
                }
    ...
    private void extractStateReferencesFromStatement(
            Map<String, Object> statement, Set<Integer> references) {
    ...
    private void checkOverlappingRegions(
            List<Map<String, Object>> regions, String stateName, ValidationResult result) {
