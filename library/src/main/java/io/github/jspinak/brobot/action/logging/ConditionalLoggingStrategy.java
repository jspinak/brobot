...
public class ConditionalLoggingStrategy {
    ...
    public static class LoggingCondition {
        ...
        public LoggingCondition(
                Predicate<ActionResult> condition,
    ...
    public void logIfSuccess(ActionResult result, String message) {
        if (result.isSuccess() && logger != null) {
            logger.log().level(LogEvent.Level.INFO).message(message).log();
        }
    }
    ...
    public void logIfFailure(ActionResult result, String message) {
        if (!result.isSuccess() && logger != null) {
            logger.log().level(LogEvent.Level.WARNING).message(message).log();
        }
    }

    /** Add a custom condition for logging */
    public ConditionalLoggingStrategy withCondition(
            Predicate<ActionResult> condition,
            Runnable loggingAction,
            LogLevel logLevel,
            String description) {
    ...
    public ConditionalLoggingStrategy withScoreThresholdCondition(
    ...
    public ConditionalLoggingStrategy withDurationThresholdCondition(
            long milliseconds, String message) {
        return withCondition(
                result ->
    ...
    public void executeConditionalLogging(ActionResult result) {
        conditions.stream().filter(c -> c.test(result)).forEach(LoggingCondition::execute);
    }
