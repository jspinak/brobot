 ...
 *
 * <p>This component manages the iteration process for finding patterns, coordinating searches
 * across multiple scenes and state images. It supports various find strategies (FIRST, EACH, ALL,
 * BEST) and manages the action lifecycle during the search process. The component intelligently
 * routes searches based on pattern variability, using either standard image finding or
 * region-in-pattern (RIP) techniques as appropriate.
 *
 * <p>Key features:
 *
 * <ul>
 *   <li>Iterates through multiple scenes and state images systematically
 *   <li>Manages early termination for FIRST strategy when matches are found
 *   <li>Aggregates matches both globally and per-scene for analysis
 *   <li>Monitors action lifecycle to handle interruptions or failures
 * </ul>
 *
 * @see FindAll
...
public class IterativePatternFinder {
     ...
     *
     * @param actionLifecycleManagement Service for managing action lifecycle and monitoring
     *     execution
     ...
     *
     * <p>This method systematically searches for patterns from each state image within each
     * provided scene. The search behavior adapts based on the find strategy:
     *
     * <ul>
     *   <li>For FIRST strategy: Returns immediately upon finding sufficient matches
     *   <li>For other strategies: Continues searching all scenes and images exhaustively
     * </ul>
     *
     * <p>The method maintains two levels of match aggregation:
     *
     * <ol>
     *   <li>Global matches - All matches across all scenes (stored in the ActionResult)
     *   <li>Per-scene matches - Matches grouped by scene (stored as SceneAnalysis)
     * </ol>
     *
     * <p>Action lifecycle management ensures the search can be interrupted if:
     *
     * <ul>
     *   <li>Sufficient matches are found (for FIRST strategy)
     *   <li>The action is cancelled or times out
     *   <li>An error condition occurs
     * </ul>
     *
     * @param matches The ActionResult to populate with found matches and scene analyses. This
     *     object is modified throughout execution.
     * @param stateImages The list of state images containing patterns to search for. Must not be
     *     null or empty.
        ...
        for (Scene scene : scenes) {
            ...
            for (int i = 0; i < stateImages.size(); i++) { // run for each StateImage
                ...
                if (!newMatches.isEmpty()) {
                    log.info(
                            "[ITERATIVE] Found {} matches for {}, updating dependent search"
                                    + " regions",
                            newMatches.size(),
                            currentImage.getName());
                    ...
                    // This ensures that if ImageA is found, ImageB (which might be later in the
                    // list)
                    // gets its search regions updated before being searched
                    List<StateImage> remainingImages =
                            stateImages.subList(i + 1, stateImages.size());
                    if (!remainingImages.isEmpty()) {
                        log.info(
                                "[ITERATIVE] Updating search regions for {} remaining state images",
                                remainingImages.size());
                        for (StateImage img : remainingImages) {
                            log.info(
                                    "[ITERATIVE]   - Will update: '{}' (has dependency: {})",
                                    img.getName(),
                ...
                if (!actionLifecycleManagement.isOkToContinueAction(matches, stateImages.size()))
                    return;
            }
