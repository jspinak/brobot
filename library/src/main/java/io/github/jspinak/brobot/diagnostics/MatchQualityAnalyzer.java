 ...
 *
 * <ul>
 *   <li>Similarity threshold issues (too high/too low)
 *   <li>Image quality problems
 *   <li>Resolution and DPI mismatches
 *   <li>False positive detection
 *   <li>Pattern ambiguity (multiple similar matches)
 *   <li>Screen capture problems
 * </ul>
 *
 * <p>The analyzer provides detailed diagnostics including:
 *
 * <ul>
 *   <li>Match quality distribution analysis
 *   <li>Clustering detection for multiple matches
 *   <li>Confidence score evaluation
 *   <li>Recommendations for threshold adjustments
 *   <li>Warnings about potential issues
 * </ul>
 *
 * <p>Usage example:
 *
...
public class MatchQualityAnalyzer {
    ...
    public String generateReport(String patternName, ActionResult result) {
        ...
        for (int i = 0; i < Math.min(matches.size(), 10); i++) {
            ...
            report.append(
                    String.format(
                            "  [%d] Score: %.3f, Location: (%d, %d), Size: %dx%d%n",
    ...
    private void analyzeMatchQuality(List<Match> matches) {
        List<Match> goodMatches =
                matches.stream()
                        .filter(m -> m.getScore() >= goodMatchThreshold)
                        .collect(Collectors.toList());

        List<Match> acceptableMatches =
                matches.stream()
                        .filter(
                                m ->
                                        m.getScore() >= acceptableMatchThreshold
                                                && m.getScore() < goodMatchThreshold)
                        .collect(Collectors.toList());

        List<Match> poorMatches =
                matches.stream()
                        .filter(
                                m ->
                                        m.getScore() < acceptableMatchThreshold
                                                && m.getScore() >= suspiciousMatchThreshold)
                        .collect(Collectors.toList());

        List<Match> suspiciousMatches =
                matches.stream()
        ...
        log.info(
                "  - Acceptable (>= {}): {} matches",
                acceptableMatchThreshold,
                acceptableMatches.size());
        ...
        if (!suspiciousMatches.isEmpty()) {
            log.warn(
                    "WARNING: {} suspicious matches detected with very low confidence",
                    suspiciousMatches.size());
        }
    }
    ...
    private void analyzeMatchDistribution(List<Match> matches) {
        if (matches.size() > maxExpectedMatches) {
            log.warn(
                    "WARNING: Found {} matches, expected at most {}. Possible issues:",
                    matches.size(),
                    maxExpectedMatches);
        ...
        for (int i = 0; i < matches.size(); i++) {
            ...
            for (int j = i + 1; j < matches.size(); j++) {
                ...
                if (distance < clusterDistance) {
                    inCluster = true;
                    log.debug(
                            "Matches {} and {} are clustered (distance: {:.1f} pixels)",
                            i,
                            j,
                            distance);
                }
    ...
    private void provideRecommendations(List<Match> matches) {
        ...
        if (avgScore < acceptableMatchThreshold) {
            log.info(
                    "• Consider lowering similarity threshold to {:.2f}",
        ...
        if (matches.stream().anyMatch(m -> m.getScore() < suspiciousMatchThreshold)) {
            log.info(
                    "• Remove suspicious matches by setting minimum threshold to {}",
                    suspiciousMatchThreshold);
        }
    }
    ...
    private String getQualityDistribution(List<Match> matches) {
        ...
        long acceptable =
                matches.stream().filter(m -> m.getScore() >= acceptableMatchThreshold).count();
        long poor = matches.stream().filter(m -> m.getScore() < acceptableMatchThreshold).count();

        return String.format(
                "  Good (>= %.2f): %d%n  Acceptable (>= %.2f): %d%n  Poor (< %.2f): %d%n",
                goodMatchThreshold,
                good,
                acceptableMatchThreshold,
                acceptable - good,
    ...
    private String getClusterAnalysis(List<Match> matches) {
        ...
        if (clustered > 0) {
            analysis.append(
                    String.format(
                            "  Found %d clustered match pairs (within %.1f pixels)%n",
    ...
    private String getRecommendations(List<Match> matches) {
        ...
        if (avgScore < acceptableMatchThreshold) {
            recommendations.append(
                    String.format(
                            "  • Lower similarity threshold to %.2f%n",
                            Math.max(avgScore - 0.1, 0.5)));
        }
