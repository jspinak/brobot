...
public class FrameworkSettings {
     ...
     * Allows GUI elements to stabilize before interaction begins.
     */
    public static double pauseBeforeMouseDown = 0;
    
    /**
     * Delay in seconds after completing a mouse down action.
     * Provides time for the GUI to react to the press event.
     */
    public static double pauseAfterMouseDown = 0;
    
    /**
     * Delay in seconds before initiating a mouse up action.
     * Used in drag operations to ensure stable drag state.
     */
    public static double pauseBeforeMouseUp = 0;
    
    /**
     * Delay in seconds after completing a mouse up action.
     * Allows GUI to process the release event before continuing.
     */
    public static double pauseAfterMouseUp = 0;
    
    /**
     * Horizontal offset in pixels to move the mouse after pressing down.
     * Used for drag initiation or gesture recognition.
     */
    public static int xMoveAfterMouseDown = 0;
    
    /**
     * Vertical offset in pixels to move the mouse after pressing down.
     * Used for drag initiation or gesture recognition.
     */
     ...
     * <p>
     * When true, captures are stored for later analysis or dataset building.
     * Should be initialized to false in Brobot 1.x versions (not using a database).
     * Screenshots are saved to the path specified by {@link #screenshotPath}.
     */
     ...
     * <p>
     * When true, saves annotated screenshots showing action results to the
     * path specified by {@link #historyPath}. Ensure the history folder exists
     * before enabling this setting to avoid runtime errors.
     * 
     * @see #historyPath
     * @see #historyFilename
     */
     ...
     * <p>
     * When true, actions are simulated rather than performed on the actual GUI.
     * Useful for unit testing, development, and scenarios where GUI interaction
     * is not possible or desired.
     */
    public static boolean mock = false;
    
    /**
     * Simulated execution time in seconds for finding the first match.
     * Used in mock mode to approximate real find operation timing.
     */
    public static double mockTimeFindFirst = 0.1;
    
    /**
     * Simulated execution time in seconds for finding all matches.
     * Typically longer than findFirst to reflect scanning entire search area.
     */
    public static double mockTimeFindAll = 0.2;
    
    /**
     * Simulated execution time in seconds for drag operations.
     * Reflects the combined time for press, move, and release actions.
     */
    public static double mockTimeDrag = 0.3;
    
    /**
     * Simulated execution time in seconds for click operations.
     * Represents a simple mouse click action timing.
     */
    public static double mockTimeClick = 0.05;
    
    /**
     * Simulated execution time in seconds for mouse move operations.
     * Used when moving the mouse without clicking.
     */
    public static double mockTimeMove = 0.1;
    
    /**
     * Simulated execution time in seconds for histogram-based find operations.
     * Reflects the additional processing time for color histogram analysis.
     */
    public static double mockTimeFindHistogram = 0.3;
    
    /**
     * Simulated execution time in seconds for color-based find operations.
     * Accounts for pixel-by-pixel color matching algorithms.
     */
    public static double mockTimeFindColor = 0.3;
    
    /**
     * Simulated execution time in seconds for classification operations.
     * Represents machine learning inference timing.
     */
     ...
     * <p>
     * When populated, Find and Highlight operations will search within these images
     * rather than capturing the current screen. Images are loaded from {@link #screenshotPath}.
     * The filenames do not need to start with the value in {@link #screenshotFilename}.
     * <p>
     * This enables deterministic testing by using known screen states.
     * 
     * @see #screenshotPath
     */
     ...
     * <p>
     * This path is relative to the application's working directory. Ensure the
     * directory exists or can be created before capturing screenshots.
     */
     ...
     * <p>
     * Only files starting with this prefix will be processed by the StateStructure
     * builder. This allows separation of builder screenshots from other images
     ...
     * <p>
     * Used when the framework generates code files, such as state definitions
     * or automation scripts. Should follow Java package naming conventions.
     */
     ...
     * <p>
     * These screenshots are annotated with visual indicators showing where
     * actions were performed. Path is relative to the application directory.
     * 
     * @see #saveHistory
     */
    ...
    /**
     * Filename prefix for history screenshots.
     * Files are typically named as: hist_timestamp.png
     */
    public static String historyFilename = "hist";
    
    /**
     * Controls whether Find action results are drawn on history screenshots.
     * When true, highlights regions where patterns were found.
     */
    public static boolean drawFind = true;
    
    /**
     * Controls whether Click action locations are marked on history screenshots.
     * When true, shows click points with visual indicators.
     */
    public static boolean drawClick = true;
    
    /**
     * Controls whether Drag action paths are illustrated on history screenshots.
     * When true, draws lines showing drag start and end points.
     */
    public static boolean drawDrag = true;
    
    /**
     * Controls whether Move action paths are shown on history screenshots.
     * When true, illustrates mouse movement trajectories.
     */
    public static boolean drawMove = true;
    
    /**
     * Controls whether Highlight action regions are shown on history screenshots.
     * When true, preserves the highlight rectangles in the saved image.
     */
    public static boolean drawHighlight = true;
    
    /**
     * Controls whether repeated actions are drawn multiple times.
     * When false, only the first occurrence of repeated actions is illustrated.
     */
    public static boolean drawRepeatedActions = true;
    
    /**
     * Controls whether Classify action results are visualized.
     * When true, shows classification regions and labels.
     */
    public static boolean drawClassify = true;
    
    /**
     * Controls whether Define action regions are illustrated.
     * When true, shows the newly defined regions with boundaries.
     */
     ...
     * <p>
     * Used when analyzing image color distributions. Higher values provide
     * more detailed color profiles but increase processing time.
     */
     ...
     * <p>
     * When processing DynamicImages, k-means profiles for k values from 2 to
     * this maximum are computed and stored. This enables fast color-based
     * matching with different clustering granularities.
     * <p>
     * Higher values increase memory usage but provide more matching options.
     */
     ...
     * <p>
     * When true, color analysis is performed on static images during initialization.
     * This increases startup time but improves runtime performance for color-based matching.
     */
     ...
     * <p>
     * When true, generates k-means profiles for dynamic images that change during execution.
     ...
     * <p>
     * When true, the analysis considers all visible state objects, not just the
     * primary search targets. This provides more comprehensive scene understanding
     ...
     * <p>
     * These settings are used for both capture and replay operations,
     * ensuring consistency between recording and playback.
     */
    
    /**
     * Maximum duration in seconds for screen capture sessions.
     * Acts as a safety limit to prevent indefinite recording.
     */
    public static int secondsToCapture = 1000;
    
    /**
     * Frequency of screen captures per second (frames per second).
     * Higher values provide smoother replay but increase storage requirements.
     */
    public static int captureFrequency = 1;
    
    /**
     * Directory name for storing screen recordings.
     * Created relative to the application's working directory.
     */
     ...
     * <p>
     * When true, captures and saves:
     * <ul>
     * <li>Screenshot before each action</li>
     * <li>Action representation as a feature vector</li>
     * <li>Human-readable text description of the action</li>
     * <li>Screenshot after action completion</li>
     * </ul>
     * <p>
     * This data can be used to train machine learning models for automated
     * GUI interaction or action prediction.
     */
    public static boolean buildDataset = false;
    
    /**
     * Directory path for storing generated training datasets.
     * Dataset files are organized by timestamp and action type.
     */
     ...
     * <p>
     * Used to track multiple test executions and can be incorporated into
     * log files and reports for test run identification.
     */
     ...
     * <p>
     * When true, test results and execution logs can be transmitted to
     * monitoring systems or test management platforms.
     */
     ...
     * <p>
     * Test screenshots provide consistent, reproducible screen states for
     * automated testing. Path is relative to the test resources directory.
     */
