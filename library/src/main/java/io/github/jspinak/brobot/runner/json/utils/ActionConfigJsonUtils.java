 ...
 *       FindOptions, TypeOptions, etc.) through type discrimination
 *   <li><b>Type safety</b>: Ensures correct concrete type is restored during deserialization
 *   <li><b>Deep copying</b>: Enables true deep copies through serialization/deserialization
 *   <li><b>Configuration persistence</b>: Allows configurations to be saved and restored across
 *       sessions
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>Saving action configurations to JSON files for reuse
 *   <li>Creating independent copies of configurations for parallel execution
 *   <li>Transmitting action configurations over network protocols
 *   <li>Converting between different ActionConfig types
 * </ul>
 *
 * @see ActionConfig
...
public class ActionConfigJsonUtils {
    ...
    /**
     * Custom serializer for OpenCV Mat that serializes it as null. This prevents issues with Mat's
     * conflicting setter methods.
     */
    static class MatSerializer extends JsonSerializer<org.bytedeco.opencv.opencv_core.Mat> {
        @Override
        public void serialize(
                org.bytedeco.opencv.opencv_core.Mat value,
                JsonGenerator gen,
    ...
    /**
     * Custom deserializer for OpenCV Mat that always returns null. This prevents issues with Mat's
     * conflicting setter methods.
     */
    static class MatDeserializer extends JsonDeserializer<org.bytedeco.opencv.opencv_core.Mat> {
        @Override
        public org.bytedeco.opencv.opencv_core.Mat deserialize(
    ...
    public ActionConfigJsonUtils(JsonUtils jsonUtils, ConfigurationParser jsonParser) {
        ...
        try {
            ...
            module.addDeserializer(
                    org.bytedeco.opencv.opencv_core.Mat.class, new MatDeserializer());
        } catch (Exception e) {
     ...
     *
     * <p>This method handles polymorphic serialization by including type information in the JSON
     * output. The resulting JSON can be deserialized back to the correct concrete ActionConfig
     * subclass.
     *
     * <p>Usage example:
     *
     ...
     *
     * <p>This method uses the ActionConfigDeserializer to determine the correct concrete type based
     * on the @type field or other type discriminators in the JSON.
     *
     * <p>Supported types include:
     *
     * <ul>
     *   <li>ClickOptions
     *   <li>BaseFindOptions and its subclasses
     *   <li>TypeOptions
     *   <li>DragOptions
     *   <li>MouseMoveOptions, MouseDownOptions, MouseUpOptions
     *   <li>And all other ActionConfig subclasses
     * </ul>
     *
     ...
     *
     * <p>This method implements a serialization-based deep copy strategy that ensures complete
     * independence between the original and copied ActionConfig. The copy will be of the same
     * concrete type as the original.
     *
     * <p>Benefits:
     *
     * <ul>
     *   <li>Guarantees true deep copy with no shared references
     *   <li>Preserves the concrete type (e.g., ClickOptions remains ClickOptions)
     *   <li>Handles complex nested objects automatically
     *   <li>Thread-safe - can be used for parallel execution scenarios
     * </ul>
     *
     * <p>Common use cases:
     *
     * <ul>
     *   <li>Creating action variations for testing
     *   <li>Preserving original configuration while experimenting
     *   <li>Parallel execution with different parameters
     * </ul>
     *
    ...
    public <T extends ActionConfig> T deepCopy(T actionConfig) throws ConfigurationException {
        ...
        if (!copy.getClass().equals(actionConfig.getClass())) {
            throw new ConfigurationException(
                    String.format(
                            "Type mismatch during deep copy: expected %s, got %s",
     ...
     *
     * <p>This method can be useful when migrating from one action type to another or when creating
     * a new action based on an existing one's configuration.
     *
     * <p>Note: Only compatible fields will be copied. Fields specific to the source type that don't
     * exist in the target type will be ignored.
     *
