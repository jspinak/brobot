 ...
 *
 * <p>This tool captures screens and reports the resolution to help verify that captures are
 * occurring at physical resolution (e.g., 1920x1080) rather than logical resolution (e.g.,
 * 1536x864).
 *
 * @since 1.1.0
 */
public class DPIVerificationTool {
    ...
    public static void main(String[] args) throws Exception {
        ...
        BufferedImage robotCapture =
                robot.createScreenCapture(new Rectangle(0, 0, screenSize.width, screenSize.height));
        System.out.println(
                "   Captured Size: " + robotCapture.getWidth() + "x" + robotCapture.getHeight());
        ...
        System.out.println(
                "   Captured Size: " + sikuliCapture.getWidth() + "x" + sikuliCapture.getHeight());

        // Analysis
        System.out.println("\n6. ANALYSIS:");
        System.out.println("-".repeat(70));

        boolean capturesMatch =
                (robotCapture.getWidth() == sikuliCapture.getWidth())
                        && (robotCapture.getHeight() == sikuliCapture.getHeight());

        if (capturesMatch) {
            System.out.println(
                    "   ✓ Robot and SikuliX captures match: "
                            + sikuliCapture.getWidth()
                            + "x"
                            + sikuliCapture.getHeight());
        } else {
            System.out.println("   ⚠ Captures don't match!");
            System.out.println(
                    "     Robot: " + robotCapture.getWidth() + "x" + robotCapture.getHeight());
            System.out.println(
        ...
        if (scaleX > 1.0) {
            ...
            if (Math.abs(sikuliCapture.getWidth() - expectedPhysicalWidth) < 10) {
                ...
                System.out.println(
                        "   This is the desired behavior for SikuliX IDE compatibility.");
                ...
                System.out.println("\n   To fix this:");
                System.out.println(
                        "   1. Ensure brobot.dpi.disable=true in application.properties");
            ...
            System.out.println(
                    "   Captures are at native resolution: "
                            + sikuliCapture.getWidth()
                            + "x"
                            + sikuliCapture.getHeight());
        }
