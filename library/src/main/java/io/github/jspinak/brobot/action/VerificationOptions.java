 ...
 *
 * <p>This class defines the criteria for repeatedly executing an action until a specific condition,
 * such as the appearance of text or an image, is met. It allows any action to be converted into a
 * "wait-like" operation.
 *
...
public final class VerificationOptions {
    ...
    @Builder.Default private final Condition condition = Condition.CONTINUE_UNTIL_CONDITION_MET;

    @Builder.Default private final Event event = Event.NONE;

    @Builder.Default private final String text = "";

    @Builder.Default
    private final ObjectCollection objectCollection =
            new ObjectCollection.Builder().build(); // For OBJECTS_APPEAR/VANISH
