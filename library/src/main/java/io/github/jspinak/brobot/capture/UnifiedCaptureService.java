 ...
 * 
 * This service is the primary entry point for screen capture in Brobot and can be
 * configured entirely through properties to use different capture providers.
 * 
 * Configuration example in application.properties:
 * ```properties
 * # Choose capture provider: AUTO, ROBOT, FFMPEG, SIKULIX, or fully qualified class name
 * brobot.capture.provider=ROBOT
 * 
 * # Or use a custom provider class
 * brobot.capture.provider=com.example.MyCustomCaptureProvider
 * ```
 * 
 * Features:
 * - Single interface for all capture operations
 * - Property-based provider selection
...
public class UnifiedCaptureService {
     ...
     * This is useful for testing or when different providers are needed for different operations.
     * 
    ...
    private BufferedImage executeWithRetry(CaptureOperation operation) throws IOException {
        ...
        for (int i = 0; i <= (autoRetry ? retryCount : 0); i++) {
            try {
                ...
                if (enableLogging) {
                    System.err.println("[UnifiedCapture] Capture attempt " + (i + 1) + " failed: " + e.getMessage());
                }
    ...
    /**
     * Loads a custom capture provider by class name.
     * This allows users to implement their own CaptureProvider and use it via properties.
     */
    private void loadCustomProvider(String className) {
        try {
            ...
            if (CaptureProvider.class.isAssignableFrom(clazz)) {
                ...
                System.out.println("[UnifiedCapture] Custom provider loading not yet implemented: " + className);
                System.out.println("[UnifiedCapture] Please register your provider as a Spring bean");
            } else {
                throw new IllegalArgumentException("Class does not implement CaptureProvider: " + className);
            }
