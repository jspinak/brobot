 ...
 *
 * <ul>
 *   <li>Finding overlapping regions between scene transitions
 *   <li>Identifying opposite-direction movements that indicate object motion
 *   <li>Verifying consistent distances to confirm the same object is moving
 * </ul>
 *
 * <p>A moving object is identified when:
 *
 * <ol>
 *   <li>A match region overlaps between two scene transitions
 *   <li>There's a match in the opposite direction (>150° difference)
 *   <li>The distances are consistent, indicating the same object
 * </ol>
 *
 * <p>This approach effectively filters out camera movements and background changes, focusing on
 * objects that move independently within the scene.
 *
...
public class MovingObjectSelector {
    ...
    /**
     * Selects moving objects by analyzing motion patterns across three scenes. The algorithm
     * identifies objects that move in opposite directions between scene transitions, which
     * indicates independent object motion rather than camera movement or background changes.
     *
     * <p>The returned list contains three sublists:
     *
     * <ul>
     *   <li>Index 0: Matches from scene 1 (starting positions)
     *   <li>Index 1: Overlapping matches in scene 2 (intermediate positions)
     *   <li>Index 2: Matches from scene 3 (final positions)
     * </ul>
     *
     ...
     */
    public List<List<Match>> select(
            List<Match> matchList1, List<Match> matchList2, int maxDistance) {
        ...
        for (Match overlappingMatch : overlappingMatches) {
            List<Match> matchesWithinDistance1 =
                    getMatchesWithinDistance(matchList1, overlappingMatch, maxDistance);
            List<Match> matchesWithinDistance2 =
                    getMatchesWithinDistance(matchList2, overlappingMatch, maxDistance);
            for (Match match2 : matchesWithinDistance2) {
                for (Match match1 : matchesWithinDistance1) {
                    double degreesBetweenMatches =
                            distance.getDegreesBetween(overlappingMatch, match1, match2);
    ...
    /**
     * Finds matches that overlap between two match lists. Overlapping matches indicate regions that
     * changed in both scene transitions, which are candidates for containing moving objects.
     *
    ...
    /**
     * Filters matches to find those within a specified distance of a center match. This helps
     * identify related motion regions that could represent the same moving object at different
     * positions.
     *
     ...
     */
    private List<Match> getMatchesWithinDistance(
            List<Match> matchList, Match centerMatch, int maxDist) {
