...
/**
 * Configures DPI scaling for Brobot applications. Automatically detects and applies appropriate
 * pattern scaling based on monitor DPI.
 *
 * <p>This configuration is part of the Brobot library and applies to all Brobot applications.
 */
@Component
...
public class DPIConfiguration {
    ...
    public void configureDPIScalingEarly() {
        ...
        if (resizeFactor != 1.0f) {
            ...
            System.out.println(
                    "  - A 100x100 pattern will be matched as "
                            + (int) (100 * resizeFactor)
                            + "x"
                            + (int) (100 * resizeFactor));
            if (resizeFactor < 1.0f) {
                System.out.println(
                        "  - Patterns are being DOWNSCALED (display has higher DPI than pattern"
                                + " source)");
            } else {
                System.out.println(
                        "  - Patterns are being UPSCALED (display has lower DPI than pattern"
                                + " source)");
            }
        }
