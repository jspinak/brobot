 ...
 * Provides detailed logging for complex action sequences.
 */
@Component
public class ActionChainLogger {
    ...
    public static class StepLog {
        ...
        public StepLog(int stepNumber, String description, ActionConfig config, ActionResult result) {
     ...
     */
    public void logStepTransition(String chainId, String fromStep, String toStep, 
                                  ActionConfig config, ActionResult result) {
        ...
        if (context.isStepLoggingEnabled() && logger != null) {
            logger.log()
                ...
                .metadata("configType", config != null ? config.getClass().getSimpleName() : "null")
        ...
        if (logger != null) {
            ...
            if (!success) {
                context.getSteps().stream()
                    ...
                    .forEach(step -> logger.log()
                        .level(LogEvent.Level.WARNING)
                        .message(String.format("  Failed Step %d: %s", 
    ...
    public void logStep(String chainId, String stepDescription) {
        ...
        if (context.isStepLoggingEnabled() && logger != null) {
            logger.log()
                .level(LogEvent.Level.INFO)
                .message(String.format("  Step %d: %s", context.getCurrentStepIndex(), stepDescription))
                .log();
        }
