 ...
 *
 * <p>This configuration: 1. Explicitly configures the ApplicationEventMulticaster 2. Ensures all
 * ApplicationListener beans are properly registered 3. Provides synchronous event handling for
 * critical initialization events 4. Adds logging for event publishing diagnostics
 */
@Configuration
@Slf4j
public class EventListenerConfiguration {
    ...
    /**
     * Configure a custom ApplicationEventMulticaster with enhanced logging. We use synchronous
     * event publishing for initialization events to ensure proper ordering and completion.
     */
    ...
    public static class LoggingApplicationEventMulticaster
        ...
        protected void invokeListener(
            ...
            try {
                ...
                log.error("Error in listener {} handling {}", listenerName, eventName, e);
    ...
    /**
     * Optional: Configure an async executor for performance-critical runtime events. This is not
     * used for initialization events but can be enabled for runtime events.
     */
    ...
    public Executor asyncEventExecutor() {
        ...
        executor.initialize();
        log.info(
                "[EVENT CONFIG] Async event executor configured (not used for initialization"
                        + " events)");
        return executor;
    }
}
