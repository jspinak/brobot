 ...
 * 
 * This class manages state and search region initialization including:
 * 1. State registration and validation
 * 2. Search region dependency resolution
 * 3. Cross-state reference validation
 * 4. Declarative region setup
 * 
 * This orchestrator works in conjunction with BrobotStartupOrchestrator
 * to handle the state-specific aspects of initialization.
 * 
 * By centralizing state initialization, we ensure:
 * - Proper ordering of initialization steps
 * - Clear visibility into what's being initialized
 * - Single point of control for declarative regions
...
public class StateInitializationOrchestrator {
     ...
     * This coordinates all initialization phases in the correct order.
     */
    ...
    public void orchestrateInitialization(StatesRegisteredEvent event) {
        ...
        log.info("[STATE-INIT] States: {}, Transitions: {}", 
        ...
        try {
            ...
            log.info("[STATE-INIT] Completed phases: {}", 
            ...
            log.error("[STATE-INIT] Initialization failed at phase: {}", 
                    getCurrentPhase(), e);
    ...
    private void validateStateRegistration() {
        ...
        for (State state : states) {
            ...
            log.debug("[STATE-INIT]   - State: {} (Images: {}, Regions: {}, Locations: {})",
                    stateName,
            ...
            if (!stateService.getAllStates().contains(state)) {
                log.warn("[STATE-INIT]   - WARNING: State {} is in StateStore but not in StateService",
                        stateName);
            }
        }
    ...
    private void initializeSearchRegionDependencies() {
        ...
        for (State state : stateStore.getAllStates()) {
            ...
            log.debug("[STATE-INIT]   - State {} has {} objects", 
                    stateName, stateObjects.size());
        }
        ...
        log.info("[STATE-INIT]   - Registered {} objects with DynamicRegionResolver", allObjects.size());
    ...
    private void validateCrossStateReferences() {
        ...
        Set<String> availableStates = stateStore.getAllStates().stream()
        ...
        for (State state : stateStore.getAllStates()) {
            for (StateObject obj : getAllStateObjects(state)) {
                ...
                if (obj instanceof io.github.jspinak.brobot.model.state.StateImage) {
                    io.github.jspinak.brobot.model.state.StateImage img = (io.github.jspinak.brobot.model.state.StateImage) obj;
                    if (img.getSearchRegionOnObject() != null) {
                        ...
                        if (targetState != null && !targetState.equals(obj.getOwnerStateName())) {
                            ...
                            if (!availableStates.contains(targetState)) {
                                log.warn("[STATE-INIT]   - INVALID: {} references non-existent state: {}",
                                        obj.getName(), targetState);
                                invalidReferences++;
                            } else {
                                log.debug("[STATE-INIT]   - Valid cross-reference: {} -> {}",
                                        obj.getName(), targetState);
                            }
                        }
                    }
        ...
        log.info("[STATE-INIT]   - Cross-state references: {} (invalid: {})",
                crossStateReferences, invalidReferences);
        
        if (invalidReferences > 0) {
            log.warn("[STATE-INIT]   - WARNING: Found {} invalid cross-state references",
                    invalidReferences);
        }
            ...
            for (StateObject obj : getAllStateObjects(state)) {
                ...
                if (obj instanceof io.github.jspinak.brobot.model.state.StateImage) {
                    ...
                    if (img.getSearchRegionOnObject() != null) {
                        ...
                        regionsSetup++;
                        log.debug("[STATE-INIT]   - Registered regions for {}.{}",
    ...
    public void reinitialize() {
        ...
        StatesRegisteredEvent event = new StatesRegisteredEvent(
            this, 
            stateStore.getAllStates().size(), 
            0
        );
