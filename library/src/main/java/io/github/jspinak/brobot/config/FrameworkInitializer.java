 ...
 * 
 * <p>FrameworkInitializer provides essential startup operations that prepare the framework for automation 
 ...
 * </p>
 * 
 * <p>In the model-based approach, FrameworkInitializer transforms the static state and image definitions 
...
public class FrameworkInitializer {
    ...
    /**
     * Counter for assigning unique indices to StateImages.
     * Starts at 1 because 0 typically represents "no class" in matrix operations
     * and machine learning contexts where these indices are used.
     */
     ...
     * <p>
     * Dependencies are injected by Spring to ensure proper initialization order
     * and availability of all required services.
     *
     ...
     */
    public FrameworkInitializer(StateService allStatesInProjectService, 
     ...
     * <p>
     * This method should be called after Spring initialization completes, typically
     * from {@link FrameworkLifecycleManager}. It performs two critical operations:
     * <ol>
     * <li>Configures SikuliX's image search path (unless in mock/headless mode)</li>
     * <li>Preprocesses all StateImages by assigning indices and generating profiles</li>
     * </ol>
     * <p>
     * <strong>Side effects:</strong>
     * <ul>
     * <li>Sets the global SikuliX bundle path</li>
     * <li>Assigns unique indices to all StateImages (increments {@code lastImageIndex})</li>
     * <li>Generates color profiles for each image</li>
     * <li>Creates k-means profiles based on configuration settings</li>
     * <li>Outputs progress to the Report system</li>
     * </ul>
     * <p>
     * The preprocessing step is crucial for performance as it moves expensive
     * operations to initialization time rather than during action execution.
     * 
     ...
     * <p>
     * For each StateImage in the given state:
     * <ol>
     * <li>Assigns a unique index (starting from 1)</li>
     * <li>Generates OpenCV Mat objects and color profiles</li>
     * <li>Optionally creates k-means profiles based on image type and settings</li>
     * </ol>
     * <p>
     * <strong>Side effects:</strong>
     * <ul>
     * <li>Modifies each StateImage by setting its index field</li>
     * <li>Increments {@code lastImageIndex} for each processed image</li>
     * <li>Creates and stores color profile data in each StateImage</li>
     * <li>Outputs progress information via Report</li>
     * </ul>
     * <p>
     * K-means profiles are generated when:
     * <ul>
     * <li>Dynamic images: {@link FrameworkSettings#initProfilesForDynamicImages} is true</li>
     * <li>Static images: {@link FrameworkSettings#initProfilesForStaticfImages} is true</li>
     * </ul>
     *
    ...
    private void preProcessImages(State state) {
        if (!state.getStateImages().isEmpty())
            ConsoleReporter.print(state.getName() + ": ");
        for (StateImage stateImage : state.getStateImages()) {
            ...
            if (FrameworkSettings.initProfilesForDynamicImages && stateImage.isDynamic() ||
                    (FrameworkSettings.initProfilesForStaticfImages && !stateImage.isDynamic())) {
     ...
     * <p>
     * This method extends the image search path without replacing the existing
     * bundle path. Useful for adding supplementary image directories after initial
     * configuration. The path is only added when SikuliX operations are enabled.
     * <p>
     * <strong>Side effects:</strong> Modifies SikuliX's global image search path
     *
     ...
     * <p>
     * State transitions are initially defined using state names for readability.
     * This method converts those name references to numeric IDs for efficient
     * runtime lookups and path finding algorithms.
     * <p>
     * <strong>Side effects:</strong>
     * <ul>
     * <li>Modifies all StateTransitions objects to use state IDs</li>
     * <li>Initializes the transitions repository for runtime use</li>
     * </ul>
     */
     ...
     * <p>
     * States can be configured to hide other states (e.g., a popup hiding the
     * main window). This method converts the string-based state names in the
     * canHide list to numeric IDs for efficient runtime checking.
     * <p>
     * <strong>Side effects:</strong>
     * <ul>
     * <li>Populates the canHideIds list in each State object</li>
     * <li>Silently ignores invalid state names (states that don't exist)</li>
     * </ul>
     */
     ...
     * <p>
     * Coordinates the conversion of string-based state references to numeric IDs
     * in both transitions and "can hide" relationships. This conversion is essential
     * for runtime performance as ID lookups are significantly faster than string
     * comparisons.
     * <p>
     * <strong>Side effects:</strong>
     * <ul>
     * <li>Modifies all StateTransitions to use state IDs</li>
     * <li>Populates canHideIds in all State objects</li>
     * <li>Initializes the transitions repository</li>
     * </ul>
     *
     ...
     * <p>
     * This method orchestrates the final initialization steps needed to prepare
     * the state management system:
     * <ol>
     * <li>Checks if states exist (beyond the default UNKNOWN state)</li>
     * <li>Converts all state name references to IDs</li>
     * <li>Builds the state transitions joint table for path finding</li>
     * </ol>
     * <p>
     * The joint table is a critical data structure that enables efficient
     * navigation between states by pre-computing all possible transition paths.
     * <p>
     * <strong>Side effects:</strong>
     * <ul>
     * <li>Modifies all state-related objects to use IDs instead of names</li>
     * <li>Creates and populates the state transitions joint table</li>
     * <li>Outputs initialization status via Report</li>
     * </ul>
     * <p>
     * This method should be called after all states have been defined and
     * registered with the framework, typically during application startup.
     *
