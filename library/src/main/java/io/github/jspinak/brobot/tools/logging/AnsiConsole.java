 ...
 *
 * <p>AnsiConsole provides assertion-style methods with colored console output to visually indicate
 * test results during automation execution. Unlike traditional unit test assertions that throw
 * exceptions, these methods return boolean results while providing immediate visual feedback
 * through color-coded output.
 *
 * <p>Key features:
 *
 * <ul>
 *   <li><b>Visual Assertions</b>: Green for passed, red for failed comparisons
 *   <li><b>Multi-type Support</b>: Overloaded methods for int, double, boolean, String, StateEnum
 *   <li><b>Pair-wise Comparison</b>: Expected values followed by actual values
 *   <li><b>Inline Results</b>: Compact output format for real-time monitoring
 *   <li><b>Non-throwing</b>: Returns boolean instead of throwing exceptions
 * </ul>
 *
 * <p>Use cases:
 *
 * <ul>
 *   <li><b>Live Testing</b>: Monitor automation execution with visual feedback
 *   <li><b>Mock Verification</b>: Validate mock behavior with colored output
 *   <li><b>Companion Apps</b>: Watch screens and report expected/unexpected events
 *   <li><b>Debug Mode</b>: Quick visual verification during development
 * </ul>
 *
 * <p>Output format:
 *
 ...
 *
 * <ul>
 *   <li>Disable other console output when using live testing to avoid clutter
 *   <li>Group related assertions with descriptive messages
 *   <li>Use for visual monitoring, not critical test infrastructure
 *   <li>Consider terminal width when formatting messages
 * </ul>
 *
 * <p>Thread safety: Methods are not synchronized. In multi-threaded environments, output may
 * interleave. Consider external synchronization if needed.
 *
 * @since 1.0
...
public class AnsiConsole {
     ...
     *
     * <p>Format: value1:value2 (where value2 is colored based on match result)
     *
     ...
     *
     * <p>Values must be provided in expected/actual pairs. Each pair is compared, with green output
     * for matches and red for mismatches.
     *
    ...
    public static boolean assertTrue(String message, int... valuesToCompare) {
        ...
        for (int i = 0; i < valuesToCompare.length; i++)
            values[i] = Integer.toString(valuesToCompare[i]);
     ...
     *
     * <p>Values must be provided in expected/actual pairs. Note: Uses exact equality comparison,
     * consider using a tolerance-based comparison for floating point values.
     *
     * @param message descriptive message about what is being tested
    ...
    public static boolean assertTrue(String message, double... valuesToCompare) {
        String[] values = new String[valuesToCompare.length];
        for (int i = 0; i < valuesToCompare.length; i++)
            values[i] = Double.toString(valuesToCompare[i]);
        return assertTrue(message, values);
    }
    ...
    public static boolean assertTrue(String message, boolean... valuesToCompare) {
        String[] values = new String[valuesToCompare.length];
        for (int i = 0; i < valuesToCompare.length; i++)
            values[i] = Boolean.toString(valuesToCompare[i]);
        return assertTrue(message, values);
    }
     ...
     *
     * <p>Useful for validating state transitions in the automation framework.
     *
     * @param message descriptive message about what is being tested
     ...
     *
     * <p>This is the base implementation used by all type-specific assertion methods. Compares
     * values in expected/actual pairs, printing each comparison with appropriate color coding. The
     * final message is colored based on overall result.
     *
     * <p>Output format:
     *
     * <pre>
     ...
     * </pre>
     *
     * Where mismatches are shown in red and matches in green.
     *
     * @param message descriptive message about what is being tested
