 ...
 * consolidating the functionality of SLF4J, ActionLogger, and ConsoleReporter into a cohesive API.
 * This design simplifies the developer experience while providing powerful capabilities for
 * debugging, monitoring, and analysis.
 *
 * <p>Key features:
 *
 * <ul>
 *   <li>Simple methods for common logging scenarios (80% use case)
 *   <li>Fluent builder API for complex logging needs
 *   <li>Automatic context propagation (session, state, action)
 *   <li>Performance tracking utilities
 *   <li>Configurable output levels and formats
 * </ul>
 *
 * <p>Usage examples:
 *
...
public class BrobotLogger {
     ...
     *
     * <p>This method provides the simplest way to log automation actions. It automatically captures
     * the current context (session, state) and routes the log entry to appropriate handlers based
     * on configuration.
     *
    ...
    public void action(String action, StateObject target, ActionResult result) {
        ...
        if (!result.getMatchList().isEmpty()) {
            ...
            if (firstMatch instanceof Match) {
                ...
                eventBuilder.metadata("matchX", match.x()).metadata("matchY", match.y());
            }
        }
     ...
     *
     * <p>The builder pattern allows for more detailed log entries with custom metadata, performance
     * metrics, screenshots, and other contextual information.
     *
     ...
     *
     * <p>All logs within the session scope will automatically include the session ID for
     * correlation. The session is automatically closed when used with try-with-resources.
     *
     * <pre>{@code
     ...
     *
     * <p>Operations are automatically timed and logged when the scope closes.
     *
    ...
    /**
     * Routes a log event through the message router. Package-private to allow LogBuilder access.
     *
