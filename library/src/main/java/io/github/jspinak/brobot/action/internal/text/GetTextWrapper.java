 ...
 * <p>
 * This wrapper class abstracts optical character recognition operations, extracting
 * text from {@link Match} objects and their associated image regions. It integrates
 * with Sikuli's OCR capabilities while providing a consistent interface for both
 * real and mocked text extraction.
 * <p>
 * The class handles two main workflows:
 * <ul>
 * <li>Batch processing: Extract text from all matches in an {@link ActionResult}</li>
 * <li>Individual processing: Extract text from a single {@link Match}</li>
 * </ul>
 * <p>
 * In mock mode, text extraction uses probabilistic models based on match history
...
public class GetTextWrapper {
     ...
     * <p>
     * This method processes each match in the provided {@link ActionResult}, performing
     * OCR on their associated image regions. Extracted text is stored in two places:
     * <ul>
     ...
     * </ul>
     * <p>
     * In mock mode, text is generated based on match history probabilities. The mock
     ...
     *                is modified by adding extracted text strings to its collection.
     *                
     ...
     * In mock mode, generates text based on the match's history and probability models.
