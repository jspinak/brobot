 ...
 * Mat objects (matrix representations of images). These operations are fundamental to Brobot's
 * computer vision capabilities, supporting everything from basic matrix manipulation to complex
 * image analysis. The class bridges JavaCV and OpenCV APIs, provides debugging tools, and
 * implements common patterns used throughout the framework.
 *
 * <p>Core operation categories:
 *
 * <ul>
 *   <li><b>Data Access</b>: Get/put values at specific positions with type safety
 *   <li><b>Debugging</b>: Print matrix contents with intelligent formatting
 *   <li><b>Conversion</b>: Transform between JavaCV/OpenCV formats and color spaces
 *   <li><b>Creation</b>: Factory methods for creating Mats with specific properties
 *   <li><b>Analysis</b>: Min/max values, statistics, and content inspection
 *   <li><b>Manipulation</b>: Channel operations, ROI extraction, aggregation
 * </ul>
 *
 * <p>Debugging features:
 *
 * <ul>
 *   <li>Partial matrix printing to avoid overwhelming output
 *   <li>Consecutive zero compression for sparse matrices
 *   <li>Multi-channel support with per-channel display
 *   <li>Dimension and type information display
 *   <li>Min/max value reporting for quick data validation
 * </ul>
 *
 * <p>Type handling:
 *
 * <ul>
 *   <li>Automatic indexer selection based on Mat type
 *   <li>Support for 8-bit, 32-bit integer, and 32-bit float types
 *   <li>Safe conversions between different Mat representations
 *   <li>BufferedImage to Mat conversion for Java integration
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>Extract individual channels from multi-channel Mats
 *   <li>Find min/max values across channels
 *   <li>Merge single-channel Mats into multi-channel
 *   <li>Per-cell operations across multiple Mats
 * </ul>
 *
 * <p>Factory methods:
 *
 * <ul>
 *   <li>Create Mats with uniform values
 *   <li>3x3 kernel creation for morphological operations
 *   <li>Multi-channel Mat creation with per-channel values
 *   <li>Size-based and dimension-based constructors
 * </ul>
 *
 * <p>Safety features:
 *
 * <ul>
 *   <li>Null checks and empty Mat handling
 *   <li>Bounds checking for ROI operations
 *   <li>Type compatibility verification
 *   <li>Optional return types for operations that may fail
 * </ul>
 *
 * <p>Performance considerations:
 *
 * <ul>
 *   <li>Direct indexer access for efficient pixel operations
 *   <li>Minimal copying through careful use of references
 *   <li>Batch operations for multiple Mats
 *   <li>Lazy evaluation where possible
 * </ul>
 *
 * <p>In the model-based approach, MatrixUtilities serves as the foundation for all image processing
 * operations. It provides the low-level tools needed to implement pattern matching, color analysis,
 * motion detection, and other computer vision algorithms that enable Brobot to "see" and understand
 * GUI elements.
 *
 * @since 1.0
 * @see Mat
...
public class MatrixUtilities {
    ...
    public static void putInt(Mat mat, int row, int col, short... values) {
        ...
        UByteRawIndexer indexer =
                mat.createIndexer(); // IntRawIndexer indexer32s = mat.createIndexer();
    ...
    /**
     * In a multi-dimensional array, such as a 3D matrix or tensor, the indices i,j,k typically
     * represent the indices along the first, second, and third dimensions, respectively. The
     * variable i is likely used to refer to the row index.
     *
    ...
    public static double getDouble(int row, int col, int channel, Mat mat) {
        UByteRawIndexer indexer8u;
        IntRawIndexer indexer32s;
        FloatIndexer indexer32f;
    ...
    /**
     * Prints part of the matrix, starting from the top left corner. Consecutive 0 values will be
     * printed as ... The first channel is printed first and its 2d form is maintained.
     *
    ...
    public static org.opencv.core.Mat convertToOpenCVmat(Mat mat) {
        ...
        OpenCVFrameConverter.ToOrgOpenCvCoreMat converter2 =
    ...
    /**
     * minMaxLoc requires a single channel array. This method uses the first channel if the Mat is a
     * multi-channel array. miniMaxLoc does not consider 0 as a minimum but this function does.
     */
    ...
    public static boolean firstChannelContains(Mat mat, int x) {
        ...
        inRange(
                singleChannelMat,
                new Mat(new Scalar(x)),
                new Mat(new Scalar(x)),
                booleanMat); // 255 where inRange, 0 where not
    ...
    public static Mat makeMat(
            Size size, int type, double channel1, double channel2, double channel3) {
        return makeMat(size.height(), size.width(), type, channel1, channel2, channel3);
    }

    /**
     * Makes either a one channel or 3 channel Mat. If another number of channels is provided, an
     * empty Mat is returned.
     *
    ...
    public static Mat makeMat(
            int rows, int cols, int type, double channel1, double channel2, double channel3) {
    ...
    public static boolean matsDontMatch(int minMats, List<Mat> mats) {
        ...
        for (Mat mat : mats) {
            if (mat.rows() != rows || mat.cols() != cols || mat.channels() != channels) {
                System.out.println(
                        "All Mats must have the same number of rows, columns, and channels");
                return true;
            }
        }
        return false;
    ...
    public static Optional<Mat> applyIfOk(Mat mat, Rect roi) {
        ...
        if (matIsNull || smallX || smallY || smallW || smallH || outsideMatX || outsideMatY)
