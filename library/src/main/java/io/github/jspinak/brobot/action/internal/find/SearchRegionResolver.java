 ...
 *
 * <p>This component implements a sophisticated region selection strategy that respects the
 * precedence of different region sources. It ensures that the most specific and intentional region
 * definitions take priority over more general ones, while always guaranteeing that at least one
 * search region is available for the operation.
 *
 * <p><b>Region Selection Priority (highest to lowest):</b>
 *
 * <ol>
...
public class SearchRegionResolver {
    ...
    /**
     * Logs a message while preventing consecutive duplicates. If the same message is repeated, it
     * increments a counter instead of logging again. When a different message comes in, it logs any
     * suppressed count and the new message.
     */
     ...
     * @return A non-empty list of regions to search within. Always contains at least one region
     *     (full screen) if no specific regions are defined.
     */
    ...
    public List<Region> getRegionsForAllImages(
        ...
        if (actionConfig instanceof BaseFindOptions) {
            BaseFindOptions findOptions = (BaseFindOptions) actionConfig;
            if (findOptions.getSearchRegions() != null
                    && findOptions.getSearchRegions().isAnyRegionDefined()) {
                return findOptions.getSearchRegions().getRegionsForSearch();
            }
        }
