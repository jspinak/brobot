 ...
 * <p>
 * This class is a core component of the Brobot visualization system, offering various methods
 * to draw rectangles around or directly on matches, regions, and other rectangular areas.
 * It handles boundary checking to ensure rectangles stay within image bounds and provides
 * both single and batch drawing operations.
 * <p>
 * Key features:
 * <ul>
 * <li>Draw rectangles around matches with 1-pixel padding</li>
 * <li>Draw rectangles directly on match boundaries</li>
 * <li>Batch operations for multiple regions</li>
 * <li>Automatic boundary clipping to prevent out-of-bounds drawing</li>
 * <li>Support for custom colors via OpenCV Scalar</li>
 * </ul>
 * <p>
 * Drawing modes:
 * <ul>
 * <li><b>Around</b> - Adds 1-pixel padding around the target area</li>
 * <li><b>On</b> - Draws exactly on the target boundaries</li>
 * </ul>
 * <p>
 * Common use cases:
 * <ul>
 * <li>Highlighting found matches in automation results</li>
 * <li>Visualizing search regions during debugging</li>
 * <li>Creating annotated screenshots for documentation</li>
 * <li>Drawing bounding boxes for object detection results</li>
 * </ul>
 * <p>
 * Thread safety: This class is stateless and thread-safe. Multiple threads can
...
public class DrawRect {
     ...
     * <p>
     * This method adds visual emphasis by drawing the rectangle slightly outside
     * the match boundaries, making it easier to see the match edges. The rectangle
     * is automatically clipped to stay within the image bounds.
     * <p>
     * Boundary handling:
     * <ul>
     * <li>X coordinates are clamped to [0, image width]</li>
     * <li>Y coordinates are clamped to [0, image height]</li>
     * <li>1-pixel padding is added where possible</li>
     * </ul>
     *
     ...
     * @param color The color of the rectangle as an OpenCV Scalar (e.g., new Scalar(0,255,0) for green).
     */
     ...
     * <p>
     * This method draws the rectangle exactly on the match edges, useful when
     * precise boundary visualization is needed. The rectangle is automatically
     * clipped to stay within the image bounds.
     *
     * @param scene The OpenCV Mat image on which to draw. Must not be null.
     * @param match The match whose boundaries define the rectangle. Must not be null.
     ...
     * <p>
     * This is a lower-level method that works directly with OpenCV Rect objects.
     * The rectangle is drawn exactly on the rect boundaries with automatic
     * boundary clipping.
     *
     ...
     * <p>
     * Similar to {@link #drawRectAroundMatch(Mat, Match, Scalar)} but works
     * directly with OpenCV Rect objects. Adds 1-pixel padding on all sides
     * where possible within image bounds.
     *
     * @param mat The OpenCV Mat image on which to draw. Must not be null.
     ...
     * <p>
     * This batch operation method draws rectangles on two different visualization
     * layers provided by the Illustrations object. This is useful for showing
     * the same regions on different visual representations simultaneously.
     *
     * @param illScn The Illustrations object containing both scene and class visualizations.
     * @param searchRegions List of regions to draw rectangles around. Empty lists are handled safely.
     * @param color The color of the rectangles as an OpenCV Scalar.
     */
    public void drawRectAroundMatch(Visualization illScn, List<Region> searchRegions, Scalar color) {
     ...
     * <p>
     * This method efficiently draws rectangles around all provided regions.
     * Each region is converted to a Match object for drawing. Null Mat objects
     * are handled safely by returning early.
     *
     * @param mat The OpenCV Mat image on which to draw. Can be null (method returns early).
     * @param searchRegions List of regions to draw rectangles around. Each region is converted to a match.
     * @param color The color of the rectangles as an OpenCV Scalar.
     */
     ...
     * <p>
     * Convenience method for drawing a rectangle around a single Region object.
     * The region is converted to a Match for drawing. Null Mat objects are
     * handled safely.
     *
     * @param mat The OpenCV Mat image on which to draw. Can be null (method returns early).
