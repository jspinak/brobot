...
/**
 * Diagnostic tool for analyzing and resolving DPI scaling issues. Helps determine why patterns that
 * match in SikuliX IDE don't match in Brobot.
 *
 * <p>Key findings from testing: - SikuliX IDE captures patterns in physical pixels - Windows
 * snipping tool captures in logical pixels - With 125% Windows scaling: logical 100px = physical
 * 125px - Pattern scale factor = 1/display_scale (e.g., 0.8 for 125%)
 */
public class DPIScalingDiagnostic {
    ...
    public static DiagnosticResult analyzePattern(String patternPath) {
        ...
        try {
            ...
            DPIScalingStrategy.PatternSource likelySource =
                    guessPatternSource(width, height, displayScale);
            ...
            for (float dpiSetting : testSettings) {
                ...
                try {
                    ...
                    if (match != null) {
                        ...
                        System.out.printf(
                                "  DPI %.2f: %.1f%% similarity", dpiSetting, similarity * 100);
            ...
            System.out.println(
                    "  Best similarity: " + String.format("%.1f%%", bestSimilarity * 100));
            System.out.println("\nRECOMMENDATION:");
    ...
    public static void analyzePatterns(List<String> patternPaths) {
        ...
        if (!results.isEmpty()) {
            ...
            double avgSimilarity =
                    results.stream().mapToDouble(r -> r.bestSimilarity).average().orElse(0);

            System.out.println("Across " + results.size() + " patterns:");
            System.out.println("  Most common optimal DPI: " + commonDPI);
            System.out.println(
                    "  Average best similarity: " + String.format("%.1f%%", avgSimilarity * 100));
    ...
    private static DPIScalingStrategy.PatternSource guessPatternSource(
        ...
        int[][] commonLogicalSizes = {{100, 50}, {150, 75}, {200, 100}, {103, 60}, {195, 80}};
    ...
    private static String generateRecommendation(
        ...
        if (bestSimilarity > 0.90) {
            ...
            sb.append("  Current best: Settings.AlwaysResize = ")
                    .append(bestDPISetting)
                    .append("\n");
            sb.append("\n  To improve:\n");

            if (source == DPIScalingStrategy.PatternSource.SIKULI_IDE) {
                sb.append("  - Pattern appears to be from SikuliX IDE (physical pixels)\n");
                sb.append("  - Expected DPI setting for ")
                        .append((int) (displayScale * 100))
                        .append("% scaling: ")
                        .append((float) (1.0 / displayScale))
                        .append("\n");
            } else {
            ...
            sb.append("  Best achieved: ")
                    .append(String.format("%.1f%%", bestSimilarity * 100))
                    .append(" with DPI ")
                    .append(bestDPISetting)
                    .append("\n");
