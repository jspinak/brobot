 ...
 * operations work in logical resolution (e.g., 1536x864 with 125% DPI), coordinates need to be
 * scaled appropriately.
 *
 * <p>This centralizes the coordinate scaling logic to ensure consistency across all mouse
 * operations, highlighting, and other screen-coordinate dependent features.
 *
 * @since 1.0
 */
@Slf4j
...
public class CoordinateScaler {
    ...
    public org.sikuli.script.Location scaleToLogical(int x, int y) {
        ...
        if (captureWidth == logicalWidth && captureHeight == logicalHeight) {
            log.trace(
                    "No coordinate scaling needed: capture {}x{} matches logical {}x{}",
                    captureWidth,
                    captureHeight,
                    logicalWidth,
                    logicalHeight);
        ...
        log.debug(
                "Scaled coordinates from ({},{}) to ({},{}) - scale factors: {}x{}",
                x,
                y,
                scaledX,
                scaledY,
    ...
    /**
     * Scales a region from capture coordinates to logical coordinates. Used for highlighting and
