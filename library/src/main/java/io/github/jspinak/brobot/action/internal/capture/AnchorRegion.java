 ...
 * definition by using anchor points from found matches to establish the boundaries of a new region.
 *
 * <p>In the model-based automation paradigm, this class facilitates the creation of regions that
 * are relative to existing GUI elements, allowing for more flexible and adaptive automation
 * scripts. The anchor points can define one or two boundaries of the region depending on their
 * position (e.g., corner positions define two boundaries, while middle positions define only one).
 *
 * <p>Key functionality includes:
 *
 * <ul>
 *   <li>Processing anchor points from multiple matches to define region boundaries
 *   <li>Adjusting region coordinates based on anchor positions
 *   <li>Supporting both absolute locations and match-relative anchor points
 * </ul>
 *
 * @see DefinedBorders
 * @see DefineRegion
...
public class AnchorRegion {
    ...
    /**
     * The anchors for each match are passed to the method that defines a boundary of the new
     * region.
     *
     * @param region the new region to define
     * @param actionResult the matches that are used to define the new region
     */
    public void fitRegionToAnchors(
            DefinedBorders definedBorders, Region region, ActionResult actionResult) {
            ...
            match.getAnchors()
                    ...
                    .forEach(
                            anchor -> {
                                ...
                                Location locationInMatch =
                                        new Location(match, anchor.getPositionInMatch());
                                adjustDefinedRegion(
                                        region,
                                        locationInMatch,
                                        anchor.getAnchorInNewDefinedRegion());
                            });
        }
    }

    /**
     * MIDDLE in this scenario is considered an unspecified value and is not used to define the
     * region. The other positions, LEFT RIGHT TOP BOTTOM, are considered specific and are used to
     * define boundaries. This is useful for matches that should define only the top, bottom, left,
     * or right side of the new region and not a specific point with x and y values.
     */
    ...
    /**
     * Locations can also be used to define boundaries. These are considered the same as anchors
     * with x and y coordinates (not MIDDLE).
     */
    private void fitRegionToLocations(
            DefinedBorders definedBorders, Region region, List<StateLocation> locations) {
