 ...
 *
 * <p>ActionHistory accumulates MatchSnapshot records over time, building a statistical model of how
 * GUI elements behave in different contexts. This historical data serves two critical purposes:
 * enabling realistic mock execution during development/testing and providing empirical data for
 * optimizing automation strategies.
 *
 * <p>Key components:
 *
 * <ul>
 *   <li><b>Search Statistics</b>: Tracks total searches and successful finds for probability
 *       calculations
 *   <li><b>Snapshot Collection</b>: Stores detailed records of each match attempt with full context
 *   <li><b>Action-specific Data</b>: Separates snapshots by action type for accurate simulation
 *   <li><b>State Context</b>: Links matches to their containing states for context-aware mocking
 * </ul>
 *
 * <p>Mock execution support:
 *
 * <ul>
 *   <li>When history exists, uses actual snapshot data for realistic behavior
 *   <li>Falls back to state probability variables when history is empty
 *   <li>Provides action-specific and state-specific snapshot selection
 *   <li>Handles special cases like text extraction and vanish operations
 * </ul>
 *
 * <p>Snapshot management:
 *
 * <ul>
 *   <li><b>Automatic Text Handling</b>: Adds matches to text-only snapshots when needed
 *   <li><b>Find Type Filtering</b>: Respects Find.ALL, Find.EACH, Find.FIRST distinctions
 *   <li><b>Vanish Separation</b>: Treats vanish operations differently from find operations
 *   <li><b>Random Selection</b>: Provides stochastic behavior based on historical distribution
 * </ul>
 *
 * <p>Data collection strategies:
 *
 * <ul>
 *   <li>Can be pre-populated during state setup for immediate mocking capability
 *   <li>Accumulates naturally during real execution for learning
 *   <li>Persists to database in production environments for long-term learning
 *   <li>Merges histories from multiple sources for comprehensive coverage
 * </ul>
 *
 ...
 * sophisticated simulation and optimization of GUI interactions.
 *
 * <p>The distinction between action types (especially vanish vs. other actions) ensures that mock
 * behavior accurately reflects the different success criteria and timing characteristics of various
 * operations.
 *
 * @since 1.0
 * @see ActionRecord
...
public class ActionHistory {
    ...
    /**
     * A Snapshot is either: 1. A successful/unsuccessful Match 2. A successful/unsuccessful GetText
     * Action from a successful Match 3. A successful/unsuccessful Vanish There cannot be a GetText
     * Snapshot without a successful Match. If a Snapshot is passed with Text and without a Match, a
     * new Match is added to the Snapshot. If the ActionHistory already has a Successful Match
     * Snapshot, this Match is added. Otherwise, a Match is created using probabilities.
     *
    ...
    public Optional<ActionRecord> getRandomSnapshot(ActionConfig actionConfig) {
        ...
        List<ActionRecord> selectedSnapshots =
                snapshots.stream()
    ...
    public Optional<ActionRecord> getRandomSnapshot(ActionConfig actionConfig, Long state) {
        ...
        List<ActionRecord> selectedSnapshots =
                snapshots.stream()
                        .filter(
                                snapshot ->
                                        getSnapshotActionType(snapshot) == action
                                                && snapshot.getStateId().equals(state))
                        .collect(toList());
        return getRandomSnapshot(selectedSnapshots);
    }
    ...
    /**
     * Most Actions succeed or fail based on a Find operation. VANISH has different criteria and its
     * Snapshots are separated. Return the Snapshots of the specific Find type.
     *
    ...
    private List<ActionRecord> getSnapshotOfFindType(
        ...
        List<ActionRecord> sameTypeSnapshots =
                snapshots.stream()
                        .filter(
                                snapshot ->
                                        snapshot.getActionConfig() != null
                                                && snapshot.getActionConfig()
                                                        .getClass()
                                                        .equals(configClass))
                        .collect(toList());
        ...
        if (actionConfig instanceof PatternFindOptions) {
            ...
            return sameTypeSnapshots.stream()
                    .filter(
                            snapshot -> {
                                PatternFindOptions.Strategy snapshotFind =
                                        getSnapshotFindType(snapshot);
