...
/**
 * Manages regions created or defined during action execution. Handles both anonymous and named
 * regions.
 *
 * <p>This class encapsulates region management functionality that was previously embedded in
 * ActionResult.
 *
 * @since 2.0
 */
...
public class RegionManager {
    ...
    /**
     * Gets the union of all defined regions. Creates a bounding box containing all regions.
     *
    ...
    /**
     * Gets the intersection of all defined regions. Returns the area common to all regions.
     *
    ...
    public Optional<Region> getIntersection() {
        ...
        for (int i = 1; i < definedRegions.size(); i++) {
            ...
            int x2 =
                    Math.min(
                            intersection.getX() + intersection.getW(),
                            current.getX() + current.getW());
            int y2 =
                    Math.min(
                            intersection.getY() + intersection.getH(),
    ...
    public List<Region> getRegionsByArea(boolean ascending) {
        ...
        return definedRegions.stream().sorted(comparator).collect(Collectors.toList());
    }
