 ...
 *
 * <p>This implementation uses multiple strategies to capture at physical resolution: 1. Robot with
 * DPI scaling compensation 2. FFmpeg gdigrab with forced resolution 3. PowerShell screen capture
 *
 * @since 1.1.0
 */
public class WindowsPhysicalCapture {
    ...
    public static BufferedImage captureWithScaledRobot() throws AWTException {
        ...
        BufferedImage logicalCapture =
                robot.createScreenCapture(new Rectangle(0, 0, logicalWidth, logicalHeight));

        // Scale up to physical resolution if needed
        if (dpiScale > 1.0) {
            BufferedImage physicalCapture =
                    new BufferedImage(physicalWidth, physicalHeight, BufferedImage.TYPE_INT_RGB);
            ...
            System.out.println(
                    "[WindowsPhysical] Scaled capture to: "
                            + physicalCapture.getWidth()
                            + "x"
                            + physicalCapture.getHeight());
    ...
    public static BufferedImage captureWithPowerShell() throws IOException {
        ...
        String script =
                String.format(
                        "[System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms');"
                            ...
                            + " $bounds = [System.Windows.Forms.Screen]::PrimaryScreen.Bounds; $bmp"
                            + " = New-Object System.Drawing.Bitmap($bounds.Width, $bounds.Height);"
                            + " $graphics = [System.Drawing.Graphics]::FromImage($bmp);"
                            + " $graphics.CopyFromScreen($bounds.Location,"
                            + " [System.Drawing.Point]::Empty, $bounds.Size); $bmp.Save('%s',"
        ...
        System.out.println(
                "[WindowsPhysical] PowerShell captured: "
                        + image.getWidth()
                        + "x"
                        + image.getHeight());
