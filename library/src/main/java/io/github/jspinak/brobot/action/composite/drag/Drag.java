 ...
 *
 * <p>This class provides a pure drag action that performs mouse drag operations between provided
 * locations. Unlike composite actions, it does NOT perform any Find operations - it expects the
 * source and target locations to be provided directly through ObjectCollections.
 *
 * <p><b>Usage Pattern:</b>
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>This pure implementation follows the Single Responsibility Principle: it only handles the drag
 * operation itself, not the finding of elements. This design eliminates circular dependencies and
 * makes the action more composable with other actions.
 *
 * @since 2.0
 * @see MouseDown
 * @see MoveMouse
...
public class Drag implements ActionInterface {
     ...
     *
     * <p>This method performs a pure drag operation by:
     *
     * <ol>
     *   <li>Extracting the source location from the first ObjectCollection
     *   <li>Extracting the target location from the second ObjectCollection
     *   <li>Moving to the source location
     *   <li>Pressing the mouse button
     *   <li>Moving to the target location (while holding the button)
     *   <li>Releasing the mouse button
     * </ol>
     *
     * <p><b>Important behaviors:</b>
     *
     * <ul>
     *   <li>Requires exactly 2 ObjectCollections with at least one location each
     *   <li>Uses the first location from each collection
     *   <li>Records the drag path as a Movement in the results
     *   <li>Returns matches for both source and target locations
     * </ul>
     *
     * @param actionResult The ActionResult to populate with execution results
     * @param objectCollections Requires exactly 2 collections: [0] = source location to drag from,
    ...
    public void perform(ActionResult actionResult, ObjectCollection... objectCollections) {
        ...
        MouseMoveOptions moveToSourceOptions =
                new MouseMoveOptions.Builder().setPauseAfterEnd(0.1).build();
        ...
        ObjectCollection sourceCol =
                new ObjectCollection.Builder().withLocations(sourceStateLocation).build();
        ...
        MouseMoveOptions moveToTargetOptions =
                new MouseMoveOptions.Builder().setPauseAfterEnd(0.1).build();
        ...
        ObjectCollection targetCol =
                new ObjectCollection.Builder().withLocations(targetStateLocation).build();
    ...
    /**
     * Extracts the first location from an ObjectCollection. Checks StateLocations, then
     * StateRegions, then matches.
     *
