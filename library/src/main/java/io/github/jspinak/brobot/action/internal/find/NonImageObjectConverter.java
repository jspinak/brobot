 ...
 *
 * <p>This component handles the conversion of various non-image elements (regions, locations, and
 * existing matches) from ObjectCollections into Match objects. Unlike image-based finding which
 * requires pattern matching, these objects are directly converted since their positions are already
 * known.
 *
 * <p>Non-image objects include:
 *
 * <ul>
 *   <li>{@link StateRegion} - Predefined regions on screen
 *   <li>{@link StateLocation} - Specific screen coordinates
 *   <li>Existing {@link Match} objects from previous operations
 * </ul>
 *
 * @see Match
...
public class NonImageObjectConverter {
     ...
     *
     * <p>This method processes regions, locations, and existing matches from the collection,
     ...
     * @return An ActionResult containing Match objects created from all non-image elements in the
     *     collection
     */
     ...
     *
     * <p>This method transfers all pre-existing match results from the collection to the provided
     * ActionResult. The matches parameter is modified by adding all results from the
     * ObjectCollection's matches.
     *
     * @param matches The ActionResult to which matches will be added. This object is modified by
     *     the method.
     ...
     *
     * <p>Each StateRegion is transformed into a Match object that preserves the region's
     * boundaries, anchor points, and associated state data. Note that text finding within regions
     * is handled separately and not part of this conversion process.
     *
     * @param matches The ActionResult to which region-based matches will be added. This object is
     ...
     *
     * <p>Each StateLocation represents a specific point on screen and is converted into a Match
     * object centered at that location. The original state data is preserved in the resulting
     * Match.
     *
     * @param matches The ActionResult to which location-based matches will be added. This object is
     *     modified by the method.
