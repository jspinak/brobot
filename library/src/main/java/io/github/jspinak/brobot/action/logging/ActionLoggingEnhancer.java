...
/**
 * Provides fluent API methods for adding comprehensive logging capabilities to ActionConfig
 * objects. This enhancer allows for before/after action logging, conditional logging, and custom
 * log levels.
 */
@Component
public class ActionLoggingEnhancer {
    ...
    public ActionConfig withBeforeActionLog(ActionConfig config, Consumer<ActionConfig> beforeLog) {
        LoggingConfiguration loggingConfig =
                loggingConfigs.computeIfAbsent(config, k -> LoggingConfiguration.defaultConfig());
        loggingConfig.setBeforeActionLog(beforeLog);
        return config;
    }

    /** Adds an after-action logging callback to the ActionConfig */
    public ActionConfig withAfterActionLog(
            ActionConfig config, BiConsumer<ActionConfig, ActionResult> afterLog) {
        LoggingConfiguration loggingConfig =
                loggingConfigs.computeIfAbsent(config, k -> LoggingConfiguration.defaultConfig());
    ...
    public void executeAfterLogging(ActionConfig config, ActionResult result) {
        ...
        if (loggingConfig != null && loggingConfig.getAfterActionLog() != null) {
            boolean shouldLog =
                    (result.isSuccess() && loggingConfig.isLogOnSuccess())
