 ...
 * 
 * <p>This implementation uses visual pattern matching to locate GUI elements
 * on screen. It orchestrates the ScreenCaptureService and PatternMatcher
 * to find elements without any dependency on Find or Action classes.</p>
 * 
 * <p>This class is the key to breaking circular dependencies. Actions that
 * previously depended on Find now depend on ElementLocator instead.</p>
 * 
 * <p>Key characteristics:
 * <ul>
 *   <li>Depends ONLY on core services (Layer 1)</li>
 *   <li>NO dependencies on Find, Actions, or any higher layers</li>
 *   <li>Thread-safe through stateless operations</li>
 *   <li>Supports all find strategies (ALL, BEST, FIRST, EACH)</li>
 * </ul>
 * </p>
 * 
 * @since 2.0.0
...
public class VisualElementLocator implements ElementLocator {
    ...
    public VisualElementLocator(PatternMatcher patternMatcher, 
    ...
    public boolean verifyElement(Element element) {
        ...
        List<PatternMatcher.MatchResult> results = patternMatcher.findPatterns(regionImage, pattern, options);
    ...
    private List<Element> findPatternElements(BufferedImage screen, Pattern pattern, LocateRequest request) {
        ...
        // Find matches
        List<PatternMatcher.MatchResult> results = patternMatcher.findPatterns(screen, pattern, options);
    ...
    private List<Element> applyStrategy(List<Element> allElements, LocateRequest request) {
        ...
        switch (request.getStrategy()) {
            ...
            case BEST:
                ...
                Element best = allElements.stream()
