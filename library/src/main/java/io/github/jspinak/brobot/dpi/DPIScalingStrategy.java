 ...
 * 
 * Key concepts:
 * - Physical pixels: Actual screen pixels (what SikuliX IDE captures)
 * - Logical pixels: DPI-scaled pixels (what Windows/applications report)
 * - Pattern scaling: Adjusting patterns to match screen capture dimensions
 * 
 * Windows with 125% scaling:
 * - Logical 100x100 = Physical 125x125
 * - Pattern scale factor = 1/1.25 = 0.8
 */
public class DPIScalingStrategy {
     ...
     */
    public static PatternSource analyzePatternSource(BufferedImage image, 
        ...
        if (Math.abs(actualWidth - expectedLogicalWidth) <= expectedLogicalWidth * tolerance &&
            Math.abs(actualHeight - expectedLogicalHeight) <= expectedLogicalHeight * tolerance) {
        ...
        if (Math.abs(actualWidth - expectedPhysicalWidth) <= expectedPhysicalWidth * tolerance &&
            Math.abs(actualHeight - expectedPhysicalHeight) <= expectedPhysicalHeight * tolerance) {
    ...
    public static void printDiagnostics() {
        ...
        System.out.println("  Physical pixels: " + (int)(100 * displayScale) + "x" + (int)(100 * displayScale));
        ...
        if (Math.abs(displayScale - 1.0) < 0.01) {
            ...
            System.out.println("  - SikuliX IDE patterns: Use Settings.AlwaysResize = " + patternScale);
