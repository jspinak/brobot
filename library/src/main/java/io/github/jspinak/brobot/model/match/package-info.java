 ...
 * 
 * <p>This package provides data structures for representing pattern matching
 * results in the Brobot framework. Matches are the fundamental output of find
 * operations, containing location data, similarity scores, and metadata about
 * what was found on the screen.</p>
 * 
 * <h2>Core Components</h2>
 * 
 * <ul>
 *   <li>{@link io.github.jspinak.brobot.model.match.Match} - 
 *       Primary result object containing location, score, and metadata</li>
 *   <li>{@link io.github.jspinak.brobot.model.match.EmptyMatch} - 
 *       Represents unsuccessful match attempts with failure context</li>
 * </ul>
 * 
 * <h2>Match Structure</h2>
 * 
 * <p>A Match contains comprehensive information about a successful pattern match:</p>
 * <ul>
 *   <li><b>Location</b> - Screen coordinates where pattern was found</li>
 *   <li><b>Region</b> - Bounding box of the matched area</li>
 *   <li><b>Score</b> - Similarity score (0.0 to 1.0)</li>
 *   <li><b>StateObjectData</b> - Reference to source pattern and state</li>
 *   <li><b>Timestamp</b> - When the match was found</li>
 *   <li><b>Scene</b> - Screenshot context (optional)</li>
 *   <li><b>Text</b> - OCR text if applicable</li>
 * </ul>
 * 
 ...
 * <ul>
 *   <li>1.0 = Perfect match (pixel-identical)</li>
 *   <li>0.95+ = Excellent match (imperceptible differences)</li>
 *   <li>0.90+ = Good match (minor variations)</li>
 *   <li>0.80+ = Acceptable match (noticeable differences)</li>
 *   <li>&lt;0.80 = Poor match (significant differences)</li>
 * </ul>
 * 
 * <h3>Target vs Region</h3>
 * <p>Matches distinguish between the matched area and interaction point:</p>
 * <ul>
 *   <li><b>Region</b> - The full area that matched the pattern</li>
 *   <li><b>Target</b> - The specific point for interaction (often center, but configurable)</li>
 * </ul>
 * 
 * <pre>{@code
 ...
 * <h2>Empty Matches</h2>
 * 
 * <p>EmptyMatch provides context for failed searches:</p>
 * <pre>{@code
 * if (result.isEmpty()) {
 ...
 * 
 * <p>Matches carry rich metadata for analysis:</p>
 * <pre>{@code
 * // State association
 * String ownerState = match.getOwnerStateName();
 ...
 * <p>Use matches as reference points for relative operations:</p>
 * <pre>{@code
 ...
 * 
 * <ol>
 *   <li>Always check match scores before using results</li>
 *   <li>Use EmptyMatch instead of null for failed searches</li>
 *   <li>Validate matches are within expected screen regions</li>
 *   <li>Log match scores for debugging pattern quality</li>
 *   <li>Consider match context (state, timestamp) for validation</li>
 *   <li>Use appropriate score thresholds for different use cases</li>
 * </ol>
 * 
 * <h2>Thread Safety</h2>
 * 
 * <p>Match objects are designed to be immutable after creation, making them
 * thread-safe for concurrent access. The builder pattern should be used by
 * single threads only. EmptyMatch is a singleton and inherently thread-safe.</p>
 * 
 * @since 1.0
