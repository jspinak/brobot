 ...
 * 
 * This class encapsulates metrics functionality that was
...
public class ActionMetrics {
    ...
    public long getOverheadTimeMs() {
        long totalPhaseTime = phaseTimings.values().stream()
     ...
     * Based on match confidence and retry overhead.
     * 
    ...
    public void merge(ActionMetrics other) {
        if (other != null) {
            ...
            // Merge phase timings
            other.phaseTimings.forEach((phase, time) -> 
                phaseTimings.merge(phase, time, Long::sum));
            other.phaseCounts.forEach((phase, count) -> 
                phaseCounts.merge(phase, count, Integer::sum));
        }
    }
    ...
    public String formatPerformance() {
        ...
        if (matchCount > 0) {
            sb.append(String.format(", Matches: %d (best: %.2f)", 
                matchCount, bestMatchConfidence));
        }
        
        if (hasRetries()) {
            sb.append(String.format(", Retries: %d (%dms, %.1f%%)", 
    ...
    public String formatPhases() {
        ...
        phaseTimings.entrySet().stream()
                ...
                .forEach(entry -> {
                    ...
                    sb.append(String.format("  %s: %dms (%.1f%%) x%d",
