 ...
 * BufferedImage class, bridging the gap between various image representations used throughout the
 * framework. It handles conversions, screen captures, file operations, and format transformations
 * while abstracting platform-specific details.
 *
 * <p>Key operation categories:
 *
 * <ul>
 *   <li><b>File Operations</b>: Loading images from disk with platform abstraction
 *   <li><b>Screen Capture</b>: Capturing screen regions with headless/mock support
 *   <li><b>Format Conversion</b>: Converting between BufferedImage, Mat, and byte arrays
 *   <li><b>Image Manipulation</b>: Extracting sub-images and type conversions
 *   <li><b>Serialization</b>: Base64 encoding/decoding for data transmission
 * </ul>
 *
 * <p>Platform abstraction features:
 *
 * <ul>
 *   <li>Uses SikuliX for cross-platform file loading when available
 *   <li>Falls back to direct file reading in headless environments
 *   <li>Provides dummy images for mock/test scenarios
 *   <li>Handles various image formats with automatic PNG extension
 * </ul>
 *
 * <p>Integration points:
 *
 * <ul>
 *   <li><b>SikuliX</b>: Leverages Pattern class for platform-independent file access
 *   <li><b>OpenCV/JavaCV</b>: Converts between Mat and BufferedImage formats
 *   <li><b>Screen Capture</b>: Integrates with Screen class for region capture
 *   <li><b>Environment Awareness</b>: Adapts behavior based on BrobotEnvironment
 * </ul>
 *
 * <p>Common use cases:
 *
 * <ul>
 *   <li>Loading pattern images from files for visual matching
 *   <li>Capturing screen regions for analysis or comparison
 *   <li>Converting images for OpenCV processing
 *   <li>Serializing images for network transmission or storage
 *   <li>Extracting regions of interest from larger images
 * </ul>
 *
 * <p>In the model-based approach, BufferedImageUtilities serves as the foundation for all image
 * handling operations. By providing consistent interfaces and handling edge cases like headless
 * environments, it enables the framework to work reliably across different deployment scenarios
 * while maintaining clean separation between image processing and automation logic.
 *
 * @since 1.0
 * @see Image
 * @see Pattern
...
public class BufferedImageUtilities {
    ...
    private static final long MONITOR_LOG_INTERVAL =
            60000; // Log monitor info at most once per minute
    ...
    private static boolean isRunningInWSL() {
        try {
            ...
            return osName.contains("linux")
                    && (osVersion.contains("microsoft")
                            || osVersion.contains("wsl")
                            || new File("/proc/version").exists()
                                    && java.nio.file.Files.readString(
                                                    java.nio.file.Paths.get("/proc/version"))
                                            .toLowerCase()
                                            .contains("microsoft"));
        } catch (Exception e) {
            return false;
    ...
    /**
     * To use this method, you need to deal with paths. In SikuliX, there is a bundle path. SikuliX
     * has code to abstract the OS. If you want Brobot to be platform independent, it should use
     * SikuliX as much as possible to retrieve BufferedImages from file.
     *
    ...
    public static BufferedImage getBufferedImageFromScreen(Region region) {
        ...
        try {
            ...
            // Use UnifiedCaptureService if available, which respects the configured capture
            // provider
    ...
    public BufferedImage getBuffImgFromScreen(Region region) {
        ...
        if (!environmentLogged) {
            ...
            log.debug(
                    "[SCREEN_CAPTURE] canCaptureScreen: {}, hasDisplay: {}, mockMode: {}",
        ...
        try {
            ...
            if (now - lastMonitorLogTime > MONITOR_LOG_INTERVAL) {
                log.debug(
                        "[SCREEN_CAPTURE] Using screen {} to capture region {}",
                        screen.getID(),
                        region);
            ...
            if (!environmentLogged) {
                log.debug(
                        "[SCREEN_CAPTURE] Successfully captured screen: {}x{}",
                        captured.getWidth(),
                        captured.getHeight());
            }
            return captured;
    ...
    public BufferedImage convert(Mat mat) {
        try {
            ...
            // Delete the temp file (may fail if file is still in use, but that's acceptable
            // for temp files)
            temp.delete();
    ...
    public static BufferedImage base64StringToImage(String base64String) {
        ...
        try {
            byte[] imageBytes =
                    Base64.getDecoder().decode(base64String); // Decode Base64 String to byte array
            ByteArrayInputStream bis =
                    new ByteArrayInputStream(imageBytes); // Create ByteArrayInputStream from the
            // byte array
            BufferedImage image =
                    null; // Read image from ByteArrayInputStream and create BufferedImage
            try {
