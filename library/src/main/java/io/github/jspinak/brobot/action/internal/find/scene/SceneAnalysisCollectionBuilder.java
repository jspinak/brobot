...
/**
 * Orchestrates the creation and population of scene analysis collections for
 * color-based matching.
 * 
 * <p>
 * SceneAnalysisCollectionBuilder serves as the entry point for comprehensive
 * color analysis
 * workflows in the Brobot framework. It coordinates scene acquisition, target
 * selection,
 * and pixel-level analysis to produce collections ready for matching
 * operations.
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * When scene analysis is not required (non-color matching), scenes are
 * simply wrapped in SceneAnalysis objects without pixel processing.
 * </p>
 * 
 * @see SceneAnalyses
...
public class SceneAnalysisCollectionBuilder {
     ...
     * 
     * <p>
     * When color analysis is not required (non-COLOR find operations),
     * scenes are wrapped without pixel processing for efficiency.
     * </p>
     * 
     * <p>
     ...
     * </p>
     * 
     * @param objectCollections      collections containing targets, context, and
     *                               scenes
     * @param scenesToCapture        number of screenshots to take if no scenes
     *                               provided
     * @param secondsBetweenCaptures delay between multiple screenshots
     * @param actionConfig           configuration including find type and
     *                               thresholds
    ...
    public SceneAnalyses get(List<ObjectCollection> objectCollections,
        ...
        List<Scene> scenes = getScenes.getScenes(actionConfig, objectCollections, scenesToCapture,
        ...
        for (Scene scene : scenes) {
            SceneAnalysis sceneAnalysis = analyzePixels.getAnalysisForOneScene(scene, targetImages, allImages,
                    actionConfig);
            sceneAnalysisCollection.add(sceneAnalysis);
        }
        return sceneAnalysisCollection;
     ...
     * 
     * <p>
     * These images are used for classification but not for matching.
     ...
     * 
     * <p>
     * Target images are the primary patterns to find in the scene.
     * These images will be both classified and matched, generating
     * concrete Match objects when found.
     * </p>
     * 
     * <p>
     * The first ObjectCollection by convention contains targets.
     * An empty first collection indicates classify-only mode with
     * no specific matching targets.
     * </p>
     * 
     ...
     * 
     * <p>
     * Scene analysis is only required when using COLOR-based
