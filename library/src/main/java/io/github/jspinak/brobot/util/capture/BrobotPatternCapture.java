...
/**
 * Brobot Pattern Capture Tool - Alternative to SikuliX IDE for capturing patterns. This tool
 * captures patterns with metadata about the environment to ensure they work correctly across
...
public class BrobotPatternCapture {
    ...
    private void savePatternWithMetadata(BufferedImage image, String name, PatternMetadata metadata)
            throws IOException {
    ...
    private void savePNGWithMetadata(BufferedImage image, File file, PatternMetadata metadata)
            throws IOException {
    ...
    /** Create scaled versions for common DPI settings. */
    private void createScaledVersions(
            BufferedImage original, String name, PatternMetadata metadata) {
        ...
        for (double scale : scales) {
            ...
            try {
                ...
                log.debug("Created scaled version: {} ({}x{})", scaledName, newWidth, newHeight);
            } catch (IOException e) {
