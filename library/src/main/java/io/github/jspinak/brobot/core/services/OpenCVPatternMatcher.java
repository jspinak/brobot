 ...
 *
 * <p>This implementation uses Sikuli's Finder (which wraps OpenCV) for pattern matching operations.
 * It is completely independent of the Find action and other high-level Brobot components, providing
 * pure pattern matching functionality.
 *
 ...
 *   <li>Properly manages Finder resources
...
public class OpenCVPatternMatcher implements PatternMatcher {
    ...
    @Override
    public List<MatchResult> findPatterns(
            BufferedImage screen, Pattern pattern, MatchOptions options) {
        // Validate inputs
    ...
    @Override
    public List<MatchResult> findPatternsInRegion(
    ...
    public boolean supportsPattern(Pattern pattern) {
        ...
        return pattern != null && pattern.getImage() != null && !pattern.getImage().isEmpty();
    }
