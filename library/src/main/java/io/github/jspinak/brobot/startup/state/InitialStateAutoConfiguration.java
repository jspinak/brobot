 ...
 * 
 * <p>
 * This configuration automatically handles states marked with @State(initial =
 * true)
 * by creating the necessary startup configuration and activating initial states
 ...
 * </p>
 * <ul>
 * <li>Automatic BrobotStartupConfiguration creation from annotations</li>
 * <li>ApplicationReadyEvent listener for state activation</li>
 * <li>Configurable delay before verification in real mode</li>
 * <li>Profile-specific behavior (test vs production)</li>
 * <li>Opt-out capability via properties</li>
 * </ul>
 * 
 * <p>
...
public class InitialStateAutoConfiguration {
    ...
    /**
     * Creates BrobotStartupConfiguration automatically from states marked
     * with @State(initial = true).
     * Applications can override this by providing their own
     * BrobotStartupConfiguration bean.
     * 
    ...
    public StartupConfiguration brobotStartupConfiguration() {
        log.info("Auto-configuring BrobotStartupConfiguration from @State(initial = true) annotations");

        StartupConfiguration config = new StartupConfiguration();

        // Enable verification if initial states are registered
        config.setVerifyInitialStates(verifyInitialStates && initialStates.hasRegisteredInitialStates());
        ...
        if (!registeredStates.isEmpty()) {
            ...
            log.info("Found {} initial states from annotations: {}",
        ...
        log.info("Initial state configuration: verify={}, delay={}s, fallback={}, activateFirstOnly={}",
                config.isVerifyInitialStates(), config.getStartupDelay(),
                config.isFallbackSearch(), config.isActivateFirstOnly());
     ...
     * This listener is triggered after all beans are initialized and the
     * application is fully started.
     * 
     * <p>
     * The activation includes a configurable delay for real mode to allow the GUI
     * to stabilize
    ...
    @ConditionalOnProperty(value = "brobot.startup.auto-activate", havingValue = "true", matchIfMissing = true)
    public void autoActivateInitialStates(ApplicationReadyEvent event) {
        ...
        try {
            ...
            log.info("Searching for initial states: {}", initialStates.getRegisteredInitialStates());
            initialStates.findInitialStates();

            // Log results
            if (!stateMemory.getActiveStates().isEmpty()) {
                log.info("✅ Successfully activated initial states: {}",
    ...
    /**
     * Test profile specific configuration that optimizes initial state handling for
     * tests.
     */
    @Configuration
