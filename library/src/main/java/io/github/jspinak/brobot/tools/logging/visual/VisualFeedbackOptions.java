...
/**
 * Options for visual feedback during action execution. Allows fine-grained control over
 * highlighting behavior for individual actions.
 *
 * <p>This class provides a builder pattern for creating custom visual feedback configurations that
 * can override the global settings for specific actions.
 *
...
public class VisualFeedbackOptions {
    ...
    /** Whether highlighting is enabled for this action. Default: true */
    @Builder.Default private boolean highlightEnabled = true;

    /** Whether to highlight successful finds. Default: true */
    @Builder.Default private boolean highlightFinds = true;

    /** Whether to highlight search regions. Default: false */
    @Builder.Default private boolean highlightSearchRegions = false;

    /** Whether to highlight errors/failures. Default: false */
    @Builder.Default private boolean highlightErrors = false;
    ...
    /** Whether to use flashing effect for highlights. Default: false */
    @Builder.Default private boolean flashHighlight = false;

    /** Number of times to flash if enabled. Default: 2 */
    @Builder.Default private int flashCount = 2;

    /** Whether to show match score as overlay text. Default: false */
    @Builder.Default private boolean showMatchScore = false;

    /** Whether to persist highlights until manually cleared. Default: false */
    @Builder.Default private boolean persistHighlight = false;
    ...
    public static VisualFeedbackOptions none() {
        return VisualFeedbackOptions.builder().highlightEnabled(false).build();
    }
    ...
    /**
     * Merges these options with global configuration. Custom values in this object override global
     * settings.
     *
