 ...
 *
 * <ul>
 *   <li>Mock mode - Use fake data for testing workflows
 *   <li>Display availability - Whether screen capture operations are possible
 *   <li>Headless mode - Running without a GUI (can still process files)
 * </ul>
 *
 * <p><strong>Default Headless Configuration:</strong><br>
 * Brobot automatically sets {@code java.awt.headless=false} during class initialization to enable
 * GUI automation by default. This overrides any existing headless setting (including those set by
 * build tools like Gradle). Applications that specifically need to preserve the original headless
 * setting can use:
 *
 * <pre>{@code
...
public class ExecutionEnvironment {
        ...
        if ("unit".equals(testType) || "true".equals(System.getProperty("brobot.test.mode"))) {
            ...
            if ("true".equals(preserveHeadless)) {
                ...
                if (currentHeadless != null && !"false".equals(currentHeadless)) {
                    log.info(
                            "Overrode java.awt.headless from '{}' to 'false' for GUI automation."
                                    + " Set -Dbrobot.preserve.headless.setting=true to preserve"
                                    + " original setting.",
                            currentHeadless);
                } else {
    ...
    /**
     * Checks if Brobot is running in mock mode. In mock mode, operations return predefined fake
     * data instead of performing real actions.
     *
    ...
    /**
     * Checks if a display is available for GUI operations. This determines whether screen capture
     * and GUI automation are possible. Results are cached to avoid repeated expensive checks during
     * automation.
     *
    ...
    public boolean hasDisplay() {
        ...
        if (cachedHasDisplay != null
                && (currentTime - lastDisplayCheckTime) < DISPLAY_CHECK_CACHE_DURATION) {
    ...
    /**
     * Forces a refresh of the display check cache. Useful when the environment might have changed.
     */
    ...
    /**
     * Performs the actual display check without caching. This method contains the original display
     * checking logic.
     *
     * @return true if display is available
     */
        ...
        if (isMac) {
            ...
            try {
                ...
                if (javaHeadless) {
                    if (cachedHasDisplay == null) {
                        log.debug(
                                "[DISPLAY_CHECK] macOS: Java reports headless, cannot use display");
                    }
                    return false;
                }
                ...
                if (notInCI) {
                    if (cachedHasDisplay == null) {
                        log.debug(
                                "[DISPLAY_CHECK] macOS: Not in CI and not headless, assuming"
                                        + " display available");
                    }
                    return true;
                }
                ...
                if (cachedHasDisplay == null) {
                    log.debug(
                            "[DISPLAY_CHECK] macOS: Error checking GraphicsEnvironment: {}",
                            e.getMessage());
                }
                return false;
            }
            ...
            try {
                ...
                if (cachedHasDisplay == null) {
                    log.debug(
                            "[DISPLAY_CHECK] macOS CI: HeadlessException caught, assuming no"
                                    + " display in CI");
                }
                return false;
            } catch (Exception e) {
    ...
    /**
     * Checks if screen capture operations are allowed. This requires both display availability and
    ...
    public boolean canCaptureScreen() {
        ...
        if (!result) {
            log.debug(
                    "[CAPTURE_CHECK] Cannot capture screen - hasDisplay: {}, allowScreenCapture:"
                            + " {}, mockMode: {}",
                    hasDisp,
                    allowScreenCapture,
                    mockMode);
    ...
    /**
     * Checks if image file operations should use real files. File operations work in headless mode
     * but not in mock mode.
     *
    ...
    /**
     * Checks if SikuliX operations should be skipped. This is true when either in mock mode or when
    ...
    public String getEnvironmentInfo() {
        return String.format(
                "ExecutionEnvironment[mockMode=%s, hasDisplay=%s, canCaptureScreen=%s,"
                        + " useRealFiles=%s]",
    ...
    public static class Builder {
        ...
        /**
         * Forces headless mode regardless of actual display availability. Useful for testing
         * headless behavior on systems with displays.
         *
         * @param forceHeadless true to force headless, false to force display mode, null to
         *     auto-detect
         * @return this builder
         */
         ...
         *
         * <p>Environment variables: - BROBOT_MOCK_MODE=true/false -
         * BROBOT_FORCE_HEADLESS=true/false - BROBOT_ALLOW_SCREEN_CAPTURE=true/false
         *
