 ...
 *
 * <p>This class implements an "outside" anchor strategy where the resulting region expands to
 * include all anchor points from matches and locations. Unlike {@link DefineInsideAnchors} which
 * creates the smallest rectangle, this creates a bounding box by expanding outward to ensure all
 * anchors are contained.
 *
 * <p>The algorithm works by:
 *
 * <ul>
 *   <li>Initializing a 1x1 region at the first anchor point
 *   <li>Expanding the region boundaries outward for each subsequent anchor
 *   <li>Including any additional locations specified in DefineRegionOptions
 *   <li>Applying final adjustments from DefineRegionOptions
 * </ul>
 *
 * <p>This strategy is useful when:
 *
 * <ul>
 *   <li>You need a region that guarantees inclusion of all reference points
 *   <li>Creating safe boundaries around a set of UI elements
 *   <li>Defining an area for operations that must not miss any anchors
 * </ul>
 *
...
public class DefineOutsideAnchors implements ActionInterface {
    ...
    public DefineOutsideAnchors(RegionDefiner defineHelper) {
     ...
     *   <li>Returns early with appropriate message if no matches are found
     *   <li>Initializes a region from the first anchor point
     *   <li>Expands the region to include all other anchor points
     *   <li>Further expands to include any specified locations
     *   <li>Applies final adjustments from DefineRegionOptions
     *   <li>Adds the expanded region to the ActionResult
     * </ol>
     *
     * @param matches The ActionResult containing DefineRegionOptions and to which the defined
     *     region will be added. Output text is set to describe the result or indicate no matches
     *     were found.
     * @param objectCollections The collections containing objects to find. Anchor points from
     *     matches in these collections determine the region boundaries.
     */
    @Override
     ...
     *
     * <ul>
     *   <li>The first explicit anchor if the match has custom anchors defined
     *   <li>The match's default anchor position if no custom anchors exist
     * </ul>
     *
    ...
    private Region initializeRegionFromFirstAnchor(ActionResult matches) {
        ...
        if (!firstMatch.getAnchors().getAnchorList().isEmpty()) {
            ...
            return new Region(
                    anchorLocation.getCalculatedX(), anchorLocation.getCalculatedY(), 1, 1);
        }
     ...
     *
     * <ul>
     *   <li>Processes all explicit anchors if defined
     *   <li>Uses the match's default anchor if no explicit anchors exist
     *   <li>Expands the region in the necessary directions to include each point
     * </ul>
     *
     * <p>The region is modified in-place through calls to {@link #expandRegionToPoint}.
     *
            ...
            if (!match.getAnchors().getAnchorList().isEmpty()) {
                match.getAnchors()
                        ...
                        .forEach(
                                anchor -> {
                                    Location anchorLocation =
                                            new Location(match, anchor.getPositionInMatch());
                                    expandRegionToPoint(
                                            region,
                                            anchorLocation.getCalculatedX(),
                                            anchorLocation.getCalculatedY());
                                });
            } else {
                // Use the match's target as the default anchor
                Location defaultLocation = match.getTarget();
                expandRegionToPoint(
                        region, defaultLocation.getCalculatedX(), defaultLocation.getCalculatedY());
            }
        }
    }
     ...
     *
     * <ul>
     *   <li>Moving the left boundary left if the point is to the left
     *   <li>Extending the width if the point is to the right
     *   <li>Moving the top boundary up if the point is above
     *   <li>Extending the height if the point is below
     * </ul>
     *
     * <p>The +1 adjustments ensure the point is fully contained within the region boundaries, not
