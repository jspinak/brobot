...
public class DPIScalingDetector {
    ...
    public static float detectScalingFactor() {
        ...
        if (io.github.jspinak.brobot.config.ExecutionEnvironment.getInstance().isMockMode()) {
            // In mock mode, return no scaling
            System.out.println("[DPI DETECTOR] Mock mode detected - returning 1.0 (no scaling)");
        ...
        try {
            ...
            return detectScalingFactorForBounds(logicalBounds, displayMode);
        } catch (Exception e) {
    ...
    static float detectScalingFactorForBounds(Rectangle logicalBounds, DisplayMode displayMode) {
        if (logicalBounds == null || displayMode == null) {
            return 1.0f;
        }
    ...
    /**
     * Applies scaling to Settings.AlwaysResize if needed.
     * This method checks if scaling is detected and applies it.
     */
    public static void applyScalingIfNeeded() {
        float factor = detectScalingFactor();
        if (Math.abs(factor - 1.0f) > 0.01) {
            System.out.println("[DPI DETECTOR] Applying scaling factor: " + factor);
            // In a real implementation, this would set Settings.AlwaysResize = factor
            // For now, we just log it
        }
    }
    
    /**
     * Outputs debug information about the current scaling configuration.
     */
    public static void debugScaling() {
        System.out.println("[DPI DETECTOR] Debug Information:");
        System.out.println("  Scaling factor: " + detectScalingFactor());
        System.out.println("  Has scaling: " + hasScaling());
        System.out.println("  Description: " + getScalingDescription());
    }
}