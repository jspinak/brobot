 ...
 * 
 * <p>This component bridges the gap between action execution and state management
 ...
 * are currently active based on visual evidence.</p>
 * 
 * @since 1.1
 */
@Component
...
public class StateMemoryUpdater {
    ...
    public void updateFromActionResult(ActionResult actionResult) {
        ...
        for (Match match : actionResult.getMatchList()) {
            if (match != null && match.getStateObjectData() != null) {
                ...
                if (ownerStateName != null && !ownerStateName.isEmpty()) {
                    ...
                    stateService.getState(ownerStateName).ifPresent(state -> {
        ...
        for (Long stateId : stateIdsToActivate) {
            if (!stateMemory.getActiveStates().contains(stateId)) {
                log.debug("Activating state {} based on found match", 
        ...
        if (!stateIdsToActivate.isEmpty()) {
            log.info("Updated active states based on {} found matches. Active states: {}", 
    ...
    public void updateFromMatch(Match match) {
        ...
        stateService.getState(ownerStateName).ifPresent(state -> {
            Long stateId = state.getId();
            if (stateId != null && stateId > 0 && !stateMemory.getActiveStates().contains(stateId)) {
                log.debug("Activating state {} ({}) based on found match", ownerStateName, stateId);
                stateMemory.addActiveState(stateId);
            }
        });
    }
