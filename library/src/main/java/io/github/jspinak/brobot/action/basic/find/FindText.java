 ...
 *
 * <p>This component specializes in locating and extracting text from screenshots, supporting the
 * ALL_WORDS find strategy. It performs comprehensive text detection across multiple scenes,
 * organizing results into scene analyses for structured access to text matches.
 *
 * <p>The text finding process includes:
 *
 * <ul>
 *   <li>Scene acquisition based on action options
 *   <li>OCR processing to detect all text regions
 *   <li>Region filtering based on search constraints
 *   <li>Scene-specific match aggregation
 *   <li>Action lifecycle management for proper execution control
 * </ul>
 *
 * <p>This component is essential for:
 *
 * <ul>
 *   <li>GUI automation that needs to interact with text elements
 *   <li>Screen content validation based on text presence
 *   <li>Data extraction from application interfaces
 *   <li>Text-based navigation and state detection
 * </ul>
 *
 * @see FindAll
 * @see SceneAnalysis
...
public class FindText {
     ...
     *
     * <p>This method implements a repetitive text search strategy that continues acquiring and
     * processing scenes until the action lifecycle indicates completion. This approach is useful
     * for:
     *
     * <ul>
     *   <li>Monitoring changing text content over time
     *   <li>Waiting for specific text to appear
     *   <li>Capturing text from dynamic interfaces
     * </ul>
     *
     * <p>The method respects action lifecycle constraints, allowing for:
     *
     * <ul>
     *   <li>Maximum repetition limits
     *   <li>Timeout conditions
     *   <li>Early termination when sufficient matches are found
     * </ul>
     *
     * @param matches The ActionResult to populate with text matches and scene analyses. Modified
     *     throughout execution with cumulative results.
     * @param objectCollections Collections containing configuration for scene acquisition. The
     *     first collection's state image count is used for lifecycle management.
     */
    void findAllWordMatches(ActionResult matches, List<ObjectCollection> objectCollections) {
        while (actionLifecycleManagement.isOkToContinueAction(
                matches, objectCollections.get(0).getStateImages().size())) {
            List<Scene> scenes =
                    getScenes.getScenes(matches.getActionConfig(), objectCollections, 1, 0);
     ...
     *
     * <p>This method processes each scene to detect all text elements, creating both a global
     * collection of matches and scene-specific analyses. Each scene analysis maintains its own
     * match list, enabling scene-by-scene text examination.
     *
     * <p>The method performs the following for each scene:
     *
     * <ol>
     *   <li>Executes OCR to find all text words
     *   <li>Filters results based on action options (search regions)
     *   <li>Adds matches to the global result set
     *   <li>Creates a SceneAnalysis with scene-specific matches
