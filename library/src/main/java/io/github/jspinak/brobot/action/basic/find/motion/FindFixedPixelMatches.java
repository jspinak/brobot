...
/**
 * Finds matches based on fixed (unchanging) pixels across multiple patterns. This class identifies
 * regions that remain constant across different images, which is useful for finding stable UI
 * elements or static backgrounds.
 *
 * <p>The process involves:
 *
 * <ul>
 *   <li>Collecting all patterns from object collections
 *   <li>Creating a fixed pixel mask showing unchanging areas
 *   <li>Finding contours in the mask to identify distinct regions
 *   <li>Validating matches against search regions and area constraints
 * </ul>
 *
 * <p>This approach is particularly effective for:
 *
 * <ul>
 *   <li>Identifying static UI elements in dynamic interfaces
 *   <li>Finding stable reference points for navigation
 *   <li>Detecting unchanging background regions
 * </ul>
 *
...
public class FindFixedPixelMatches {
    ...
    /**
     * Finds matches based on pixels that remain fixed across multiple patterns. This method creates
     * a mask of unchanging pixels and uses contour detection to identify distinct regions of fixed
     * content.
     *
     * <p>Side effects:
     *
     * <ul>
     *   <li>Sets the mask in the ActionResult to the fixed pixel mask
     *   <li>Populates the ActionResult with matches for each fixed region found
     * </ul>
     *
    ...
    /**
     * Extracts all patterns from the provided object collections. Collects patterns from both
     * StateImages and Scenes within each collection.
     *
    ...
    /**
     * Creates matches from contours found in the provided mask. Each contour represents a region of
     * fixed pixels that could be a match. Matches are validated against area constraints and search
     * regions before being added to the results.
     *
     * <p>Side effects:
     *
     * <ul>
     *   <li>Adds validated matches to the ActionResult
     *   <li>Sets the search image for each match to the provided pattern
     * </ul>
     *
