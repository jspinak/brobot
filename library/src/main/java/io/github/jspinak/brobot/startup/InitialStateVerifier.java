 ...
 * 
 * <p>This component provides a clean API for verifying and establishing initial states
 * when automation begins. It supports both real execution and mock testing scenarios.</p>
 * 
 * <p>Key features:
 * <ul>
...
public class InitialStateVerifier {
     ...
     * 
     * <p>In normal mode, searches for the states on screen in the order provided.
     * In mock mode, randomly selects one state with equal probability.</p>
     * 
    ...
    public class VerificationBuilder {
        ...
        private boolean verifyReal() {
            ...
            foundStates.forEach(stateId -> {
                ...
                stateService.getState(stateId).ifPresent(state ->
                    log.info("Verified and activated state '{}' (ID: {})", state.getName(), stateId)
                );
            });
        ...
        private boolean searchAllStates() {
            ...
            foundStates.forEach(stateId -> {
                stateMemory.addActiveState(stateId);
                stateService.getState(stateId).ifPresent(state ->
                    log.info("Found state '{}' during fallback search", state.getName())
                );
            });
