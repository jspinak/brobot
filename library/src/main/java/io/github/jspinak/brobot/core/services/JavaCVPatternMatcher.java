 ...
 *
 * <p>This implementation bypasses SikuliX entirely and uses JavaCV's OpenCV bindings directly for
 * template matching. It provides the same functionality as SikuliX's Finder but with direct control
...
public class JavaCVPatternMatcher implements PatternMatcher {
    ...
    @Override
    public List<PatternMatcher.MatchResult> findPatterns(
    ...
    @Override
    public List<PatternMatcher.MatchResult> findPatternsInRegion(
            BufferedImage screen,
            io.github.jspinak.brobot.model.element.Pattern pattern,
            int regionX,
            int regionY,
        ...
        for (PatternMatcher.MatchResult result : results) {
            adjustedResults.add(
                    new PatternMatcher.MatchResult(
                            result.getX() + regionX,
                            result.getY() + regionY,
                            result.getWidth(),
                            result.getHeight(),
    ...
    private List<PatternMatcher.MatchResult> performTemplateMatching(
            Mat screen, Mat pattern, PatternMatcher.MatchOptions options) {
    ...
    private List<PatternMatcher.MatchResult> findAllMatches(
            Mat result, Mat pattern, PatternMatcher.MatchOptions options) {
            ...
            for (int x = 0; x < result.cols(); x++) {
                ...
                if (score >= threshold) {
                    ...
                    if (!isSupressed(matches, x, y, pattern.cols(), pattern.rows())) {
                        matches.add(
                                new PatternMatcher.MatchResult(
                                        x, y, pattern.cols(), pattern.rows(), score));
    ...
    private List<PatternMatcher.MatchResult> findBestMatch(
            Mat result, Mat pattern, PatternMatcher.MatchOptions options) {
        List<PatternMatcher.MatchResult> matches = new ArrayList<>();
        ...
        if (score >= options.getSimilarity()) {
            matches.add(
                    new PatternMatcher.MatchResult(
                            maxLoc.x(), maxLoc.y(), pattern.cols(), pattern.rows(), score));
        }
    ...
    private boolean isSupressed(
            List<PatternMatcher.MatchResult> existing, int x, int y, int width, int height) {
        // Check if this match overlaps significantly with existing matches
        for (PatternMatcher.MatchResult match : existing) {
            int overlapX =
                    Math.max(
                            0,
                            Math.min(x + width, match.getX() + match.getWidth())
                                    - Math.max(x, match.getX()));
            int overlapY =
                    Math.max(
                            0,
                            Math.min(y + height, match.getY() + match.getHeight())
                                    - Math.max(y, match.getY()));
