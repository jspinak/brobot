...
/**
 * Diagnostic runner for testing image loading capabilities. Enabled via property:
...
public class ImageLoadingDiagnosticsRunner {
    ...
    private void printRecommendations() {
        ...
        long failureCount =
                loadHistory != null
                        ? loadHistory.values().stream().filter(r -> !r.isSuccess()).count()
                        : 0;
        ...
        if (loadHistory != null
                && loadHistory.size() > 10
                && cacheHits < loadHistory.size() / 2.0) {
