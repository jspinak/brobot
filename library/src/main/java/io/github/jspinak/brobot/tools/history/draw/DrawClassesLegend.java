 ...
 *
 * <p>DrawClassesLegend generates a multi-column legend that displays StateImage objects alongside
 * their computed k-means color centers. This provides a visual reference for understanding how
 * images are classified and what color profiles they contain.
 *
 * <p><b>Visual Output Structure:</b>
 *
 * <ul>
 *   <li>Organized in paired columns: images on left, k-means centers on right
 *   <li>Each entry is 40x40 pixels with 4-pixel spacing between entries
 *   <li>Multiple column pairs are created as needed to fit all images
 *   <li>Columns are dynamically sized based on scene height
 * </ul>
 *
 * <p><b>Configuration Parameters:</b>
 *
 * <ul>
 *   <li>Entry dimensions: 40x40 pixels (entryW x entryH)
 *   <li>Spacing between entries: 4 pixels
 *   <li>Spacing between column pairs: 8 pixels (2x entry spacing)
 *   <li>Automatic column calculation based on scene dimensions
 * </ul>
 *
 * <p><b>Use Cases:</b>
 *
 * <ul>
 *   <li>Creating visual documentation of classification results
 *   <li>Debugging image classification by showing input images with their color profiles
 *   <li>Generating legends for automated testing reports
 *   <li>Providing visual context for pattern matching operations
 * </ul>
 *
 * <p><b>Relationships:</b>
 *
 * <ul>
 *   <li>Works with {@link Visualization} to manage the complete visualization
 *   <li>Uses {@link ProfileMatrixBuilder} to generate image and profile matrices
 *   <li>Integrates with {@link VisualizationLayout} for multi-column layout management
 *   <li>Complements {@link DrawColorProfile} by showing k-means centers instead of mean colors
 * </ul>
 *
...
public class DrawClassesLegend {
    ...
    public DrawClassesLegend(
            ProfileMatrixBuilder profileMatrixBuilder, VisualizationLayout columnMatOps) {
     ...
     *
     * <p>Calculates how many image entries can fit vertically in the scene and determines the
     * required number of columns to display all images. This dynamic sizing ensures the legend
     * scales appropriately with different scene dimensions.
     *
     ...
     *
     * <p>Creates a visual legend showing:
     *
     * <ol>
     *   <li>Original images from each StateImage's patterns
     *   <li>Corresponding k-means color centers for each StateImage
     * </ol>
     *
     * <p>The legend is arranged in paired columns where each StateImage's patterns appear alongside
     * their computed color centers. This provides immediate visual correlation between source
     * images and their color analysis.
     *
    ...
    public void drawLegend(
            Visualization visualization, List<StateImage> imgs, ActionConfig actionConfig) {
     ...
     *
     * <p>Merges the matches-on-classes visualization with the generated legend, creating a complete
     * visual report. The legend appears to the right of the classification results, providing
     * context for interpreting the matches.
     *
    ...
    public void mergeClassesAndLegend(Visualization visualization) {
        ...
        Mat classesAndLegend =
                new MatBuilder()
                        .addHorizontalSubmats(
                                visualization.getMatchesOnClasses(), visualization.getLegend())
     ...
     *
     * <p>Creates a multi-column visualization where:
     *
     * <ul>
     *   <li>Each column pair shows images on the left, k-means centers on the right
     *   <li>Columns are filled vertically before starting new column pairs
     *   <li>Double spacing (8 pixels) separates column pairs for clarity
     * </ul>
     *
     ...
     *
     * <p>Processes each StateImage to create a horizontal arrangement of its pattern images.
     * Multiple patterns within a StateImage are displayed side-by-side with standard spacing.
     *
     ...
     *
     * <p>Arranges multiple pattern images horizontally if the StateImage contains more than one
     * pattern. This provides a complete view of all images that contribute to the StateImage's
     * classification.
     *
     ...
     *
     * <p>Processes each StateImage to create visual representations of its computed k-means color
     * centers. These centers represent the dominant colors found through clustering analysis.
     *
     * @param imgs list of StateImage objects to process
     ...
     *
     * <p>Generates visual swatches representing the k-means cluster centers computed from the
     * StateImage's color analysis. Multiple centers are arranged horizontally to show the color
     * distribution.
     *
