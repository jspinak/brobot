 ...
 *
 * <p>In mock mode with no screenshots, highlighting operations are logged instead of displayed.
 * When screenshots are available in mock mode, actual highlighting is performed on the screenshots
 * for visual verification.
 *
 * <p>The class ensures minimum visibility by setting a default size of 10x10 pixels for
 * zero-dimension matches to make them visible when highlighted.
 *
 * @see Match
...
public class MatchHighlighter {
     ...
     *
     * <p>The highlight remains visible until explicitly turned off with {@link #turnOff}. In mock
     * mode without screenshots, the operation is logged instead of displayed. The highlight color
     * is determined by the action configuration.
     *
     ...
     *
     * <p>This method removes highlights previously activated with {@link #turnOn}. In mock mode
     * without screenshots, no action is taken as there's no actual highlight to remove.
     *
     ...
     *
     * <p>This method works around a Sikuli limitation where Match objects don't highlight properly
     * by converting the match to a Region. For zero-dimension matches (e.g., point clicks), it
     * ensures visibility by setting a minimum size of 10x10 pixels.
     *
     * <p>The method performs a brief 1-second highlight on the match itself before highlighting the
     * region, which helps with visual debugging.
     *
     * @param match The match to highlight. Zero-dimension matches are expanded to 10x10 pixels for
     *     visibility.
     * @param stateObject Additional state information for logging in mock mode.
     * @param config Configuration containing highlight duration (seconds) and color settings.
     * @return {@code true} if the highlight was displayed (or logged in mock mode), always returns
     *     {@code true} in current implementation.
