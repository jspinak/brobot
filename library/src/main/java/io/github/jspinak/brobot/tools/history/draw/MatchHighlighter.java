...
/**
 * Provides visual highlighting functionality for matches on screen with mock
 * support.
 * <p>
 ...
 * matched regions on the screen. It supports both persistent highlights
 * (turnOn/turnOff)
 * and timed highlights. The class handles a Sikuli limitation where Match
 * objects
 * don't highlight properly by converting them to Region objects.
 * <p>
 * In mock mode with no screenshots, highlighting operations are logged instead
 * of
 * displayed. When screenshots are available in mock mode, actual highlighting
 * is
 * performed on the screenshots for visual verification.
 * <p>
 * The class ensures minimum visibility by setting a default size of 10x10
 * pixels
...
public class MatchHighlighter {
     ...
     * <p>
     * The highlight remains visible until explicitly turned off with
     * {@link #turnOff}.
     * In mock mode without screenshots, the operation is logged instead of
     * displayed.
     * The highlight color is determined by the action configuration.
     * 
     * @param match       The match to highlight. Must not be null.
     * @param stateObject Additional state information for logging purposes in mock
     *                    mode.
    ...
    public void turnOn(Match match, StateObjectMetadata stateObject, ActionConfig config) {
        ...
        if (FrameworkSettings.mock && FrameworkSettings.screenshots.isEmpty()) {
            ...
            try {
                ...
                String stateName = match.getStateObjectData() != null
                        && match.getStateObjectData().getOwnerStateName() != null
                                ? match.getStateObjectData().getOwnerStateName()
                                : "Unknown";
                String objectName = match.getStateObjectData() != null
     ...
     * 
     * @param match       The match to highlight. Zero-dimension matches are
     *                    expanded
     ...
     * 
     * @implNote This method logs the highlighted region dimensions for debugging
     *           purposes.
     */
