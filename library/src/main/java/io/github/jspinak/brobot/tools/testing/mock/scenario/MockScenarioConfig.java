 ...
 *
 * <p>This class enables sophisticated test scenarios by providing configurable:
 *
 * <ul>
 *   <li>State appearance probabilities for different GUI states
 *   <li>Action failure patterns with temporal and conditional triggers
 *   <li>Time-based conditions for simulating real-world timing variations
 *   <li>Cascading failure scenarios for robust error handling testing
 * </ul>
 *
 * <p>Example usage:
 *
...
public class MockScenarioConfig {
    ...
    /**
     * Probability (0.0-1.0) that each state will appear when searched for. States not in this map
    ...
    /**
     * Failure patterns for different action types, enabling simulation of intermittent failures,
     * cascading errors, and recovery scenarios.
     */
    @Singular("actionFailurePattern")
    private final Map<ActionType, FailurePattern> actionFailurePatterns;

    /**
     * Time-based conditions that affect mock behavior, such as network delays, processing time
     * variations, or timeout simulations.
     */
    @Singular("temporalCondition")
    private final Map<String, TemporalConditions> temporalConditions;

    /**
     * Global conditions that must be met for this scenario to be active. Useful for
     * environment-specific testing or progressive scenarios.
     */
    @Singular("activationCondition")
    private final Map<String, Predicate<MockTestContext>> activationConditions;

    /**
     * Maximum duration this scenario should run before automatically switching to default mock
     * behavior.
     */
    ...
    /**
     * Whether this scenario should cascade failures (one failure increases probability of
     * subsequent failures).
     */
    @Builder.Default private final boolean cascadingFailures = false;
    ...
    public MockScenarioConfig activate() {
        return this.toBuilder().activatedAt(LocalDateTime.now()).build();
    }
}
