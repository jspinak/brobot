 ...
 * 
 * <p>This class maintains a set of pre-computed K-means clustering results for different
 * numbers of cluster centers (k values). By calculating these profiles in advance for
 * commonly used values (typically 2-10), the framework can quickly access clustering
 * results without the computational overhead of running K-means at runtime.</p>
 * 
 * <p>Benefits of pre-computation:
 * <ul>
 *   <li>Significant performance improvement for color-based matching operations</li>
 *   <li>Consistent clustering results across multiple runs</li>
 *   <li>Reduced CPU/GPU usage during automation execution</li>
 *   <li>Ability to analyze and optimize cluster configurations offline</li>
 * </ul>
 * </p>
 * 
 * <p>The class stores profiles indexed by the number of centers, allowing quick
 * retrieval of the appropriate clustering configuration based on the complexity
 * of the color analysis required.</p>
 * 
 * @see KmeansProfile
...
public class KmeansProfiles {
     ...
     * 
     * <p>The profile is indexed by its number of centers, replacing any existing
     * profile with the same number of centers.</p>
     * 
     ...
     * @return An Optional containing the {@link KmeansProfile} if found, or empty if
     *         no profile exists for the specified number of centers
     */
     ...
     * 
     * <p>This method would analyze all stored profiles and select the one with
     * the best compactness metric, indicating well-separated clusters. Compactness
     * is typically measured by the within-cluster sum of squares (WCSS).</p>
     * 
     * <p>TODO: Implement this method to analyze compactness metrics and return
     * the optimal number of clusters.</p>
     * 
     * @return The number of centers that produces the most compact clustering,
     ...
     * 
     * <p>This utility method combines color information from both color spaces into
     * a single {@link ColorCluster} object, which provides a complete color profile
     * for matching operations.</p>
     * 
