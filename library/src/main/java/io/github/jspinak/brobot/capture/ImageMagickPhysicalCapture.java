 ...
 *
 * <p>ImageMagick captures at physical resolution, bypassing Java's DPI awareness.
 *
 * <p>Requirements: - ImageMagick must be installed - Windows: Download from
 * https://imagemagick.org/script/download.php#windows - macOS: brew install imagemagick - Linux:
 * apt-get install imagemagick or yum install imagemagick
 *
 * @since 1.1.0
 */
public class ImageMagickPhysicalCapture {
    ...
    public static BufferedImage captureRegion(int x, int y, int width, int height)
        ...
        if (IS_WINDOWS) {
            ...
            command =
                    String.format(
                            "screencapture -x -R%d,%d,%d,%d \"%s\"", x, y, width, height, tmpFile);
        } else {
