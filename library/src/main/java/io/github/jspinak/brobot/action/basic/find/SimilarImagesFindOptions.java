 ...
 *
 * <p>This class encapsulates parameters specific to finding similar images between two
 * ObjectCollections. The first ObjectCollection contains base images for comparison, and the second
 * ObjectCollection contains images to compare against the base images. For each image in the second
 * collection, it finds the best matching image from the first collection and returns a Match object
 * with similarity scores.
 *
 * <p>This is particularly useful for:
 *
 * <ul>
 *   <li>Screen state recognition - determining which known screen is currently displayed
 *   <li>Image classification - categorizing images based on similarity to known templates
 *   <li>Change detection - finding which images have changed between collections
 *   <li>Duplicate detection - identifying similar or duplicate images across collections
 * </ul>
 *
 * <p>The similarity comparison is performed at the Pattern level, where each StateImage's patterns
 * are compared to find the best match. The larger image automatically becomes the scene, while the
 * smaller becomes the search target.
 *
 * <p>Example usage:
 *
 * <pre>{@code
...
public final class SimilarImagesFindOptions extends BaseFindOptions {
    ...
    public enum ComparisonMethod {
        /**
         * Returns the single best match across all pattern combinations. This is the default and
         * most common method.
         */
        BEST_MATCH,

        /**
         * Returns an average similarity score across all pattern combinations. Useful when
         * consistency across multiple patterns is important.
         */
        AVERAGE_SCORE,

        /**
         * Returns matches only if all patterns meet the similarity threshold. Most restrictive
         * method, ensuring high confidence matches.
         */
        ALL_PATTERNS_MATCH,

        /**
         * Returns matches if any pattern meets the similarity threshold. Most permissive method,
         * useful for finding partial matches.
         */
        ANY_PATTERN_MATCHES
    }
     ...
     *
     * <p>This preset uses high similarity thresholds and strict comparison to accurately identify
     * which known screen state is currently displayed.
     *
     ...
     *
     * <p>This preset uses very high similarity thresholds to identify images that are nearly
     * identical.
     *
     ...
     *
     * <p>This preset uses lower similarity thresholds and includes all results to identify which
     * images have changed between collections.
     *
    ...
    public static class Builder extends BaseFindOptions.Builder<Builder> {
         ...
         *
         * <p>When true, images from the second collection that don't meet the similarity threshold
         * will still be included in the results as EmptyMatch objects. This is useful for
         * identifying which images couldn't be matched.
         *
         ...
         *
         * <p>When true, the Match objects will include detailed information about all
         * pattern-to-pattern comparison scores, not just the best one. This is useful for analysis
         * and debugging.
         *
