 ...
 *
 * <p>NestedFinds performs hierarchical searches where each find operation searches within the
 * results of the previous find. This is useful for:
 *
 * <ul>
 *   <li>Finding elements within specific UI containers
 *   <li>Narrowing down search areas progressively
 *   <li>Implementing hierarchical object detection
 * </ul>
 *
 * <p>Example usage:
 *
...
public class NestedFinds implements ActionInterface {
     ...
     * @param objectCollections Variable array of search targets. Each find action uses the
     *     collection at its index.
     */
    @Override
    public void perform(ActionResult matches, ObjectCollection... objectCollections) {
        ...
        NestedFindsOptions options =
                (config instanceof NestedFindsOptions)
                        ? (NestedFindsOptions) config
                        : createDefaultOptions(objectCollections.length);

        // Build the nested action chain
        ActionChainOptions.Builder chainBuilder = null;

        for (int i = 0;
                i < Math.min(options.getFindSteps().size(), objectCollections.length);
                i++) {
        ...
        ActionResult result =
                actionChainExecutor.executeChain(chainOptions, matches, objectCollections);
