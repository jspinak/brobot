 ...
 *
 * <p>This component handles the deserialization of {@link TrainingExample} objects from a binary
 * file format. It reads the custom serialization format that includes the action vectors,
 * descriptive text, and PNG-encoded screenshots.
 *
 * <p>The data is read from the configured file in the current working directory (default:
 * "trainingdata.dat"). The file format expects an integer count followed by that many serialized
...
public class TrainingExampleReader {
     ...
     *
     * <p>This method reads the serialized training data from disk and populates the internal list.
     * The file format consists of an integer indicating the number of TrainingData objects,
     * followed by the serialized objects themselves.
     *
     * <p>Each TrainingData object is printed to console as it's read for debugging purposes. If the
     * file doesn't exist or an error occurs during reading, the exception is printed to stderr and
     * the internal list remains unchanged.
     *
     * <p>Side effects:
     *
     * <ul>
     *   <li>Updates the internal trainingData list
     *   <li>Prints each loaded TrainingData object to stdout
    ...
    public void getDataFromFile() {
        ...
        try (FileInputStream fis = new FileInputStream(filename);
            ...
            System.out.println(
                    "Successfully loaded " + size + " training examples from " + filename);
        } catch (IOException e) {
            System.err.println(
                    "Error reading training data from " + filename + ": " + e.getMessage());
            throw new RuntimeException("Failed to read training data", e);
        } catch (ClassNotFoundException e) {
            System.err.println(
                    "Class not found while deserializing from " + filename + ": " + e.getMessage());
