 ...
 *
 * <p>Match fusion is a critical post-processing step that prevents duplicate detections and
 * improves result quality by merging matches that likely represent the same UI element. The fusion
 * process iteratively combines matches based on configurable distance thresholds and fusion
 * strategies.
 *
 * <p>Key features:
 *
 * <ul>
 *   <li>Supports multiple fusion strategies (absolute size, relative size)
 *   <li>Iterative fusion until no more matches can be combined
 *   <li>Preserves match names through string fusion
 *   <li>Configurable distance thresholds for X and Y axes
 * </ul>
 *
 * <p><b>Important:</b> Fused matches lose their original pattern, state object, and other metadata.
 * If this information is needed, perform fusion on a copy of the matches or retain the original
 * matches separately.
 *
...
public class MatchFusion {
    ...
    public MatchFusion(
            AbsoluteSizeFusionDecider matchFusionDeciderAbsoluteSize,
     ...
     *
     * <p>This method modifies the match list in-place by replacing all matches with their fused
     * equivalents. The fusion process continues iteratively until no more matches can be combined.
     * If fusion method is set to NONE, no changes are made.
     *
     * @param matches The ActionResult containing matches to fuse. The match list within this object
     *     is modified in-place with fused results.
     */
    public void setFusedMatches(ActionResult matches) {
        if (matches.getActionConfig() == null || getFusionMethod(matches.getActionConfig()) == NONE)
            return;
     ...
     *
     * <p>This method repeatedly applies the fusion algorithm until a stable state is reached where
     * no additional matches can be fused. This ensures that all possible combinations are found,
     ...
     *
     * <p>This method implements the core fusion algorithm, examining each match to find others that
     * should be combined with it based on the fusion strategy and distance thresholds. The
     * algorithm:
     *
     * <ol>
     *   <li>Processes each unallocated match as a potential fusion seed
     *   <li>Iteratively expands the seed by fusing nearby matches
     *   <li>Creates a new combined match with merged regions and concatenated names
     *   <li>Continues until all matches have been allocated to fusion groups
     * </ol>
     *
     * <p>The fusion decision is delegated to the configured {@link MatchFusionDecider}, allowing
     * for different fusion strategies. Match names are preserved and combined using string fusion
     * to maintain traceability.
     *
     * @param matchList The list of matches to fuse. This list is not modified.
     * @param actionConfig Configuration containing the fusion method and distance thresholds
     *     (maxFusionDistanceX, maxFusionDistanceY)
     * @return A new list containing the fused matches. May be smaller than the input list if
     *     matches were combined.
     */
    public List<Match> getFusedMatchObjects(List<Match> matchList, ActionConfig actionConfig) {
        ...
        while (!toCheck.isEmpty()) {
            ...
            Match m =
                    new Match.Builder().setRegion(originalMatch.getRegion()).setName(name).build();
            toCheck.remove(0);
            /*
            ...
            while (fused) {
                ...
                for (int i : new ArrayList<>(toCheck)) {
                    if (decider.isSameMatchGroup(
                            m, originalMatches.get(i), maxXDistance, maxYDistance)) {
                        Region r = new Region(m).getUnion(new Region(originalMatches.get(i)));
                        name = StringFusion.fuse(name, originalMatches.get(i).getName());
                        m = new Match.Builder().setRegion(r).setName(name).build();
    ...
    /**
     * Helper method to extract the fusion method from ActionConfig. Since ActionConfig is now a
     * base class for specific config types, we need to check the type and provide appropriate
     * defaults.
     *
