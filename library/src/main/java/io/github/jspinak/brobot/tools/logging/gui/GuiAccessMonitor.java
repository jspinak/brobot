...
/**
 * Monitors and reports GUI access problems that may prevent automation
 * from working correctly. Provides platform-specific diagnostics and
 * solutions for common GUI access issues.
 * 
 * <p>Common issues detected:</p>
 * <ul>
 *   <li>Headless environment (no display)</li>
 *   <li>Missing DISPLAY variable on Linux/Unix</li>
 *   <li>X11 server not running</li>
 *   <li>Wayland compatibility issues</li>
 *   <li>Remote desktop limitations</li>
 *   <li>Screen resolution problems</li>
 * </ul>
 * 
...
public class GuiAccessMonitor {
    ...
    private boolean performSilentCheck() {
        try {
            return !GraphicsEnvironment.isHeadless() && 
                   GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice() != null;
        } catch (Exception e) {
            return false;
        }
    }
    ...
    private boolean checkHeadlessMode(List<GuiAccessProblem> problems) {
        try {
            if (GraphicsEnvironment.isHeadless()) {
                problems.add(new GuiAccessProblem(
            ...
            try {
                ...
                problems.add(new GuiAccessProblem(
                    ...
                    List.of(
                        ...
                        "Try: xhost +local:docker or xhost +SI:localuser:$(whoami)"
                    )
                ));
            }
        }
        ...
        try {
            ...
            if (bounds.width < 800 || bounds.height < 600) {
                problems.add(new GuiAccessProblem(
                    ...
                    String.format("Screen resolution %dx%d may be too low for automation",
        ...
        try {
            ...
            if (devices.length > 1) {
                ...
                if (config.isVerboseErrors()) {
                    problems.add(new GuiAccessProblem(
                        "Multiple Screens",
                        String.format("%d screens detected - automation will use primary screen",
                            devices.length),
                        GuiAccessProblem.Severity.INFO,
                        List.of(
    ...
    private static class GuiAccessProblem {
        ...
        public GuiAccessProblem(String title, String description, 
                              Severity severity, List<String> solutions) {
            this.title = title;
